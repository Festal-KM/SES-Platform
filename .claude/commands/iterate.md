...
You are orchestrating the `/iterate` loop.
...
- 2 回目以降: 直前のフィードバック（reviewer または e2e-tester からのもの）を次のフォーマットで先頭に付加してから `$ARGUMENTS` を続ける：
...
- **タスクが `CLAUDE.md` §3 のどのハードルールに触れるか**を明示する（reviewer が該当チェック項目を誤って N/A にしないため）
...
- 🔴 **外部 API は必ずモックすること**を毎回明示する（実行すると不可逆な事故になる）
...
- 推奨される次の一手:
  - docs/05-program-design.md を見直す / タスク定義を再分解する / e2e-tester の前提環境（dev サーバ + ジョブワーカー + 外部 API モック）を確認する
```
...
- 🔴 **外部サービスへの不可逆な操作が発生し得る処理をループ内で許可しない**。`programmer` / `e2e-tester` のいずれかが実 API を叩こうとした形跡があれば、即座にループを停止してユーザーに報告する。
