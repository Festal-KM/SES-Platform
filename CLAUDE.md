...
12. **`sandbox` の有効期間と、本契約へのデータ引き継ぎ手順** — 見込み客が自分の実データで試す環境（§11）について、有効期間、期限到来時のデータの扱い、本契約への移行手順が未定。実在の個人情報を預かるため、§9-8 の保持期間と併せて決める
13. 🔴 **匿名共有（§3.1 経路 4）の再識別リスクの評価 — Phase 1 のリリース条件** — スキル・経験年数・単価レンジ・稼働可能時期・勤務地の 5 項目でも、母集団が小さいと個人が特定されうる。どの粒度まで丸めるか（経験年数を範囲で出す、稼働可能時期を月単位にする等）を決める。**経路 4 が Phase 1 に入ったため（§5）、「実データで確かめてから」では間に合わない。Phase 1 の実装前に暫定の丸め方を決め、実データが溜まった時点で見直す。**
