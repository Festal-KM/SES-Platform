...
🔴 **リリースは Phase 1 / Phase 2 / Phase 3 の完了時点でそれぞれ行う（計 3 回）**（2026-09-10 に人間が決定。[Issue #46](https://github.com/Festal-KM/SES-Platform/issues/46)）。理由: **実機能を触って出る修正を、次のフェーズを積み上げる前に回収する**ため。

| リリース | 時点 | その時点で自動化されていないもの |
|---|---|---|
| 第 1 回 | **Phase 1 完了（SP-12 の後）** | ⑤ 契約（メール添付で手作業）/ 課金（手動請求）/ ② マッチングスコア・チャット・⑥ 還流 |
| 第 2 回 | **Phase 2 完了（SP-16 の後）** | ⑤ 契約 / 課金。**⑥ → ① の還流（§1.3 の中核）はここで閉じる** |
| 第 3 回 | **Phase 3 完了（SP-20 の後）** | — |

🔴 **第 1 回リリースの時点で `production` 環境が実在する。** したがって §11 の環境分離のハードルールは Phase 1 の完了条件であり、後回しにできない。
...
🔴 **レビューはリスク基準で回す**（2026-09-15 に人間が「レビューの実行回数を減らして開発速度を上げる」と指示）:

| タスクが触るもの | code-reviewer | e2e-tester |
|---|---|---|
| **DB アクセス / RLS / 越境経路（§3.1）/ AI 層（§3.2）/ 外部送信・冪等性（§3.4）/ 状態機械（§4.2）/ 管理平面（§10）** | **1 回だけ**。`REQUEST_CHANGES` の修正は**オーケストレーターが対象テストで機械確認**し、再レビューはしない | 省略（programmer の E2E 実行 + CI で代替） |
| **UI のみ / 文言 / ドキュメント / テストのみ** | **省略**。オーケストレーターが lint / typecheck / 対象テストで確認 | 省略 |

理由: 本スプリントでレビューが捕まえた実バグ 2 件（`T-08-03` の実名漏洩 / `T-08-06` の AI 実行系 import）は**いずれも上段の領域**で起きた。下段では見た目の差分が中心で、CI の静的テスト群（testid 凍結 / ブレークポイント / RSC 境界 / 折り返し検出器）が回帰を機械で捕まえる。**上段を 0 回にはしない**（§7 の KPI「漏洩 0 件」は許容が無い）。
...
| Tenant | `PARTNER_VIEWER` | 自社に公開された案件情報と、自社の要員情報の**閲覧のみ**。**承認・送信・ダウンロード・提案の作成・チャットの投稿は一切不可**（2026-09-10 に人間が追加を決定。[Issue #34](https://github.com/Festal-KM/SES-Platform/issues/34)。実装は `T-16-12`） |
| Tenant | `VIEWER` | 閲覧のみ。**承認・送信・ダウンロードは一切不可**。ホスト所属専用であり、**パートナー所属の閲覧専用は `PARTNER_VIEWER` を用いる** |
...
- **運営者コンソールは既定 read-only**。テナントの業務データ（エンジニア・案件・提案・チャット・契約）を運営者が書き換えられる経路を作らない。書き込みが許されるのは契約・クォータ・機能フラグ・お知らせと、**テナントの開設・初期 `OWNER` の招待・送信ドメインの登録（いずれも INSERT のみ）**に限る（2026-09-10 に人間が承認。[Issue #21](https://github.com/Festal-KM/SES-Platform/issues/21)。`A-014` の開設フローを成立させるための最小の書き込みであり、**この列挙に無いものは「契約の一部だから」という解釈で足してはならない**）。
