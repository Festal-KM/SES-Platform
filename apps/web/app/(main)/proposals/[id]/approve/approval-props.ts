...
export function proposalApprovalScreenMessages(): ProposalApprovalScreenMessages {
  return {
    ...
    // 🔴 T-09-06: 送信（#43）。「送信済み」ではなく「受け付けた / 送信中」の語を使う。
    submit: t('proposals.approval.action.submit'),
    submitting: t('proposals.approval.action.submitting'),
    submitRequested: t('proposals.approval.action.submitRequested'),
    submitLead: t('proposals.approval.action.submitLead'),
    submitScrollRequired: t('proposals.approval.action.submitScrollRequired'),
    errorSubmitState: t('proposals.approval.error.submitState'),
    errorSendBlocked: t('proposals.approval.error.sendBlocked'),
