...
export function proposalDetailScreenMessages(): ProposalDetailScreenMessages {
  return {
    ...
    // ✅ T-12-14 ②③: セクション 4「ゲート結果の履歴」+ 折りたたみの「すべて表示」。HELD / 判定不能の語は `S-021` と同じ出所。
    sectionGateHistory: t('proposals.detail.gateHistory.section'),
    gateHistoryLead: t('proposals.detail.gateHistory.lead'),
    showAll: t('proposals.detail.showAll'),
    ...
    gateHistoryAiFailed: t('proposals.detail.gateHistory.aiFailed'),
    ...
    // 🔴 AI の警告の見出しは「合否に影響しない」ことを語に含める（現在の結果・履歴とも同じ 1 部品で描くため同じ語）。
    gateWarningsTitle: t('proposals.detail.gateHistory.warningsTitle'),
