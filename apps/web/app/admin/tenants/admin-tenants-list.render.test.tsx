...
import { DEFAULT_TENANT_HEALTH_THRESHOLDS } from '@ses/domain';
...
describe('AdminTenantsList（A-002 / F-056 AC-2）', () => {
  it('① 異常のシグナルが行にバッジで出る（列挙値の文言。API の順）', () => {
    const html = render({ page: { items: [ABNORMAL, HEALTHY], nextCursor: null, observedAt: NOW } });
  ...
  it('🔴 ② 異常 0 件（先頭ページ・異常度順）→ 全体の帯 + 一覧は引き続き表示（画面を空にしない）', () => {
    const html = render({ page: { items: [HEALTHY, CLOSING], nextCursor: null, observedAt: NOW } });
  ...
  it('② 異常 0 件でも、2 ページ目や別の並びでは帯を出さない（先頭が最上位である保証が無い）', () => {
    const page: PlatformTenantListPage = { items: [HEALTHY], nextCursor: null, observedAt: NOW };
  ...
  it('③ テナント 0 件 → 「テナントがまだありません」', () => {
    const html = render({ page: { items: [], nextCursor: null, observedAt: NOW } });
  ...
  it('④ CLOSING / PURGED は「対象外」、シグナル無しの対象テナントは「異常なし」', () => {
    const html = render({ page: { items: [HEALTHY, CLOSING, PURGED], nextCursor: null, observedAt: NOW } });
  ...
  it('⑤ 並び替え: 現在の並びは aria-current、他はリンク。health 以外は ?sort=、カーソルは付かない', () => {
    const html = render({
      page: { items: [HEALTHY], nextCursor: TENANT_B, observedAt: NOW },
      sort: 'name',
      isFirstPage: false,
    });
  ...
  it('⑥ 席の列は「利用中 / 有効」、集計日時（JST）と閾値が明示される', () => {
    const html = render({ page: { items: [ABNORMAL], nextCursor: null, observedAt: NOW } });
  ...
  it('⑧ 操作導線（承認 / 送信 / 削除 / 停止 / フォーム）が 1 つも無い。行の遷移先は A-003 だけ', () => {
    const html = render({ page: { items: [ABNORMAL, HEALTHY, CLOSING], nextCursor: null, observedAt: NOW } });
