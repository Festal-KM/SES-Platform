 ...
 *
 * 🔴 ② を外さない（Issue #33 / `docs/05` §3.3.1）。
 *    `invitations.partner_company_id` ほか A 群 13 列は **複合 FK**
 *    `(tenant_id, partner_company_id) REFERENCES partner_companies(tenant_id, id)` になり、
 *    他テナントの取引先を指す行は **DB が拒否する**ようになった。だが照合の役割が
 *    「防御の本体」から「一次防御（正しい応答を返す責務）」へ**降格しただけ**であり、
 *    不要になったわけではない。**「FK が守るから照合は要らない」と読み替えてはならない。**
 *      1. 🔴 FK 違反は `23503` であり、写像しなければ 500 になる。500 と 404 が区別できると
 *         **他テナントに実在する ID かどうかを応答コードで探れる**（500 = 実在するが他テナント /
 *         404 = そもそも無い）。§4.8「見えない ＝ 存在しない」が崩れる。アプリ層照合が先に
 *         404 を返すことで、FK 違反が利用者応答に到達する経路そのものが無くなる。
 *      2. 🔴 FK は**テナント境界しか見ない**。第二境界の判定はそれより狭い。同一テナント内で
 *         実行者に**見えてよい**取引先か（C5 の母集団に入るか）は FK では判定できず、
 *         パートナー文脈の実行者が他パートナーの ID を指定するケースは FK を通ってしまう。
 *
