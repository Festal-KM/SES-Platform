 ...
 * 登録直後の遷移先パターン。
 *
 * ⚠️ **暫定で編集画面へ戻す。** `docs/04` §S-012 関連画面は `→ S-011`（案件詳細）だが、
 *    `S-011` は **T-06-02** で実装される。T-05-01 の `S-007` が `S-006` 未実装のあいだ
 *    編集画面へ送り返していたのと同じ形であり、T-06-02 で `/projects/{id}` に差し替える。
 * 🔴 関数ではなく**文字列**で持つ（サーバコンポーネントからクライアントコンポーネントへ渡す
 *    props は直列化できる必要があり、関数は渡せない。docs/05 §6.1「Server Actions を使わない」）。
 */
export const PROJECT_CREATED_HREF_PATTERN = `/projects/${CREATED_HREF_ID_PLACEHOLDER}/edit`;
