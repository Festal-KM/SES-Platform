...
| **トライアル** | 見込み客が**サンドボックス環境で自分の実データを使って**試用する段階（§7.8.1）。**業務上の外部送信（宛先が取引先や第三者のもの）だけが疑似**で、それ以外の挙動と統制は本番と同じ（BR-62） | 初期フェーズは運営者が手動でサンドボックスのテナントを開設し、期限を管理する運用を許容する。**本契約への移行はテナントの状態遷移として行い、データを環境間で複製しない**（BR-64）。有効期間と引き継ぎ手順は未確定（§9-12 / Q-16） |
...
| **外部への送信** | **一切起きない**（合成データのみのため、すべて疑似送信） | 🔴 **宛先が誰かで分かれる**（**暫定。[Issue #9](https://github.com/Festal-KM/SES-Platform/issues/9) で確認中**）。**業務上の外部送信は疑似** — 提案メール・面談調整・契約書の送付・電子署名依頼など、宛先が**取引先や第三者**であるもの。**実際に届くのは、契約 SES 企業（ホスト）の社員宛の案内メールだけ** — 招待、パスワード再設定、試用期限の予告、削除の予告、スキャン失敗の周知、上限接近の通知。**本人に届かないと試用環境に入れず、期限にも気づけない**ため。🔴 **取引先企業の担当者宛は、テナントの利用者であっても疑似**（理由は §7.8.3 / Q-19）。**判断に迷うものは疑似の側に倒す。** 送信以外の挙動は本番と同じ | 実際に起きる |
...
- 🔴 **実演・試用中の「業務上の外部送信」（提案メール・面談調整・契約書の送付・電子署名依頼）で、実在の取引先に何かが届くことは構造上起こらない**（BR-45）。「気をつけて操作する」で担保しない。**`CLAUDE.md` §11.1 の改訂（宛先がそのテナントの利用者であるメールは試用環境でも実送信）によっても、この趣旨は一切弱まらない** — 試用環境で実際に届くのは**ホスト（契約 SES 企業）の社員宛の案内メール**（招待・パスワード再設定・試用期限の予告・削除の予告・スキャン失敗の周知・上限接近の通知）に限る
- 🔴 **取引先企業の担当者宛のメールは、試用環境では実際には届かない。** 本プロダクトでは取引先の担当者も**招待されてそのテナントに所属する利用者**（`PARTNER_ADMIN` / `PARTNER_SALES`）になるため、§11.1 の原則を字義どおり当てはめると**見込み客の試用環境から実在の取引先企業へ招待メールが届く**ことになり、**BR-45（0 件・許容しない）に抵触する**。したがって §11.1 のタイブレーカー「**判断に迷うものは業務上の外部送信の側に倒す**」を適用し、**試用環境で実送信するのはホストの社員宛のみ**とする（**暫定。[Issue #10](https://github.com/Festal-KM/SES-Platform/issues/10) で確認中**。Q-19）。**取引先を招待する業務は、招待用リンクを画面に表示して担当者が自分の手段で渡せる形とし、試用中も「取引先を招く → 提案が返ってくる」流れを体験できるようにする**
- **本番でないことが画面に常時表示される**（BR-48）。実演を見た顧客も、試用中の見込み客も、あとで本番と取り違えないため
- **サンドボックスは契約前でも「預かった個人情報」を扱う場である**（BR-62）。有効期間、期限到来時のデータの扱い、本契約への引き継ぎ手順は未確定であり、決まるまでは運営者が個別に期限を管理する（`CLAUDE.md` §9-12 / Q-16）。**実演・試用中の誤操作が失注に直結するという業務リスクは、章 10 の R-1 / R-2 に記載する**
...
| Q-19 | 🔴 **試用環境（サンドボックス）で、取引先企業の担当者宛のメールを実際に送るか** — 本書は「**送らない（業務上の外部送信として疑似にする）**」を既定として置いた | **暫定。[Issue #10](https://github.com/Festal-KM/SES-Platform/issues/10) で確認中**（`CLAUDE.md` §11.1 自体が暫定であり [Issue #9](https://github.com/Festal-KM/SES-Platform/issues/9) と連動する）。§11.1 は 2026-08-31 に改訂され、試用環境のモックの射程を「**宛先が、そのテナントに所属する利用者か**」で分ける原則になった。しかし本プロダクトでは**取引先の担当者も招待されてテナントの利用者になる**（`PARTNER_ADMIN` / `PARTNER_SALES`）ため、字義どおり適用すると**見込み客の試用環境から実在の取引先企業へ招待メールが届き、BR-45（0 件・許容しない）に抵触する**。そこで §11.1 のタイブレーカー「**判断に迷うものは業務上の外部送信の側に倒す**」を適用し、**試用環境で実送信するのはホスト（契約 SES 企業）の社員宛のみ**とした（章 7.8.1 / 7.8.3 / BR-62）。試用中の取引先招待は**招待リンクを画面に表示して担当者が渡す**形で体験させる。**確認したいのは、この解釈でよいか（＝§11.1 に「取引先担当者宛は業務上の外部送信として扱う」と明記するか）**。決めるのは人間（`CLAUDE.md` §11.1 の改訂を伴うため §8.6）。**BR-45 は無傷であり、本判断は BR-45 を守るためのものである** |
...
**`CLAUDE.md` の再検討が必要な事項**: **1 件（Q-19）**。本書は `CLAUDE.md` と正面から矛盾する記述を含まないが、🔴 **§11.1 の「宛先がそのテナントの利用者なら実送信」という原則と、その例示（招待）については、本プロダクト固有の事情で解釈が要る** — 取引先の担当者も招待されてテナントの利用者になるため、字義どおりでは BR-45 に抵触する。本書は §11.1 自身のタイブレーカー（迷うものは業務上の外部送信の側に倒す）に従って「取引先担当者宛は疑似」とし、**§11.1 にその旨を明記するかを Q-19 / [Issue #10](https://github.com/Festal-KM/SES-Platform/issues/10) で人間に確認中**とした。未決着の Q-1 / Q-2 / Q-4〜Q-7 / Q-9 / Q-10 / Q-12 / Q-15〜Q-17 / Q-19 は、いずれも `CLAUDE.md` の記述と**矛盾する**ものではなく、未確定または未記載である旨（**Q-19 のみ、暫定である §11.1 の解釈の確認**）をそのまま引き継いだものである。**決着済みは Q-3 / Q-8 / Q-11 / Q-13 / Q-14 / Q-18**（2026-08-31 の Issue #2 / #4 の決定を含む）。**上流への反映漏れも 0 件** — Q-11（取引先の利用時間 1 日 4〜5 時間）は `CLAUDE.md` §1.2 に反映済み（2026-08-31。表の 2 行と、その直後の 🔴 段落）であり、本書は同節を典拠として引いている（章 2.1 / 2.3）。残る未決着のうち性質が異なるものが 2 つある: 🔴 **Q-17 は `CLAUDE.md` §5 により Phase 1 のリリース条件**であり、Phase 1 の実装前に暫定値を決めなければ着手できない。**Q-5 は方針（稼働開始日と勤務地の重みを大きくする）と暫定値を得たが、「開始日の遅れ・勤務地の不一致を減点で扱うか候補から外すかで扱うか」の論点が未決**である（§8.6 の事業判断）。
