...
- **受け入れ基準**:
  - AC-1 🔴 **テナント外へ共有される 5 種（提案 / スキルシートの外部共有 / 案件の公開 / チャット添付 / 契約書）は、ゲートを経ずに共有状態へ進めない**（`BR-15`）。契約書は `DRAFT` → `SENDING`（章 5.5 遷移 2）の前にゲートを通り、FAIL のまま送付できない（`F-047` 処理 ⑥ / `F-048 AC-3`）。**発注書（`F-050`）はゲート対象ではない。**
...
| 対象 | 上限（既定） | 超過時の挙動 |
|---|---|---|
| AI コスト（日次） | テナントあたり 1 日の上限（USD。**運営者の内部指標**）。プランごとに設定 | AI 機能を停止し、**残量（件数）・上限（件数）・リセット時刻・停止理由**を画面に表示する。**金額は表示しない**（`F-027 AC-6`）。🔴 **ゲートは実行されず未実行のまま保持され、対象は `GATE_RUNNING` に留まる（`GATE_FAILED` にしない）。送信はできない**（`F-027 AC-5`。上限到達は「保留」であって元データの欠陥ではないため。`CLAUDE.md` §4.2） |
...
| **コスト上限到達** | 再試行せず即座に停止し、残量と理由を表示する（`F-027 AC-1`）。🔴 **`gate-inspector` は実行せず、対象を `GATE_RUNNING` のまま保持する（`GATE_FAILED` にしない）。送信はできない**（`F-027 AC-5` / 章 8.5）。**これは LLM の失敗ではなく「未実行」であり、障害指標に加算しない**（`F-059 AC-6`） |
...
| BR-15 | `F-020 AC-1`（**5 種。契約書を含む**）/ `F-014` / `F-038` / `F-047` 処理 ⑥ / `F-048 AC-3` / 章 8.5 | BR-47 | `F-053 AC-1` / **NFR-ENV-8** |
