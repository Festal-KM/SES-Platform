...
- **AI 利用**: なし。
- **外部 API 依存**: メール送信（招待・ロール変更通知）。上限は `F-027` のガードに従う。🔴 **`sandbox` での扱いは宛先の分類で分かれる**（章 7.6 NFR-ENV-1 の判定表 / `CLAUDE.md` §11.1）。**ホスト所属利用者（`OWNER` / `ADMIN` / `SALES` / `VIEWER`。招待中の本人を含む）宛は分類 1 で実送信**。**パートナー所属利用者（`PARTNER_ADMIN` / `PARTNER_SALES`）宛は分類 2 でモックとし、代わりに招待リンクを画面に表示する**（取引先招待の起点は `F-007`。`F-007 AC-4`）。`development` / `demo` では分類を問わずモック。
- **受け入れ基準**:
...
- **関連ロール**: `OWNER` / `ADMIN` / `SALES`、`VIEWER`（閲覧のみ）。**`PARTNER_ADMIN` / `PARTNER_SALES` は不可**（章 4.2）。`Assignment` はホストが作成するレコードであり、パートナーがこれをアプリ内で閲覧することは `CLAUDE.md` §3.1 の越境 4 経路のいずれにも該当しない（`## Assumptions` A-21）。パートナーが知り得るのは、自社が作成した提案の状態（`F-024`。経路 2）と、**自社エンジニアの稼働状況と稼働可能時期という属性**（`F-045 AC-5`。自社が持ち込んだ `Engineer` レコードの属性であり越境ではない）に限られる。**案件名・エンド企業名・単価・契約には到達しない。** 延長の打診・満了の連絡はチャット（`F-038`。経路 3）で行う。
...
- **受け入れ基準**:
  ...
  - AC-5 🔴 **`PARTNER_ADMIN` / `PARTNER_SALES` が参照できるのは、自社エンジニアの「稼働状況」（稼働中 / 待機予定 / 待機中）と「稼働可能時期」に限られる。** 案件名・エンド企業名・単価・契約・`Assignment` そのものには到達せず、それらは画面・API・エクスポート・通知のいずれにも現れない（`F-042` はパートナーに `−`。`BR-04` / `BR-06`）。**稼働可能時期が満了日と同一の値になることは意図した挙動であり**（処理 ①）、これは自社が持ち込んだ `Engineer` レコードの属性であって `Assignment` の内容ではないため、`CLAUDE.md` §3.1 の越境にあたらない。
- **関連ロール**: `system`（還流）、`OWNER` / `ADMIN` / `SALES`（結果の参照）。`PARTNER_ADMIN` / `PARTNER_SALES` が参照できるのは**自社エンジニアの稼働状況と稼働可能時期という属性のみ**（`AC-5`）である。🔴 **満了日が「稼働可能時期」としてこの属性に載ることを本書は明示的に認める。** `Engineer` はパートナー自身が自社台帳に持ち込んだレコードであり（章 6.2。`F-008` で `PA` / `PS` = `◐`）、その属性を自社が参照・編集することは越境ではない。実務上も、取引先は「自社の誰がいつ空くか」を知らなければ次の提案ができない。到達しないのは `Assignment` の内容（**案件・エンド企業名・単価・契約**）であり、`Assignment` レコード自体をアプリ内で閲覧することも認めない（`F-042` はパートナーに `−`。`## Assumptions` A-21）。この属性が `F-017` の匿名 5 項目（稼働可能時期は A-04 の 5 段階に丸める）としてホストへ出る経路は A-04 の範囲内である。
...
- 🔴 **`F-042`（稼働）と `F-047`（契約）はパートナーに `−` である。** いずれもホストが作成するレコードであり、パートナーがこれをアプリ内で閲覧することは `CLAUDE.md` §3.1 の越境 4 経路（章 1.1）のいずれにも該当しない。**5 つ目の越境経路を本書で作らない**（`F-004 AC-5` / `## Assumptions` A-21）。パートナーが知り得るのは、自社が作成した提案の状態（`F-024`。経路 2）、**自社エンジニアの稼働状況と稼働可能時期という属性**（`F-045 AC-5`。自社が持ち込んだ `Engineer` レコードの属性であり越境ではない。**案件名・エンド企業名・単価・契約には到達しない**）、およびチャットでのやり取り（`F-038`。経路 3）に限られる。
- `F-010`（スキル辞書・別名）の `PA` / `PS` = `◐` は、**語彙マスタの参照と、自社の入力から生じた新語候補の起票**を指す。`Skill`（グローバル辞書）と `SkillAlias` は**分類のためのマスタであって業務データではなく**、他パートナーが持ち込んだ情報を含まない（章 6 の分離キー規約 / `F-010 AC-2`）。採否の決定はホスト（`ADMIN` / `SALES`）が行い、**採用後のテナント別名はホストの決定物としてテナント内で共有される**（分類語彙であり、他パートナーの業務データを含まないため第二境界の対象にならない）。
- `F-003`（認証・2FA）の `PA` / `PS` = `●` は、**認証が境界そのものを確定させる操作であり、第二境界の内側での参照ではない**ことによる。パートナー所属の利用者も自分自身のログイン・2 要素認証の設定は完全に実行できる（`F-003 AC-1`。認証の結果として `partner_company_id` が確定し、以後のすべての参照がその内側に限定される）。ここに `◐` を付けると「認証がパートナースコープの内側の操作」という誤った読みを生む。
...
| **A-19** | **`BR` の担保状況**: `BR-01`〜`BR-64` のうち、受け入れ基準または非機能要件で担保できなかったものは **0 件**。ただし `BR-51`（外部サービスの規約と送信元名義）のみ、**`Q-2` の裏取り結果に依存**するため、現時点では「規約に反する使い方を既定にしない」という要件までの担保に留まる | 付録 B のカバレッジ表 | `A-11` と連動 |
...
>
> **確認中（本書は既定を置いたうえで進める）**: **`sandbox` におけるメール送信の射程を「宛先の分類」で分ける扱い**は **[Issue #9](https://github.com/Festal-KM/SES-Platform/issues/9)** で確認中である。本書は `CLAUDE.md` §11.1 を正とし、**提案先・エンド企業・エンジニア本人宛の業務上の外部送信はモック、ホスト所属利用者宛のアカウント運用のメールは実送信**を既定として記述した（`## Assumptions` **A-22**。章 7.6 の判定表 / NFR-ENV-1 / `F-054 AC-9` / `F-064 AC-10`）。🔴 **このうち「パートナー所属利用者（`PARTNER_ADMIN` / `PARTNER_SALES`）宛をモック側に置き、取引先招待は招待リンクの画面表示で代替する」扱いは [Issue #10](https://github.com/Festal-KM/SES-Platform/issues/10) の決定であり**、`CLAUDE.md` §11.1 の判定原則（宛先がそのテナントに所属する利用者か）だけでは実送信側に落ちて `BR-45`（0 件）に抵触するため、**同 §11.1 のタイブレーカー（迷うものはモック側）を適用して分類 2 を新設した**（`F-007 AC-4` / `F-054 AC-1` / NFR-ENV-1 ③）。**§11.1 の改訂は §8.6 の人間承認事項**であり、本書からは追随のみを行う。`Q-` 番号は `docs/01` からの引き継ぎに使うため本項には振らない（A-21 と同じ扱い）。
>
...
| BR-05 | `F-004 AC-5` / 章 1.1 の経路表 | BR-37 | `F-056 AC-3` / 章 2.8 共通前提 / 章 4.4 / **`F-053 AC-6`**（`demo` 以外での投入・リセットの禁止） |
