...
- **関連ロール**: `OWNER` / `ADMIN` / `SALES`、`VIEWER`（閲覧のみ）。`PARTNER_ADMIN` / `PARTNER_SALES` は**自社が当事者となる契約（NDA / 基本契約 / 個別契約 / 発注書）に限り閲覧できる**（章 4.1 の `◐`）。**他社が当事者の契約は 1 件も見えず、一覧の件数・存在の示唆も返らない**（`BR-07`）。単価・支払条件は**自社が当事者となる契約に含まれるものだけ**が見える。締結の意思決定・送付はホスト側（`OWNER` / `ADMIN` / `SALES`）が行う。運営者は状態を変更できない（`BR-37`）。
...
| ◐ | **パートナースコープ内に限定される（第二境界）**。自パートナー企業が持ち込んだ情報、および自社に公開・自社が当事者となる情報に限る。**閲覧のみの場合も、パートナースコープに限定されるなら `○` ではなく `◐` を用いる** |
