...
- **受け入れ基準**:
  ...
  - AC-7 テナントが `SUSPENDED` の間、**実行系の操作**（提案の承認 `F-021` / 送信・再送 `F-022` `F-023` / 面談調整の連絡 `F-041` / 契約書の送付・電子署名依頼 `F-047` `F-049` / 提案依頼の発行 `F-018`）は、`OWNER` を含むすべてのロールで拒否され、導線も表示されない。**ログインと閲覧は可能で、データは削除されない**（章 5.4）。
  - AC-8 テナントが `CLOSING` の間、業務データの**新規作成**（エンジニア・案件・提案・チャット・契約）と実行系の操作ができず、実行できるのは閲覧と `F-052` のエクスポートのみである（章 5.4）。
  - AC-9 AC-7 / AC-8 の拒否は、画面の導線非表示だけでなく **API を直接呼んでも拒否される**。拒否の理由（停止中 / 解約手続き中）が利用者に表示される。
- **関連ロール**: 全テナントロール（適用対象）。設定変更は不可（コードとデータ構造で担保する統制であり、画面設定を持たない）。テナントのライフサイクル状態を変更できるのは `PLATFORM_OWNER` のみ（`F-062`）。
...
- **入力**: エンジニアの最終の稼働終了日 / 提案終了日、保持期間の設定（既定 **3 年**。`## Assumptions` A-05）、テナントのライフサイクル状態（章 5.4）、削除の実行。
- **処理**: ①保持期間を過ぎた対象を抽出する ②連絡先とスキルシート原本を削除する ③削除前にテナント管理者へ予告通知する ④削除の実行と対象件数を監査ログに記録する ⑤`sandbox`（`SANDBOX`）テナントの期限到来時は `F-054` の規則で `CLOSING` へ遷移させる ⑥**テナントが `CLOSING` から `PURGED` へ遷移するとき、エンジニアの連絡先・スキルシート原本・チャット本文を削除する。監査ログは法令上の保持義務がある範囲のみ残す**（章 5.4）。
- **出力**: 削除対象の一覧、削除実行結果、`PURGED` 到達時の削除完了記録、監査ログ。
- **AI 利用**: なし。
...
- **受け入れ基準**:
  ...
  - AC-5 テナントが `PURGED` に到達した時点で、**エンジニアの連絡先・スキルシート原本・チャット本文が削除されている**。削除後にこれらへ到達できる経路（画面・API・エクスポート・共有 URL）が 1 つも存在しない（章 5.4）。
  - AC-6 `PURGED` 後も、**法令上の保持義務がある範囲の監査ログは残る**。残す範囲と消す範囲が設定として明示され、暗黙に全件削除・全件保持のいずれにもならない。
  - AC-7 `PURGED` は終端であり、そこから `ACTIVE` / `CLOSING` に戻す操作が存在しない（章 5.4 の遷移表に無い遷移として 422 で拒否される）。
- **関連ロール**: `OWNER` / `ADMIN`（設定・確認）、`system`（実行）、`PLATFORM_OWNER`（`PURGED` 到達の確認。`F-062`）。
...
- **関連ロール**: `OWNER` / `ADMIN` / `SALES`、`VIEWER`（閲覧のみ）。運営者は状態を変更できない（`BR-37`）。
...
- **目的**: 解約手続き中（`CLOSING`）に顧客のデータを返却し、期限（`PURGED`）をもって削除する（`BR-29` / `## Assumptions` A-07）。
- **入力**: エクスポート対象範囲（エンジニア台帳・案件・提案履歴・契約・監査ログ）、テナントのライフサイクル状態（章 5.4）、実行の指示。
- **処理**: ①CSV 形式で一括エクスポートを生成する ②生成をジョブとして実行し、完了を通知する ③エクスポートの実行を監査ログに記録する ④**`CLOSING` の間は新規作成ができず、実行できるのは閲覧とエクスポートのみである**（章 5.4）⑤`CLOSING` から 30 日経過で `PURGED` へ遷移し、`F-046` の削除が実行される（A-07）。
- **出力**: CSV 一式、`PURGED` への遷移と削除の実行結果、監査ログ。
- **AI 利用**: なし。
...
- **受け入れ基準**:
  ...
  - AC-4 `CLOSING` に入ってから 30 日経過した時点で `PURGED` へ遷移し、`F-046 AC-5` の削除が完了する。削除の実行結果が運営者に確認できる（`F-062`）。
...
- **処理**: ①テナントを `SANDBOX` で開設し、有効期限を持たせる ②テナント分離・パートナースコープ・監査ログ・暗号化・保持期間を**本番と同じ規律**で適用する ③送信系（メール / 電子署名）のみモックとする ④**本契約は `SANDBOX` → `ACTIVE` の状態遷移として行い、データを環境間で複製しない**（章 5.4 遷移 1）⑤**30 日の期限到来、または見送りの決定で `SANDBOX` → `CLOSING` に遷移する**（章 5.4 遷移 2）。以後の削除は `F-052` / `F-046` の `CLOSING` → `PURGED` に従う ⑥期限接近を運営者と見込み客に通知する。
...
- **受け入れ基準**:
  ...
  - AC-4 開設から 30 日を過ぎ本契約に至らない場合、または見送りが決定された場合、テナントは **`CLOSING`** に遷移する。遷移後は新規作成ができず、エクスポートのみ実行できる（`F-052 AC-3`）。削除前に運営者と見込み客へ通知される。
  - AC-5 `CLOSING` から 30 日経過で `PURGED` に遷移し、個人情報が削除される（`F-046 AC-5`）。
...
- **処理**: ①プランを変更し、席数とクォータを反映する ②トライアル期限を延長できる（`F-054` と連動）③**テナントのライフサイクル状態を遷移させる（章 5.4）。停止は `ACTIVE` → `SUSPENDED`、解消は `SUSPENDED` → `ACTIVE`、解約は `CLOSING` への遷移** ④請求を記録する（Phase 3 で決済連携。それ以前は手作業の請求を前提とし、`F-026` の計測データを根拠として提示する）⑤`CLOSING` → `PURGED` の到達と削除結果を確認する（`F-046` / `F-052`）⑥すべての操作を監査ログに記録する。
...
- **受け入れ基準**:
  ...
  - AC-2 **`ACTIVE` → `SUSPENDED` の遷移を実行できるのは `PLATFORM_OWNER` のみ**である。テナント側のどのロール（`OWNER` を含む）からも実行できない。`SUSPENDED` → `ACTIVE`（解消）も同じく `PLATFORM_OWNER` のみ。
...
- **手順**: 1. 見込み客が自社の実データ（実在のエンジニアの経歴を含む）を投入する。2. `SANDBOX` でも、テナント分離・監査ログ・暗号化・保持期間が `production` と同じ規律で適用される（`F-054 AC-1`）。3. 提案の送信を試すが、メールは実在の宛先に到達しない（`F-054 AC-2`）。画面上部に `sandbox` の表示が出ている（`F-028`）。4-a **本契約に至った場合**: `PLATFORM_OWNER` が **`SANDBOX` → `ACTIVE`** に遷移させる（`F-062`）。データのコピー・別テナントの作成・別環境への複製はいずれも発生せず、投入済みのデータが同じ識別子のまま引き継がれる（`F-054 AC-3` / NFR-ENV-7）。4-b **至らなかった場合**: 30 日の期限到来または見送りの決定で **`SANDBOX` → `CLOSING`** に遷移する（`F-054 AC-4`）。以後は新規作成ができず、エクスポートのみ可能になる。5. `CLOSING` から 30 日経過で **`PURGED`** に遷移し、連絡先・スキルシート原本・チャット本文が削除される（`F-046 AC-5`）。
- **結果**: 契約前でも預かった個人情報が本番と同じ水準で保護され、本契約は状態遷移だけで完了し、至らなかった場合は `CLOSING` → `PURGED` で確実に片付く。
- **関連機能**: `F-028` `F-046` `F-052` `F-054` `F-062`（章 5.4）
...
- **手順**: 1. 解約申請を受けて、テナントが **`CLOSING`** に遷移する（`F-062`。`ACTIVE` → `CLOSING` / `SUSPENDED` → `CLOSING`）。2. `CLOSING` の間は業務データの新規作成と実行系の操作ができず、**閲覧とエクスポートのみが可能**である（`F-004 AC-8` / `F-052 AC-3`）。3. `OWNER` が CSV 一括エクスポートを実行する（`F-052`）。生成完了が通知され、ダウンロードする。エクスポートの実行が監査ログに残る。4. `CLOSING` から 30 日経過で **`PURGED`** に遷移し、エンジニアの連絡先・スキルシート原本・チャット本文が削除される（`F-046 AC-5`）。**監査ログは法令上の保持義務がある範囲のみ残る**（`F-046 AC-6`）。5. `PURGED` は終端であり、`ACTIVE` に戻す操作は存在しない（`F-046 AC-7`）。削除の実行結果を運営者が確認する（`F-062`）。
- **結果**: 顧客が取引先に対して返却・削除の説明責任を果たせる。返却の窓（`CLOSING` の 30 日）と削除の完了（`PURGED`）が状態として記録に残る。
- **関連機能**: `F-004` `F-046` `F-052` `F-062`（章 5.4）
...
| **テナントの解約（`ACTIVE` / `SUSPENDED` → `CLOSING`）** | `F-062 AC-3`（章 5.4 遷移 5 / 6） |
| クォータの設定・上書き・引き下げ | `F-057` |
| **`SANDBOX` → `ACTIVE` の移行**・トライアル期限の延長 | `F-054 AC-7` / `F-062`（章 5.4 遷移 1） |
...
| 4 | **`Tenant`（契約のライフサイクル）** | **5.4** | **5** | Phase 0（開設 = `SANDBOX` / `ACTIVE`。`F-001`）/ Phase 1（`SANDBOX` → `ACTIVE` / `CLOSING`。`F-054`）/ Phase 3（`SUSPENDED` と解約。`F-062` / `F-052`） |
...
| # | 遷移元 | 遷移先 | トリガー | 実行者 | 不正時のエラー |
|---|---|---|---|---|---|
| 1 | `SANDBOX` | `ACTIVE` | 本契約の締結（`F-054 AC-3`）。**データを引き継ぎ、複製しない** | **`PLATFORM_OWNER` のみ**（`F-062`） | 422 |
| 2 | `SANDBOX` | `CLOSING` | 開設から 30 日の期限到来、または見送りの決定（`F-054 AC-4`。A-08） | `system`（期限）/ `PLATFORM_OWNER`（見送り） | 422 |
| 3 | `ACTIVE` | `SUSPENDED` | 運営者による停止（`F-062 AC-2`） | 🔴 **`PLATFORM_OWNER` のみ。`PLATFORM_SUPPORT` は不可**（`BR-44` / 章 4.4） | 422（`PLATFORM_SUPPORT` の要求は 403） |
| 4 | `SUSPENDED` | `ACTIVE` | 停止事由の解消（`F-062 AC-2`） | **`PLATFORM_OWNER` のみ** | 422 |
| 5 | `ACTIVE` | `CLOSING` | 解約申請（`UC-24`） | `PLATFORM_OWNER`（テナントの `OWNER` の申請を受けて実行） | 422 |
| 6 | `SUSPENDED` | `CLOSING` | 解約 | **`PLATFORM_OWNER` のみ** | 422 |
| 7 | `CLOSING` | `PURGED` | `CLOSING` から 30 日経過（`F-052 AC-4`。A-07） | `system` | 422 |

**各状態で何ができるか**（`F-004 AC-7`〜`AC-9` が全画面・全 API に適用する）:

| 状態 | ログイン | 閲覧 | 新規作成・編集 | 実行系（承認 / 送信 / 契約書送付 / 提案依頼） | エクスポート（`F-052`） | データ |
|---|---|---|---|---|---|---|
...
- 🔴 **`CLOSING` では新規作成ができず、エクスポートのみ可能である**（`F-004 AC-8` / `F-052 AC-3`）。返却の手段を停止によって塞がないため、エクスポートは `SUSPENDED` / `CLOSING` のいずれでも実行できる（`F-052 AC-5`）。
- 🔴 **`PURGED` は終端である。** `ACTIVE` / `CLOSING` に戻す遷移は存在しない（`F-046 AC-7`）。
...
| `SANDBOX` テナントの個人情報 | 開設から **30 日**。未移行なら `CLOSING` へ遷移し、そこから 30 日で `PURGED`（A-08 / `F-054 AC-4` `AC-5`） | `BR-64` |
| 解約テナントの業務データ | **`CLOSING` から 30 日**で `PURGED`（A-07 / `F-052 AC-4`）。`CLOSING` の間は返却のためのエクスポートが可能 | `BR-29` |
| `PURGED` 時に削除するもの / 残すもの | 削除: エンジニアの連絡先・`SkillSheet` 原本・`Message` 本文。残す: **法令上の保持義務がある範囲の `AuditLog`**（`F-046 AC-5` `AC-6` / 章 5.4） | `BR-27` / `BR-29` |
...
| **応答不明時の運用** | 外部 API がタイムアウトし到達が不明な場合、`SUBMIT_FAILED` / **`SEND_FAILED`** として確定させ、**自動で再送しない**。`F-059` に「未対応の `SUBMIT_FAILED` / `SEND_FAILED`」として現れ、人間が実際の到達を確認したうえで `F-023` / `F-049` から再送する。再送時は「届いている可能性がある」旨の確認を表示する（`F-023 AC-2` / `F-049 AC-3`） |
...
| **A-12** | **`Tenant` のライフサイクル状態は `SANDBOX` / `ACTIVE` / `SUSPENDED` / `CLOSING` / `PURGED` の 5 つ**。`SUSPENDED` はデータを消さず実行系のみ不可、`CLOSING` は新規作成不可でエクスポートのみ可、`PURGED` は終端で連絡先・スキルシート原本・チャット本文を削除、`SANDBOX` → `ACTIVE` はデータを引き継ぐ。停止（`ACTIVE` → `SUSPENDED`）は `PLATFORM_OWNER` のみ | **章 5.4**（状態遷移表 + 状態別の可否表）/ `F-004 AC-7`〜`AC-9` / `F-046 AC-5`〜`AC-7` / `F-052 AC-3`〜`AC-5` / `F-054 AC-3` `AC-4` `AC-7` / `F-062 AC-1`〜`AC-5` / 章 6.1 / 章 6.8 / `UC-22` / `UC-24` | **`CLAUDE.md` §4.2 に暫定として反映済み。[Issue #6](https://github.com/Festal-KM/SES-Platform/issues/6) で確認中。** 本書は §4.2 を正として引き写しており、**そこに無い遷移を追加していない**（`SANDBOX` → `SUSPENDED` 等は 422）。確認の結果 §4.2 が変わった場合は §8.7 の手順で本書も追随する |
...
| BR-12 | `F-031 AC-2` / `F-034 AC-1` / `F-044 AC-2` / `F-048`（AI 不使用） | BR-44 | `F-062 AC-1` `AC-2`（**`SUSPENDED` への遷移は `PLATFORM_OWNER` のみ**）/ `F-057 AC-2` / `F-054 AC-7` / **章 4.4** / **章 5.4 遷移 3** |
...
| BR-29 | `F-046 AC-1`〜`AC-3` `AC-5`〜`AC-7` / `F-052 AC-3`〜`AC-5` / **章 5.4**（`CLOSING` → `PURGED`）/ 章 6.8 | BR-61 | `F-020 AC-3` `AC-4` / **章 8.5** |
...
| BR-32 | 章 7.11 | BR-64 | `F-054 AC-3`〜`AC-5` / **章 5.4 遷移 1 / 2 / 7** / **NFR-ENV-7** |
...
10. **テナントが `SUSPENDED` / `CLOSING` のときの画面**（章 5.4 / `F-004 AC-7`〜`AC-9`）— **閲覧はできるが実行系の導線が出ない**状態を、「壊れている」ではなく「停止中 / 解約手続き中」として理由とともに示す。`CLOSING` では**エクスポートだけが実行できる**ことが分かる導線にする（`F-052 AC-3`）。「黙って劣化させない」は項目 8 と同じ原則。
