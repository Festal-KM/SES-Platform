...
- **関連ロール**: `OWNER` / `ADMIN`（設定・確認）、`system`（実行）、`PLATFORM_OWNER`（削除ジョブの実行結果の確認。**件数・状態のみで業務データの内容には到達しない**。`BR-40` / `F-062`）。
...
- **処理**: ①テナントを `SANDBOX` で開設し、有効期限を持たせる ②テナント分離・パートナースコープ・監査ログ・暗号化・保持期間を**本番と同じ規律**で適用する ③送信系（メール / 電子署名）のみモックとする ④**本契約は `SANDBOX` → `ACTIVE` の状態遷移として行い、データを環境間で複製しない**（章 5.4 遷移 1）⑤**30 日の期限到来、または見送りの決定で `SANDBOX` → `CLOSING` に遷移する**（章 5.4 遷移 2）。以後の返却と削除（`CLOSING` → `PURGED`）は **`F-064`**（P0 / Phase 1）が担う ⑥期限接近を運営者と見込み客に通知する。
...
- **受け入れ基準**:
  ...
  - AC-9 `sandbox` 環境で本機能が `production` と**同一の実装・同一の統制**で動作する。「試用環境だから削除しない / 期限を延ばす」に相当する設定項目が存在しない（`BR-62`）。期限の延長は `F-062` の運営者操作としてのみ成立し、監査ログに残る。
...
- **処理**: ①プランを変更し、席数とクォータを反映する ②トライアル期限を延長できる（`F-054` と連動）③**テナントのライフサイクル状態を遷移させる（章 5.4）。停止は `ACTIVE` → `SUSPENDED`、解消は `SUSPENDED` → `ACTIVE`、解約は `CLOSING` への遷移** ④請求を記録する（Phase 3 で決済連携。それ以前は手作業の請求を前提とし、`F-026` の計測データを根拠として提示する）⑤**`CLOSING` → `PURGED` の到達と削除結果（完了 / 未完了と件数）を確認する。これが運営者にとっての削除確認の唯一の経路であり、返却データの内容には到達しない**（`F-064 AC-7` / `F-052 AC-6` / `BR-40`）⑥すべての操作を監査ログに記録する。
...
- **受け入れ基準**:
  ...
  - AC-7 🔴 **`CLOSING` → `PURGED` の削除完了の確認は本機能の権限として成立する**が、表示されるのは**完了 / 未完了の別と削除件数のみ**であり、削除された内容・返却データ（`F-064` / `F-052`）に到達する導線が存在しない（`BR-40`）。閲覧は `PLATFORM_SUPPORT` にも許される。
...
- `F-011` / `F-023` / `F-046` の `VI` `PO` = `○` は**閲覧のみ**であり、実行（ダウンロード・再送・削除の実行）は含まない（章 4.3 / `F-011 AC-1`〜`AC-4` / `F-023 AC-1` / `BR-31`）。
- `F-062` の `PP` = `○` は**閲覧のみ**の意味である。章 4.4 が `PLATFORM_SUPPORT` に禁じているのは**変更操作**（プラン変更・請求の確定・停止 / 解消 / 解約）であって、契約状態の閲覧ではない（`F-062 AC-1` / `AC-7`）。
- `F-046` の `PO` = `○` は、**削除ジョブの実行結果（件数・状態）の確認**に限る。業務データの内容には到達しない（`BR-40` / `F-046` 関連ロール）。
- 🔴 **`F-052` と `F-064` は運営者に `−` である。** いずれも**業務データそのものを取り出す**機能であり、記号定義どおりに `○` を付けると「運営者がその内容を閲覧できる」と読める。`BR-40` はこれを禁じる。**削除完了の確認は `F-062` 側の権限として表現する**（`F-062 AC-7` / `F-052 AC-6` / `F-064 AC-7`）。
...
|---|---|---|---|---|---|---|
| 1 | `SANDBOX` | `ACTIVE` | 本契約の締結（`F-054 AC-3`）。**データを引き継ぎ、複製しない** | **`PLATFORM_OWNER` のみ**（`F-062`） | **Phase 1**（`F-054`） | 422 |
...
| 5 | `ACTIVE` | `CLOSING` | 解約申請（`UC-24`） | `PLATFORM_OWNER`（テナントの `OWNER` の申請を受けて実行） | Phase 3（`F-062`） | 422 |
| 6 | `SUSPENDED` | `CLOSING` | 解約 | **`PLATFORM_OWNER` のみ** | Phase 3（`F-062`） | 422 |
...
| **A-20** | 🔴 **削除・返却の機能を 3 つに分けた**: **`F-064`（P0 / Phase 1）= `SANDBOX` の 30 日期限と `CLOSING` の 30 日経過による自動削除 + 返却の最小版**、`F-046`（P1 / Phase 2）= 保持期間 3 年の一般ルールに基づく定期削除、`F-052`（P2 / Phase 3）= 運用機能としての一括エクスポート（書式・項目の選択）。**`docs/01` 章 7.5 が許容する「初期フェーズは運営者が手動で期限を管理する運用」は採らない** | `F-064` / `F-046` / `F-052` / 章 5.4 遷移 7 / 章 1.4 | **本書が置いた分割。** 理由: `sandbox` には見込み客が**実在するエンジニアの経歴**を入れる（`CLAUDE.md` §11.1）。削除の仕組みが無いまま期限切れテナントを抱えると**第三者の個人情報を削除手段のないまま保持する**ことになり、これはスコープではなく統制の問題である（`BR-62` / `BR-64`）。一方、3 年ルールと運用機能としてのエクスポートまで Phase 1 に上げると P0 が膨らむため、**Phase 1 には必要最小限のみを置いた**（章 1.4 の対応表を参照） |
