...
| Phase 1: 「案件登録 → パートナーへ公開 → パートナーが提案 → ゲート実行 → ホストが承認 → 送信 → 結果記録」を E2E で完遂できる | `F-007` `F-008` `F-010` `F-011` `F-013` `F-014` `F-015` `F-019` `F-020` `F-021` `F-022` `F-024` `F-025` |
...
| Phase 1（`docs/01` 章 11.3-4）: デモ環境とサンドボックス環境が Phase 1 完了時点で用意される | `F-028` `F-053` `F-054` |
...
受け入れ基準は `AC-n` の連番で機能内に閉じる（参照は `F-020 AC-3` のように書く）。
...
- **処理**: ①すべての業務データ参照に対しテナント境界を適用する ②パートナー所属の利用者にはパートナー境界を重ねて適用する ③越境が許されるのは経路 1〜4 の定義に合致する場合のみで、それ以外は 0 件として扱う ④存在しないものとして扱うため、件数・存在の示唆も返さない。
- **出力**: 境界適用後の参照結果、越境試行の記録。
- **AI 利用**: なし。
- **外部 API 依存**: なし。
- **受け入れ基準**:
...
- **関連ロール**: 全テナントロール（適用対象）。設定変更は不可（コードとデータ構造で担保する統制であり、画面設定を持たない）。
...
- **処理**: ①ロール別に表示ブロックを決める（自社営業 = 承認待ち / 送信失敗 / 満了間近、パートナー = 公開案件 / 提案依頼 / 自社提案の状態、管理者 = メンバーと利用量）②`F-004` の境界適用後の件数のみを集計する ③パートナーには「何が見えて何が見えないか」の説明を常時表示する。
- **出力**: ロール別ホーム画面の表示データ。
- **AI 利用**: なし。
- **外部 API 依存**: なし。
- **受け入れ基準**:
...
- **処理**: ①アップロードを受け付け、版を採番する ②ウイルススキャンを実行し、結果が `CLEAN` になるまで共有 URL を発行しない ③`CLEAN` になった版のみ最新版フラグを持てる ④スキャン失敗・感染検出時は隔離し、担当者と `F-059` に通知する ⑤アップロード・版の切替を監査ログに記録する。
...
- **処理**: ①**PII 層** — 氏名・生年月日・連絡先・顔写真・現所属会社名の残存を検査する ②**商流層** — 単価・エンド企業名・他社名が、その公開範囲で出してはならない相手に出ていないかを検査する ③**整合層** — 案件の必須要件との齟齬・重複提案・スキルシートと登録スキルの矛盾を**機械的な照合のみで判定する**。AI（`gate-inspector`）はスキルシート本文との意味的な齟齬を**警告として併記**するだけで、**警告は合否を変えない** ④1 層でも FAIL なら `GATE_FAILED` とし、指摘を該当箇所とともに返す ⑤結果を `ReviewGate` として保存する。
...
- **処理**: ①実行前に上限を判定し、超過する要求を実行しない ②AI の日次コスト上限に達したら AI 機能を停止し、**残量と停止理由を画面に表示する** ③メールは日次・分次の両方を判定し、分次超過は待機、日次超過は停止として区別する ④上限接近（既定 80%）でテナント管理者と `F-057` に通知する ⑤上限到達・停止・解除を監査ログに記録する。
- **出力**: 実行可否、残量表示、停止理由、通知、監査ログ。
- **AI 利用**: なし。
- **外部 API 依存**: なし（判定はアプリ層）。
- **受け入れ基準**:
  - AC-1 AI の日次コスト上限に到達した状態で AI 機能を起動すると、実行されず「上限に達したため停止中」と残量が表示される。エラーコードのみの無言停止にならない。
...
- **関連ロール**: `OWNER` / `ADMIN` / `SALES`、`PARTNER_ADMIN` / `PARTNER_SALES`（自社エンジニアの稼働の閲覧）、`VIEWER`（閲覧のみ）。
...
- **関連ロール**: `system`（還流）、`OWNER` / `ADMIN` / `SALES` / パートナー各ロール（結果の参照）。

#### F-046 個人情報の保持期間管理と削除

- **目的**: 稼働終了・提案終了から一定期間を過ぎた個人情報を削除できる状態を作る（`BR-29` / `BR-52`）。
- **入力**: エンジニアの最終の稼働終了日 / 提案終了日、保持期間の設定（既定 **3 年**。`## Assumptions` A-05）、削除の実行。
- **処理**: ①保持期間を過ぎた対象を抽出する ②連絡先とスキルシート原本を削除する ③削除前にテナント管理者へ予告通知する ④削除の実行と対象件数を監査ログに記録する ⑤`sandbox` テナントの期限到来時は `F-054` の規則で削除する。
...
- **受け入れ基準**:
  ...
  - AC-4 `sandbox` 環境でも本番と同じ保持期間の規律が適用される（`BR-62`）。
- **関連ロール**: `OWNER` / `ADMIN`（設定・確認）、`system`（実行）。
...
- **入力**: 決定した提案の内容（案件・エンジニア・単価・期間）、取引先の企業情報（商号・住所・支払条件）、契約種別、契約書ファイルと版。
- **処理**: ①契約を作成し、締結状態を追跡する ②締結待ちをタスク化し督促通知を出す（`F-040` / `F-039`）③取引先へ渡る書類は `F-020` のゲート対象とする ④契約締結の意思決定は人間のみ。
- **出力**: `Contract`、`ContractDocument`（版・締結状態）、タスク、監査ログ。
- **AI 利用**: なし。
- **外部 API 依存**: 電子署名（`F-049`）、メール送信。
- **受け入れ基準**:
  - AC-1 契約の締結状態が、送付前 / 署名依頼済 / 署名完了 / 失効の区別をもって追跡できる（**この状態群は `CLAUDE.md` §4.2 に定義がない。`## Assumptions` A-13 として §4.2 の改訂を提起する**）。
  - AC-2 契約書ファイルは `F-011` と同じくスキャンが `CLEAN` になるまで外部に渡らない。
  - AC-3 締結の意思決定・署名依頼の送付は人間の明示操作でのみ行われる。
- **関連ロール**: `OWNER` / `ADMIN` / `SALES`、`VIEWER`（閲覧のみ）。
...
- **入力**: 契約書の版、署名者、`idempotency_key`、外部サービスの Webhook 通知。
- **処理**: ①送付直前に状態を CAS 更新して多重実行を排除する ②署名依頼を 1 回だけ発行する ③自動再送しない。再送は人間の明示操作のみ ④Webhook を正規化して締結状態に反映する ⑤未着を `F-059` に出す。
- **出力**: 署名依頼の記録、締結状態、監査ログ。
- **AI 利用**: なし。
- **外部 API 依存**: 電子署名サービス（**選定未確定。`## Assumptions` A-11 / `Q-2`**）。**送信元名義をテナントごとに分けられるかが未確定**であり、確定するまで本機能の送信元設計を固定しない。
- **受け入れ基準**:
  - AC-1 同一契約に対する署名依頼が外部 API へ 2 回以上発行されない（二重実行しても外部呼び出しは 1 回）。
  - AC-2 自動再送の仕組みが存在しない。再送は人間の明示操作で、確認を経て実行される。
  - AC-3 `production` 以外では実在の署名者に依頼が到達しない（章 7.6）。
  - AC-4 外部 API の応答は正規化された内部型として保存され、生の応答をそのまま業務データに持ち込まない。
- **関連ロール**: `OWNER` / `ADMIN` / `SALES`、`VIEWER`（不可）。
...
#### F-052 データ一括エクスポート（解約時の返却）

- **目的**: 解約時に顧客のデータを返却し、期限をもって削除する（`BR-29` / `## Assumptions` A-07）。
- **入力**: エクスポート対象範囲（エンジニア台帳・案件・提案履歴・契約・監査ログ）、実行の指示。
- **処理**: ①CSV 形式で一括エクスポートを生成する ②生成をジョブとして実行し、完了を通知する ③エクスポートの実行を監査ログに記録する ④解約から **30 日後**にテナントのデータを削除する（A-07）。
- **出力**: CSV 一式、削除の実行結果、監査ログ。
- **AI 利用**: なし。
- **外部 API 依存**: オブジェクトストレージ。
- **受け入れ基準**:
  - AC-1 エクスポートに含まれる範囲が、実行者の境界内に限られる（パートナーは自社分のみ）。
  - AC-2 エクスポートの実行と対象範囲が監査ログに残る。
  - AC-3 解約から 30 日後に、テナントの業務データと個人情報が削除される。削除の実行結果が運営者に確認できる（`F-062`）。
- **関連ロール**: `OWNER`（実行）、`ADMIN`（実行）、`VIEWER`（不可）、運営者（削除の確認のみ）。
...
- **入力**: `sandbox` テナント、開設日、有効期間（既定 **30 日**。`## Assumptions` A-08）、本契約への移行の指示、期限到来。
- **処理**: ①`sandbox` テナントを開設し、有効期限を持たせる ②テナント分離・パートナースコープ・監査ログ・暗号化・保持期間を**本番と同じ規律**で適用する ③送信系（メール / 電子署名）のみモックとする ④本契約への移行は**テナントの状態遷移**として行い、データを環境間で複製しない ⑤期限到来かつ未移行の場合、個人情報を削除する ⑥期限接近を運営者と見込み客に通知する。
- **出力**: `sandbox` テナントの状態、移行結果、削除の実行結果、監査ログ。
- **AI 利用**: なし。
- **外部 API 依存**: 送信系のみモック。それ以外（AI・ストレージ・スキャン）は本番同等。
- **受け入れ基準**:
  - AC-1 `sandbox` において、テナント分離・パートナースコープ・監査ログ・暗号化・保持期間の各テストが `production` と同じ内容で PASS する（`BR-62`）。
  - AC-2 `sandbox` からのメール・電子署名が実在の宛先に到達しない（`BR-45`）。それ以外の機能は本番と同じ挙動を示す。
  - AC-3 本契約への移行がテナントの状態遷移として行われ、データのコピー処理が発生しない（`BR-64`。**Tenant のライフサイクル状態は `CLAUDE.md` §4.2 に定義がない。`## Assumptions` A-12 として §4.2 の改訂を提起する**）。
  - AC-4 開設から 30 日を過ぎ未移行の `sandbox` テナントについて、個人情報が削除される。削除前に運営者と見込み客へ通知される。
  - AC-5 `demo` と `sandbox` が同一のテナント・同一の環境で兼用されない（`BR-63`）。
- **関連ロール**: `PLATFORM_OWNER`（開設・移行）、`PLATFORM_SUPPORT`（期限の確認・通知。移行と停止は不可）、見込み客側の `OWNER`。
...
- **入力**: 失敗ジョブ、`SUBMITTING` の滞留、未対応の `SUBMIT_FAILED`、ウイルススキャン失敗、電子署名の未着、ゲート FAIL 率、`F-043` の未起票検知、`F-026` の計測欠測。
- **処理**: ①上記を種別ごとに一覧化し、滞留時間と件数を表示する ②閾値を超えた項目をアラートする ③テナント別に集計するが、**内容には立ち入らない** ④ゲート FAIL 率の急変を異常として検知する。
- **出力**: 運用監視の一覧、アラート。
- **AI 利用**: なし。
- **外部 API 依存**: エラー追跡（Sentry）との併用。
- **受け入れ基準**:
  - AC-1 `SUBMITTING` のまま一定時間を超えた提案が一覧に現れる（`SUBMITTING` は片道であるため、滞留は必ず検知対象になる）。
  - AC-2 未対応の `SUBMIT_FAILED` が、`LOST` / `GATE_FAILED` / `DECLINED` と混ざらずに集計される（`BR-23`）。
...
- **入力**: プラン、席数、クォータ、トライアル（`sandbox`）期限と延長、停止 / 再開、請求の記録。
- **処理**: ①プランを変更し、席数とクォータを反映する ②トライアル期限を延長できる（`F-054` と連動）③テナントを停止・再開する ④請求を記録する（Phase 3 で決済連携。それ以前は手作業の請求を前提とし、`F-026` の計測データを根拠として提示する）⑤すべての操作を監査ログに記録する。
- **出力**: `Plan` / `Subscription` の状態、請求記録、監査ログ。
- **AI 利用**: なし。
- **外部 API 依存**: 決済（Stripe。Phase 3）。
- **受け入れ基準**:
  - AC-1 **`PLATFORM_SUPPORT` は課金設定の変更とテナント停止ができない**。導線が表示されず、API を直接呼んでも拒否される（`BR-44`）。
  - AC-2 プラン変更・トライアル延長・停止・再開が、実施者・対象・変更前後の値とともに監査ログに残る。
  - AC-3 テナント停止時、そのテナントの利用者は業務操作ができず、理由が画面に表示される。データは削除されない。
  - AC-4 Phase 3 以前でも、`F-026` の計測データから請求根拠（席数・AI 利用量・超過分）を遡って提示できる。
- **関連ロール**: `PLATFORM_OWNER`（全操作）、`PLATFORM_SUPPORT`（閲覧のみ）。
...
- **手順**: 1. `PLATFORM_SUPPORT` が消化率を確認し、顧客に事前連絡する（`F-057`。閲覧のみ）。2. `PLATFORM_SUPPORT` がプラン変更を試みるが、**導線が存在せず実行できない**（`BR-44` / `F-062 AC-1`）。3. `PLATFORM_OWNER` が上位プランへの変更を実行する（`F-062`）。4. 必要に応じてテナント個別のクォータ上書きを設定する（`F-057`）。5. `sandbox` テナントについては期限を延長する（`F-062` / `F-054`）。6. すべての操作が監査ログに残る。
...
- **前提**: `sandbox` テナントが開設され、有効期間 30 日が設定されている（`F-054`）。
- **手順**: 1. 見込み客が自社の実データ（実在のエンジニアの経歴を含む）を投入する。2. テナント分離・監査ログ・暗号化・保持期間が本番と同じ規律で適用される（`F-054 AC-1`）。3. 提案の送信を試すが、メールは実在の宛先に到達しない（`F-054 AC-2`）。画面上部に `sandbox` の表示が出ている（`F-028`）。4-a 本契約に至った場合: **テナントの状態遷移**として移行し、データを複製しない（`F-054 AC-3`）。4-b 至らなかった場合: 30 日の期限到来で個人情報が削除される（`F-054 AC-4`）。
- **結果**: 契約前でも預かった個人情報が本番と同じ水準で保護され、期限で確実に片付く。
- **関連機能**: `F-028` `F-046` `F-054` `F-062`
...
- **前提**: 解約が決定している。
- **手順**: 1. `OWNER` が CSV 一括エクスポートを実行する（`F-052`）。2. 生成完了が通知され、ダウンロードする。エクスポートの実行が監査ログに残る。3. 運営者が契約を終了状態にする（`F-062`）。4. 解約から 30 日後にテナントの業務データと個人情報が削除される（`F-052 AC-3`）。5. 削除の実行結果を運営者が確認する。
- **結果**: 顧客が取引先に対して返却・削除の説明責任を果たせる。
- **関連機能**: `F-046` `F-052` `F-062`
...
| ◐ | **パートナースコープ内のみ**実行可。自パートナー企業が持ち込んだ情報、および自社に公開された情報に限る |
| ○ | 閲覧のみ |
...
|---|---|
| プラン変更・契約の停止 / 再開・請求の確定 | `F-062` |
| クォータの設定・上書き・引き下げ | `F-057` |
| `sandbox` の本契約への移行・期限延長 | `F-054` / `F-062` |
...
🔴 **`CLAUDE.md` §4.2 を正とし、本章はそこに定義された状態と遷移だけを機能要件に落としたものである。状態も遷移も追加していない。** 追加が必要と判断した 2 点（`Tenant` のライフサイクル、`Contract` / `ContractDocument` の締結状態）は本文に書かず、`## Assumptions` A-12 / A-13 に「`CLAUDE.md` §4.2 の改訂が必要」として上げた。
...
| 概念 | 主要属性 |
|---|---|
| `Tenant` | 企業名 / 商号、環境種別（`production` / `sandbox` / `demo`）、ライフサイクル状態（**A-12 参照**）、`sandbox` の有効期限、自動承認フラグ `autoApproveEnabled`、マッチング重み設定（`F-030`）、AI ロール別の承認モードとモデル（`F-035` / `F-036`）、個人情報の保持期間（既定 3 年）、開設日 |
...
| `Contract` | `tenant_id`、契約種別（NDA / 基本契約 / 個別契約）、相手方、対象案件・エンジニア、単価、期間、支払条件、**締結状態（A-13 参照）** |
| `ContractDocument` | `contract_id`、版番号、ファイル参照、スキャン状態、署名依頼の `idempotency_key`、署名依頼日時、署名完了日時、外部サービスの正規化済み状態 |
...
| `sandbox` テナントの個人情報 | 開設から **30 日**、未移行なら削除（A-08 / `F-054`） | `BR-64` |
| 解約テナントの業務データ | 解約から **30 日**（A-07 / `F-052`） | `BR-29` |
...
- **NFR-ENV-1**（`BR-45`）: `development` / `demo` / `sandbox` / `staging` のいずれの環境からも、**実在のドメイン宛にメール・契約書・電子署名依頼が 1 通も送出されない**。E2E で全送信系機能（`F-022` / `F-041` / `F-047` / `F-049` / `F-002` の招待 / `F-039` の通知）を実行し、外部エンドポイントへの発信が 0 件であることを検証する。
...
- **NFR-ENV-5**（`BR-62`）: `sandbox` において、テナント分離・パートナースコープ・監査ログ・暗号化・保持期間の各テストが `production` と**同一の内容**で PASS する（`F-054 AC-1`）。「試用環境だから」を理由に統制を緩めた設定項目が存在しない。
...
| **CAS による多重実行の排除** | 送信直前に状態を `SUBMITTING` へ CAS 更新し、失敗した実行は送信しない（章 5.1 遷移 7）。DB レベルで多重実行が排除される |
| **片道の遷移** | `SUBMITTING` に入ったら必ず `SUBMITTED` / `SUBMIT_FAILED` に確定する。自動で `APPROVED` に戻らない |
| **自動リトライの禁止** | 外部 API 呼び出しの結果が確定した後にリトライしない。送信系ジョブに自動再試行の設定が存在しない（`F-023 AC-1`） |
| **応答不明時の運用** | 外部 API がタイムアウトし到達が不明な場合、`SUBMIT_FAILED` として確定させ、**自動で再送しない**。`F-059` に「未対応の `SUBMIT_FAILED`」として現れ、人間が実際の到達を確認したうえで `F-023` から再送する。再送時は「届いている可能性がある」旨の確認を表示する（`F-023 AC-2`） |
| **外部応答の正規化** | 外部 API の応答は生のまま保存せず、正規化した内部型に変換してから永続化する（`F-049 AC-4`） |
...
| **整合層** | 🔴 **機械的な照合のみ** | ①案件の必須要件との齟齬（`ProjectRequirement` と `EngineerSkill` / `EngineerSnapshot` の突合）②重複提案（`F-037`）③スキルシートと登録スキルの矛盾（登録値どうしの突合） | **機械的照合で不一致があれば FAIL。** AI の警告は合否に**一切影響しない** |
...
| 7 | **契約前に預かる個人情報** — `sandbox` でも本番と同じ規律を適用する | `F-054 AC-1`（`BR-62`） |
...
| **`Proposal` / `Assignment` / `ProposalRequest` の状態の追加** | 状態の意味が発散し、KPI が意味を失う（`BR-33`） | 必要が生じたら `CLAUDE.md` §4.2 の改訂を人間に提起する（A-12 / A-13） |
...
| **A-07** | **解約時のデータ返却は CSV 一括エクスポート、解約から 30 日後に削除** | `F-052` / 章 6.8 / `UC-24` | **暫定**（Issue #4、`Q-9`） |
| **A-08** | **`sandbox` の有効期間は 30 日**。期限到来で本契約に移行しなければデータ削除 | `F-054` / 章 6.8 / `UC-22` | **暫定**（Issue #4、`Q-16`） |
...
| **A-12** | 🔴 **`CLAUDE.md` §4.2 の改訂が必要 — `Tenant` のライフサイクル状態** | `BR-64`（`sandbox` から本契約への移行を**テナントの状態遷移**として行う）を満たすには、`Tenant` に「試用 / 有効 / 停止 / 解約」に相当するライフサイクル状態が要る。**しかし §4.2 は中核エンティティを `Proposal` / `Assignment` / `ProposalRequest` の 3 つと定めており、`Tenant` の状態機械を定義していない。** 本書は状態を追加せず、`F-054 AC-3` / 章 6.1 で「ライフサイクル状態（A-12 参照）」と記すに留めた | **人間の判断が必要。** §4.2 に `Tenant` の状態機械を追記するか、状態機械として扱わない旨を明記するかを決める（`CLAUDE.md` §8.6） |
| **A-13** | 🔴 **`CLAUDE.md` §4.2 の改訂が必要 — `Contract` / `ContractDocument` の締結状態** | Phase 3 の `F-047` / `F-049` は締結状態（送付前 / 署名依頼済 / 署名完了 / 失効）の追跡を要するが、**§4.2 に定義がない。** 本書は状態を追加せず、`F-047 AC-1` / 章 6.5 で「締結状態（A-13 参照）」と記すに留めた | **人間の判断が必要。Phase 3 着手前に §4.2 の改訂が要る**（`CLAUDE.md` §8.6） |
...
| **新規 `Q-19`** | 🔴 **`CLAUDE.md` §4.2 に `Tenant` のライフサイクル状態を追加するか** | **人間**（`CLAUDE.md` の改訂は承認事項） | A-12。本書は状態を追加していない |
| **新規 `Q-20`** | 🔴 **`CLAUDE.md` §4.2 に `Contract` / `ContractDocument` の締結状態を追加するか** | **人間**。Phase 3 着手前 | A-13。本書は状態を追加していない |
...
| 6 | 提案・提案依頼・稼働の状態を §4.2 どおりに機能化する | `F-018` / `F-024` / `F-042` / **章 5**（3 つのステートマシン）/ `F-051 AC-2` |
| 7 | 送信の一回性と人間による再送を機能化する | `F-022`（AC-1 / AC-3）/ `F-023`（AC-1 / AC-2）/ 章 7.8 / `UC-20` |
| 8 | 上限到達時の挙動を機能化する（残量と理由の表示） | `F-027`（AC-1〜AC-4）/ `F-049 AC-2` / 章 7.5 / 章 7.7 / `UC-06` |
...
| 17 | 環境の分離を非機能要件として書く（demo / sandbox / 本番の 3 者） | **章 7.6**（NFR-ENV-1〜9）/ `F-028` / `F-053` / `F-054` / `UC-21` / `UC-22` |
...
| 22 | Open Questions の未決着分を引き継ぐ | **`## Assumptions`**（A-01 / A-03〜A-05 / A-07〜A-11 / A-14 / A-15）/ **`## Open Questions`**（`Q-1` / `Q-2` / `Q-4`〜`Q-6` / `Q-9` / `Q-10` / `Q-12` / `Q-15`〜`Q-17` + 新規 `Q-19` / `Q-20`）。関連機能: `F-030`（`Q-5`）/ `F-017`（`Q-17`）/ `F-032`（`Q-6`）/ `F-057`（`Q-15`）/ `F-054`（`Q-16`）/ `F-049`（`Q-2`）/ `F-046`（`Q-4`） |
...
| BR | 担保箇所 | BR | 担保箇所 |
|---|---|---|---|
| BR-01 | `F-004 AC-1` / 章 7.3 | BR-33 | `F-024 AC-1` / `F-042 AC-2` / 章 5 共通ルール |
...
| BR-08 | `F-037 AC-1` | BR-40 | `F-056 AC-1` / `F-058 AC-2` `AC-3` / `F-059 AC-3` / `F-060 AC-4` / `F-063 AC-4` |
...
| BR-12 | `F-031 AC-2` / `F-034 AC-1` / `F-044 AC-2` / `F-048`（AI 不使用） | BR-44 | `F-062 AC-1` / `F-057 AC-2` / **章 4.4** |
...
| BR-21 | `F-022 AC-1` / `F-041 AC-1` / `F-049 AC-1` / 章 7.8 | BR-53 | `F-016 AC-1` / `AC-2` |
| BR-22 | `F-022 AC-3` / `F-023 AC-1` / 章 5.1 遷移 10 / 章 7.8 | BR-54 | `F-017 AC-1` / `AC-3` / `F-031 AC-3` |
...
| BR-29 | `F-046 AC-1`〜`AC-3` / `F-052 AC-3` / 章 6.8 | BR-61 | `F-020 AC-3` `AC-4` / **章 8.5** |
| BR-30 | `F-003 AC-2` / `F-055 AC-3` | BR-62 | `F-054 AC-1` / **NFR-ENV-5** |
| BR-31 | `F-004 AC-6` / `F-012 AC-3` / **章 4.3** | BR-63 | `F-053 AC-4` / `F-054 AC-5` / **NFR-ENV-6** |
| BR-32 | 章 7.11 | BR-64 | `F-054 AC-3` / **NFR-ENV-7** |
...
11. **`Tenant` のライフサイクルと `Contract` の締結状態**（A-12 / A-13）— **`CLAUDE.md` §4.2 の改訂が済むまで、状態機械として実装しない。** 属性としての最小限の保持に留め、改訂後に設計を確定する。
