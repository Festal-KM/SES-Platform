...
- `F-010`（スキル辞書・別名）の `PA` / `PS` = `◐` は、**語彙マスタの参照と、自社の入力から生じた新語候補の起票**を指す。`Skill`（グローバル辞書）と `SkillAlias` は**分類のためのマスタであって業務データではなく**、他パートナーが持ち込んだ情報を含まない（章 6 の分離キー規約 / `F-010 AC-2`）。採否の決定はホスト（`ADMIN` / `SALES`）が行い、採用後のテナント別名はホストの決定物としてテナント内で共有される。/ `F-003`（認証・2FA）の `PA` / `PS` = `●` は、**認証が境界そのものを確定させる操作であり、第二境界の内側での参照ではない**ことによる（`F-003 AC-1`。認証の結果として `partner_company_id` が確定し、以後のすべての参照がその内側に限定される）。ここに `◐` を付けると「認証がパートナースコープの内側の操作」という誤った読みを生む。
