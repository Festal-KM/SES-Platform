...
- **AI 利用**: なし。
- **外部 API 依存**: メール送信（招待）。**招待の宛先はそのテナントの利用者本人（招待中の本人を含む）であるため、`sandbox` では実送信される**。`development` / `demo` ではモック（`F-028` / 章 7.6 NFR-ENV-1 / `CLAUDE.md` §11.1）。
- **受け入れ基準**:
...
- **AI 利用**: なし。
- **外部 API 依存**: メール送信（招待・ロール変更通知）。上限は `F-027` のガードに従う。**宛先はそのテナントの利用者（招待中の本人を含む）であるため、`sandbox` では実送信される**。`development` / `demo` ではモック（章 7.6 NFR-ENV-1 / `CLAUDE.md` §11.1）。
- **受け入れ基準**:
...
- **AI 利用**: なし。
- **外部 API 依存**: メール送信（パスワード再設定等）。**宛先は利用者本人であるため、`sandbox` では実送信される**。`development` / `demo` ではモック（章 7.6 NFR-ENV-1 / `CLAUDE.md` §11.1）。
- **受け入れ基準**:
...
- **処理**: ①アップロードを受け付け、版を採番する ②ウイルススキャンを実行し、結果が `CLEAN` になるまで共有 URL を発行しない ③`CLEAN` になった版のみ最新版フラグを持てる ④スキャン失敗・感染検出時は隔離し、担当者と `F-059` に通知する（**Phase 1 の担当者への周知はアプリ内表示とメール送信**（`F-001` / `F-002` と同じ送信経路）。**宛先は自テナントの担当者であるため、`sandbox` では実送信される**（章 7.6 NFR-ENV-1）。Phase 2 以降は `F-039` に集約する）⑤アップロード・版の切替を監査ログに記録する。
...
- **処理**: ①`production` 以外では、画面最上部に環境名を固定表示する ②`demo` では「デモ環境 — 送信は行われません」を表示する ③`sandbox` では「サンドボックス環境 — **取引先への提案・契約書・署名依頼は送信されません。招待・期限のお知らせはお使いのアドレスに届きます。**それ以外は本番と同じ動作です」を表示する（宛先による区別。章 7.6 NFR-ENV-1 / `CLAUDE.md` §11.1）④主平面・管理平面の両方に表示する。
- **出力**: 環境バナー表示。
- **AI 利用**: なし。
- **外部 API 依存**: なし。
- **受け入れ基準**:
  - AC-1 `production` 以外のすべての画面（主平面・管理平面・モバイルビューポートを含む）で環境表示が視認でき、スクロールしても消えない。
  - AC-2 `demo` と `sandbox` で表示文言が異なり、`sandbox` では「**業務上の外部送信（取引先・第三者宛の提案 / 面談調整 / 契約書 / 署名依頼）だけが疑似であり、自分のアドレス宛のメール（招待・期限のお知らせ）は実際に届く**」ことが読み取れる。「メールは一切送信されない」と誤読させる文言にしない。
...
- **処理**: ①終了が決定した時点（`ENDING`）でエンジニアを「待機予定」とし、**稼働可能時期を満了日として設定する**（`Engineer` の属性。章 6.2。パートナー所属エンジニアの場合、この属性はそのパートナーが自社台帳で参照・編集できる。`AC-5`）②満了日到来で `ENDED` とし、エンジニアを探索母集団に戻す ③案件側は「後任募集」の状態にし、① の案件台帳に戻す ④**担当者の操作を要さない** ⑤還流を監査ログに記録する。
...
- **受け入れ基準**:
  ...
  - AC-6 🔴 **合成データの投入とリセットは `APP_ENV` が `demo` の場合にのみ実行できる。** `production` / `sandbox` / `staging` では管理平面に導線が表示されず、API を直接呼んでも拒否される（`BR-63` / `BR-37` / `CLAUDE.md` §10.5 / §11.1）。リセットは対象テナントの全業務データの削除にあたるため、環境ガードを実装の注意ではなく実行前の判定として持つ。
...
- **処理**: ①テナントを `SANDBOX` で開設し、有効期限を持たせる ②テナント分離・パートナースコープ・監査ログ・暗号化・保持期間を**本番と同じ規律**で適用する ③**外部送信は「宛先が誰か」で分ける** — 取引先・第三者が宛先となる**業務上の外部送信**（提案メール `F-022` / 面談調整 `F-041` / 契約書送付 `F-047` / 電子署名依頼 `F-049`）はモックとし、**宛先が当該テナントの利用者本人であるアカウント運用のメール**（招待 `F-002` / パスワード再設定 `F-003` / 期限予告 `F-054` / 削除予告 `F-064` / スキャン失敗の周知 `F-011` / 上限接近の通知 `F-027`）は実送信する（章 7.6 NFR-ENV-1 / `CLAUDE.md` §11.1）。**列挙ではなく宛先で判定し、迷うものはモック側に倒す。** 列挙のうち Phase 1 に存在するのは `F-002` / `F-003` / `F-011` / `F-022` / `F-027` / `F-064` であり、残り（`F-039` / `F-041` / `F-047` / `F-049`）は**各機能のフェーズ到来とともに同じ判定基準の対象に加わる**。本機能の成立は Phase 1 に存在する送信のみで判定できる ④**本契約は `SANDBOX` → `ACTIVE` の状態遷移として行い、データを環境間で複製しない**（章 5.4 遷移 1）⑤**30 日の期限到来、または見送りの決定で `SANDBOX` → `CLOSING` に遷移する**（章 5.4 遷移 2）。以後の返却と削除（`CLOSING` → `PURGED`）は **`F-064`**（P0 / Phase 1）が担う ⑥期限接近を運営者と見込み客に通知する（**Phase 1 の通知手段はアプリ内表示とメール送信**。Phase 2 以降は `F-039` に集約する）。
- **出力**: テナントのライフサイクル状態（`SANDBOX` / `ACTIVE` / `CLOSING`）、移行結果、通知、監査ログ。
- **AI 利用**: なし。
- **外部 API 依存**: メール送信・電子署名。🔴 **宛先で分ける** — 取引先・第三者宛の**業務上の外部送信はモック**、当該テナントの利用者本人宛の**アカウント運用のメール（期限予告を含む）は実送信**（処理 ③ / `AC-2` / `AC-9` / 章 7.6 NFR-ENV-1）。それ以外（AI・ストレージ・スキャン）は本番同等。
- **受け入れ基準**:
  ...
  - AC-2 `SANDBOX` からの**業務上の外部送信**（提案メール・面談調整の連絡・契約書の送付・電子署名依頼）が、**実在する取引先・第三者の宛先に 1 通も到達しない**（`BR-45`）。それ以外の機能は本番と同じ挙動を示す。**本 AC の射程はアカウント運用のメールを含まない**（`AC-9`）。
  ...
  - AC-9 🔴 **`sandbox` の期限予告は、見込み客本人（当該テナントに所属する利用者）のメールアドレスに実際に届く。** 予告メールの送信された全通について、宛先が当該テナントの利用者のアドレスのみであり、**取引先・エンジニアのアドレスが 1 件も含まれない**（章 7.6 NFR-ENV-1 / `CLAUDE.md` §11.1）。モック化を理由に**本人が期限に気づけない状態にならない**（`AC-2` の「0 通」は本 AC に適用しない）。同じことが招待（`F-002`）・パスワード再設定（`F-003`）・削除予告（`F-064 AC-10`）・スキャン失敗の周知（`F-011`）・上限接近の通知（`F-027`）にも成立する。
...
- **AI 利用**: なし。
- **外部 API 依存**: オブジェクトストレージ（原本の削除、CSV の生成と受け渡し）。メール送信（削除予告・完了の通知）。**宛先はテナント管理者（`SANDBOX` の場合は見込み客本人）であり、そのテナントに所属する利用者であるため、`sandbox` では実送信される**（`development` / `demo` ではモック。章 7.6 NFR-ENV-1 / `CLAUDE.md` §11.1）。
- **受け入れ基準**:
  ...
  - AC-10 削除の実行前に、テナント管理者（`SANDBOX` の場合は見込み客）へ予告が通知される。**通知されないまま削除が実行される経路が存在しない。** 🔴 **本 AC は `sandbox` でも成立する** — 予告メールの宛先はそのテナントの利用者本人であり、`sandbox` では実送信されるため、モック化によって予告が届かない状態にならない（`F-054 AC-9` / 章 7.6 NFR-ENV-1）。
...
|---|---|---|---|---|---|---|
| `SANDBOX` | ○ | ○ | ○ | ○（**取引先・第三者宛の業務上の外部送信は必ずモック**。テナント利用者宛のアカウント運用メールは実送信。`BR-45` / NFR-ENV-1 / `F-054 AC-2` `AC-9`） | ○ | 保持。統制は `production` と同一（`BR-62`） |
| `ACTIVE` | ○ | ○ | ○ | ○ | ○ | 保持 |
| `SUSPENDED` | ○ | ○ | ○ | 🔴 **不可** | ○ | 🔴 **消さない** |
...
| 見込み客サンドボックス | `sandbox` | 🔴 **宛先で分ける**（`CLAUDE.md` §11.1）。**業務上の外部送信（提案 / 面談調整 / 契約書送付 / 電子署名）はモック。アカウント運用のメール（招待 / パスワードリセット / 期限予告 / 削除予告 / スキャン失敗の周知 / 上限接近の通知）は実送信**。AI・ストレージ・スキャンは本番同等 | 🔴 **本番と同じ**（分離・記録・暗号化・保持期間） | **見込み客の実データ** |
| ステージング | `staging` | 各サービスの sandbox | 本番同等 | 合成 |
| 本番 | `production` | 実 | 本番 | 顧客の実データ |

🔴 **守るべき危険（1 行）**: **本番以外の環境から、実在する取引先・第三者へ、業務上の提案メール・面談調整・契約書・電子署名依頼が到達すること。** これを設計で物理的に不可能にする。**利用者本人宛のアカウント運用メールはこの危険にあたらない**（NFR-ENV-1）。

**受け入れ基準**:

- **NFR-ENV-1**（`BR-45`）: 🔴 **守るべき条件は「実在する取引先・第三者（取引先企業・エンド企業・エンジニア本人の実アドレス）宛に、業務上の外部送信が 1 通も到達しない」ことである。** `BR-45` の射程は**宛先が第三者である送信**であり、そのテナント自身の利用者に宛てたアカウント運用のメールは射程外である（`docs/01` `BR-45` / `CLAUDE.md` §11.1）。
  - 🔴 **この区別は列挙ではなく宛先で判定する。** 判定基準は「**宛先が、そのテナントに所属する利用者（招待中の本人を含む）か**」の一点である。**本書の列挙に無い送信が新たに増えた場合も、列挙を増やすのではなく宛先で決める。判断に迷うものは業務上の外部送信（モック）側に倒す**（`CLAUDE.md` §11.1）。
  - **業務上の外部送信**（宛先が取引先・第三者。モック側）: 提案メール（`F-022`）、面談日程調整の連絡（`F-041`）、契約書の送付（`F-047`）、電子署名依頼（`F-049`）。
  - **アカウント運用のメール**（宛先がそのテナントの利用者。実送信側）: 招待（`F-002`）、パスワード再設定（`F-003`）、`sandbox` の期限予告（`F-054`）、削除予告（`F-064`）、スキャン失敗の担当者周知（`F-011` 処理 ④）、上限接近の通知（`F-027` 処理 ④）、アプリ内 / メール通知（`F-039`。受信者は全テナントロール）。
  - **`development` / `demo`**（🔴 **区別を適用せず全送信系モック**）: E2E で上記の**両群すべて**を実行し、**外部エンドポイントへの発信が 0 件**であることを検証する。`demo` は合成データのみを扱い、宛先自体が架空であるため区別する必要がない（`CLAUDE.md` §11.1「この区別は `sandbox` にのみ適用する」）。
  - **`sandbox`**（🔴 **宛先で分ける**）: 次の 2 点を**別々の受け入れ基準として**検証する。
    - ① **業務上の外部送信**（`F-022` / `F-041` / `F-047` / `F-049`）を E2E で実行し、**外部エンドポイントへの発信が 0 件**であることを検証する。
    - ② **アカウント運用のメール**（`F-002` / `F-003` / `F-011` / `F-027` / `F-039` / `F-054` / `F-064`）を E2E で実行し、**実際に送信されること**、および送信された全通の宛先が**当該テナントに所属する利用者（招待中の本人を含む）のアドレスのみ**で、**取引先・エンド企業・エンジニアのアドレスが 1 件も含まれない**ことを検証する。①の「0 件」を②に適用しない（**本人に届かないと `sandbox` に入れず、期限にも気づけない**ため。`F-054 AC-9` / `F-064 AC-10`）。
  - **`staging`**（外部 API は各サービスの sandbox）: 両群の E2E を実行し、**各サービスの sandbox エンドポイント以外への発信が 0 件**であること、および sandbox エンドポイントへ渡す宛先が**テスト用アドレスのみで、実在の取引先ドメインを 1 件も含まない**ことを検証する。「`staging` では発信が 0 件」と読み替えない。
...
- **NFR-ENV-6**（`BR-63`）: `demo` と `sandbox` が同一環境・同一テナントで兼用されない。`demo` に見込み客の実データを投入する経路が存在せず、`sandbox` に合成データが投入されない（`F-053 AC-4`）。**合成データの投入とリセットは `APP_ENV` が `demo` の場合にのみ実行でき、`production` / `sandbox` / `staging` では導線が存在せず API も拒否される**（`F-053 AC-6`）。
...
| **A-22** | 🔴 **`sandbox` におけるメール送信の射程を「宛先」で分ける。** 宛先が取引先・第三者である**業務上の外部送信**（提案メール `F-022` / 面談調整 `F-041` / 契約書送付 `F-047` / 電子署名依頼 `F-049`）はモックとし、宛先が**そのテナントに所属する利用者（招待中の本人を含む）**であるアカウント運用のメール（招待 `F-002` / パスワード再設定 `F-003` / スキャン失敗の周知 `F-011` / 上限接近の通知 `F-027` / 通知 `F-039` / 期限予告 `F-054` / 削除予告 `F-064`）は実送信する。**この区別は列挙ではなく宛先で判定し、迷うものはモック側に倒す。** `development` / `demo` には区別を適用せず全送信系モックとする | **章 7.6**（環境表の `sandbox` 行 / **NFR-ENV-1**）/ `F-001` `F-002` `F-003` `F-011` `F-027` `F-064` の外部 API 依存・処理 / **`F-028` 処理 ③ / `AC-2`**（バナー文言）/ **`F-054` 処理 ③ / `AC-2` / `AC-9`**（新設）/ **`F-064` 外部 API 依存 / `AC-10`** / 章 5.4 の `SANDBOX` 行 / `UC-22` 手順 3・5 / 付録 B の `BR-45` 行 | **`CLAUDE.md` §11.1 に暫定として反映済み。[Issue #9](https://github.com/Festal-KM/SES-Platform/issues/9) で確認中。** 本書は §11.1 を正として引き写しており、**判定基準（宛先が当該テナントの利用者か）を原則として書き、列挙に依存しない形にしている**。`BR-45` の射程は「実在する取引先・第三者への到達」であり、本人宛のアカウント運用メールは射程外である（`docs/01` `BR-45`）。確認の結果 §11.1 が変わった場合は §8.7 の手順で本書も追随する |
...
>
> **確認中（本書は既定を置いたうえで進める）**: **`sandbox` におけるメール送信の射程を「宛先」で分ける扱い**は **[Issue #9](https://github.com/Festal-KM/SES-Platform/issues/9)** で確認中である。本書は `CLAUDE.md` §11.1 を正とし、**宛先が取引先・第三者である業務上の外部送信はモック、宛先がそのテナントの利用者であるアカウント運用のメールは実送信**を既定として記述した（`## Assumptions` **A-22**。章 7.6 NFR-ENV-1 / `F-054 AC-9` / `F-064 AC-10`）。**§11.1 の改訂は §8.6 の人間承認事項**であり、本書からは追随のみを行う。`Q-` 番号は `docs/01` からの引き継ぎに使うため本項には振らない（A-21 と同じ扱い）。
>
...
| BR-13 | 章 8.2 再現性 / `F-031 AC-4` / `F-032 AC-5` / `F-034 AC-4` / `F-044 AC-4` | BR-45 | `F-022 AC-4` / `F-041 AC-3` / `F-049 AC-5` / `F-053 AC-5` / **`F-054 AC-2`（業務上の外部送信は 0 通）+ `F-054 AC-9`（本人宛は実送信。A-22）** / **NFR-ENV-1** |
...
8. **`APP_ENV` による起動時 DI の実現方式**（章 7.6 NFR-ENV-2〜4）— `production` でモックが選ばれたら起動失敗、非本番に本番キーがあれば起動失敗を、環境変数の検証でどう担保するか。あわせて 🔴 **`sandbox` だけは「宛先が当該テナントの利用者か」でメールの実送信 / モックが分かれる**（NFR-ENV-1 / A-22。Issue #9）。メール送信基盤の選定（`CLAUDE.md` §9-3）で、**送信ドメインの許可リストや宛先制限（allowlist）を基盤側で設定できるか**を裏取りし、アプリ層の分類だけに頼らない二重防御が組めるかを判断すること。
...
10. **`APP_ENV` による外部連携の差し替えを起動時 1 箇所に集約**（章 7.6 NFR-ENV-2）— リクエストごとの分岐にしない。`production` でのモック選択、非本番での本番キー設定は**起動失敗**にする。🔴 **`sandbox` のメールだけは「宛先が当該テナントの利用者か」で実送信とモックが分かれる**（NFR-ENV-1 / A-22）。これを送信箇所ごとの `if` で実装せず、**メール送信の単一経路に「宛先の分類」を必須の入力として持たせ、分類が未指定の送信をコンパイル時または実行前に成立させない**設計にする（分類を省略できると、新しい通知が既定で実送信側に落ちる）。既定値を置く場合は**モック側**に倒す。
