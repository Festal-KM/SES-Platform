...
> **最終改訂**: 2026-09-01（**越境経路 5「当事者レコードの参照」の追加**（Issue #8）/ `Tenant`・`Contract` の状態機械の確定（Issue #6 / #7）/ 試用環境のメール射程の確定（Issue #9 / #10）/ 電子署名の BYO 方式と DocuSign（Issue #11）/ テナント独自ドメインの送信認証（Issue #13）/ クォータの単位を件数に（Issue #12）/ 契約書の品質ゲート対象化（Issue #15）/ 代理閲覧中の操作不可 UI（Issue #14））。**改訂 2**: 2026-09-01（レビュー指摘の反映。①`F-001 AC-4` の独自ドメイン検証の対象から `F-049` を除外し `F-047 AC-7` との矛盾を解消 ②`ui-design` 申し送り 12 の画面 ID を `S-029`（稼働 / Phase 2）・`S-025`（契約 / Phase 3）に訂正 ③付録 A の申し送り 22 を現状に同期（`Q-2` を未決着から除き `Q-7` / `Q-20` を追加）④**AI の日次コスト上限到達時はゲートを未実行のまま保持し `GATE_RUNNING` に留める**（`GATE_FAILED` にしない。`CLAUDE.md` §4.2「失敗と保留を混同しない」）⑤章 4.2 に `F-043` と `F-065` の関係の補足を追加）。**状態・遷移・機能 ID の追加削除は無い。** **改訂 3**: 2026-09-10（🔴 **エンジニアの経験内容と従事期間（`EngineerCareer`）を Phase 1 の機能要件として明示**。[Issue #35](https://github.com/Festal-KM/SES-Platform/issues/35) = 回答「A」。`F-008`（入力 / 処理 ⑤〜⑦ / `AC-5`〜`AC-8`）/ `F-019`（処理 ② / `AC-5`）/ `F-017 AC-1` / `F-032` 処理 ⑤ / 章 6.2 / 章 6.4 / 章 6.8 / **A-24** / 申し送り。**機能 ID・ユースケース ID・状態・遷移の追加削除は無い**（既存 `AC-n` の番号も振り直していない）。**越境経路と匿名 5 項目は不変** — 経験内容は匿名候補に出ない）。 **改訂 4**: 2026-09-15（🔴 **匿名候補への検索条件は丸め後の区分で評価し、開示していない属性は適用しない**。`T-08-05` の実装で置いた判断。`F-009` 処理① / `F-017` 処理⑥ / **A-25**（暫定。[Issue #51](https://github.com/Festal-KM/SES-Platform/issues/51) で確認中）。**機能 ID・状態・遷移・越境 5 経路・匿名 5 項目・A-04 の粒度はいずれも不変**）。
...
| **BR** | `docs/01` 章 9 のビジネスルール。主要なもののみ記載（全対応は付録 B の `BR` カバレッジ表） |

🔴 **越境経路の対応**（`CLAUDE.md` §3.1 / `BR-05`）。**この 5 つ以外に越境経路を持つ機能は本書に存在しない。** **経路 5 は 2026-09-01 に人間の承認で追加された**（[Issue #8](https://github.com/Festal-KM/SES-Platform/issues/8)。`BR-65`〜`BR-69`）。**経路の追加、経路 4 の匿名 5 項目の追加、経路 5 の開示項目の追加は、いずれも人間の承認事項である**（`CLAUDE.md` §8.6）。
...
| **経路 5** | **当事者レコードの参照**（取引先が、自社所属エンジニアの稼働と、自社が当事者の契約・契約書・発注を**読み取りのみ**で参照する） | **`F-065`**（稼働。Phase 2）/ **`F-066`**（契約・契約書・発注。Phase 3） |

### 1.2 主平面（テナント平面）

| ID | 機能 | 優先度 | フェーズ | 平面 | ステージ | 主な BR |
|---|---|---|---|---|---|---|
| `F-001` | テナント開設と初期設定 | P0 | Phase 0 | 主 / 管 | - | BR-01 / BR-02 |
...
| `F-007` | 取引先企業の招待とパートナーアカウント管理 | P0 | Phase 1 | 主 | ① | BR-04 / BR-27 / BR-45 |
| `F-008` | エンジニア台帳の登録・編集（**経験内容と従事期間 `EngineerCareer` を含む**） | P0 | Phase 1 | 主 | ① | BR-04 / BR-52 / BR-55 / BR-06 |
...
| `F-014` | 案件の公開範囲設定（**越境経路 1**。**公開後の再検査と自動解除を含む**） | P0 | Phase 1 | 主 | ① | BR-05 / BR-15 / BR-18 / BR-27 |
...
| `F-065` | 取引先による自社エンジニアの稼働の参照（**越境経路 5**） | P1 | Phase 2 | 主 | ⑥ | BR-65〜BR-69 / BR-07 |
| `F-066` | 取引先による自社が当事者の契約・契約書・発注の参照（**越境経路 5**） | P2 | Phase 3 | 主 | ⑤ | BR-65〜BR-69 / BR-07 |
...
| Phase 1: 「案件登録 → パートナーへ公開 → パートナーが提案 → ゲート実行 → ホストが承認 → 送信 → 結果記録」を E2E で完遂できる | **`F-001`（送信ドメインの検証。`AC-4`。`BR-71`。これが未完のテナントは取引先へ送れないため、E2E の前提条件になる）** `F-007` `F-008` `F-010` `F-011` `F-013` `F-014` `F-015` `F-019` `F-020` `F-021` `F-022` `F-023`（送信経路の一部。`BR-22` の人手再送）`F-024` `F-025` |
...
- **入力**: 企業名・商号、契約プラン、環境種別（`production` / `sandbox` / `demo`）、初期 `OWNER` の氏名とメールアドレス、テナント設定（タイムゾーン、通貨は日本円固定）、🔴 **取引先へ届く送信に用いるテナント独自ドメイン**（`BR-71`）。
- **処理**: ①運営者（`F-055`）または初期セットアップ経路からテナントを作成する ②初期 `OWNER` を 1 名作成し招待メールを送る ③テナント既定設定（自動承認 = 無効、AI ロール承認モード = すべて都度承認、公開範囲の既定 = 非公開、**送信ドメインの検証状態 = 未検証**）を投入する ④開設を監査ログに記録する ⑤🔴 **送信ドメインの登録と、顧客が設定すべき DNS レコードの提示 → 検証完了の確認までを、開設フローの工程として持つ**（`BR-71` / `docs/03` §3.2.7 規律 3）。**検証が完了するまで、そのテナントは「取引先へ送信できない状態」として扱う**（機能を隠さず理由とともに示す）⑥検証状態の変化（未検証 / 検証中 / 検証済み / 失効）を監査ログに記録し、`F-059` の監視対象に載せる。
- **出力**: 開設済みテナント、初期 `OWNER` の招待、テナント設定の初期値、**送信ドメインの検証状態と DNS 設定の指示**、監査ログ。
- **AI 利用**: なし。
- **外部 API 依存**: メール送信（招待）。**招待の宛先は初期 `OWNER` 本人（ホスト所属利用者 = 章 7.6 NFR-ENV-1 の分類 1）であるため、`sandbox` では実送信される**。`development` / `demo` ではモック（`F-028` / 章 7.6 NFR-ENV-1 / `CLAUDE.md` §11.1）。
- **受け入れ基準**:
  ...
  - AC-4 🔴 **開設フローに「送信ドメインの登録 → DNS レコードの提示 → 検証完了の確認」の工程があり、検証が完了していないテナントでは、自テナントのメール送信基盤から取引先へ届く送信（`F-007` の取引先招待 / `F-022` / `F-041` / `F-047` のメール送付）が実行できない**（`BR-71` / NFR-ENV-10）。未検証のまま送信を要求すると、対象は状態を進めず（`Proposal` は `APPROVED` のまま、`Contract` は `DRAFT` のまま）、**「送信元ドメインが未検証である」ことと、設定すべき DNS レコードが実行者に表示される**。共通ドメインへのフォールバックが存在しない（`BR-46` と同じ考え方）。🔴 **`F-049`（電子署名依頼）はこの列挙に含まない** — メールを送るのは電子署名サービス側でありテナントのメール送信基盤を通らないため、**前提条件は独自ドメインの検証ではなく電子署名アカウントの接続である**（`F-049 AC-8` / `F-047 AC-7`。`docs/03` §3.2.7 の ⚠️）。**独自ドメインが未検証でも、電子署名が接続済みであれば `F-049` は実行でき、⑤ 契約は完了できる**（`BR-70` / `F-049 AC-9`）。
  - AC-5 テナント所属利用者宛（`F-002` の自社メンバー招待 / `F-003`）と運営者宛（`F-055`）の送信は、送信ドメインの検証状態に依存せず実行できる（`docs/03` §3.2.7 規律 2）。**`F-002`（自社メンバーの招待）と `F-007`（取引先の招待）を同じ扱いにしない。**
- **関連ロール**: `PLATFORM_OWNER`（作成）、`OWNER`（初期設定・**送信ドメインの登録**）、`ADMIN`（送信ドメインの検証状態の確認）。
...
- **AI 利用**: なし。
- **外部 API 依存**: メール送信（招待・ロール変更通知）。上限は `F-027` のガードに従う。🔴 **本機能の宛先はテナント所属利用者であるため、共通ドメインから送ってよく、送信ドメインの検証状態に依存しない**（`F-001 AC-5` / `docs/03` §3.2.7 規律 2）。**取引先企業の担当者を招く `F-007` は別扱いであり、独自ドメインの検証が前提になる**（`F-007 AC-5`）。🔴 **`sandbox` での扱いは宛先の分類で分かれる**（章 7.6 NFR-ENV-1 の判定表 / `CLAUDE.md` §11.1）。**ホスト所属利用者（`OWNER` / `ADMIN` / `SALES` / `VIEWER`。招待中の本人を含む）宛は分類 1 で実送信**。**パートナー所属利用者（`PARTNER_ADMIN` / `PARTNER_SALES`）宛は分類 2 でモックとし、代わりに招待リンクを画面に表示する**（取引先招待の起点は `F-007`。`F-007 AC-4`）。`development` / `demo` では分類を問わずモック。
- **受け入れ基準**:
...
- **AI 利用**: なし。
- **外部 API 依存**: メール送信（パスワード再設定等）。🔴 **`sandbox` での扱いは宛先の分類による**（章 7.6 NFR-ENV-1 の判定表）— **ホスト所属利用者本人宛（分類 1）は実送信**、**パートナー所属利用者宛（分類 2）はモック**とし、後者の再開通はホストの `ADMIN` が招待を再発行し、画面に表示されたリンクを渡すことで行う（`F-007 AC-4`）。`development` / `demo` ではいずれもモック。
- **受け入れ基準**:
...
- **目的**: `CLAUDE.md` §3.1 の二重境界を、機能として全画面・全経路に適用する。**越境は 5 経路のみ**とし、パートナー同士が相互参照できる経路を 1 つも作らない（`BR-01`〜`BR-08` / `BR-53`〜`BR-59` / **`BR-65`〜`BR-69`**）。
- **入力**: 認証コンテキスト（テナント / パートナー所属 / ロール）、参照対象の種別と識別子。
- **処理**: ①すべての業務データ参照に対しテナント境界を適用する ②パートナー所属の利用者にはパートナー境界を重ねて適用する ③越境が許されるのは経路 1〜5 の定義に合致する場合のみで、それ以外は 0 件として扱う（**経路 5 は読み取りのみ**） ④存在しないものとして扱うため、件数・存在の示唆も返さない ⑤**テナントのライフサイクル状態（章 5.4）が `SUSPENDED` / `CLOSING` の場合、境界判定と同じ経路で実行系の操作を拒否する**（ロールの権限より優先する）。
...
- **受け入れ基準**:
  ...
  - AC-5 越境が成立するのは経路 1（`F-014`）・経路 2（`F-019`）・経路 3（`F-038`）・経路 4（`F-016`〜`F-018`）・**経路 5（`F-065` / `F-066`。当事者レコードの読み取り参照）**に限られ、他の機能はいずれも越境読み取りを行わない。**経路 5 は読み取り専用であり、越境した書き込み経路は 1 つも存在しない**（`BR-68`）。
...
- **処理**: ①ホストの `ADMIN` が取引先企業を登録する ②`PARTNER_ADMIN` を招待する（`F-002`）③受諾後、そのパートナー企業に紐づくアカウントを `PARTNER_ADMIN` が自ら追加できる ④停止時は配下アカウントの業務操作を止める（データは残す）⑤登録・招待・停止を監査ログに記録する ⑥🔴 **`sandbox` では招待メールを送らず（宛先の分類 2。章 7.6 NFR-ENV-1）、招待リンクを招待一覧の画面に表示してコピーできるようにする。** 見込み客は自社の同僚などを疑似的な取引先として招き、パートナー側の画面まで確認できる。**リンクの有効期限・1 回限りの受諾・受諾後の失効は `production` の招待と同一の規律とする。**
- **出力**: パートナー企業、配下アカウント、招待の状態、監査ログ、**`sandbox` では招待リンクの画面表示**。
- **AI 利用**: なし。
- **外部 API 依存**: メール送信（招待）。🔴 **`sandbox` ではモック**（宛先はパートナー所属利用者 = 分類 2。実在する取引先企業の担当者であるため。章 7.6 NFR-ENV-1 / `F-002` 外部 API 依存欄）。`development` / `demo` でもモック。`production` / `staging` は実送信（`staging` は各サービスの sandbox エンドポイント）。
- **受け入れ基準**:
  ...
  - AC-4 🔴 **`sandbox` において、取引先招待のメールが外部エンドポイントへ 1 通も発行されない**（章 7.6 NFR-ENV-1 ③）。**代わりに招待リンクが招待一覧の画面に表示され、コピーできる。** そのリンクから受諾した利用者は `PARTNER_ADMIN` としてログインでき、以後のパートナー側の画面・境界の挙動が `production` と同一である（`F-054 AC-1` のパートナースコープ検証はこの経路で成立する）。リンクは受諾後に失効し、2 回目の受諾はできない。`production` ではリンクの画面表示を行わず、招待は実送信のみで成立する。
  - AC-5 🔴 **`production` において、送信ドメインの検証（`F-001 AC-4`）が完了していないテナントでは取引先招待メールを送信できない**（`BR-71`。宛先が取引先企業の担当者であるため。`docs/03` §3.2.7 規律 1）。実行者には「送信元ドメインが未検証である」ことと設定すべき DNS レコードが表示され、共通ドメインで送られることはない。**招待そのものは作成できるが、送達は検証完了後になる。**
- **関連ロール**: `OWNER` / `ADMIN`（登録・招待・停止、**`sandbox` での招待リンクの取得**）、`PARTNER_ADMIN`（自社配下の管理）。

#### F-008 エンジニア台帳の登録・編集

- **目的**: 自社・取引先の双方のエンジニアを、同じ土俵で検索できる形に整える（`docs/01` 章 6.1 / `BR-04` / `BR-52`）。あわせて 🔴 **経験内容と従事期間（`EngineerCareer`）を構造化して保持し、提案時に行単位で凍結できる状態を作る**（`BR-52` の収集範囲に含まれる項目。**決定済み（2026-09-10、[Issue #35](https://github.com/Festal-KM/SES-Platform/issues/35) = 回答 A）**。A-24）。
- **入力**: 氏名（社内表示用）、所属区分（自社 / パートナー）、スキルと経験年数・レベル、🔴 **経験内容と従事期間（`EngineerCareer`。1 行 = 期間〈開始年月・終了年月。終了は「継続中」を取りうる〉/ 役割 / 業務内容 / 使用技術。1 エンジニアが複数行を持てる。Phase 1 は手入力）**、稼働可能時期、単価レンジ、希望条件、勤務地・リモート可否、連絡先（必要最小限）、稼働状況。
- **処理**: ①入力項目は `BR-52` の範囲（SES の営業判断に必要な情報）に限る ②スキルは `F-010` の辞書から選ぶ（自由入力は別名候補として起票）③登録者の所属に応じて所有パートナーを自動で確定する（入力で指定させない）④作成・更新・削除を監査ログに記録する ⑤🔴 **経験内容は行単位で追加・編集・削除でき、表示順は期間の降順の決定的順序とする**（自由文の 1 塊として保持しない）⑥🔴 **Phase 2 の `F-032`（`sheet-parser`）の抽出結果は、この同じ項目構成へ採否確認を経て反映される**（別の保存先を新設せず、手入力と AI 抽出で二重入力を作らない）⑦🔴 **経験内容は匿名共有（`F-017`）の開示対象に含めない**（`BR-55`）。
- **出力**: エンジニア台帳のレコード、**経験内容の行集合（`EngineerCareer`）**、スキル関連、稼働状況、監査ログ。
...
- **受け入れ基準**:
  ...
  - AC-5 経験内容の 1 行が**期間（開始年月・終了年月。終了は「継続中」を取りうる）/ 役割 / 業務内容 / 使用技術**の 4 項目を持ち、1 人のエンジニアに複数行を**登録・編集・削除**できる。ある行の削除・編集が他の行の内容を変えない。行の追加・更新・削除が監査ログに残る。同一データに対する表示順は実行のたびに同じである（期間の降順、同期間は登録順）。**経験内容が 0 行のエンジニアも登録できる**（必須にしない。スキルシートのみで運用している既存データを壊さないため）。
  - AC-6 🔴 **`Proposal` の作成時、経験内容は `EngineerSnapshot` へ「行単位」で凍結される**（`F-019 AC-5`）。凍結後に台帳側の行を編集・削除しても、既存提案が保持する行の内容・行数は変化しない。**行単位でなければならない理由**: ①Phase 2 のマッチングスコア（`F-029`）は期間・使用技術を**項目として**参照する ②越境経路 4 の匿名化制御（`F-017 AC-1`）は「経歴の並びが 1 項目も出ていない」ことを**項目単位で**示す必要がある。自由文や 1 塊の構造で凍結すると、①②のいずれでも後から分解が必要になり、**分解結果が凍結時点の値と一致することを証明できない**（凍結の意味が失われる）。
  - AC-7 🔴 **経験内容は、匿名候補（`F-017`）の表示・API 応答・エクスポート・根拠文のいずれにも 1 項目も現れない**（`BR-55` / `BR-59`。匿名 5 項目に経歴は含まれない。**細かい経歴の並びは同一人物を案件をまたいで追跡させる代表的な情報である**）。ホストが経験内容に到達できるのは、`Proposal` が作成され実名が開示された後の `EngineerSnapshot` 経由に限る（`F-018 AC-3` / `F-019 AC-1`）。**表示項目を 5 から増やすことは人間の承認事項**（`CLAUDE.md` §8.6）。
  - AC-8 Phase 2 で `F-032` の抽出結果を台帳へ反映する際、反映先は AC-5 と同一の項目構成であり、**手入力済みの行が黙って上書きされない**（追加 / 置換を人が選び、選択の結果が記録される。`F-032 AC-3`）。同じ経歴を手入力用と抽出用の 2 か所へ入れる導線が存在しない。
    - 🔴 **実装時期**: **`T-09-12`（SP-09 の先頭タスク）。Phase 1（第 1 回リリース）に含む。** 提案フローより前に置く理由は、**凍結が遡れない**ためである — 元データが無いまま `F-019` を動かすと、経験内容が空の `EngineerSnapshot` を作り続け、**作成済みのスナップショットは後から埋められない**（AC-6 / `F-019 AC-2`）。
...
- **処理**: ①検索条件を境界適用後の母集団に対して評価する（ホスト = 自社台帳 + `F-017` の匿名候補、パートナー = 自社台帳のみ）。🔴 **匿名候補に対する評価は A-04 の丸め後の区分に対して「区分が条件と重なりうるか」で行い、開示していない属性（フリーワード / 稼働状況）は匿名候補に適用しない**（A-25。生値で評価すると `yearsMin=7` と `8` の当たり方の差から「7 年」が、開示していない属性の当たり外れが 6 項目目の開示になる） ②**Phase 1 の並び順は、検索条件への適合と更新日時による決定的な順序**とする。重みという概念を持たない ③Phase 2 では `F-029` のスコア順に切り替わる ④匿名候補は `F-017` の表示規則に従って混在表示する。
...
- **外部 API 依存**: なし。
- **受け入れ基準**:
  - AC-1 新語候補は、**`OWNER` / `ADMIN` / `SALES`** のいずれかが明示的に採用するまで検索の正規化に使われない。
    - ⚠️ **`OWNER` は 2026-09-06 に追加した（暫定。[Issue #36](https://github.com/Festal-KM/SES-Platform/issues/36) で確認中）。** 当初の記述は `ADMIN` / `SALES` の 2 ロールだったが、**章 4.2 の権限マトリクスは `F-010` の `OW` を `●` としており食い違っていた**。既定 A（`OWNER` を追加してマトリクス側に寄せる）で実装する（`docs/dev-plan.md` §9 / SP-06 の T-06-01）。回答が来たら本行と `docs/04` §S-009 / `docs/05` §6.4 #24 を同時に更新する。
  - AC-2 テナント固有の別名は他テナントに影響しない。グローバル辞書はテナントから編集できない（`BR-02` の射程）。
  - AC-3 別名の採用・却下が監査ログに残る。
- **関連ロール**: **`OWNER`** / `ADMIN` / `SALES`（採否。`OWNER` は暫定。[Issue #36](https://github.com/Festal-KM/SES-Platform/issues/36)）、`PARTNER_ADMIN` / `PARTNER_SALES`（候補の起票のみ）、`VIEWER`（閲覧のみ）。

#### F-011 スキルシートのアップロードと版管理

- **目的**: スキルシートを版付きで保管し、**安全と判定されるまで外部に渡らない**状態を保つ（`BR-26` / `BR-15`）。
- **入力**: ファイル（xlsx / docx / pdf 等）、対象エンジニア、版のメモ。
- **処理**: ①アップロードを受け付け、版を採番する ②ウイルススキャンを実行し、結果が `CLEAN` になるまで共有 URL を発行しない ③`CLEAN` になった版のみ最新版フラグを持てる ④スキャン失敗・感染検出時は隔離し、担当者と `F-059` に通知する（**Phase 1 の担当者への周知はアプリ内表示とメール送信**（`F-001` / `F-002` と同じ送信経路）。🔴 **`sandbox` でのメールの扱いは宛先の分類で分かれる — 担当者がホスト所属（分類 1）なら実送信、パートナー所属（分類 2）ならモック**とし、後者はアプリ内表示のみで周知する（章 7.6 NFR-ENV-1）。**アプリ内表示は分類によらず必ず行うため、パートナーの担当者が隔離に気づけない状態にならない。** Phase 2 以降は `F-039` に集約する）⑤アップロード・版の切替を監査ログに記録する。
  - ✅ **処理④の「担当者」の定義（決定済み（2026-09-06、T-05-08））**: 🔴 **そのファイルの所有側の管理ロール**に限る——ホスト所有なら `OWNER` / `ADMIN`、パートナー所有ならその取引先の `PARTNER_ADMIN`。🔴 **所有側を越えない**（取引先所有の隔離をホストへ送ると、パートナーの台帳にファイルが存在することを漏らす。`BR-06`）。`SALES` / `PARTNER_SALES` / `VIEWER` は含めない（ホストの `SALES` は 3〜30 名おり、他人のファイルの隔離を全員に配ると通知が読まれなくなる）。—— **アップロードした本人はメールの宛先に含めないが、アプリ内表示（分類によらず必ず行う）で必ず気づける**。🔴 **周知メールの本文に内容を載せない**（アプリへのリンクのみ。氏名・版・ファイル名を載せない。`docs/05` §9.6.1）
- **出力**: スキルシートの版、スキャン結果、最新版フラグ、監査ログ。
- **AI 利用**: なし（本文の構造化抽出は `F-032`）。
- **外部 API 依存**: ウイルススキャン（`production` 以外の扱いは章 7.6）。**メール送信**（処理 ④ の担当者周知。上限は `F-027`）。🔴 **`sandbox` ではホスト所属の担当者宛（分類 1）は実送信、パートナー所属の担当者宛（分類 2）はモック**（章 7.6 NFR-ENV-1 の判定表）。`development` / `demo` ではいずれもモック。
- **受け入れ基準**:
...
- **目的**: 案件をどの取引先に見せるかを**明示的に**指定する。既定で広げない（`BR-05` 経路 1 / `BR-15` / `BR-18` / `BR-27`）。🔴 **検査の射程は「公開の瞬間」に限らない** — 公開中の案件であっても、取引先に見えている内容が後から商流情報（エンド企業名・内部単価・他社名）を含む状態になってはならない（**決定済み。2026-09-10、[Issue #42](https://github.com/Festal-KM/SES-Platform/issues/42) = 回答②**。`## Assumptions` A-26）。
- **入力**: 案件、公開先パートナー企業の集合、公開する項目のプロファイル（外部公開用の記載）、公開 / 非公開の切替、🔴 **公開後の「公開欄」の編集**（**3 欄 = 案件名 / 外部公開用の記載 / `ProjectRequirement` の自由記述**。編集そのものは `F-013` で行う）。
- **処理**: ①公開先を明示的に選択する（既定は誰にも公開されない）②**公開が成立する時点で**品質ゲート（`F-020`）を実行し、PII 層・商流層で FAIL したら公開しない ③公開範囲の変更を監査ログに記録する ④公開解除時、その案件は対象パートナーの一覧から消える（作成済みの提案は残る）⑤🔴 **公開中の案件で公開欄 3 欄のいずれかが実際に変更されたら、品質ゲート（`F-020`）を再実行する。商流層が FAIL なら公開を解除する**（対象パートナーの公開範囲が落ち、④ と同じ状態になる）。**PASS なら公開をそのまま維持する** ⑥**公開先の増減は再検査の契機にしない**（増えた公開先に対する検査は、その公開が成立する時点で ② が行う）⑦自動解除の事実と理由（どの欄が原因か）をホスト側に表示・通知し、**取引先には理由を出さない** ⑧AI の日次コスト上限に達していてゲートを実行できない場合は、**再検査を未実行のまま保留し、公開を維持する**（`F-027 AC-5`）。
- **出力**: 公開範囲、公開用の案件表示、ゲート結果、**再検査による自動解除の記録と理由（ホスト向け）**、監査ログ。
- **AI 利用**: `gate-inspector`（`F-020` 経由）。🔴 **公開後の再検査も同じ経路を通り、1 回の再検査が `F-026` に 1 件として計上される**（コスト影響。**公開欄 3 欄が実際に変わったときだけ実行するため、保存のたびには計上されない** — AC-6）。**失敗時のフォールバックは `F-020` に従う**（LLM の失敗・タイムアウト・スキーマ違反による判定不能は商流層 FAIL として扱い、AC-7 に従って公開を解除する。**AI の日次コスト上限による未実行はこれに含まない** — AC-12）。
- **外部 API 依存**: なし（通知は `F-039`。Phase 1 の通知手段はアプリ内表示とメール送信で、`F-027` と同じ経路）。
- **受け入れ基準**:
  - AC-1 公開範囲に含まれないパートナーの画面・検索・件数・通知のいずれにも、その案件が現れない。
  - AC-2 公開範囲は「全公開」を既定値として持たない。新規案件の初期状態は非公開である。
  - AC-3 公開された案件の表示に、エンド企業名・内部単価・他社名が含まれない（`F-020` の商流層 FAIL となり公開できない）。🔴 **本 AC は公開が成立する時点だけでなく、公開が続いている間ずっと成立する**（公開後の編集は AC-6〜AC-8 が受け持つ）。
  - AC-4 公開先パートナーは、同じ案件に公開されている他のパートナーの社名・件数を知る手段を持たない（`BR-07`）。
  - AC-5 公開範囲の変更が、実施者・変更前後の公開先とともに監査ログに残る。
  - AC-6 🔴 **再検査の契機は「公開欄 3 欄（案件名 / 外部公開用の記載 / `ProjectRequirement` の自由記述）のいずれかが実際に変更されたこと」に限る。** 3 欄の値が変わらない保存（3 欄以外の属性だけの編集、同一値での再保存、公開先の追加・削除）では再検査が実行されず、`F-026` の AI 利用量も増えない。
  - AC-7 公開中の案件で 3 欄のいずれかが変更され、再検査の**商流層が FAIL** となった場合、**その案件の公開が解除される** — 公開先だったパートナーの画面・検索結果・件数・並び順・通知のいずれにも現れず（AC-1 と同じ状態）、URL を直接開いても「存在しない」として扱われる（`F-004 AC-3`）。**FAIL の内容が公開されたまま残る経路が存在しない**（`BR-15` / `BR-18`）。🔴 **PII 層の FAIL も同じく公開を解除する** — 公開が成立する時点の規律（処理 ②）と揃える（**公開中だけ PII 層の FAIL を許す理由が無い**。[Issue #42](https://github.com/Festal-KM/SES-Platform/issues/42) の決定は商流層を明示したものであり、本書は 3 欄の再検査で FAIL した層を区別しない。A-26 の既定③）。
  - AC-8 再検査が全層 PASS の場合、**公開はそのまま維持される。** 解除 → 再公開の往復が発生せず、公開先の画面から案件が一時的にも消えない。
  - AC-9 自動解除が起きたとき、ホスト側に「検査で問題が見つかったため公開を解除した」ことと**どの欄が原因か**が示され、`F-020` の指摘に辿れる。🔴 **指摘を無視して公開を続ける / 再公開する操作・API・設定が存在しない**（`BR-18` / `F-020 AC-2`）。解消手段は 3 欄の修正と再検査のみである。
  - AC-10 🔴 **取引先側には、解除の理由も、解除が起きたこと自体も表示されない。** 取引先から見えるのは「公開されていない」状態だけであり、はじめから公開されていない案件（AC-1）と区別できない。**ホスト内部の検査結果・指摘が、取引先の画面・通知・API 応答・エクスポートのいずれにも現れない**（`BR-07`。ホスト内部の検討を出さない点は `BR-67` と同じ考え方）。
  - AC-11 自動解除が監査ログに残り、**実施者が `system`、変更前後の公開先が記録される**（AC-5 と同じ形式）。🔴 **人の操作による公開解除と、再検査による自動解除が記録から区別できる。**
  - AC-12 🔴 **AI の日次コスト上限に達していて再検査を実行できない場合、公開を解除しない。** 再検査は未実行のまま保持され、上限のリセットまたは引き上げの後に実行される（`F-027 AC-5` / 章 8.5）。**上限到達による保留を商流層 FAIL として扱わない**（`CLAUDE.md` §4.2「失敗と保留を混同しない」）。この保留はゲート FAIL 率（`F-059 AC-6`）に加算されない。⚠️ **保留中は編集後の内容が未検査のまま公開先に見えている** — これは「原価で止まっただけの案件を取引先の画面から消さない」ための決定（A-26）であり、保留の解除後に再検査して FAIL なら AC-7 に従って解除される。
  - AC-13 **公開が成立する経路と解除される経路は、ゲート結果の確定を経る単一の判定に集約される。** ゲート結果を参照せずに公開範囲を成立させる 2 本目の経路が存在しない（越境経路 1 を 1 箇所に閉じる。`F-004 AC-5` / `program-design` への申し送り 2）。
- **関連ロール**: `SALES` / `ADMIN`（設定）、`OWNER`（設定）、`VIEWER`（閲覧のみ）。**再検査の実行と自動解除は `system`**（`F-020` と同じ）。**自動解除を取り消せる / 上書きできるロールは存在しない**（AC-9）。
...
- **処理**: ①開示は **5 項目のみ**（スキル / 経験年数 / 単価レンジ / 稼働可能時期 / 勤務地・リモート可否）②表示前に丸める — 経験年数は 5 段階（`1 年未満` / `1〜3 年` / `3〜5 年` / `5〜10 年` / `10 年以上`）、単価は **10 万円刻み**のレンジ（例 `60〜70 万円`。上限は `100 万円以上` で打ち止め）、稼働可能時期は 5 段階（`即日` / `当月中` / `翌月` / `翌々月` / `3 か月以降`）、勤務地は**都道府県のみ**（市区町村・沿線・駅名は出さない）+ リモート可否 3 値（`フルリモート可` / `一部リモート可` / `常駐のみ`）、スキルは辞書の正規化済み名称を**上位 8 件まで**（経験年数の降順、同順は辞書 ID 順の決定的な順序）（`## Assumptions` A-04）③候補には**案件をまたいで同一人物を追跡できる識別子を出さない** ④Phase 1 は `F-009` の決定的な並び順の中に自社エンジニアと混在させる。Phase 2 は `F-029` のスコア順に並べ、**匿名のまま**順位が付く ⑤候補に付く操作は `F-018` の提案依頼のみ ⑥🔴 **匿名候補への検索条件（`F-009`）は、②で丸めた後の区分に対して「区分が条件と重なりうるか」で評価する**（例: 経験年数 7 年の候補は `5〜10 年` の区分として評価され、`yearsMin` を 5〜9.9 のどこに置いても当たり方が変わらない）。**開示していない属性（フリーワード `q` / 稼働状況 `availability`）は匿名候補に適用しない** —— 当たり外れが 6 項目目の開示になる（A-25。`F-031 AC-3` が根拠文に 5 項目を超える情報を含めないのと同じ原理）。
- **出力**: 匿名候補の一覧表示、提案依頼への導線。
- **AI 利用**: なし。Phase 2 の根拠文（`F-031`）は匿名候補にも付くが、**根拠文にも 5 項目を超える情報を含めない**（`F-031 AC-3`）。
- **外部 API 依存**: なし。
- **受け入れ基準**:
  - AC-1 匿名候補の表示・API 応答・エクスポート・根拠文のいずれにも、実名・所属会社名・社内 ID・営業メモ・スキルシート・**詳細な経歴の並び（`F-008 AC-5` の `EngineerCareer` の行。期間 / 役割 / 業務内容 / 使用技術のいずれも 1 項目たりとも）**が含まれない。表示項目は 5 項目を超えない（`BR-54`）。**スキルは辞書の正規化済み名称のみで、フリーテキストを含まず、1 候補あたり 8 件を超えない**（9 件以上のスキルを持つ候補でも、表示・API 応答・エクスポートに載るのは経験年数の降順で上位 8 件のみ。A-04。**これは 1 項目内の粒度であり、開示項目を 5 から増やすものではない**）。
  - AC-2 同一の匿名候補が複数の案件の候補一覧に現れても、**それが同一人物であることをホストが突き合わせられる識別子・値の組が応答に含まれない**（`BR-55`）。候補ごとに案件スコープの参照子のみを返す。
  - AC-3 経験年数・単価・稼働可能時期・勤務地が、A-04 の丸め規則どおりの粒度でのみ表示される — 経験年数は 5 段階（`1 年未満` / `1〜3 年` / `3〜5 年` / `5〜10 年` / `10 年以上`）、単価は 10 万円刻みのレンジと `100 万円以上`、稼働可能時期は 5 段階（`即日` / `当月中` / `翌月` / `翌々月` / `3 か月以降`）、勤務地は都道府県 + リモート可否 3 値。例: 経験年数 7 年の候補は `5〜10 年`、単価 65 万円の候補は `60〜70 万円`、勤務地「東京都渋谷区」の候補は `東京都` と表示され、`7 年` / `65 万円` / `渋谷区` および具体的な稼働開始日（`2026-09-16` 等）はいずれも表示されない。
...
- **処理**: ①`Proposal` を `DRAFT` で作成する ②作成時点のエンジニア情報を `EngineerSnapshot` として凍結する（以後の台帳更新は提案内容を変えない）。🔴 **凍結の対象には `F-008 AC-5` の経験内容を「行単位で」含める**（`F-008 AC-6`）③添付するスキルシートは `F-011` で `CLEAN` の版に限る ④提案の作成・更新を `ProposalEvent` と監査ログに記録する。
...
- **受け入れ基準**:
  ...
  - AC-5 🔴 **提案作成時点で台帳に N 行の経験内容があるエンジニアについて、`EngineerSnapshot` にも N 行が個別の行として保持される**（`F-008 AC-6`）。台帳側の行を後から追加・編集・削除しても、その提案が保持する行数と各行の 4 項目は変化しない。**経験内容が 0 行のまま作成された提案は 0 行のまま確定し、後から遡って埋まらない**（凍結は遡れない。だから `F-008 AC-5` の実装を提案フローより前に置く）。
...
- **入力**: 検査対象（**5 種: 提案 / スキルシートの外部共有 / 案件の公開 / チャット添付 / 契約書**。🔴 **契約書の追加は決定済み（2026-09-01、[Issue #15](https://github.com/Festal-KM/SES-Platform/issues/15)。`BR-15`）** — 契約書には単価とエンド企業名が載るため商流層の対象そのものである。**発注書は対象外**）、対象の公開範囲、案件の必須 / 尚可要件、登録スキル、既存の提案履歴。
...
- **AI 利用**: **あり**。ロール = `gate-inspector`。用途 = PII 層・商流層の検査実行と、整合層の警告の生成。期待する構造化出力 = 層ごとの判定（PII / 商流）、指摘の配列（種別・該当箇所・重大度）、整合層の警告の配列。**失敗時のフォールバック** = LLM が失敗・タイムアウト・スキーマ違反の場合、PII 層と商流層は**判定不能として FAIL 扱い**とし、「検査を完了できなかったため送信できない」と表示する（PASS へフォールバックしない）。整合層は機械的照合の結果のみで合否が決まるため、警告が生成できなくても合否は変わらない。🔴 **これは「LLM を呼んで失敗した」場合の扱いであり、「AI の日次コスト上限に達していて呼べない」場合とは区別する** — 後者はゲートを実行せず未実行のまま保持し、対象は `GATE_RUNNING` に留まる（`GATE_FAILED` にしない。`F-027 AC-5` / 章 8.5）。コスト影響 = 提案 1 件あたり 1 回、再実行のたびに 1 回計上（`F-026`）。
- **外部 API 依存**: Anthropic Claude API（`packages/ai` 経由の単一経路）。
- **受け入れ基準**:
  - AC-1 🔴 **テナント外へ共有される 5 種（提案 / スキルシートの外部共有 / 案件の公開 / チャット添付 / 契約書）は、ゲートを経ずに共有状態へ進めない**（`BR-15`）。契約書は `DRAFT` → `SENDING`（章 5.5 遷移 2）の前にゲートを通り、FAIL のまま送付できない（`F-047` 処理 ⑥ / `F-048 AC-3`）。🔴 **案件の公開は「共有状態へ進む時点」だけでなく、共有状態が続いている間も射程である** — 公開中に公開欄が編集されたら再検査し、商流層 FAIL なら公開を解除する（`F-014 AC-6`〜`AC-8`。A-26。[Issue #42](https://github.com/Festal-KM/SES-Platform/issues/42)）。**発注書（`F-050`）はゲート対象ではない。**
...
- **処理**: ①送信直前に `APPROVED` → `SUBMITTING` を CAS で更新し、失敗したら実行しない（多重実行の排除）②送信を実行する ③成功で `SUBMITTED`、失敗で `SUBMIT_FAILED` に確定する ④**外部 API 呼び出しの結果が確定した後に自動リトライしない** ⑤**本機能の送信は**`production` 以外ではモック実装が選択され、実在の宛先へ到達しない（宛先は提案先・エンド企業 = 章 7.6 NFR-ENV-1 の分類 3 であり、全非本番環境でモックとなる）⑥送信を監査ログと `ProposalEvent` に記録する。
...
- **受け入れ基準**:
  ...
  - AC-7 🔴 **送信ドメインの検証（`F-001 AC-4`）が完了していないテナントでは送信を実行できない**（`BR-71`）。提案は `APPROVED` のままで `SUBMITTING` に入らず、実行者に「送信元ドメインが未検証である」ことと設定すべき DNS レコードが表示される。**共通ドメインへフォールバックして送信することはない**（`BR-51`）。この状態は `SUBMIT_FAILED`（障害）ではなく設定未了として区別され、`F-059` に「送信ドメイン未検証のテナント」として現れる。
...
- **処理**: ①`packages/ai` の単一経路を通るすべての AI 呼び出しを `AiUsage` に記録する ②メール・ストレージ・席・電子署名を期間ごとの `UsageCounter` に集計する ③日次・月次の連続性を検査し、欠測を `F-059` に通知する ④売上（席課金 + 超過従量）と原価（AI / メール / ストレージ / 電子署名）を、テナント × 月 × ロール別に分解できる形で保持する ⑤🔴 **利用者に見せる単位（件数）を、金額とは独立のカウンタとして計測する**（章 7.5。スキルシート解析 / マッチング候補の根拠文 / 提案ドラフト / 延長論点の整理 / メール通数 / ストレージ GB）。**件数を金額から割り戻さない**（1 件あたり標準原価を後から見直したときに過去の残量表示が遡って変わるため。`docs/03` §7.6.3-1）⑥**`gate-inspector` の呼び出しも `AiUsage` に必ず記録する**が、**利用者向けクォータの分母・分子には入れない**（`docs/03` §7.6.1。記録しないのではなく見せ方の問題である）。
...
- **受け入れ基準**:
  ...
  - AC-6 🔴 **件数のカウンタが金額のカウンタと独立に存在し、件数が金額から割り戻されていない**（章 7.5 / `docs/03` §7.6.3-1）。1 件あたり標準原価の値を変更しても、過去の期間の件数消費と残量表示が変化しない。**`gate-inspector` の呼び出しは `AiUsage` に記録されるが、件数クォータの消費に加算されない**（`docs/03` §7.6.1）。
...
- **入力**: `UsageCounter` の現在値（**金額と件数の両方**）、プラン別の上限（**利用者に見せる件数クォータ**: スキルシート解析 N 件 / 月・マッチング候補の根拠文 N 件 / 月・提案ドラフト N 件 / 月・延長論点の整理 N 件 / 月・メール N 通 / 月・ストレージ N GB。**運営者の内部指標**: メール 1 テナント 1 日 500 通 / 1 分 30 通、AI の 1 日のコスト上限（USD）、電子署名 1 契約 1 リクエスト）、機能の実行要求。
- **処理**: ①実行前に上限を判定し、超過する要求を実行しない ②AI の日次コスト上限に達したら AI 機能を停止し、**残量と停止理由を画面に表示する** ③メールは日次・分次の両方を判定し、分次超過は待機、日次超過は停止として区別する ④上限接近（既定 80%）でテナント管理者と `F-057` に通知する（**Phase 1 の通知手段はアプリ内表示とメール送信**（`F-001` / `F-002` と同じ送信経路）。**宛先は自テナントの管理者であるため、`sandbox` では実送信される**（章 7.6 NFR-ENV-1）。Phase 2 以降は `F-039` に集約する）⑤上限到達・停止・解除を監査ログに記録する。
- **出力**: 実行可否、残量表示、停止理由、通知、監査ログ。
- **AI 利用**: なし。
- **外部 API 依存**: なし（判定はアプリ層）。
- **受け入れ基準**:
  - AC-1 AI の日次コスト上限に到達した状態で AI 機能を起動すると、実行されず「上限に達したため停止中」と**停止理由が実行者に表示される**。エラーコードのみの無言停止にならない（`BR-24`）。**残量・上限値・リセット時刻はテナントの契約情報としてホスト所属ロール（`OWNER` / `ADMIN` / `SALES` / `VIEWER`）に表示され、パートナー所属ロールには表示されない**（`BR-04` の第二境界。パートナーに表示されるのは停止の事実と理由のみ。章 4.2 の補足）。🔴 **残量・上限値は件数で表示する**（`AC-6`）。
  ...
  - AC-4 上限が 80% に達した時点でテナント管理者に通知が届く（Phase 1 はアプリ内表示とメール、Phase 2 以降は `F-039`。いずれのフェーズでも通知が出ない状態にならない）。🔴 **例外: AI 日次コスト上限（処理①）はテナントへは到達時のみ通知する**（`docs/04` U-12 の「遮断器」。テナントには金額の単位が無く 80% を件数で説明できないため。運営者 `F-057` には 80% でも通知する。**暫定。[Issue #56](https://github.com/Festal-KM/SES-Platform/issues/56) で確認中**。2026-09-16、T-10-03）。
  - AC-5 🔴 **AI の日次コスト上限による停止中にレビュー依頼を行っても、ゲートは実行されず未実行のまま保持され、対象は `GATE_RUNNING` に留まる。`GATE_FAILED` にはならない。** 承認・送信のいずれにも進めず、画面には「AI が上限到達で停止しているためゲートを実行できない」ことと再開の条件（リセット時刻 / 上限の引き上げ）が表示される。整合層の機械的照合は動作し、その結果は保持されて上限解除後の再実行に用いられる。🔴 **上限解除後（日次リセット / 上限の引き上げ）の再実行はゲートジョブが自動で行い、対象は `GATE_RUNNING` のまま検査が完了して `APPROVAL_PENDING` か `GATE_FAILED` に確定する**（ゲートは外部送信系ではないため自動再試行が許される。章 7.8 の自動リトライ禁止の対象は外部送信ジョブに限る）。**利用者（作成者 / `SALES` / `ADMIN`）が手動でレビュー依頼を再実行することもでき、自動・手動のいずれの経路でも同一対象へのゲート実行が多重化しない**（`UC-06` 手順 7）。**新しい状態は作らない**（章 5.1 の遷移表は変わらない）。🔴 **上限到達による未実行を `GATE_FAILED` に倒さないのは、`GATE_FAILED` が「元データの欠陥」を意味する状態だからである**（`CLAUDE.md` §4.2「失敗と保留を混同しない」）。混ぜるとゲート FAIL 率（`F-059`）が汚れ、直すべき元データが無いのに「修正して再実行」を促す誤った導線になる。**LLM の失敗・タイムアウト・スキーマ違反（`F-020` の AI 失敗時フォールバック）は別扱いであり、そちらは判定不能 = FAIL のままである**（章 8.5）。
  - AC-6 🔴 **テナント側の画面・通知・エクスポートに、クォータと残量を金額（USD / 円）で示す表示が 1 つも存在しない**（`BR-24`。2026-09-01、[Issue #12](https://github.com/Festal-KM/SES-Platform/issues/12)）。示すのは件数（スキルシート解析 / 根拠文 / 提案ドラフト / 延長論点の整理 / メール通数 / ストレージ GB）のみである。**例外は超過分の請求見込み額**であり、これは「残量の提示」ではなく請求であるため金額で示してよい（章 7.5）。**金額（USD）の上限は運営者の内部指標であり、`F-057` / `F-063` にのみ現れる。**
  - AC-7 🔴 **`gate-inspector`（`F-020`）の実行は、利用者向けクォータの消費に加算されない**（`docs/03` §7.6.1）。件数クォータを使い切った状態でも品質ゲートは実行でき、「クォータ切れでゲートが実行できない」状態が発生しない（ゲートは外部共有の前提条件であり、前提条件に従量の上限を掛けないため）。
...
- **処理**: ①`production` 以外では、画面最上部に環境名を固定表示する ②`demo` では「デモ環境 — 送信は行われません」を表示する ③`sandbox` では「サンドボックス環境 — **取引先への提案・契約書・署名依頼、および取引先の担当者宛のメール（招待を含む）は送信されません。取引先の招待は画面に表示されるリンクをお渡しください。自社メンバー宛の招待・期限のお知らせは、お使いのアドレスに実際に届きます。**それ以外は本番と同じ動作です」を表示する（宛先の分類による区別。章 7.6 NFR-ENV-1 の判定表 / `CLAUDE.md` §11.1）④主平面・管理平面の両方に表示する。
- **出力**: 環境バナー表示。
- **AI 利用**: なし。
- **外部 API 依存**: なし。
- **受け入れ基準**:
  - AC-1 `production` 以外のすべての画面（主平面・管理平面・モバイルビューポートを含む）で環境表示が視認でき、スクロールしても消えない。
  - AC-2 `demo` と `sandbox` で表示文言が異なり、`sandbox` では次の 3 点が文言から読み取れる。①**提案先・エンド企業宛の業務上の外部送信（提案 / 面談調整 / 契約書 / 署名依頼）は疑似である** ②**取引先の担当者宛のメールも疑似であり、取引先の招待は画面に表示されるリンクで行う** ③**自社メンバー宛のメール（招待・期限のお知らせ）は実際に届く**。「メールは一切送信されない」とも「すべてのメールが届く」とも誤読させる文言にしない（章 7.6 NFR-ENV-1 の判定表と同じ分類で表現する）。
...
- **処理**: ①スコア算出後に実行する ②構造化出力で根拠文を得る ③生成物にロール・プロンプト版・モデルを記録する ④匿名候補の根拠文は 5 項目の範囲を超えず、**A-04 の丸め後の値のみを用いる**。
...
- **受け入れ基準**:
  ...
  - AC-3 匿名候補の根拠文に、5 項目を超える情報（実名・所属会社名・詳細な経歴の並び）が含まれない（`BR-54`）。**根拠文が触れてよいのは A-04 の丸め後の値のみ**であり、丸め前の値（`7 年` / `65 万円` / 具体的な稼働開始日 / 市区町村）や、上位 8 件に入らなかったスキルを含まない（`F-017 AC-1` / `AC-3`）。
...
- **処理**: ①`packages/ai` の入口で氏名・生年月日・連絡先・顔写真・現所属会社名をマスキングする ②マスキング後の内容のみを LLM に渡す ③構造化出力を `SkillSheetExtraction` として保存し、ロール・プロンプト版・モデルを記録する ④抽出できなかった項目は「未抽出」として明示し、人が埋める前提とする ⑤登録者が採否を確認して台帳に反映する。🔴 **経歴の反映先は `F-008 AC-5` の経験内容と同一の項目構成であり、抽出専用の別の保存先を新設しない**（`F-008 AC-8`。手入力と AI 抽出で同じ経歴を 2 か所に持たせない）。
...
- **処理**: ①事象の発生時に通知を生成する ②メール通知は `F-027` の上限ガードを通す ③通知の本文に境界を越える情報を含めない ④既読・未読を管理する ⑤🔴 **`sandbox` ではメール通知の扱いが宛先の分類で分かれる — 受信者がホスト所属（分類 1）なら実送信、パートナー所属（分類 2）ならモック**とし、後者はアプリ内通知のみで届ける（章 7.6 NFR-ENV-1）。**アプリ内通知は分類によらず必ず生成されるため、パートナーが通知を受け取れない状態にならない。**
- **出力**: `Notification`、メール送信（`sandbox` は分類 1 のみ）。
- **AI 利用**: なし。
- **外部 API 依存**: メール送信（上限は `F-027`）。🔴 **`sandbox` ではホスト所属の受信者宛（分類 1）は実送信、パートナー所属の受信者宛（分類 2）はモック**（章 7.6 NFR-ENV-1 の判定表 / `CLAUDE.md` §11.1）。`development` / `demo` では分類を問わずモック。
- **受け入れ基準**:
...
- **受け入れ基準**:
  ...
  - AC-4 🔴 送信ドメインの検証が未完了のテナントでは実行できず、理由と設定すべき DNS レコードが表示される（`F-001 AC-4` / `F-022 AC-7` と同じ規律。`BR-71`）。
...
- **関連ロール**: `OWNER` / `ADMIN` / `SALES`（作成・更新・状態遷移）、`VIEWER`（閲覧のみ）。🔴 **`PARTNER_ADMIN` / `PARTNER_SALES` = `◐`（章 4.2）** — **自社所属エンジニアの稼働に限り、`F-065`（越境経路 5。Phase 2）を通じて読み取りのみで参照できる**（2026-09-01、[Issue #8](https://github.com/Festal-KM/SES-Platform/issues/8) で人間が経路 5 を承認。`BR-65`〜`BR-69`）。**開示は `BR-66` の 4 項目（案件名〈自社に公開済みの範囲〉/ 稼働期間 / 契約満了日 / 延長確認の状態）に限られ、本機能の作成・更新・削除・状態遷移はパートナーから実行できない**（`BR-68`。`F-065 AC-4`）。延長の意思表示・単価の交渉はチャット（`F-038`。経路 3）で行う。
...
- **受け入れ基準**:
  ...
  - AC-3 延長 / 終了 / 単価改定の決定を AI が確定させる経路が存在しない。🔴 **`ExtensionReview.summary`（論点・根拠データ・留意事項）は取引先に開示されない** — 越境経路 5 で取引先が見えるのは「延長確認が動いているかの状態」までである（`F-065 AC-2` / `BR-67`）。
  - AC-4 生成物からロール・プロンプト版・モデルを辿れる。
...
- **処理**: ①終了が決定した時点（`ENDING`）でエンジニアを「待機予定」とし、**稼働可能時期を満了日として設定する**（`Engineer` の属性。章 6.2。パートナー所属エンジニアの場合、この属性はそのパートナーが自社台帳で参照・編集できる。`AC-5`）。**緊急離任（章 5.2 遷移 6）で `ENDING` に到達した場合は、満了日ではなく「実際の離任日」を稼働可能時期として設定する**（離任日が未確定なら「即時」として設定し、担当者が上書きできる）②満了日到来で `ENDED` とし、エンジニアを探索母集団に戻す ③案件側は「後任募集」の状態にし、① の案件台帳に戻す ④**担当者の操作を要さない** ⑤還流を監査ログに記録する。
...
- **受け入れ基準**:
  ...
  - AC-5 🔴 **`PARTNER_ADMIN` / `PARTNER_SALES` が自社エンジニアについて参照できるのは、次の 2 つに限られる。** ①**自社台帳の属性**である「稼働状況」（稼働中 / 待機予定 / 待機中）と「稼働可能時期」（自社が持ち込んだ `Engineer` レコードの属性であり、越境にあたらない。**稼働可能時期が満了日と同一の値になることは意図した挙動である** — 処理 ①）。②**越境経路 5 による `Assignment` の読み取り参照**（**`F-065`。Phase 2**。開示は `BR-66` の 4 項目に限る。2026-09-01、[Issue #8](https://github.com/Festal-KM/SES-Platform/issues/8)）。🔴 **①②のいずれにおいても、ホストの内部単価・販売単価・エンド企業名・粗利、`ExtensionReview` の論点整理の内容、他社の当事者レコード（存在・件数を含む）は、画面・API・エクスポート・通知のいずれにも現れない**（`BR-67` / `BR-07`）。Phase 1 の時点では②が存在しないため、参照できるのは①のみである。
- **関連ロール**: `system`（還流）、`OWNER` / `ADMIN` / `SALES`（結果の参照）、`PARTNER_ADMIN` / `PARTNER_SALES`（`AC-5` の①②の範囲）。この属性が `F-017` の匿名 5 項目（稼働可能時期は A-04 の 5 段階に丸める）としてホストへ出る経路は A-04 の範囲内である。

#### F-046 個人情報の保持期間管理と削除（保持期間 3 年の一般ルール）

- **目的**: 稼働終了・提案終了から一定期間を過ぎた個人情報を、**保持期間の一般ルール**に基づいて定期的に削除できる状態を作る（`BR-29` / `BR-52`）。🔴 **本機能の範囲は「保持期間 3 年の一般ルールに基づく定期削除」に限る。** テナントのライフサイクルに伴う削除（`SANDBOX` の期限到来・`CLOSING` → `PURGED`）と、解約時の返却は **`F-064`（P0 / Phase 1）が担う**（`sandbox` には実在するエンジニアの経歴が入るため、削除手段を Phase 2 まで持ち越せない。`BR-62` / `BR-64`）。
...
#### F-065 取引先による自社エンジニアの稼働の参照（越境経路 5 / 稼働。Phase 2）

- **目的**: 取引先が**自社所属エンジニアの稼働**（`Assignment`）を読み取り専用で確認し、「あの人はいつまでですか」の問い合わせをホストと往復させない（`BR-65`〜`BR-69` / `docs/01` 章 6.6。2026-09-01、[Issue #8](https://github.com/Festal-KM/SES-Platform/issues/8) で人間が経路 5 を承認）。
- **入力**: 認証コンテキスト（テナント / パートナー所属 / ロール）、一覧の絞り込み条件（稼働中 / 満了間近 / 終了）、対象稼働の識別子。
- **処理**: ①**当事者判定を認証コンテキストのみから行う** — 対象 `Assignment` のエンジニアの所有パートナーが自社であるものに限る（リクエスト入力で当事者を指定させない。`BR-03`）②開示項目を **`BR-66` の 4 項目に固定する**（案件名〈**自社に公開済みの範囲に限る**。未公開の案件は案件名を出さず「非公開の案件」と表示する〉/ 稼働期間 / 契約満了日 / **延長確認の状態**）③延長確認は**起票されているか否かの状態のみ**を返し、`ExtensionReview.summary`（`F-044` の論点整理）を返さない ④**読み取り専用**とし、作成・更新・削除・状態遷移の API を持たない ⑤当事者でない稼働は 0 件として扱い、件数・存在の示唆も返さない ⑥閲覧を監査ログに記録する（`BR-27`）。
- **出力**: 自社が当事者の稼働の一覧・詳細（`BR-66` の 4 項目のみ）、監査ログ。
- **AI 利用**: なし。🔴 **`renewal-advisor`（`F-044`）の生成物を取引先に出さない**（`BR-67`。ホスト内部の検討内容にあたる）。
- **外部 API 依存**: なし（延長の意思表示・単価交渉は `F-038` のチャットで行う。`BR-68`）。
- **受け入れ基準**:
  - AC-1 表示・API 応答・エクスポート・通知に現れるのは `BR-66` の 4 項目のみである。**案件名は自社に公開済み（`F-014`）の案件に限り、未公開の案件では案件名が現れない。**
  - AC-2 🔴 **ホストの上流契約・販売単価・エンド企業名・粗利、および `ExtensionReview` の内部検討（論点整理・根拠データ・留意事項）が 1 項目も現れない**（`BR-67`）。API 応答のフィールドとしても存在しない。
  - AC-3 🔴 **他社が当事者の稼働は、存在も件数も推測できない**（`BR-07` / `BR-67`）。一覧・件数バッジ・並び順・集計・通知・エクスポートのいずれからも、同一案件に他社が稼働していることが分からない。**「この案件には他に N 件の稼働があります」に相当する表示を作らない。**
  - AC-4 🔴 **パートナー側からの書き込み API が存在しない**（`BR-68`）。作成・更新・削除・状態遷移を直接呼び出しても **403** で拒否され、状態は変化しない。導線も表示されない。
  - AC-5 参照（一覧・詳細）が監査ログに記録され、「誰が・いつ・どの稼働を見たか」を後から辿れる（`BR-27`）。
  - AC-6 **本機能は Phase 2 に限られる。** 契約・契約書・発注の参照は `F-066`（Phase 3）であり、Phase 2 で先に開かない（`BR-69`）。
- **関連ロール**: `PARTNER_ADMIN` / `PARTNER_SALES`（自社が当事者の分のみ・読み取り）、`OWNER` / `ADMIN` / `SALES`（**取引先に何が見えているかのプレビュー**）、`VIEWER`（閲覧のみ）。**運営者は不可**（`BR-40`）。
...
- **処理**: ①契約を `DRAFT` で作成する ②送付は `F-049` の送付ジョブが `SENDING` へ CAS 更新して実行し、成功で `UNDER_REVIEW`、失敗で `SEND_FAILED` に確定する ③`UNDER_REVIEW` から、先方の署名で `EXECUTED`、差し戻しで `DRAFT`、取り下げで `WITHDRAWN` に遷移する ④`EXECUTED` の契約は内容を書き換えられず、訂正は新しい契約を起こす ⑤締結待ちをタスク化し督促通知を出す（`F-040` / `F-039`）⑥🔴 **契約書は品質ゲートの 5 種の対象である**（`BR-15`。2026-09-01、[Issue #15](https://github.com/Festal-KM/SES-Platform/issues/15)）— `DRAFT` → `SENDING` の前に `F-020` の 3 層を通し、FAIL のまま送付できない（`F-020 AC-1` / `F-048 AC-3`）。**発注書（`F-050`）は対象外** ⑦契約締結の意思決定は人間のみ ⑧送付手段は 2 つある — **電子署名（`F-049`。テナントが自社アカウントを接続している場合のみ）**と、**契約書のメール添付（未接続テナントの既定経路）**。いずれも人間の明示操作で 1 回だけ実行し、締結の事実は人間が記録できる（`BR-70`）。
- **出力**: `Contract`（章 5.5 の 7 状態）、`ContractDocument`（版・署名依頼の記録・**ゲート結果の参照**。章 6.5）、タスク、監査ログ。
- **AI 利用**: なし。
...
- **受け入れ基準**:
  ...
  - AC-7 🔴 **契約書をメールで送付する経路は、送信ドメインの検証（`F-001 AC-4`）が完了していなければ実行できない**（`BR-71`）。契約は `DRAFT` のまま `SENDING` に入らず、理由と設定すべき DNS レコードが表示される。**共通ドメインへフォールバックしない**（`BR-51`）。**電子署名で送る経路（`F-049`）の前提条件は、独自ドメインの検証ではなく電子署名アカウントの接続である**（`F-049 AC-8`）。
  - AC-8 🔴 **契約書のドラフト版（署名前の版）が取引先の参照（`F-066`）に現れない**。取引先が参照できるのは `BR-66` の項目と**署名済みの最終版のみ**であり、締結に至らなかった条件の履歴に到達できない（`F-066 AC-2`）。
- **関連ロール**: `OWNER` / `ADMIN` / `SALES`（作成・送付・状態遷移）、`VIEWER`（閲覧のみ）。🔴 **`PARTNER_ADMIN` / `PARTNER_SALES` = `◐`（章 4.2）** — **自社が当事者の契約に限り、`F-066`（越境経路 5。Phase 3）を通じて読み取りのみで参照できる**（`BR-65`〜`BR-69`。2026-09-01、[Issue #8](https://github.com/Festal-KM/SES-Platform/issues/8)）。**作成・更新・送付・状態遷移はパートナーから実行できない**（`BR-68`）。締結の意思決定・送付はホスト側が行う。運営者は状態を変更できない（`BR-37`）。
...
- **入力**: `DRAFT` または `SEND_FAILED` の契約、契約書の版、署名者、`idempotency_key`、外部サービスの Webhook 通知、🔴 **テナントの電子署名アカウントの接続状態**（BYO。未接続 / 接続済み / 失効）。
- **処理**: ①送付ジョブが状態を `DRAFT` → `SENDING` へ CAS 更新し、失敗した実行は送付しない ②署名依頼を 1 回だけ発行する ③成功で `UNDER_REVIEW`、失敗で `SEND_FAILED` に確定する ④**自動リトライしない。再送は人間が `SEND_FAILED` → `DRAFT` に戻したうえでの明示操作のみ** ⑤Webhook を正規化し、署名完了で `EXECUTED` に反映する ⑥`SENDING` の滞留と署名の未着を `F-059` に出す ⑦🔴 **テナントごとの電子署名アカウントの接続（BYO）を設定として保持する**（接続・再接続・失効の検知。資格情報は暗号化して保存し、画面・ログ・監査ログ・通知・LLM に出さない。`BR-25`）。**未接続・失効のテナントでは本機能を実行せず、`F-047` 処理 ⑧ のメール添付が代替経路になる。**
- **出力**: 署名依頼の記録、`Contract` の状態遷移、**電子署名アカウントの接続状態**、監査ログ。
- **AI 利用**: なし。
- **外部 API 依存**: 電子署名サービス。🔴 **決定済み（2026-09-01、[Issue #11](https://github.com/Festal-KM/SES-Platform/issues/11) / [Issue #7](https://github.com/Festal-KM/SES-Platform/issues/7)）: テナントが自社のアカウントを接続する BYO 方式。第一コネクタは DocuSign、第二候補にクラウドサイン**（`BR-70` / `CLAUDE.md` §9-2 / `docs/03` §3.1.2 / §3.1.2a）。**署名依頼はテナント自身の名義で届き、運営者名義にならない**（`BR-51`）。運営者の 1 アカウントから全テナント分を送る方式は採らない。
- **受け入れ基準**:
  ...
  - AC-8 🔴 **電子署名アカウントを接続していないテナントでは本機能を実行できない**（`BR-70`）。導線に「電子署名が未接続である」ことと接続手順が示され、**契約書はメール添付で送付できる**（`F-047` 処理 ⑧）。未接続を理由に `Contract` が `DRAFT` から進めなくなる状態にしない。
  - AC-9 🔴 **電子署名の接続は、⑤ 契約業務を完了できる条件ではない**（`BR-70`）。**未接続のテナントでも、契約書の作成 → ゲート → メール添付での送付 → 締結の記録 → `Assignment` の生成までが完了できる**ことを E2E で証明できる。締結の事実を人間が記録する経路が存在する。
- **関連ロール**: `OWNER` / `ADMIN`（**電子署名アカウントの接続・再接続**）、`OWNER` / `ADMIN` / `SALES`（送付・再送）、`VIEWER`（不可）。代理閲覧中の運営者は不可（章 4.5）。
...
#### F-066 取引先による自社が当事者の契約・契約書・発注の参照（越境経路 5 / 契約。Phase 3）

- **目的**: 取引先が**自社が当事者となる契約・契約書・発注**を読み取り専用で確認し、締結の進捗を問い合わせずに追えるようにする（`BR-65`〜`BR-69` / `docs/01` 章 6.5。2026-09-01、[Issue #8](https://github.com/Festal-KM/SES-Platform/issues/8)）。
- **入力**: 認証コンテキスト（テナント / パートナー所属 / ロール）、一覧の絞り込み条件（契約種別 / 状態）、対象契約・契約書・発注の識別子。
- **処理**: ①**当事者判定を認証コンテキストのみから行う** — 契約の相手方が自社であるものに限る（リクエスト入力で当事者を指定させない。`BR-03`）②開示項目を **`BR-66` に固定する** — 契約＝種別 / 状態（章 5.5 の 7 状態）/ 期間 / **自社との契約単価**、契約書＝版 / 署名の状態 / **署名済みの最終版のみ**、発注＝状態 / 期間 / 金額 ③**契約書のドラフト版（署名前の版）と、締結に至らなかった条件の履歴を返さない** ④**読み取り専用**とし、作成・更新・削除・状態遷移・送付の API を持たない ⑤当事者でない契約・発注は 0 件として扱い、件数・存在の示唆も返さない ⑥閲覧とダウンロードを監査ログに記録する（`BR-27`）。
- **出力**: 自社が当事者の契約・契約書（署名済み最終版）・発注の一覧と詳細（`BR-66` の項目のみ）、監査ログ。
- **AI 利用**: なし。
- **外部 API 依存**: オブジェクトストレージ（署名済み最終版の取得。スキャンが `CLEAN` の版に限る。`BR-26`）。単価交渉・締結の意思表示は `F-038` のチャットで行う（`BR-68`）。
- **受け入れ基準**:
  - AC-1 表示・API 応答・エクスポート・通知に現れるのは `BR-66` の項目のみである（契約＝種別 / 状態 / 期間 / 自社との契約単価、契約書＝版 / 署名の状態 / 署名済み最終版、発注＝状態 / 期間 / 金額）。
  - AC-2 🔴 **`ContractDocument` として取得できるのは署名済みの最終版のみであり、ドラフト版・差し戻し前の版を取得する導線が存在しない**（API を直接呼んでも 403 / 404）。`SEND_FAILED` / `WITHDRAWN` に終わった契約書の中身に到達できない。
  - AC-3 🔴 **ホストの上流契約・販売単価・エンド企業名・粗利が 1 項目も現れない**（`BR-67`）。API 応答のフィールドとしても存在しない。**自社との契約単価は開示するが、ホストの販売単価は開示しない。**
  - AC-4 🔴 **他社が当事者の契約・契約書・発注は、存在も件数も推測できない**（`BR-07` / `BR-67`）。一覧・件数バッジ・並び順・集計・通知のいずれからも分からない。
  - AC-5 🔴 **パートナー側からの書き込み API が存在しない**（`BR-68`）。作成・更新・削除・状態遷移・送付・再送を直接呼び出しても **403** で拒否され、状態は変化しない。導線も表示されない。
  - AC-6 参照とダウンロードが監査ログに記録される（`BR-27`）。**本機能は Phase 3 に限られ、Phase 2 で先に開かない**（`BR-69`）。
- **関連ロール**: `PARTNER_ADMIN` / `PARTNER_SALES`（自社が当事者の分のみ・読み取り）、`OWNER` / `ADMIN` / `SALES`（**取引先に何が見えているかのプレビュー**）、`VIEWER`（閲覧のみ）。**運営者は不可**（`BR-40`）。
...
- **目的**: 顧客が自テナントの業務データを、**運用機能として**対象範囲・書式を選んで一括エクスポートできるようにする（`BR-29` / `## Assumptions` A-07）。🔴 **本機能の範囲は「運用機能としての一括エクスポート」に限る。** 解約手続き中（`CLOSING`）の**返却の最小版**（自テナントの業務データを CSV で返却する）は **`F-064`（P0 / Phase 1）が担う**。本機能は対象範囲の選択・書式指定・項目選択・再実行といった運用上の利便を Phase 3 で足すものであり、**返却手段そのものは Phase 1 から `F-064` で成立している**。
...
- **受け入れ基準**:
  ...
  - AC-5 `demo` 環境からの送信操作で、実在のドメイン宛にメール・契約書・署名依頼が 1 通も送出されない（`BR-45`）。`demo` は合成データのみを扱うため、`sandbox` の宛先による区別（章 7.6 NFR-ENV-1）を適用せず**全送信系がモック**である。
  - AC-6 🔴 **合成データの投入とリセットは `APP_ENV` が `demo` の場合にのみ実行できる。** `production` / `sandbox` / `staging` では管理平面に導線が表示されず、API を直接呼んでも拒否される（`BR-63` / `BR-37` / `CLAUDE.md` §10.5 / §11.1）。**`development` のみ、ローカル開発と自動テストのために実行を許可する**（合成データしか存在せず、顧客データも実在の宛先も持たないため）。リセットは対象テナントの全業務データの削除にあたるため、環境ガードを実装の注意ではなく実行前の判定として持つ。
- **関連ロール**: 運営者（`PLATFORM_OWNER` / `PLATFORM_SUPPORT`。**`demo` 環境に限った投入・リセット**。`AC-6`）、`demo` 環境の閲覧者。**`production` / `sandbox` のテナントの業務データには一切及ばない**（`BR-37` / `F-056 AC-3`）。
...
- **処理**: ①テナントを `SANDBOX` で開設し、有効期限を持たせる ②テナント分離・パートナースコープ・監査ログ・暗号化・保持期間を**本番と同じ規律**で適用する ③**外部送信は「宛先の分類」で分ける**（章 7.6 NFR-ENV-1 の判定表 / `CLAUDE.md` §11.1）— **提案先・エンド企業・エンジニア本人が宛先となる業務上の外部送信**（提案メール `F-022` / 面談調整 `F-041` / 契約書送付 `F-047` / 電子署名依頼 `F-049`。分類 3 / 4）と、**パートナー所属利用者が宛先となる送信**（取引先招待 `F-007` 経由の `F-002` / パートナー担当者宛の `F-011` `F-039`。分類 2）はモックとし、**ホスト所属利用者本人が宛先のアカウント運用のメール**（招待 `F-002` / パスワード再設定 `F-003` / 期限予告 `F-054` / 削除予告 `F-064` / スキャン失敗の周知 `F-011` / 上限接近の通知 `F-027` / 通知 `F-039`。分類 1）と**運営者宛**（`F-055`。分類外）は実送信する。**列挙ではなく分類で判定し、迷うものはモック側に倒す。** 分類 2 の取引先招待は、メールの代わりに**招待リンクを画面に表示**して成立させる（`F-007 AC-4`）。列挙のうち Phase 1 に存在するのは `F-002` / `F-003` / `F-007` / `F-011` / `F-022` / `F-027` / `F-055` / `F-064` であり、残り（`F-039` / `F-041` / `F-047` / `F-049`）は**各機能のフェーズ到来とともに同じ判定基準の対象に加わる**。本機能の成立は Phase 1 に存在する送信のみで判定できる ④**本契約は `SANDBOX` → `ACTIVE` の状態遷移として行い、データを環境間で複製しない**（章 5.4 遷移 1）⑤**30 日の期限到来、または見送りの決定で `SANDBOX` → `CLOSING` に遷移する**（章 5.4 遷移 2）。以後の返却と削除（`CLOSING` → `PURGED`）は **`F-064`**（P0 / Phase 1）が担う ⑥期限接近を運営者と見込み客に通知する（**Phase 1 の通知手段はアプリ内表示とメール送信**。Phase 2 以降は `F-039` に集約する）。
- **出力**: テナントのライフサイクル状態（`SANDBOX` / `ACTIVE` / `CLOSING`）、移行結果、通知、監査ログ。
- **AI 利用**: なし。
- **外部 API 依存**: メール送信・電子署名。🔴 **宛先の分類で分ける** — 分類 2 / 3 / 4（**パートナー所属利用者・提案先・エンド企業・エンジニア本人**）宛は**モック**、分類 1（**ホスト所属利用者本人**）宛の**アカウント運用のメール（期限予告を含む）は実送信**（処理 ③ / `AC-2` / `AC-9` / 章 7.6 NFR-ENV-1）。それ以外（AI・ストレージ・スキャン）は本番同等。
- **受け入れ基準**:
  - AC-1 `SANDBOX` において、テナント分離・パートナースコープ・監査ログ・暗号化の各テストが `production` と同じ内容で PASS する（`BR-62`）。**保持期間の統制についても、`SANDBOX` の 30 日期限と `CLOSING` の 30 日経過による削除（`F-064 AC-1` / `AC-2`）が Phase 1 時点で動作することをテストで証明できる**（保持期間 3 年の一般ルール（`F-046`）は Phase 2 で加わるが、**Phase 1 時点で削除手段が無い状態にはならない**）。🔴 **このうちパートナースコープの検証は、`sandbox` では招待リンク経由でパートナーを参加させて成立させる** — 取引先招待のメールは分類 2 としてモックになるため（章 7.6 NFR-ENV-1 ③）、`F-007 AC-4` で画面に表示された招待リンクから `PARTNER_ADMIN` が受諾・ログインし、その状態でパートナー境界のテストを `production` と同じ内容で実行する。
  - AC-2 `SANDBOX` からの**業務上の外部送信**（提案メール・面談調整の連絡・契約書の送付・電子署名依頼。分類 3 / 4）**および取引先企業の担当者宛の送信**（取引先招待・パートナー担当者宛の周知 / 通知。分類 2）が、**実在する取引先・第三者の宛先に 1 通も到達しない**（`BR-45`）。それ以外の機能は本番と同じ挙動を示す。**本 AC の射程は分類 1（ホスト所属利用者）宛のアカウント運用のメールを含まない**（`AC-9`）。
  - AC-3 本契約への移行が **`SANDBOX` → `ACTIVE` の状態遷移**として行われ、データのコピー処理・別テナントの新規作成・別環境への複製がいずれも発生しない（`BR-64` / 章 5.4 遷移 1 / NFR-ENV-7）。移行後も、移行前に投入されたエンジニア・案件・提案・監査ログが同一の識別子で参照できる。🔴 **移行のチェックリストに「独自ドメインの検証完了（必須。`F-001 AC-4`）」と「電子署名アカウントの接続（任意。`F-049 AC-8`）」が含まれ、前者が未完了のまま `ACTIVE` になったテナントは `F-059` に現れる**（NFR-ENV-10 / `docs/01` `R-11`。**試用では独自ドメインなしで動くため、移行時に必要になることを移行の説明に必ず含める**）。
  ...
  - AC-9 🔴 **`sandbox` の期限予告は、見込み客本人（ホスト所属利用者。分類 1）のメールアドレスに実際に届く。** 予告メールの送信された全通について、宛先が**ホスト所属利用者のアドレスのみ**であり、**パートナー所属利用者・提案先・エンド企業・エンジニアのアドレスが 1 件も含まれない**（章 7.6 NFR-ENV-1 ②）。モック化を理由に**本人が期限に気づけない状態にならない**（`AC-2` の「0 通」は本 AC に適用しない）。同じことがホスト社員への招待（`F-002`）・パスワード再設定（`F-003`）・削除予告（`F-064 AC-10`）・ホスト担当者宛のスキャン失敗の周知（`F-011`）・上限接近の通知（`F-027`）にも成立する。**取引先の担当者宛（分類 2）は本 AC の対象外であり、`F-007 AC-4` の招待リンクで成立させる。** 🔴 **送信基盤（環境全体）の上限到達で予告が保留された場合も、保留解除後に必ず配送される**（章 7.7「送信基盤（環境全体）の上限到達時の保留と復帰」④。保留は失敗ではなく、到達と保留件数は `F-059 AC-7` で運営者に現れる）。
...
- **目的**: `SANDBOX` の期限到来と `CLOSING` からの解約完了に伴い、**預かった第三者の個人情報を Phase 1 時点で確実に削除できる状態**を作る。あわせて、削除の前にテナント自身がデータを取り戻せる最小限の返却手段を用意する（`BR-62` / `BR-64` / `BR-29` / `BR-40`）。
  - 🔴 **本機能を Phase 1（P0）に置く理由は、スコープではなく統制である。** `sandbox` には見込み客が**実在するエンジニアの経歴**を投入する（`CLAUDE.md` §11.1 / `docs/01` 章 7.8.1）。削除の仕組みが無いまま期限切れテナントを抱えると、**第三者の個人情報を削除手段のないまま保持する**ことになる。`docs/01` 章 11.3-4 / `BR-62` が求める「サンドボックスに本番と同じ統制が **Phase 1 時点で**効いていること」は、本機能によって成立する。**範囲は最小限に絞る** — 保持期間 3 年の一般ルールは `F-046`（P1 / Phase 2）、書式指定・項目選択などの運用機能としての一括エクスポートは `F-052`（P2 / Phase 3）の領分であり、本機能はそれらを先取りしない。
...
- **AI 利用**: なし。
- **外部 API 依存**: オブジェクトストレージ（原本の削除、CSV の生成と受け渡し）。メール送信（削除予告・完了の通知）。**宛先はテナント管理者（`OWNER` / `ADMIN`。`SANDBOX` の場合は見込み客本人）であり、ホスト所属利用者（章 7.6 NFR-ENV-1 の分類 1）であるため、`sandbox` では実送信される**（`development` / `demo` ではモック。`CLAUDE.md` §11.1）。
- **受け入れ基準**:
  ...
  - AC-10 削除の実行前に、テナント管理者（`SANDBOX` の場合は見込み客）へ予告が通知される。**通知されないまま削除が実行される経路が存在しない。** 🔴 **本 AC は `sandbox` でも成立する** — 予告メールの宛先はテナント管理者（ホスト所属利用者 = 章 7.6 NFR-ENV-1 の分類 1）であり、`sandbox` では実送信されるため、モック化によって予告が届かない状態にならない（`F-054 AC-9`）。🔴 **送信基盤（環境全体）の上限到達により予告が保留された場合は、保留が解除されて配送されるまで削除を実行しない**（章 7.7「送信基盤（環境全体）の上限到達時の保留と復帰」④ / `F-059 AC-7`。「通知されないまま削除が実行される経路が存在しない」は保留中にも成立し、上限到達を理由に予告を省いて削除に進む経路を作らない）。
...
- **出力**: 管理平面のセッション、監査ログ。
- **AI 利用**: なし。
- **外部 API 依存**: メール送信（招待・再設定）。🔴 **宛先は `PlatformUser` であり、テナントに所属する利用者ではないため、章 7.6 NFR-ENV-1 の判定表では「分類外（運営者）」として扱い、`sandbox` では実送信する**（運営者は自社スタッフであり実在の取引先ではない。かつ実送信でなければ `sandbox` の管理平面で運営者がパスワードを再設定できない）。`development` / `demo` ではモック。
- **受け入れ基準**:
...
- **処理**: ①テナント × 期間の消費量と残量、クォータ消化率を**金額（USD）と件数の両方**で表示する（**利用者側の `F-027` は件数のみ**。`BR-24`）②上限接近・超過をアラートする ③**テナント個別のクォータ上書き**を設定できる（`PLATFORM_OWNER` のみ）④クォータ変更を監査ログに記録する ⑤**既存顧客の上限を予告なく引き下げない**運用のため、引き下げ時は適用日と通知を必須とする。
...
- **入力**: 失敗ジョブ、`SUBMITTING` の滞留、未対応の `SUBMIT_FAILED`、**`GATE_RUNNING` の滞留とその理由（AI の日次コスト上限による停止 / ジョブ失敗。`F-027 AC-5`）**、**`SENDING`（契約書。章 5.5）の滞留と未対応の `SEND_FAILED`**、ウイルススキャン失敗、電子署名の未着（`UNDER_REVIEW` の長期滞留）、ゲート FAIL 率、`F-043` の未起票検知、`F-026` の計測欠測、🔴 **送信ドメインの検証が未完了・失効のまま滞留しているテナント**（`BR-71`。オンボーディングが止まっている兆候そのもの。`docs/01` 章 7.4）、🔴 **メール送信基盤（環境全体）の送信上限への到達・接近と、それにより保留されているメールの件数**（章 7.7。**テナント単位の日次上限（`F-027`）とは別項目**）。
- **処理**: ①上記を種別ごとに一覧化し、滞留時間と件数を表示する ②閾値を超えた項目をアラートする ③テナント別に集計するが、**内容には立ち入らない** ④ゲート FAIL 率の急変を異常として検知する。🔴 **監視対象は対象機能のフェーズに追随して増える** — **Phase 1** は失敗ジョブ・`SUBMITTING` の滞留・未対応の `SUBMIT_FAILED`・**`GATE_RUNNING` の滞留**・ウイルススキャン失敗・ゲート FAIL 率・`F-026` の計測欠測・`F-064` の削除ジョブの失敗・**メール送信基盤（環境全体）の上限到達と保留件数**、**Phase 2** に `F-043` の未起票検知、**Phase 3** に `SENDING`（契約書）の滞留・未対応の `SEND_FAILED`・電子署名の未着が加わる。**Phase 1 の監視に穴が空く項目は無い**（Phase 1 に存在しない事象のみが後から加わる）。
- **出力**: 運用監視の一覧、アラート。
- **AI 利用**: なし。
...
- **受け入れ基準**:
  ...
  - AC-5 🔴 **送信ドメインの検証が未完了・失効のテナントが一覧に現れ、検証開始からの経過日数が分かる**（`BR-71` / `F-001 AC-4`）。**このテナントは取引先へ 1 通も送れない状態にある**ため、運営者が顧客より先に気づいて伴走できる。表示されるのは検証状態と日時のみで、業務データの内容には立ち入らない（`BR-40`）。
  - AC-6 🔴 **`GATE_RUNNING` のまま一定時間を超えた対象が一覧に現れ、滞留の理由が「AI の日次コスト上限による停止」と「ジョブ失敗」に区別して表示される**（`F-027 AC-5`）。**上限到達による滞留は障害ではないため、失敗ジョブ件数・ゲート FAIL 率・未対応 `SUBMIT_FAILED` のいずれの指標にも加算されない**（`CLAUDE.md` §4.2「失敗と保留を混同しない」）。表示されるのは件数・理由・滞留時間のみで、提案本文には立ち入らない（`BR-40`）。
  - AC-7 🔴 **メール送信基盤（環境全体）の送信上限への到達・接近が監視項目として一覧に現れ、保留中の件数・上限値・到達時刻が表示される**（章 7.7「送信基盤（環境全体）の上限到達時の保留と復帰」）。🔴 **テナント単位の日次上限（`F-027`。1 テナント 1 日 500 通）とは別項目として区別して表示され**、混ぜて集計されない（前者は環境全体の制約、後者はテナントの利用量であり、対処する相手が異なる）。**上限到達による保留は障害ではないため、失敗ジョブ件数・未対応 `SUBMIT_FAILED` / `SEND_FAILED`・ゲート FAIL 率のいずれにも加算されない**（`CLAUDE.md` §4.2「失敗と保留を混同しない」。`AC-6` と同じ原則）。表示されるのは件数・上限値・日時のみで、メール本文・宛先の個人情報には立ち入らない（`BR-40`）。
- **関連ロール**: `PLATFORM_OWNER` / `PLATFORM_SUPPORT`（いずれも閲覧のみ）。
...
- **受け入れ基準**:
  ...
  - AC-3 **代理状態で実行できない操作**: 提案の送信・再送、承認・却下、契約書の送付・署名依頼、業務データの作成・更新・削除、公開範囲の変更、共有設定の変更、エクスポート、スキルシートのダウンロード。🔴 **これらは「押せない状態（無効化）」で残さず画面から取り除く。そのうえで、取り除いた位置に「代理閲覧中のため操作できない」ことを常時表示する**（`BR-39`。2026-09-01、[Issue #14](https://github.com/Festal-KM/SES-Platform/issues/14)）。API を直接呼んでも拒否される。**無効化ボタンを残す実装は誤操作の機会を残し、理由の表示が無いとサポートが顧客に誤った説明をする。**
...
- **受け入れ基準**:
  ...
  - AC-5 🔴 **運営者には、AI 利用について「金額上限に対する消費率」と「基準ユニット比の倍率」の両方が表示される**（`docs/03` §7.5-3 / §7.6.3-2）。**金額（USD）はここにのみ現れ、テナント側の画面には現れない**（`F-027 AC-6` / `BR-24`）。件数（利用者に見せている単位）と金額の対応が同一画面で確認でき、1 件あたり標準原価の改定時に両者の乖離を検知できる。
...
**取引先（`PARTNER_ADMIN` / `PARTNER_SALES`）は 1 日 4〜5 時間滞在する主利用者**であり、自社営業（1 日 1〜3 時間）より長い（`CLAUDE.md` §1.2 / `docs/01` 申し送り 24）。したがって取引先側のユースケース（`UC-13`〜`UC-18` / **`UC-25`**）は自社営業と**同じ粒度**で定義する。**取引先の業務は提案で終わらず、決まった後の稼働と契約も自社の業務として抱えている**（`UC-25`。越境経路 5。`docs/01` 章 2.3）。
...
- **手順**:
  ...
  5. **⑤** 決定した提案から契約を作成し、テンプレートに差し込み、**契約書も品質ゲートを通したうえで**（`F-020 AC-1`。`BR-15`）送付する。電子署名を接続済みなら自社名義で署名依頼、未接続ならメール添付で送付し、締結を記録する（`F-047` / `F-048` / `F-049`）。**取引先は自社が当事者の契約の状態を読み取り専用で追える**（`F-066`。Phase 3）。
...
- **手順**: 1. 運営者がテナントを開設し初期 `OWNER` を招待する（`F-001`）。2. `OWNER` が 2 要素認証を設定しログインする（`F-003`）。3. 🔴 **送信ドメインを登録し、提示された DNS レコードを顧客側で設定して検証を完了させる**（`F-001 AC-4`。`BR-71`。**完了するまで取引先へは 1 通も送れない**）。4. メンバーを招待しロールを割り当てる（`F-002`。自社メンバー宛は共通ドメインでよい）。5. 取引先企業を登録し `PARTNER_ADMIN` を招待する（`F-007`。**検証済みの独自ドメインから届く**）。6. エンジニアと案件を登録する（`F-008` / `F-013`）。7. 案件の公開範囲を指定する（`F-014`）。8. 取引先が提案を作成し、ゲート → 承認 → 送信まで到達する（`F-019`〜`F-022`）。
- **結果**: 最初の提案が送信済みとなり、監査ログに一連の操作が残っている。テナントの既定は「自動承認 = 無効」「AI ロール承認モード = すべて都度承認」のままである。🔴 **オンボーディングの到達点は「ログインできたか」ではなく「取引先へ送信できる状態になったか」である**（`docs/01` 章 7.1）。手順 3 が未完のテナントは `F-059` に「送信ドメイン未検証」として現れ、運営者が伴走する。
...
- **手順**: 1. 営業がスキルシート取込を実行する。2. `F-027` が実行前に上限を判定し、実行しない。3. 画面に「上限に達したため停止中」と残量・上限値・リセット時刻が表示される。4. レビュー依頼を行うと、整合層の機械的照合は動作するが、**PII 層・商流層は実行されず、提案は `GATE_RUNNING` のまま保持される**（`GATE_FAILED` にはならない）。承認・送信のいずれにも進めず、停止理由と再開条件が表示される（`F-027 AC-5`）。5. `ADMIN` に上限到達の通知が届く。6. 運営者側では `F-057` に超過が現れ、`F-059` に「AI 停止による `GATE_RUNNING` 滞留」として障害と区別されて現れる（`F-059 AC-6`）。7. 上限がリセットされる（または `F-057` で引き上げられる）と、**`GATE_RUNNING` のまま保持されていた提案のゲートをジョブが自動で再実行し**、全層 PASS で `APPROVAL_PENDING` へ進む。営業が手動でレビュー依頼を再実行してもよく、どちらの経路でも実行は多重化しない（`F-027 AC-5`）。
- **結果**: 黙って劣化せず、停止理由と残量が利用者に見えている。**上限到達を理由にゲートを緩めることはない。** **上限到達（保留）が `GATE_FAILED`（元データの欠陥）に混ざらず、ゲート FAIL 率の指標が汚れない**（`CLAUDE.md` §4.2）。
...
- **手順**: 1. テナント側では機能が継続する（超過分は従量課金）が、**超過が始まったこと（件数）と現時点の請求見込み額（金額）**が `ADMIN` に表示される（`F-026` / `F-027`。**残量と上限は件数、請求見込みのみ金額**。`F-027 AC-6`）。2. さらに 1 日のコスト上限に到達すると AI 機能が停止し、**残量（件数）と停止理由**が表示される（`F-027 AC-1`）。**品質ゲートはクォータの対象外であるため、クォータ切れを理由にゲートが実行不能になることはない**（`F-027 AC-7`）。3. 運営者側では `F-057` に超過テナントとして現れ（**金額と件数の両方**）、`F-063` で粗利率の低下が日次で検知される（`F-063 AC-5`）。4. `PLATFORM_OWNER` がプラン変更を提案する（`F-062`）。
...
- **手順**: 1. エンジニアの稼働可能時期・単価レンジ・スキル・**経験内容と従事期間の行**を更新する（`F-008`。`AC-5`）。2. 新しいスキルシートをアップロードする（`F-011`）。3. ウイルススキャンが完了するまで共有できない旨が表示される。4. `CLEAN` になった版が最新版になる。5. Phase 2 では構造化抽出が実行され、抽出結果の採否を確認する（`F-032` / `F-033`）。6. 共有可の候補であれば、更新内容が丸めた粒度でホストの候補一覧に反映される（`F-017`）。
...
- **手順**: 1. 見込み客が自社の実データ（実在のエンジニアの経歴を含む）を投入する。2. `SANDBOX` でも、テナント分離・監査ログ・暗号化・保持期間が `production` と同じ規律で適用される（`F-054 AC-1`）。3. 提案の送信を試すが、**提案先・エンド企業宛の業務上の外部送信（提案メール・面談調整・契約書送付・電子署名依頼）は実在の宛先に到達しない**（`F-054 AC-2`。分類 3 / 4）。**取引先の担当者宛のメールも送信されず**（分類 2）、取引先を招く場合は**画面に表示された招待リンクを渡す**（`F-007 AC-4`）。見込み客は自社の同僚を疑似的な取引先として招き、パートナー側の画面と境界の挙動まで確認できる（`F-054 AC-1`）。一方、**招待・パスワード再設定・期限のお知らせなど自社メンバー宛のメールは実際に届く**（`F-054 AC-9`。分類 1）。画面上部に `sandbox` の表示が出ており、この 3 つの区別が文言から読み取れる（`F-028 AC-2`）。4-a **本契約に至った場合**: `PLATFORM_OWNER` が **`SANDBOX` → `ACTIVE`** に遷移させる（**Phase 1 は `F-054 AC-7`、Phase 3 以降は `F-062` と連動**）。データのコピー・別テナントの作成・別環境への複製はいずれも発生せず、投入済みのデータが同じ識別子のまま引き継がれる（`F-054 AC-3` / NFR-ENV-7）。4-b **至らなかった場合**: 30 日の期限到来または見送りの決定で **`SANDBOX` → `CLOSING`** に遷移する（`F-054 AC-4`）。以後は新規作成ができず、**自テナントの業務データの返却（`F-064 AC-5`）と閲覧のみ**可能になる。5. `CLOSING` から 30 日経過で **`PURGED`** に遷移し、連絡先・スキルシート原本・チャット本文が削除される（**`F-064 AC-1` / `AC-2`。Phase 1 で成立する**）。削除前に見込み客へ予告が通知される（`F-064 AC-10`）。**この予告は `sandbox` でも実送信され、見込み客本人に届く**（`F-054 AC-9` / 章 7.6 NFR-ENV-1）。
...
#### UC-25 取引先が自社エンジニアの稼働と、自社が当事者の契約を確認する（越境経路 5）

- **アクター**: `PARTNER_ADMIN` / `PARTNER_SALES`、ホストの `SALES`
- **前提**: 自社が提案したエンジニアが `WON` から稼働中になっている（Phase 2）。Phase 3 ではその契約が締結済みである。
- **手順**: 1. 取引先がログインし、自社エンジニアの稼働一覧を開く（`F-065`）。**案件名（自社に公開済みの範囲）・稼働期間・契約満了日・延長確認の状態**が読み取り専用で見える。2. 満了 60 日前に延長確認が起票されると、状態が「延長確認中」に変わる。**論点整理の中身は見えない**（`F-065 AC-2`）。3. 延長の意思表示・単価の交渉は**チャット**で行う（`F-038`。経路 3。`BR-68`）。4. Phase 3 では、自社が当事者の契約の種別・状態・期間・**自社との契約単価**と、**署名済みの最終版**の契約書、発注の状態・期間・金額を確認する（`F-066`）。5. ホスト側は「取引先に何が見えているか」をプレビューで確認できる（`F-065` / `F-066` の関連ロール）。
- **結果**: 取引先が満了日をホストに問い合わせずに把握でき、二重台帳が解消される。🔴 **ホストの販売単価・エンド企業名・粗利・内部検討は 1 項目も見えず、他社が当事者の稼働・契約は存在も件数も推測できない**（`BR-67` / `BR-07`）。取引先からの書き込み経路は 1 つも無い（`F-065 AC-4` / `F-066 AC-5`）。
- **関連機能**: `F-042` `F-043` `F-047` `F-065` `F-066` `F-038`

#### UC-26 公開中の案件の公開欄を編集し、再検査で公開が自動解除される（越境経路 1 / [Issue #42](https://github.com/Festal-KM/SES-Platform/issues/42) = 回答②）

- **アクター**: ホストの `SALES` / `ADMIN`、`system`（再検査）、`PARTNER_SALES`（結果の受け手）
- **前提**: 案件が 1 社以上の取引先に公開されており、公開の時点では品質ゲートが全層 PASS だった（`F-014 AC-3`）。
- **手順**: 1. 自社営業が案件の**外部公開用の記載**を追記し、そこにエンド企業名が入る（`F-013`）。2. **公開欄 3 欄のいずれかが実際に変わったため**、再検査が起動する（`F-014 AC-6`。3 欄以外だけの編集や公開先の増減では起動しない）。3. 商流層が FAIL し、**その案件の公開が解除される**（`F-014 AC-7`）。4. 取引先の一覧・検索結果・件数からその案件が消え、URL を直接開いても「存在しない」として扱われる。**解除の理由も、解除が起きたこと自体も取引先には示されない**（`F-014 AC-10`）。5. ホスト側の案件画面に「検査で問題が見つかったため公開を解除した」ことと**原因の欄**が表示され、`F-020` の指摘に辿れる（`F-014 AC-9`）。6. 自社営業がエンド企業名を外し、再度公開する。再検査が全層 PASS となり公開が復帰する。**指摘を無視して公開を続ける操作は存在しない**（`BR-18`）。
- **結果**: 🔴 **公開の瞬間に検査を通した内容が、後から商流情報を含む状態で取引先に見え続ける経路が塞がる**（`F-014` 目的 / A-26）。自動解除は**実施者 `system`** として監査ログに残り、人の操作による公開解除と区別できる（`F-014 AC-11`）。⚠️ **副作用として、取引先の画面から案件が一時的に消える** — だからこそホスト側に理由が出る（手順 5）。**AI の日次コスト上限に達している間は解除されず、再検査が保留されたまま公開が維持される**（`F-014 AC-12`。`UC-06` と同じ「失敗と保留を混同しない」原則）。
- **関連機能**: `F-013` `F-014` `F-020` `F-026` `F-027` `F-005`
...
| ◐ | **パートナースコープ内に限定される（第二境界）**。自パートナー企業が持ち込んだ情報、または**越境経路 1〜5**（章 1.1）で開示された範囲に限る。🔴 **経路 5 の追加により、`◐` には「自社が当事者であるレコードを、`BR-66` の開示項目だけ、読み取りのみで参照できる」場合が含まれる**（`F-042` / `F-047` の `PA` / `PS`。参照の実体は `F-065` / `F-066`。2026-09-01、[Issue #8](https://github.com/Festal-KM/SES-Platform/issues/8)）。**当事者かどうかは認証コンテキストから決まり、リクエスト入力では決まらない**（`BR-03`）。**閲覧のみの場合も、パートナースコープに限定されるなら `○` ではなく `◐` を用いる** |
...
| `F-065` 取引先による稼働の参照（経路 5。Phase 2） | ○ | ○ | ○ | **◐** | **◐** | ○ | − | − |
| `F-066` 取引先による契約・発注の参照（経路 5。Phase 3） | ○ | ○ | ○ | **◐** | **◐** | ○ | − | − |

**表の補足**（誤読を避けるため明示する）:

- `F-021` の `PA` / `PS` = `◐` は、**自社が作成した提案の内容をパートナースコープ内で確認できる**という意味である。🔴 **ホスト宛の最終承認（`APPROVED` への遷移）はホスト側のロールのみが行う**（`F-021` 関連ロール / `BR-16`）。◐ を「パートナーが承認できる」と読まない。/ `F-011` / `F-023` の `VI` = `○` は**閲覧のみ**であり、実行（ダウンロード・再送）は含まない（章 4.3 / `F-011 AC-1`〜`AC-4` / `F-023 AC-1` / `BR-31`）。
...
- 🔴 **`F-042`（稼働）と `F-047`（契約）の `PA` / `PS` は、2026-09-01 に `−` から `◐` へ改めた**（[Issue #8](https://github.com/Festal-KM/SES-Platform/issues/8) で人間が**越境経路 5** を承認したことによる。`BR-05` / `BR-65`〜`BR-69`。**ID は削除せず、意味を変えた**）。`◐` の意味は **「自社が当事者であるレコードだけを、`BR-66` の開示項目だけ、読み取りのみで参照できる」**であり、参照の実体は **`F-065`（稼働。Phase 2）/ `F-066`（契約・契約書・発注。Phase 3）**である。🔴 **作成・更新・削除・状態遷移・送付はパートナーから実行できない**（`BR-68`）。**当事者でないレコードは 0 件で、存在も件数も推測できない**（`BR-07` / `BR-67`）。パートナーが知り得るのはこれに加えて、自社が作成した提案の状態（`F-024`。経路 2）、自社エンジニアの稼働状況と稼働可能時期という自社台帳の属性（`F-045 AC-5`）、チャットでのやり取り（`F-038`。経路 3）である。**ホストの販売単価・エンド企業名・粗利・`ExtensionReview` の内部検討には到達しない。** 🔴 **`F-043`（満了アラート・自動起票）と `F-044`（延長確認の論点整理）が `PA` / `PS` = `−` のままであることは、`F-065` と矛盾しない** — `F-065` が開示するのは `BR-66` の列挙のうち**「延長確認の状態」だけ**（例: 確認中 / 延長 / 終了）であり、起票の通知・タスク・`ExtensionReview.summary`（`F-044` の論点と根拠データ）は含まない（`F-065 AC-2`）。`◐` を付けたのは `F-042` / `F-065` であって `F-043` / `F-044` ではない。
- `F-010`（スキル辞書・別名）の `PA` / `PS` = `◐` は、**語彙マスタの参照と、自社の入力から生じた新語候補の起票**を指す。`Skill`（グローバル辞書）と `SkillAlias` は**分類のためのマスタであって業務データではなく**、他パートナーが持ち込んだ情報を含まない（章 6 の分離キー規約 / `F-010 AC-2`）。採否の決定はホスト（**`OWNER` / `ADMIN` / `SALES`**。`OWNER` は暫定。[Issue #36](https://github.com/Festal-KM/SES-Platform/issues/36)）が行い、採用後のテナント別名はホストの決定物としてテナント内で共有される。/ `F-003`（認証・2FA）の `PA` / `PS` = `●` は、**認証が境界そのものを確定させる操作であり、第二境界の内側での参照ではない**ことによる（`F-003 AC-1`。認証の結果として `partner_company_id` が確定し、以後のすべての参照がその内側に限定される）。ここに `◐` を付けると「認証がパートナースコープの内側の操作」という誤った読みを生む。
- 🔴 **`F-053`（デモ合成データ・リセット）の `PO` / `PP` = `●` は、`demo` 環境のテナントに対する合成データの投入・リセットに限られる。** `production` / `sandbox` / `staging` のテナントの業務データには一切及ばない（`F-053 AC-6`。導線が表示されず API も拒否される）。章 2.8 の共通前提（運営者の書き込みは契約・クォータ・機能フラグ・お知らせに限る）と `F-056 AC-3`（運営者はテナントの業務データを作成・更新・削除できない）に対する例外ではなく、**射程が `demo` 環境の合成データに閉じているため衝突しない**（`BR-37` / `CLAUDE.md` §10.5 / §11.1）。
...
> `Tenant`（5.4）と `Contract`（5.5）は、本書の前版で `## Assumptions` A-12 / A-13 として「§4.2 の改訂が必要」と提起していたものである。🔴 **[Issue #6](https://github.com/Festal-KM/SES-Platform/issues/6) / [Issue #7](https://github.com/Festal-KM/SES-Platform/issues/7) で 2026-09-01 に決定し、`CLAUDE.md` §4.2 に確定事項として反映済み**であり、本章はその定義をそのまま引き写している（A-12 / A-13。**「暫定 / 確認中」ではない**）。
...
- **`SUBMITTING` は片道である。** 入ったら必ず `SUBMITTED` か `SUBMIT_FAILED` に確定する。滞留は `F-059 AC-1` で検知する。**`SUBMITTING` からの自動リトライを行わない**（二重送信事故になるため）。 🔴 **AI の日次コスト上限に達している間、`GATE_RUNNING` は保持される**（`F-027 AC-5`）。ゲートが実行できないだけであり、**`GATE_FAILED` へは遷移しない**（`GATE_FAILED` は元データの欠陥を意味する状態であり、保留と混ぜない）。上限のリセットまたは引き上げの後に再実行し、遷移 2 / 遷移 3 のいずれかへ確定する。**この扱いのために状態も遷移も追加していない**（上表は `CLAUDE.md` §4.2 のままである）。滞留は `F-059 AC-6` が理由付きで検知する。
...
- **`DECLINED` と `EXPIRED` はホストから区別できてよい**が、**`DECLINED` の理由は区別できない**（`F-018 AC-2`）。`DECLINED` は `LOST`（見送り）・`GATE_FAILED`（ゲートで止めた）・`SUBMIT_FAILED`（送信失敗）の**いずれとも別の状態**であり、集計・通知・一覧のフィルタで混同しない（`BR-60` / `F-018 AC-5`）。
- `ACCEPTED` 以外の終端（`DECLINED` / `WITHDRAWN_BY_HOST` / `EXPIRED`）では、**身元は最後まで開示されない**。

### 5.4 `Tenant`（契約のライフサイクル）

`CLAUDE.md` §4.2 の `Tenant`（🔴 **決定済み（2026-09-01、[Issue #6](https://github.com/Festal-KM/SES-Platform/issues/6)）**。A-12）。§11 の `sandbox` 環境と §10.4-4 の契約管理に対応する。
...
|---|---|---|---|---|---|---|
| `SANDBOX` | ○ | ○ | ○ | ○（**提案先・エンド企業宛の業務上の外部送信と、パートナー所属利用者宛の送信は必ずモック**（分類 2 / 3 / 4。取引先招待は招待リンクの画面表示で成立）。**ホスト所属利用者宛のアカウント運用メールは実送信**（分類 1）。`BR-45` / NFR-ENV-1 / `F-054 AC-2` `AC-9` / `F-007 AC-4`） | ○ | 保持。統制は `production` と同一（`BR-62`） |
| `ACTIVE` | ○ | ○ | ○ | ○ | ○ | 保持 |
| `SUSPENDED` | ○ | ○ | ○ | 🔴 **不可** | ○ | 🔴 **消さない** |
...
`CLAUDE.md` §4.2 の `Contract`（🔴 **決定済み（2026-09-01、[Issue #7](https://github.com/Festal-KM/SES-Platform/issues/7)）**。A-13）。`F-047` / `F-049` / **`F-066`（取引先の参照）**が対応する。
...
- 🔴 **本状態機械は NDA / 基本契約 / 個別契約のすべてで共通である**（`BR-73`。種別ごとに別の進み方を定義しない）。**双方署名は状態を増やさずに表現する** — `UNDER_REVIEW` = **いずれかの署名者が未署名**、`EXECUTED` = **全署名者の署名が完了**（`docs/03` §3.1.10）。「自社署名済み・先方未署名」のための状態を追加しない（`BR-33`）。取引先が参照できるのは**署名の状態と署名済みの最終版のみ**である（`F-066 AC-2`）。
...
| 概念 | 主要属性 |
|---|---|
| `Tenant` | 企業名 / 商号、環境種別（`production` / `sandbox` / `demo`）、**ライフサイクル状態（章 5.4 の 5 状態: `SANDBOX` / `ACTIVE` / `SUSPENDED` / `CLOSING` / `PURGED`）**、状態の最終遷移日時と実行者、`SANDBOX` の有効期限、`CLOSING` に入った日時（`PURGED` の起算点）、停止理由、自動承認フラグ `autoApproveEnabled`、マッチング重み設定（`F-030`）、AI ロール別の承認モードとモデル（`F-035` / `F-036`）、個人情報の保持期間（既定 3 年）、開設日、🔴 **取引先へ届く送信に用いる独自ドメインと検証状態**（未検証 / 検証中 / 検証済み / 失効。`F-001 AC-4` / `BR-71`）、🔴 **電子署名アカウントの接続状態**（BYO。未接続 / 接続済み / 失効。資格情報は暗号化。`F-049` / `BR-70` / `BR-25`） |
...
| `EngineerCareer` | 🔴 **経験内容と従事期間の 1 行**（`F-008 AC-5`。**決定済み（2026-09-10、[Issue #35](https://github.com/Festal-KM/SES-Platform/issues/35) = A）**）。`engineer_id`、**開始年月**、**終了年月（`null` = 継続中）**、**役割**、**業務内容**、**使用技術**、入力元（手入力 / `sheet-parser` の抽出を採用）、`skill_sheet_extraction_id`（抽出由来の場合の出所）、作成・更新日時。**`Engineer` の 1 属性（自由文・1 塊の構造）としては持たない**（`F-008 AC-6` の行単位の凍結、`F-029` の項目参照、`F-017 AC-1` の項目単位の非開示が成立しないため）。🔴 **匿名共有（経路 4）の開示対象外**（`BR-55`）。**保持と削除の扱いは `SkillSheet` 原本に準じる**（本書が置いた既定。章 6.8 / A-24） |
...
| `EngineerSnapshot` | `proposal_id`、提案時点のエンジニア情報の凍結コピー（氏名 / スキル / 経験 / 単価 / 稼働可能時期 / 勤務地）、🔴 **経験内容の凍結（`EngineerCareer` の各行を「行単位で」複製し、1 行が 開始年月 / 終了年月 / 役割 / 業務内容 / 使用技術 を個別に保持する。`F-008 AC-6` / `F-019 AC-5`）**、凍結日時、参照したスキルシート版 |
| `ProposalEvent` | `proposal_id`、事象種別（状態遷移 / メモ / 添付）、日時、担当者、内容、添付参照 |
| `ReviewGate` | 🔴 **対象種別は 5 種**（提案 / スキルシート共有 / 案件公開 / チャット添付 / **契約書**。`F-020 AC-1` / `BR-15`。**発注書（`F-050`）は対象に含めない**）と対象 ID、実行日時、**層ごとの判定**（PII / 商流 / 整合）、指摘（種別・該当箇所・重大度）、**整合層の警告（AI 由来。合否に影響しない）**、`gate-inspector` のプロンプト版とモデル |
...
| `Contract` | `tenant_id`、契約種別（NDA / 基本契約 / 個別契約）、相手方、🔴 **当事者となるパートナー企業（`partner_company_id`。経路 5 の当事者判定の根拠。`F-066` 処理 ①）**、対象案件・エンジニア、単価（**自社とパートナーの間の契約単価**。ホストの販売単価・エンド企業名は `Project` 側の内部限定項目であり `Contract` の開示対象に含めない。`BR-67`）、期間、支払条件、**状態（章 5.5 の 7 状態: `DRAFT` / `SENDING` / `SEND_FAILED` / `UNDER_REVIEW` / `EXECUTED` / `WITHDRAWN` / `EXPIRED`）**、送付失敗理由、取り下げ理由、締結日時、満了 / 解除日時、`corrects_contract_id`（`EXECUTED` の訂正で起こした新契約の参照元。`F-047 AC-5`） |
| `ContractDocument` | `contract_id`、版番号、ファイル参照、スキャン状態、🔴 **送付前に実行した `ReviewGate` の結果への参照（版ごとに保持し、契約書のゲート結果をここから辿れる。`F-047` 処理 ⑥ / `F-020 AC-1`）**、署名依頼の `idempotency_key`、署名依頼日時、署名完了日時、外部サービスの正規化済み応答。**状態機械を持たない**（締結の状態は `Contract` が唯一の正。章 5.5） |
| `Order` | `tenant_id`、`contract_id` または `assignment_id`、**当事者となるパートナー企業**（経路 5 の判定根拠）、金額、期間、発行日、入金状態 |
| `Assignment` | `tenant_id`、`engineer_id`、`project_id`、`proposal_id`、開始日、**満了日（必須）**、単価、状態（章 5.2 の 5 状態）、60 日前起票済みフラグ、30 日前再通知済みフラグ。🔴 **経路 5 の当事者は `engineer_id` の所有パートナーから導出する**（独立した入力項目として持たない。`F-065` 処理 ①） |
...
| `Plan` | プラン名、席数上限、🔴 **AI 利用量クォータを 2 種類持つ** — ①**利用者に見せる件数クォータ**（スキルシート解析 / 根拠文 / 提案ドラフト / 延長論点の整理。各 N 件 / 月）②**運営者の内部指標である金額上限**（USD / 月・USD / 日）。両者の換算は 1 件あたり標準原価による（章 7.5 / `docs/03` §7.6）。メール上限（日次 / 分次）、ストレージ上限、機能フラグの既定、粗利率の閾値、月額単価（**席単価は未確定。`Q-20`**）、超過分の従量単価（**件数単位で提示する**） |
...
| 対象 | 保持期間 | 根拠 |
|---|---|---|
| エンジニアの連絡先、`SkillSheet` の原本、**`EngineerCareer`（経験内容）** | 稼働終了・提案終了から **3 年**（A-05 / `F-046`）。🔴 **経験内容を対象に含めるのは本書が置いた既定であり、`BR-29` / `CLAUDE.md` §3.5 の削除対象の列挙（連絡先・スキルシート原本）には経歴が入っていない**（A-24。列挙への追加は上流の判断事項） | `BR-29` |
...
| `PURGED` 時に削除するもの / 残すもの | 削除: エンジニアの連絡先・`SkillSheet` 原本・`Message` 本文・**`EngineerCareer`（A-24 の既定）**。残す: **法令上の保持義務がある範囲の `AuditLog`**（**`F-064 AC-2` `AC-3`** / 章 5.4） | `BR-27` / `BR-29` |
...
| **パートナースコープ（第二境界）** | `partner_company_id` によるスコープを重ねて適用（`BR-04` / `BR-07`）。🔴 **越境経路 5（`F-065` / `F-066`）はこの境界を弱めない** — 開くのは**自社が当事者のレコードだけ**であり、当事者判定も認証コンテキストから決まる（`BR-65` / `BR-03`） | パートナーの全画面・全 API・集計・通知・エクスポートに他パートナー由来の情報が 0 件。**件数・存在の示唆も 0 件**（`F-004 AC-3` / `AC-4` / **`F-065 AC-3`** / **`F-066 AC-4`**） |
...
🔴 **利用者に見せる単位と、運営者が見る単位を分ける**（`BR-24`。決定済み（2026-09-01、[Issue #12](https://github.com/Festal-KM/SES-Platform/issues/12)）/ `docs/03` §7.6）。**利用者（テナント）に見せるクォータと残量は件数**とし、**金額を出さない**（`F-027 AC-6`）。単位は 6 つ: ①**スキルシート解析 N 件 / 月**（`sheet-parser` + `skill-normalizer`。標準原価 $0.033 / 件）②**マッチング候補の根拠文 N 件 / 月**（`match-explainer`。$0.004）③**提案ドラフト N 件 / 月**（`proposal-drafter`。$0.021）④**延長論点の整理 N 件 / 月**（`renewal-advisor`。$0.017）⑤**メール N 通 / 月** ⑥**ストレージ N GB**。🔴 **`gate-inspector` は利用者クォータに含めない**（原価としては計上するが分母・分子に入れない。`F-027 AC-7` / `docs/03` §7.6.1） — **ゲートは外部共有の前提条件であり、前提条件に従量の上限を掛けると「クォータ切れでゲートが実行できない」というゲート FAIL より悪い状態を生む**（`CLAUDE.md` §3.3）。**金額（USD）の上限は運営者の内部指標**であり `F-057` / `F-063` にのみ現れる。**件数は金額から割り戻さず独立に計測する**（`F-026 AC-6`）。**プラン別の件数クォータの初期値は暫定**（`docs/03` §7.6.2。A-14 / `Q-15`）、**席単価は未確定**（`Q-20`）。

**組織ごとの上限と超過時の挙動**（`BR-24` / `F-027`）:

| 対象 | 上限（既定） | 超過時の挙動 |
|---|---|---|
| AI コスト（日次） | テナントあたり 1 日の上限（USD。**運営者の内部指標**）。プランごとに設定 | AI 機能を停止し、**残量（件数）・上限（件数）・リセット時刻・停止理由**を画面に表示する。**金額は表示しない**（`F-027 AC-6`）。🔴 **ゲートは実行されず未実行のまま保持され、対象は `GATE_RUNNING` に留まる（`GATE_FAILED` にしない）。送信はできない**（`F-027 AC-5`。上限到達は「保留」であって元データの欠陥ではないため。`CLAUDE.md` §4.2）。🔴 **公開中の案件の再検査（`F-014 AC-12`）も同じ扱いで、再検査を保留し公開を解除しない**（原価で止まっただけの案件を取引先の画面から消さない。A-26） |
| AI 利用量クォータ（月次） | プラン別。**利用者には 4 つの件数**（スキルシート解析 / 根拠文 / 提案ドラフト / 延長論点の整理）、**運営者には金額（USD）**（初期値は暫定。`docs/03` §7.6.2 / A-14 / `Q-15`） | 停止せず従量課金に移行。**超過が始まったことと現時点の請求見込み額をテナントに表示する**（**請求見込みのみ金額でよい**。残量は件数） |
| メール | 1 テナント 1 日 500 通 / 1 分 30 通 | 分次超過は待機、日次超過は停止。区別して表示する |
| ストレージ | プラン別 | 上限接近を通知し、超過でアップロードを停止する |
| 電子署名 | 1 契約 1 リクエスト | 再送は人間の明示操作のみ |

**管理者への可視化**: テナントの `OWNER` / `ADMIN` は自テナントの消費量・残量・消化率を**件数で**常時確認できる（`F-026` / `F-027`。超過分の請求見込み額のみ金額）。**80% 到達で通知**する（`F-027 AC-4`）。運営者は `F-057` / `F-063` で**金額と件数の両方 + 消費率 + 基準比の倍率**を横断的に確認する（`F-063 AC-5`）。
...
| 見込み客サンドボックス | `sandbox` | 🔴 **宛先の分類で分ける**（`CLAUDE.md` §11.1。分類表は下記）。**業務上の外部送信（提案 / 面談調整 / 契約書送付 / 電子署名）はモック。パートナー所属利用者宛もモック（取引先招待は代わりに招待リンクを画面表示）。ホスト所属利用者宛のアカウント運用のメール（招待 / パスワードリセット / 期限予告 / 削除予告 / スキャン失敗の周知 / 上限接近の通知）は実送信**。AI・ストレージ・スキャンは本番同等 | 🔴 **本番と同じ**（分離・記録・暗号化・保持期間） | **見込み客の実データ** |
| ステージング | `staging` | 各サービスの sandbox | 本番同等 | 合成 |
| 本番 | `production` | 実 | 本番 | 顧客の実データ |

🔴 **守るべき危険（1 行）**: **本番以外の環境から、実在する取引先・第三者へ、業務上の提案メール・面談調整・契約書・電子署名依頼が到達すること。** これを設計で物理的に不可能にする。**ホスト（契約 SES 企業）に所属する利用者本人宛のアカウント運用メールはこの危険にあたらない**（NFR-ENV-1）。🔴 **一方、取引先企業の担当者（`PARTNER_ADMIN` / `PARTNER_SALES`）は、テナントに所属していても「実在する取引先」であるため、この危険の側に置く**（**決定済み（2026-09-01、[Issue #9](https://github.com/Festal-KM/SES-Platform/issues/9) / [Issue #10](https://github.com/Festal-KM/SES-Platform/issues/10)）。`CLAUDE.md` §11.1 に反映済みであり、「暫定 / 確認中」ではない**。`## Assumptions` A-22）。

🔴 **宛先の分類（`sandbox` の判定表）** — **メール送信の単一経路は、次の分類のいずれかを「宛先の分類」として必須の入力に取る**（`docs/05` への申し送り 10）。分類が決まれば `sandbox` での扱いが一意に決まる。**分類を省略した送信を成立させない。既定値を置く場合はモック側に倒す。**

| # | 宛先の分類 | 具体的な宛先 | 本書で該当する送信 | `sandbox` での扱い |
|---|---|---|---|---|
| 1 | **ホスト所属利用者** | 契約 SES 企業に所属する `OWNER` / `ADMIN` / `SALES` / `VIEWER`（**招待中の本人を含む**） | 招待（`F-002`）/ パスワード再設定（`F-003`）/ スキャン失敗の周知（`F-011`）/ 上限接近の通知（`F-027`）/ 通知（`F-039`）/ 期限予告（`F-054`）/ 削除予告（`F-064`） | 🔴 **実送信** |
| 2 | **パートナー所属利用者** | `PARTNER_ADMIN` / `PARTNER_SALES`。**実在する取引先企業の担当者**であり、テナントに所属していても第三者側に置く | 取引先の招待（`F-007` 経由の `F-002`）/ パートナー担当者宛のスキャン失敗の周知（`F-011`）・通知（`F-039`） | 🔴 **モック**。取引先招待は代わりに**招待リンクを画面に表示**する（`F-007 AC-4`） |
| 3 | **提案先・エンド企業** | テナントに所属しない提案先・エンド企業の担当者 | 提案メール（`F-022`）/ 面談日程調整（`F-041`）/ 契約書の送付（`F-047`）/ 電子署名依頼（`F-049`） | 🔴 **モック** |
| 4 | **エンジニア本人** | テナントに所属しないエンジニア本人 | **本書に該当する送信は無い**（エンジニア本人へ直接送る機能を持たない）。将来追加する場合も本分類 | 🔴 **モック** |
| — | （分類外）**運営者** | `PlatformUser`。テナントに所属せず、**自社スタッフであって実在の取引先ではない** | 運営者の招待・パスワード再設定（`F-055`） | 🔴 **実送信**（`F-055` 外部 API 依存欄） |

**分類の判定順**: ①宛先が `PlatformUser` か（分類外・実送信）→ ②宛先がパートナー企業に所属する利用者か（分類 2・モック）→ ③宛先が当該テナントに所属する利用者か（分類 1・実送信）→ ④それ以外（分類 3 / 4・モック）。**②を③より先に判定する**（順序を逆にするとパートナー担当者が分類 1 に落ちる）。

**受け入れ基準**:

- **NFR-ENV-1**（`BR-45`）: 🔴 **守るべき条件は「実在する取引先・第三者（取引先企業の担当者・提案先・エンド企業・エンジニア本人の実アドレス）宛に、業務上の外部送信が 1 通も到達しない」ことである。** `BR-45` の射程は**宛先が第三者である送信**であり、**ホスト所属利用者（分類 1）と運営者（分類外）**に宛てたアカウント運用のメールは射程外である（`docs/01` `BR-45` / `CLAUDE.md` §11.1）。
  - 🔴 **この区別は列挙ではなく上表の宛先の分類で判定する。** **本書の列挙に無い送信が新たに増えた場合も、列挙を増やすのではなく分類で決める。判断に迷うものは業務上の外部送信（モック）側に倒す**（`CLAUDE.md` §11.1）。**取引先企業の担当者（分類 2）は、テナントに所属していてもこのタイブレーカーによりモック側に置く。**
  - **モック側**（分類 2 / 3 / 4）: 取引先の招待（`F-007` 経由の `F-002`）、パートナー担当者宛の周知・通知（`F-011` / `F-039`）、提案メール（`F-022`）、面談日程調整の連絡（`F-041`）、契約書の送付（`F-047`）、電子署名依頼（`F-049`）。
  - **実送信側**（分類 1 / 分類外）: ホスト所属利用者宛の招待（`F-002`）、パスワード再設定（`F-003`）、`sandbox` の期限予告（`F-054`）、削除予告（`F-064`）、スキャン失敗の担当者周知（`F-011` 処理 ④）、上限接近の通知（`F-027` 処理 ④）、アプリ内 / メール通知（`F-039`）、運営者の招待・パスワード再設定（`F-055`）。
  - **`development` / `demo`**（🔴 **区別を適用せず全送信系モック**）: E2E で**全分類の送信すべて**を実行し、**外部エンドポイントへの発信が 0 件**であることを検証する。`demo` は合成データのみを扱い、宛先自体が架空であるため区別する必要がない（`CLAUDE.md` §11.1「この区別は `sandbox` にのみ適用する」）。
  - **`sandbox`**（🔴 **宛先の分類で分ける**）: 次の 3 点を**別々の受け入れ基準として**検証する。
    - ① **分類 3 / 4 宛の業務上の外部送信**（`F-022` / `F-041` / `F-047` / `F-049`）を E2E で実行し、**外部エンドポイントへの発信が 0 件**であることを検証する。
    - ② **分類 1（ホスト所属利用者）宛のアカウント運用のメール**（ホスト社員の招待 `F-002` / `F-003` / ホスト担当者宛の `F-011` / `F-027` / ホスト受信者宛の `F-039` / `F-054` / `F-064`）と**分類外（運営者）宛の送信**（`F-055`）を E2E で実行し、**実際に送信されること**、および送信された全通の宛先が**ホスト所属利用者（招待中の本人を含む）または運営者（`PlatformUser`）のアドレスのみ**で、**パートナー所属利用者・提案先・エンド企業・エンジニア本人のアドレスが 1 件も含まれない**ことを検証する。①の「0 件」を②に適用しない（**本人に届かないと `sandbox` に入れず、期限にも気づけない**ため。`F-054 AC-9` / `F-064 AC-10`）。
    - ③ **分類 2（パートナー所属利用者）宛の送信**（`F-007` 経由の取引先招待 / パートナー担当者宛の `F-011` / `F-039`）を E2E で実行し、**外部エンドポイントへの発信が 0 件**であることを検証する。あわせて、**取引先招待については招待リンクが画面に表示されコピーでき、そのリンクから受諾してパートナーがログインできる**ことを検証する（`F-007 AC-4`）。**`F-054 AC-1` が要求するパートナースコープの検証は、この経路でパートナーを参加させて成立させる。**
  - **`staging`**（外部 API は各サービスの sandbox）: 全分類の E2E を実行し、**各サービスの sandbox エンドポイント以外への発信が 0 件**であること、および sandbox エンドポイントへ渡す宛先が**テスト用アドレスのみで、実在の取引先ドメインを 1 件も含まない**ことを検証する。「`staging` では発信が 0 件」と読み替えない。
...
- **NFR-ENV-6**（`BR-63`）: `demo` と `sandbox` が同一環境・同一テナントで兼用されない。`demo` に見込み客の実データを投入する経路が存在せず、`sandbox` に合成データが投入されない（`F-053 AC-4`）。**合成データの投入とリセットは `APP_ENV` が `demo`（およびローカル開発・自動テスト用の `development`）の場合にのみ実行でき、`production` / `sandbox` / `staging` では導線が存在せず API も拒否される**（`F-053 AC-6`）。
...
- **NFR-ENV-10**（`BR-71` / `BR-51`。**決定済み（2026-09-01、[Issue #13](https://github.com/Festal-KM/SES-Platform/issues/13)）**）: 🔴 **`production` では、取引先へ届く送信（`F-007` の取引先招待 / `F-022` / `F-041` / `F-047` のメール送付）は、送信元テナントの独自ドメインが検証済みでなければ実行されない**（`F-001 AC-4`）。**共通ドメインへのフォールバックが存在しない** — フォールバックは「成功したように見えて `BR-51` に違反している」壊れ方を生む（NFR-ENV-3 と同型の原則）。テナント所属利用者宛・運営者宛（分類 1 / 分類外）は共通ドメインでよい。**`sandbox` は共通ドメインで動かす**（取引先の担当者宛は分類 2 でモックのため実送信されない）が、🔴 **`SANDBOX` → `ACTIVE` の移行条件に独自ドメインの検証完了を含める**（`F-054` / `docs/03` §3.2.7 規律 4。試用でできたことが本番でできない状態を作らないため。`docs/01` `R-11`）。
...
- シードデータは**実在の企業名・実在の個人名・実データ由来のスキルシートを含まない**（`F-053 AC-1`）。/ リセットで初期状態に戻せること（`F-053 AC-2`）。
- Phase 0 の分離テスト用に、**2 テナント × 2 パートナー**のシードを用意する（`CLAUDE.md` §5 Phase 0 の成功条件）。/ **`sandbox` には合成データを投入しない。**
...
|---|---|---|---|
| メール | **1 テナント 1 日 500 通 / 1 分 30 通**。プランごとに上書き可。🔴 **これとは別に、送信基盤にアカウント（環境）全体の上限がある**（下記の「送信基盤（環境全体）の上限到達時の保留と復帰」） | 実行前にカウンタを判定し、超過する要求を発行しない。分次超過は待機キュー、日次超過は停止 | 80% でテナント管理者に通知（`F-027 AC-4`）。運営者は `F-057` / **環境全体の上限は `F-059 AC-7`** |
| LLM（Anthropic Claude API） | **テナントあたり 1 日のコスト上限（USD。運営者の内部指標）** + **月次の件数クォータ（利用者に見せる単位）** | 呼び出し前に当日累計コストと件数消費を判定。超過する呼び出しを実行しない。**`gate-inspector` は件数クォータの判定対象外**（`F-027 AC-7`） | 80% で通知。到達で AI 機能を停止し、**残量を件数で**表示して理由を示す（金額は出さない。`F-027 AC-6`） |
| 電子署名 | **1 契約につき 1 リクエスト** | 契約単位の一意性で担保。再送は人間の明示操作のみ | — |
| ウイルススキャン | サービス上限に依らずキュー長で制御 | 待機中は「検査中」と表示し、共有操作を選択できなくする | 滞留を `F-059` で検知 |

- 🔴 **送信基盤（環境全体）の上限到達時の保留と復帰** — 上表のテナント単位の上限とは別に、**メール送信基盤にはアカウント（環境）全体の送信上限がある**（値は `docs/03` の選定結果に従う。`docs/05` `## TBD` TBD-12）。この上限に到達している間、当該メールは**外部への送信を 1 回も試みずに「保留」とし、上限のリセット後に自動で送信する**。①🔴 **保留を「失敗」として扱わない** — 送信を試みていないため `BR-22`（`CLAUDE.md` §3.4）の自動リトライ禁止に**当たらない**（禁止されるのは外部 API 呼び出しの結果が確定した後の再試行であり、保留はその手前の状態である）。②🔴 **新しい状態機械を作らない** — 保留は**メール配送レコードの属性**として持ち、`Proposal` / `Contract`（章 5.1 / 章 5.5）の状態を追加も遷移もさせない。取引先へ届く送信（`F-022` / `F-041` / `F-047`）が保留された場合、対象は `APPROVED` / `DRAFT` のままで `SUBMITTING` / `SENDING` に入らず、`SUBMIT_FAILED` / `SEND_FAILED` にも落とさない（**送信ドメイン未検証時の保留（NFR-ENV-10 / `F-001 AC-4` / `F-022 AC-7`）と同じ構造**）。③**テナント利用者に失敗として見せない** — 保留中である旨と再送予定を示し、「送れなかった」と誤認させない。④🔴 **保留したまま取りこぼさない** — ホスト所属利用者宛のアカウント運用のメール（招待 `F-002` / 期限予告 `F-054 AC-9` / 削除予告 `F-064 AC-10`）は**保留解除後に必ず配送される**。特に削除予告は、**配送されるまで削除を実行しない**（`F-064 AC-10`）。⑤上限への到達・接近と保留件数は運営者の監視項目に現れる（`F-059 AC-7`。**テナント単位の日次上限とは別項目**）。
...
| **運用メトリクス** | 失敗ジョブ数、`SUBMITTING` 滞留、`SENDING` 滞留、未対応 `SUBMIT_FAILED` / `SEND_FAILED`、**`GATE_RUNNING` 滞留**（`F-059 AC-6`。AI コスト上限による未実行と、ジョブ障害による滞留を区別して数える）、スキャン失敗、署名未着、**ゲート FAIL 率**、満了アラート未起票、計測欠測（`F-059`） |
...
- **越境経路の固定**: 経路 1〜5 以外を追加しない前提で設計する。**経路の追加、経路 4 の匿名表示 5 項目の追加、経路 5 の開示項目（`BR-66`）の追加は、いずれも人間の承認事項**（`CLAUDE.md` §8.6）であり、拡張点として設計に埋め込まない。/ **外部サービスの差し替え**: メール（**Amazon SES**）・電子署名（**BYO / DocuSign**）・スキャン・決済は正規化された内部型を境界とし、サービス差異をドメイン層に漏らさない。**第二コネクタ（クラウドサイン）を追加しても `Contract` の状態機械（章 5.5）が変わらない**こと。/ **後続フェーズの余地**: Phase 4（メール取り込み / 面談メモの様式化 / タグ運用 / 外部連携）を見越し、`ProposalEvent` と `Task` を種別で拡張できる形にする。

### 7.11 用語・文言の統一（`BR-32`）

- ユーザー向け文言を 1 箇所に集約し、コンポーネント内にハードコーディングしない。/ **同じ概念が画面ごとに違う語で呼ばれない。** 特に境界と責任に関わる語（「提案」「提案依頼」「公開」「共有」「承認」「送信」「見送り」「辞退」）は、本書の用語を正とする。
...
|---|---|---|---|---|---|---|
| `sheet-parser` | `F-032` | スキルシートから経歴・スキル・期間・業務内容を構造化抽出 | **マスキング後**のスキルシート本文（`CLEAN` の版のみ） | `{ 経歴[]（期間 / 役割 / 業務内容 / 使用技術）, スキル[]（名称 / 経験年数）, 未抽出項目[] }`。🔴 **経歴の反映先は `F-008 AC-5` の `EngineerCareer` と同一の項目構成**（A-24） | ①出力がスキーマに適合する ②**推測で埋めない**（不明は `未抽出項目` に入れる）③マスキング前の PII を含まない ④**採用した行が手入力の行を黙って上書きしない**（`F-008 AC-8`） | 抽出結果を空にし「読み取り失敗」として手入力へ誘導。台帳の既存値を変更しない（`F-032 AC-4`） |
...
- **受け入れ基準**: ①同一の案件・候補・重み設定に対し、実行のたびに同じスコアと同じ順位が返る ②`match-explainer` が失敗しても順位が変化しない（`F-031 AC-1`）③Phase 1 の並び順（`F-009`）も決定的であり、LLM を経由しない。**Phase 1 に重み設定は存在しない**（`F-030 AC-4`）。「並び順」は Phase 1 の決定的順序、「順位 / スコア / 重み」は Phase 2 の概念であり、語を使い分ける。
...
- 🔴 **「判定不能 = FAIL」は LLM を呼んで失敗した場合の規律であり、AI の日次コスト上限で呼べない場合は含まない。** 上限到達時はゲートを実行せず未実行のまま保持し、対象は `GATE_RUNNING` に留まる（`GATE_FAILED` にしない。送信もできない。`F-027 AC-5`）。**元データの欠陥（`GATE_FAILED`）とテナントの上限到達による保留を混ぜない**（`CLAUDE.md` §4.2「失敗と保留を混同しない」）。混ぜるとゲート FAIL 率（`F-059 AC-6`）が汚れ、直すべき元データが無いのに修正を促す導線になる。**新しい状態は追加しない。** 全層 PASS のときのみ `APPROVAL_PENDING` へ進み、1 層でも FAIL なら `GATE_FAILED`（章 5.1）。🔴 **ゲートの対象は 5 種（提案 / スキルシートの外部共有 / 案件の公開 / チャット添付 / 契約書）である**（`BR-15`。契約書の追加は 2026-09-01 決定、[Issue #15](https://github.com/Festal-KM/SES-Platform/issues/15)）。**契約書も同じ規律で、FAIL の間は `DRAFT` → `SENDING` に進めない**（`F-020 AC-1` / `F-047` 処理 ⑥ / `F-048 AC-3`）。🔴 **案件の公開だけは「共有状態へ進む時点」に加えて「共有状態が続いている間」も射程であり、公開中に公開欄 3 欄が変更されたら再検査して商流層 FAIL なら公開を解除する**（`F-014 AC-6`〜`AC-8`。A-26。[Issue #42](https://github.com/Festal-KM/SES-Platform/issues/42) = 回答②。**状態機械は変えない** — 案件の公開は `Proposal` のような状態を持たず、公開範囲が成立するか否かだけで表す）。**発注書は対象外。**
...
| 6 | **外部サービスの規約遵守と送信元名義** — 取引先へ届く送信が運営者名義にならない | 🔴 **決定済み（2026-09-01）**: 電子署名は**テナントのアカウントを接続する BYO 方式**（`F-049` 外部 API 依存 / `AC-8` / `BR-70`。第一コネクタ DocuSign）、メールは**テナント独自ドメインの検証を前提**（`F-001 AC-4` / `F-022 AC-7` / NFR-ENV-10 / `BR-71`）。`BR-51` は両者で担保される（A-11） |
...
| **コスト上限到達** | 再試行せず即座に停止し、残量と理由を表示する（`F-027 AC-1`）。🔴 **`gate-inspector` は実行せず、対象を `GATE_RUNNING` のまま保持する（`GATE_FAILED` にしない）。送信はできない**（`F-027 AC-5` / 章 8.5）。**これは LLM の失敗ではなく「未実行」であり、障害指標に加算しない**（`F-059 AC-6`）。🔴 **公開中の案件の再検査では、保留のまま公開を維持する**（`F-014 AC-12`。**商流層 FAIL に倒さない**） |
| **安全側の倒し方** | `gate-inspector` の PII 層・商流層は**判定不能（LLM の失敗・タイムアウト・スキーマ違反）= FAIL**。`sheet-parser` / `skill-normalizer` は**推測で埋めない**。`match-explainer` / `renewal-advisor` は**空にして本体機能を継続**。`proposal-drafter` は**空にして手入力へ** |
| **再試行の記録** | 再試行を含むすべての呼び出しが `AiUsage` に記録され、原価に計上される（`F-026 AC-1`） |
| **業務の継続性** | 🔴 **AI の失敗が、`BR-34`（満了アラート）・`BR-16`（承認）・`BR-21`（送信の一回性）のいずれをも損なわない。** AI が全停止しても、`F-043` の起票と通知、`F-021` の承認、`F-022` の送信は成立する（LLM の失敗ならゲートの PII 層・商流層が判定不能 = FAIL、コスト上限による停止ならゲートが未実行のまま `GATE_RUNNING` に留まり、**いずれの場合も送信は安全側に止まる**。`F-027 AC-5`） |
...
| **越境経路の追加、経路 4 の匿名表示 5 項目の追加、経路 5 の開示項目（`BR-66`）の追加** | 情報境界の前提が変わる。**人間の承認事項**（`CLAUDE.md` §8.6） | 本書は**経路 1〜5**・匿名 5 項目・`BR-66` の開示項目を固定として設計している。拡張点として設計に埋め込まない。**経路 5 自体は 2026-09-01 に人間が承認して追加された**（[Issue #8](https://github.com/Festal-KM/SES-Platform/issues/8)） |
...
| **A-04** | 🔴 **匿名候補の丸め方（出典: `docs/03` §4.13.1）**: ①**スキル**は辞書の正規化済み名称のみ・フリーテキスト不可・**上位 8 件まで**（経験年数の降順、同順は辞書 ID 順の決定的な順序）②**経験年数**は 5 段階（`1 年未満` / `1〜3 年` / `3〜5 年` / `5〜10 年` / `10 年以上`）③**単価**は **10 万円刻み**のレンジ（例 `60〜70 万円`。上限は `100 万円以上` で打ち止め）④**稼働可能時期**は 5 段階（`即日` / `当月中` / `翌月` / `翌々月` / `3 か月以降`）⑤**勤務地**は**都道府県のみ**（市区町村・沿線・駅名は出さない）+ リモート可否 3 値（`フルリモート可` / `一部リモート可` / `常駐のみ`）。単価を 10 万円刻みに粗くしたのは**単価と稼働可能時期の組み合わせが個人を最も強く特定するため**、スキルに上限を設けたのは**スキルの組み合わせが事実上の指紋になるため**（`docs/03` §4.13.1）。🔴 **開示する項目そのものは 5 項目のまま増やさない**（`BR-54` / `CLAUDE.md` §8.6。スキルの上位 8 件は項目の追加ではなく 1 項目内の粒度である）。**k-匿名性による件数閾値は Phase 1 では入れない**（母集団が小さい立ち上げ期にほとんど表示されなくなり、経路 4 が使えない機能になるため。`docs/03` §4.13.2-4 も「一意率は運営平面の監視指標として出すが、Phase 1 では表示を抑止しない」としており本書と一致する） | `F-017 AC-1` / `AC-3` / `F-031 AC-3` / 章 6.3。章 9 に「Phase 1 で k-匿名性を入れない」理由を記載 | ~~暫定。[Issue #5](https://github.com/Festal-KM/SES-Platform/issues/5) で確認中~~ → 🔴 **確定**（[Issue #5](https://github.com/Festal-KM/SES-Platform/issues/5) = 回答「OK です」、2026-09-10。承認者は事業責任者単独 = [Issue #18](https://github.com/Festal-KM/SES-Platform/issues/18) 回答①。`Q-17` / `R-9` / `CLAUDE.md` §9-13 / `docs/03` §4.13.1 の `Q-T-2` と同一の論点）。🔴 **Phase 1 のリリース条件 `R-1` は充足済み**（`T-12-06`。承認の根拠 —— 母集団規模 / 丸め粒度 / 残存リスクの見立て —— は `docs/dev-plan.md` §8 の 2026-09-21 の行、一意率の実測は `tests/perf/anonymity.test.ts`）。**上表の値は 1 つも変わっていない。** 実データが溜まった時点での見直しは今回の承認の対象外であり、再評価のトリガは同じ §8 の行が持つ |
...
| **A-11** | 🔴 **電子署名は BYO 方式（テナントが自社アカウントを接続）。第一コネクタは DocuSign、第二候補にクラウドサイン。取引先へ届くメールはテナント独自ドメインの検証（SPF / DKIM。Amazon SES）を前提とする** | `F-049`（外部 API 依存 / 処理 ⑦ / `AC-8` / `AC-9`）/ `F-047` 処理 ⑧ / `AC-7` / `F-001 AC-4` `AC-5` / `F-007 AC-5` / `F-022 AC-7` / `F-041 AC-4` / NFR-ENV-10 / 章 8.6-6 / 章 6.1（`Tenant` の属性）。**未接続テナントでも ⑤ 契約が完了できる**（`F-049 AC-9`） | 🔴 **決定済み（2026-09-01、[Issue #11](https://github.com/Festal-KM/SES-Platform/issues/11) / [Issue #13](https://github.com/Festal-KM/SES-Platform/issues/13)）。`CLAUDE.md` §9-2 / §9-3 と `docs/01` `BR-70` / `BR-71` に反映済み**（旧 `Q-2` / `R-6` は決着）。第二コネクタの追加時期のみ未定 |
| **A-12** | **`Tenant` のライフサイクル状態は `SANDBOX` / `ACTIVE` / `SUSPENDED` / `CLOSING` / `PURGED` の 5 つ**。`SUSPENDED` はデータを消さず実行系のみ不可、`CLOSING` は新規作成不可でエクスポートのみ可、`PURGED` は終端で連絡先・スキルシート原本・チャット本文を削除、`SANDBOX` → `ACTIVE` はデータを引き継ぐ。停止（`ACTIVE` → `SUSPENDED`）は `PLATFORM_OWNER` のみ | **章 5.4**（状態遷移表 + 状態別の可否表）/ `F-004 AC-7`〜`AC-9` / **`F-064 AC-1`〜`AC-5`** / `F-052 AC-3`〜`AC-5` / `F-054 AC-3` `AC-4` `AC-7` / `F-062 AC-1`〜`AC-5` `AC-7` / 章 6.1 / 章 6.8 / `UC-22` / `UC-24` | 🔴 **決定済み（2026-09-01、[Issue #6](https://github.com/Festal-KM/SES-Platform/issues/6)）。`CLAUDE.md` §4.2 に確定事項として反映済み**（「暫定 / 確認中」ではない）。本書は §4.2 を正として引き写しており、**そこに無い遷移を追加していない**（`SANDBOX` → `SUSPENDED` 等は 422） |
| **A-13** | **`Contract` の状態は `DRAFT` / `SENDING` / `SEND_FAILED` / `UNDER_REVIEW` / `EXECUTED` / `WITHDRAWN` / `EXPIRED` の 7 つ**。`SENDING` は片道で自動リトライ禁止、再送は `SEND_FAILED` → `DRAFT` の人間操作のみ。`SEND_FAILED` / `WITHDRAWN` / `EXPIRED` を混同しない。`EXECUTED` は内容を書き換えられず訂正は新契約。**`ContractDocument` は状態機械を持たない** | **章 5.5**（状態遷移表 + 3 状態の区別表）/ `F-047 AC-1` `AC-4`〜`AC-6` / `F-049 AC-1`〜`AC-4` `AC-7` / 章 6.5 / 章 7.8 | 🔴 **決定済み（2026-09-01、[Issue #7](https://github.com/Festal-KM/SES-Platform/issues/7)）。`CLAUDE.md` §4.2 に確定事項として反映済み**（「暫定 / 確認中」ではない）。**状態機械は NDA / 基本契約 / 個別契約で共通**、**双方署名は状態を増やさず `UNDER_REVIEW` / `EXECUTED` で表す**（章 5.5 / `docs/03` §3.1.10）。本書は §4.2 に無い遷移（`EXECUTED` → `DRAFT` 等）を追加していない |
| **A-14** | 🔴 **利用者に見せるクォータ・残量は件数、金額（USD）の上限は運営者の内部指標**（決定済み）。**金額 → 件数の換算（1 件あたり標準原価）とプラン別の件数クォータの値は暫定** — スキルシート解析 $0.033 / 根拠文 $0.004 / 提案ドラフト $0.021 / 延長論点 $0.017、`gate-inspector` はクォータに含めない（`docs/03` §7.6.1 / §7.6.2。Starter / Standard / Business の件数は同 §7.6.2 の表） | 章 7.5（単位の定義と上限表）/ `F-026` 処理 ⑤ ⑥ / `AC-6` / `F-027` 入力 / `AC-1` `AC-6` `AC-7` / `F-057` 処理 ① / `F-063 AC-5` / 章 6.7（`Plan`）/ 章 7.7 / `UC-12` | 🔴 **単位は決定済み（2026-09-01、[Issue #12](https://github.com/Festal-KM/SES-Platform/issues/12)。`CLAUDE.md` §2 / §3.4 / `BR-24` に反映済み）。金額の初期値・件数クォータ・1 件あたり標準原価は暫定**（`Q-15`。Phase 2 の実測原価で見直す）。**席単価は未確定**（`Q-20`。取引先の席を課金対象に含めるかを含め事業判断） |
...
| **A-19** | **`BR` の担保状況**: **`BR-01`〜`BR-73`**（2026-09-01 追加の `BR-65`〜`BR-73` を含む）のうち、受け入れ基準または非機能要件で担保できなかったものは **0 件**。**`BR-51` が依存していた `Q-2` は決着した**ため、送信元名義も確定した要件として担保している（`F-049 AC-8` / NFR-ENV-10） | 付録 B のカバレッジ表 | `A-11` / `A-23` と連動 |
...
| ~~**A-21**~~ | ~~取引先が自社所属エンジニアの稼働（`Assignment`）と自社が当事者の契約（`Contract`）をアプリ内で閲覧することは認めない~~ — 🔴 **取り下げ（2026-09-01、[Issue #8](https://github.com/Festal-KM/SES-Platform/issues/8) で越境経路 5 が人間の承認により追加された）。** 本前提は成立しない。**番号は欠番にせず、なぜ前提が変わったかを残すために本行を保持する**（`CLAUDE.md` §8.8） | 置き換え先: **章 1.1 の経路表（経路 5）/ `F-065` / `F-066` / `F-004 AC-5`（越境は経路 1〜5）/ 章 4.1 の `◐` の定義（**「当事者」の概念を導入した**）/ 章 4.2 のマトリクス（`F-042` / `F-047` の `PA` / `PS` = `◐`）/ `F-042` `F-045 AC-5` `F-047` の関連ロール / `UC-25` / **A-23** | **取り下げ済み。** 経路 5 の開示範囲・当事者判定・段階開放は **A-23** が引き継ぐ |
| **A-22** | 🔴 **`sandbox` におけるメール送信の射程を「宛先の分類」で分ける（4 分類 + 分類外）。** ①**ホスト所属利用者**（契約 SES 企業の社員。招待中の本人を含む）宛のアカウント運用のメール（招待 `F-002` / パスワード再設定 `F-003` / スキャン失敗の周知 `F-011` / 上限接近の通知 `F-027` / 通知 `F-039` / 期限予告 `F-054` / 削除予告 `F-064`）は**実送信**。②**パートナー所属利用者**（`PARTNER_ADMIN` / `PARTNER_SALES`）宛は**モック**（取引先招待 `F-007` → `F-002` / パートナー担当者宛の `F-011` `F-039`）。③**提案先・エンド企業**宛の業務上の外部送信（`F-022` / `F-041` / `F-047` / `F-049`）と④**エンジニア本人**宛は**モック**。分類外の**運営者**（`PlatformUser`。`F-055`）宛は**実送信**。**この区別は列挙ではなく分類で判定し、迷うものはモック側に倒す。** `development` / `demo` には区別を適用せず全送信系モックとする | **章 7.6**（環境表の `sandbox` 行 / **宛先の分類の判定表** / **NFR-ENV-1** ①②③）/ `F-001` `F-002` `F-003` `F-007` `F-011` `F-027` `F-039` `F-055` `F-064` の外部 API 依存・処理 / **`F-028` 処理 ③ / `AC-2`**（バナー文言）/ **`F-007 AC-4`**（`sandbox` の招待リンク表示）/ **`F-054` 処理 ③ / `AC-1` / `AC-2` / `AC-9`** / **`F-064` 外部 API 依存 / `AC-10`** / 章 5.4 の `SANDBOX` 行 / `UC-22` 手順 3・5 / 付録 B の `BR-45` 行 / `docs/05` への申し送り 10 | 🔴 **②（パートナー所属利用者をモック側に置く）は本書が [Issue #10](https://github.com/Festal-KM/SES-Platform/issues/10) の決定として追記したもの**（2026-08-31）。取引先企業の担当者は**実在する取引先**であり、テナントに所属していても `CLAUDE.md` §11.1 のタイブレーカー「判断に迷うものはモック側に倒す」を適用する（`BR-45` は 0 件）。**取引先招待は招待リンクの画面表示で代替**し、見込み客は同僚を疑似的な取引先として招いてパートナー側の画面まで確認できる（`F-007 AC-4` / `F-054 AC-1`）。🔴 **全体として決定済み（2026-09-01、[Issue #9](https://github.com/Festal-KM/SES-Platform/issues/9) / [Issue #10](https://github.com/Festal-KM/SES-Platform/issues/10)）。`CLAUDE.md` §11.1 に確定事項として反映済みであり、「暫定 / 確認中」ではない**（`docs/01` `BR-62` / `Q-19` も決着）。**試用中の取引先招待は招待リンクの画面表示で成立させ、本番の招待は独自ドメインからのメールで届く**（`F-007 AC-4` / `AC-5` / NFR-ENV-10。`R-11`） |
| **A-23** | 🔴 **越境経路 5（当事者レコードの参照）の設計前提**: ①**当事者判定は認証コンテキストのみから行う**（`Assignment` はエンジニアの所有パートナー、`Contract` / `Order` は相手方のパートナー企業。リクエスト入力で指定させない）②**開示項目は `BR-66` の列挙に固定**（稼働＝案件名〈自社に公開済みの範囲〉/ 稼働期間 / 満了日 / 延長確認の状態、契約＝種別 / 状態 / 期間 / 自社との契約単価、契約書＝版 / 署名の状態 / **署名済み最終版のみ**、発注＝状態 / 期間 / 金額）③**読み取り専用**（書き込み API を持たない）④**段階開放**（稼働＝Phase 2 / 契約＝Phase 3）⑤**ホストの上流・粗利・内部検討・他社の当事者レコードは存在も件数も出さない** | 章 1.1 の経路表 / `F-065` / `F-066` / `F-004 AC-5` / 章 4.1 / 章 4.2（`F-042` / `F-047` / `F-065` / `F-066`）/ `F-042` `F-045 AC-5` `F-047 AC-8` の関連記述 / 章 5.5 / 章 6.5（当事者の属性）/ `UC-25` / 付録 B（`BR-65`〜`BR-69`） | 🔴 **経路 5 の追加は決定済み（2026-09-01、[Issue #8](https://github.com/Festal-KM/SES-Platform/issues/8)。人間の承認）。`CLAUDE.md` §3.1-5 / `docs/01` `BR-65`〜`BR-69` に反映済み**。**開示項目をこれ以上増やすことは人間の承認事項**（`CLAUDE.md` §8.6）。**取引先側から延長確認に直接回答する機能は今回作らない**（`## Open Questions` 参照） |
| **A-24** | 🔴 **エンジニアの経験内容と従事期間（`EngineerCareer`）を、Phase 1 から構造化して保持する。** 1 行 = 期間（開始年月・終了年月。終了は `null` = 継続中）/ 役割 / 業務内容 / 使用技術。複数行を持て、**`EngineerSnapshot` へは行単位で凍結する**。Phase 1 は手入力で、Phase 2 の `F-032`（`sheet-parser`）は**同じ項目構成へ**抽出結果を流し込む（保存先を 2 つ作らない）。🔴 **匿名共有（経路 4）の開示対象に含めない**（`BR-55`。匿名 5 項目は不変）。**本書が置いた既定が 2 つ残る**: ①**保持期間の削除対象に含める**（章 6.8。`BR-29` / `CLAUDE.md` §3.5 の列挙は「連絡先・スキルシート原本」であり経歴を含まないため、**列挙への追加は上流の判断事項**）②**必須入力にしない**（経験内容 0 行のエンジニアを登録できる。`F-008 AC-5`） | `F-008`（目的 / 入力 / 処理 ⑤〜⑦ / `AC-5`〜`AC-8`）/ `F-019`（処理 ② / `AC-5`）/ `F-017 AC-1` / `F-032` 処理 ⑤ / 章 6.2（`EngineerCareer`）/ 章 6.4（`EngineerSnapshot`）/ 章 6.8 / `docs/04` `docs/05` への申し送り | 🔴 **決定済み（2026-09-10、[Issue #35](https://github.com/Festal-KM/SES-Platform/issues/35) = 回答「A」）。既定として置いていた C（Phase 1 は `SkillSheet` ファイルのみで運用し、構造化は Phase 2 の AI 解析と同時に設計する）から変更された。** 実装は **`T-09-12`（SP-09 の先頭。Phase 1 / 第 1 回リリース）** — 🔴 **凍結は遡れない**ため、`F-019` の提案フローより前に置く。**`CLAUDE.md` の改訂は不要**（§4.1 `Engineer` の内訳の具体化であり、状態機械・越境 5 経路・匿名 5 項目のいずれも変えない）。**上記①②は本書の既定であり、異論は Issue で提起する** |
| **A-25** | 🔴 **匿名候補（`F-017`）への検索条件（`F-009`）は、A-04 の丸め後の区分に対して「区分が条件と重なりうるか」で評価する。開示していない属性（フリーワード `q` / 稼働状況 `availability`）は匿名候補に適用しない（母集団から外す根拠にしない）。** 理由: ①検索条件を**生値の述語**で評価すると、`yearsMin=7` で当たり `8` で外れる・`priceMax=650000` と `640000` で当たり方が変わる・`availableBy` の 1 日刻みで開始日が分かる、という**条件の当たり方から丸める前の値が復元できる**（丸めが表示にしか効かず、`F-017 AC-3` の趣旨が破れる）②開示していない属性の当たり外れは、**それ自体が 6 項目目の開示**になる（開示項目の追加は人間の承認事項。`CLAUDE.md` §8.6）。スキルは**表示される上位 8 件の名称**に対してだけ照合する（9 件目以降で当たると、表示されないスキルの有無が漏れる）。**帯の内側で生値をどう動かしても判定が変わらない**ことをテストで固定する。⚠️ **見え方の差**: 匿名候補は「間に合いうる」「重なりうる」側に残るため、自社候補より緩く当たる（例: `availableBy=翌月 5 日` に、翌月 15 日から稼働可能な共有候補は「翌月」の帯として残る）。これは丸めの帰結であり、絞り込みの不具合ではない旨を画面に 1 行で書く | `F-009` 処理① / `F-017` 処理⑥ / `docs/04` §S-016 実装の補足 / `docs/05` §4.6 線引き表 #8 / `packages/domain/src/anonymize/criteria.ts` / `packages/db/src/search/anonymous-candidates.ts` | ⚠️ **暫定。[Issue #51](https://github.com/Festal-KM/SES-Platform/issues/51) で確認中**（`T-08-05` の実装で置いた判断。2026-09-15）。`F-031 AC-3` と同じ原理であり、越境 5 経路・匿名 5 項目・A-04 の粒度のいずれも変えない。**生値の評価に戻すには A-04 の粒度の再承認と同じ重さの判断が要る**（表示で丸めた値を検索経路で復元できる状態を許すかどうか） |
| **A-26** | 🔴 **公開後に公開欄が編集されたら品質ゲートを再検査し、商流層 FAIL なら公開を解除する**（`F-014 AC-6`〜`AC-13`）。**再検査の対象は 3 欄（案件名 / 外部公開用の記載 / `ProjectRequirement` の自由記述）に限り、3 欄が実際に変わったときだけ実行する**（保存のたびには走らせない。AI 原価）。**公開先の増減は再検査の契機にしない**（増えた公開先への検査は公開が成立する時点の `AC-3` が行う）。**AI の日次コスト上限に達している間は解除せず保留する**（`AC-12`）。**ホストには理由を出し、取引先には出さない**（`AC-9` / `AC-10`）。**本書が残す既定が 3 つある**: ①**LLM の失敗・タイムアウト・スキーマ違反による「判定不能」も商流層 FAIL として扱い、公開を解除する**（`F-020` のフォールバックと本決定の合成。**上限到達による未実行とは区別する** — 前者は呼んで失敗した、後者は呼んでいない）②**保留中は編集後の内容が未検査のまま公開先に見えている**（`AC-12` の ⚠️。上限到達で取引先の画面から案件が消えることを避けるための帰結）③**PII 層の FAIL も商流層と同じく公開を解除する**（`AC-7`。Issue #42 の回答は商流層を明示したが、公開が成立する時点は PII 層・商流層の両方で止めており〔処理 ②〕、**公開中だけ PII 層の FAIL を許す理由が無い**） | `F-014`（目的 / 入力 / 処理 ⑤〜⑧ / AI 利用 / `AC-3` / `AC-6`〜`AC-13`）/ **`UC-26`** / `F-020 AC-1` / 章 7.5（AI コスト日次の行）/ 章 8.5 / 章 8.7（コスト上限到達の行）/ 付録 B（`BR-15` / `BR-18`）/ `ui-design` 申し送り 17 / `program-design` 申し送り 17 | 🔴 **決定済み（2026-09-10、[Issue #42](https://github.com/Festal-KM/SES-Platform/issues/42) = 回答②）。既定として置いていた選択肢①（現状維持 = 公開後の編集では公開を解除しない）から変更された。** 選択肢③（再検査するが FAIL でも公開を維持し警告のみ）は `BR-18` / `CLAUDE.md` §3.3 に反するため採らない。**`CLAUDE.md` の改訂は不要**（§3.3 のゲート対象 5 種・§4.2 の状態機械・越境 5 経路のいずれも変えず、`BR-15` の「ゲートを経ずに共有状態へ進めない」を公開中の期間にも及ぼしただけである）。**上記①〜③は本書の既定であり、異論は Issue で提起する** |
...
| ~~`Q-2`~~ | ~~契約書・メールの送信元名義をテナントごとに分けられるか~~ — **決着済み（2026-09-01）。電子署名は BYO / DocuSign、メールはテナント独自ドメインの検証を前提とする** | — | **A-11 に確定事項として記載**（`BR-70` / `BR-71`） |
...
| `Q-7` | 満了アラートの段階数とタイミング（60 日前のみか、30 日前の再通知を置くか） | 人間（運用の実感が前提） | A-06。**60 日前 + 30 日前の再通知**で記述。🔴 **30 日前は再通知であって新しい状態ではない**ため `CLAUDE.md` §4.2 の改訂を要しない（`F-043 AC-3` / 章 5.2） |
...
| `Q-20` | **席単価（ユーザー 1 名あたりの月額）。特に取引先（`PARTNER_ADMIN` / `PARTNER_SALES`）の席を課金対象に含めるか** | 人間（事業判断。[Issue #12](https://github.com/Festal-KM/SES-Platform/issues/12)） | A-14。**料金の数値を仮置きしていない**。章 7.5 の粗利の算出定義は席単価の値に依存しない形で書いている（`docs/01` `Q-20`。取引先を招くほどコストが上がる料金体系は、章 6.2 の中核価値と逆を向く） |

> 🔴 **決着済み（2026-09-01。いずれも `CLAUDE.md` に反映済みで、本書は確定事項として書いている）**: 越境経路 5 の追加（[#8](https://github.com/Festal-KM/SES-Platform/issues/8)。A-21 を取り下げ → **A-23**）/ `Tenant`・`Contract` の状態機械（[#6](https://github.com/Festal-KM/SES-Platform/issues/6) / [#7](https://github.com/Festal-KM/SES-Platform/issues/7)。A-12 / A-13。章 5.4 / 5.5）/ `sandbox` のメール射程と取引先招待リンク（[#9](https://github.com/Festal-KM/SES-Platform/issues/9) / [#10](https://github.com/Festal-KM/SES-Platform/issues/10)。A-22。章 7.6）/ 電子署名の BYO と DocuSign（[#11](https://github.com/Festal-KM/SES-Platform/issues/11)。A-11）/ メール基盤と独自ドメイン検証（[#13](https://github.com/Festal-KM/SES-Platform/issues/13)。A-11 / NFR-ENV-10）/ クォータの単位を件数に（[#12](https://github.com/Festal-KM/SES-Platform/issues/12)。A-14。**残る `Q-15` / `Q-20` は値の問題**）/ 契約書の品質ゲート対象化（[#15](https://github.com/Festal-KM/SES-Platform/issues/15)。`F-020 AC-1`）/ 代理閲覧中の操作不可 UI（[#14](https://github.com/Festal-KM/SES-Platform/issues/14)。`F-060 AC-3`）。**これらを「暫定 / 確認中」と書いた記述は本書に残っていない。**
>
> 🔴 **決着済み（2026-09-10）**: **経験内容と従事期間（`EngineerCareer`）を Phase 1 から構造化して保持する**（[#35](https://github.com/Festal-KM/SES-Platform/issues/35) = **回答「A」**。**A-24**。`F-008 AC-5`〜`AC-8` / `F-019 AC-5`）。既定として置いていた C から変更されたため、**本書に「Phase 1 では経歴を構造化しない」と読める記述は残っていない**。**残る論点は A-24 の①保持期間の削除対象への追加（上流 `BR-29` の列挙の改訂が要るか）と②必須入力にしないことの 2 点**であり、いずれも本書の既定で進める。
>
> 🔴 **決着済み（2026-09-10）**: **公開後に公開欄（案件名 / 外部公開用の記載 / `ProjectRequirement` の自由記述 の 3 欄）が編集されたら品質ゲートを再検査し、商流層 FAIL なら公開を解除する**（[#42](https://github.com/Festal-KM/SES-Platform/issues/42) = **回答②**。**A-26**。`F-014 AC-6`〜`AC-13`）。既定として置いていた ①（現状維持）から変更されたため、**本書に「品質ゲートは公開の瞬間だけの検査である」と読める記述は残っていない**（`F-014` 目的 / 処理 ② ⑤ / `AC-3`）。**残る論点は A-26 の①判定不能（LLM 失敗）を商流層 FAIL として公開解除に倒すこと②上限到達による保留中は未検査の内容が見えていること③PII 層の FAIL も同じく公開を解除することの 3 点**であり、いずれも本書の既定で進める。
>
> 🔴 **本改訂で意図的にスコープに入れなかったもの（次の判断が要る）**: **取引先側から延長確認（`ExtensionReview`）に直接回答する機能は今回作らない。** 経路 5 は**読み取り専用**であり（`BR-68`）、延長の意思表示・単価交渉はチャット（`F-038`。経路 3）に集約する。**取引先が画面上で「延長可 / 不可」を回答できるようにするかは Phase 2 の設計時に別 Issue として起票する**（起票はオーケストレーターが行う。`CLAUDE.md` §8.6）。回答機能を作る場合、①経路 5 に書き込みが生じるため `CLAUDE.md` §3.1 の改訂が要る ②「回答」がホスト内部の検討（`BR-67`）に触れないことを設計で示す必要がある。`Q-` 番号は `docs/01` からの引き継ぎに使うため本項には振らない。

---

## 付録 A. `docs/01` 後続エージェントへの申し送り（29 項目）→ 機能 ID のマッピング

**全 29 項目を機能 ID にマッピングした。欠けている項目は無い**（項目 25〜29 は `docs/01` の 2026-09-01 改訂で追加されたもの）。

| # | 申し送りの内容 | 対応する機能 ID / 章 |
|---|---|---|
| 1 | 二重の境界を機能として定義する（**越境 5 経路**以外を作らない） | `F-004`（AC-3 / AC-5）/ `F-007` / `F-014`（経路 1）/ `F-019`（経路 2）/ `F-038`（経路 3）/ `F-016`〜`F-018`（経路 4）/ **`F-065` / `F-066`（経路 5）** / 章 1.1 の経路表 / 章 4.1 |
...
| 22 | Open Questions の未決着分を引き継ぐ | **`## Assumptions`（未決着分）**: A-01 / A-03〜A-10 / A-14 / A-15。**`## Open Questions`（未決着分）**: `Q-1` / `Q-4`〜`Q-7` / `Q-9` / `Q-10` / `Q-12` / `Q-15`〜`Q-17` / **`Q-20`**（席単価。取引先の席を課金対象に含めるか。`docs/01` 申し送り 22 が名指しで引き継ぎを求めた項目）。🔴 **`Q-2`（送信元名義）は決着済み**であり未決着分に含めない（A-11 / 本表 26 / 付録 B の `BR-51`）。🔴 **A-11 / A-12 / A-13 は 2026-09-01 に確定し（[#11](https://github.com/Festal-KM/SES-Platform/issues/11) / [#13](https://github.com/Festal-KM/SES-Platform/issues/13) / [#6](https://github.com/Festal-KM/SES-Platform/issues/6) / [#7](https://github.com/Festal-KM/SES-Platform/issues/7)）、`CLAUDE.md` §9-2 / §9-3 / §4.2 に反映済みであるため、本書は確定事項として書いている**（章 5.4 / 5.5。「暫定 / 確認中」ではない）。関連機能: `F-030`（`Q-5`）/ `F-017`（`Q-17`）/ `F-032`（`Q-6`）/ `F-043`（`Q-7`）/ `F-046`（`Q-4`）/ `F-054`（`Q-16`）/ **`F-057` / `F-063`（`Q-15` / `Q-20`）** |
...
| 24 | 取引先を「ついでの画面」として扱わない | **`UC-13`〜`UC-18` / `UC-25`**（自社営業と同粒度の 7 ユースケース）/ 章 7.1（同時接続の主体は取引先）/ 章 7.12（デバイス階層を同基準で割り当て）/ 章 4.1（◐ の意味）/ `ui-design` への申し送り 1 |
| 25 | 🔴 **越境経路 5（当事者レコードの参照）を機能化する** | **`F-065`**（稼働。Phase 2）/ **`F-066`**（契約・契約書・発注。Phase 3）/ `F-004 AC-5` / 章 1.1 の経路表 / 章 4.1 の `◐` / 章 4.2（`F-042` / `F-047` = `◐`）/ `F-042` `F-045 AC-5` `F-047 AC-8` / `UC-25` / **A-23**（A-21 は取り下げ） |
| 26 | 電子署名の BYO と、テナント独自ドメインからの送信を機能化する | `F-049`（外部 API 依存 / 処理 ⑦ / `AC-8` / `AC-9`。**未接続でも ⑤ が完了する**）/ `F-047` 処理 ⑧ / `AC-7` / `F-001 AC-4` `AC-5`（**開設の完了条件**）/ `F-007 AC-5` / `F-022 AC-7` / `F-041 AC-4` / **NFR-ENV-10** / `F-059` 入力 / `AC-5`（未検証テナントの検知）/ `F-054 AC-3`（移行条件）/ 章 6.1 / A-11 |
| 27 | 契約管理を Phase 3 の機能として定義する（ゲート対象・片道・3 つの終わり方・締結後不変・種別共通） | `F-047`（処理 ⑥ ⑧ / `AC-1` `AC-4` `AC-5`）/ `F-048` / `F-049`（`AC-2` `AC-3`）/ **章 5.5**（遷移表 + 区別表 + **NDA / 基本契約 / 個別契約で共通・双方署名で状態を増やさない**）/ `F-020 AC-1`（ゲート 5 種）/ 章 8.5 |
| 28 | テナントの契約状態ごとの業務可否を機能化する | `F-004 AC-7`〜`AC-9` / **章 5.4**（状態別の可否表）/ `F-062 AC-1`〜`AC-5`（停止は `PLATFORM_OWNER` のみ）/ `F-064 AC-5`（`CLOSING` の返却）/ `F-052 AC-3`〜`AC-5` / `UC-24` |
| 29 | 🔴 **今回の改訂で下流が直すべき点**（経路 4 → 5 / クォータの単位 / ゲート 5 種 / 状態機械の確定 / 代理閲覧 UI / 電子署名とドメイン） | 経路: 本表 25 / クォータ: `F-026 AC-6` `F-027 AC-6` `AC-7` `F-063 AC-5` 章 7.5 / ゲート: `F-020 AC-1` 章 8.5 / 状態機械: 章 5.4 / 5.5 と A-12 / A-13（**確定として記述**）/ 代理閲覧: `F-060 AC-3`（**非表示 + 理由の常時表示**）/ 電子署名・ドメイン: 本表 26。**画面・実装への反映は「後続エージェントへの申し送り」に列挙** |

## 付録 B. `BR-01`〜`BR-73` のカバレッジ

**担保できなかったルールは 0 件**（2026-09-01 追加の `BR-65`〜`BR-73` を含む）。**`BR-51` の依存先だった `Q-2` は決着した**（A-11）。
...
| BR-04 | `F-004 AC-3` / `F-007 AC-1` / `F-006 AC-1` / **`F-045 AC-5`**（還流でパートナーに載るのは稼働状況と稼働可能時期の属性のみ） | BR-36 | `F-055 AC-1` / `AC-2` |
| BR-05 | `F-004 AC-5`（**経路 1〜5**）/ 章 1.1 の経路表 / `F-065` / `F-066` | BR-37 | `F-056 AC-3` / 章 2.8 共通前提 / 章 4.4 / **`F-053 AC-6`**（`demo` 以外での投入・リセットの禁止） |
| BR-06 | `F-019 AC-1` / **`AC-5`** / `F-008 AC-3` / **`AC-7`** | BR-38 | `F-060 AC-1` / `AC-2` / `AC-5` / `AC-6` |
| BR-07 | `F-004 AC-4` / `F-014 AC-4` / `F-019 AC-4` / `F-038 AC-2` / **`F-065 AC-3`** / **`F-066 AC-4`** | BR-39 | `F-060 AC-3` / **章 4.5** |
...
| BR-13 | 章 8.2 再現性 / `F-031 AC-4` / `F-032 AC-5` / `F-034 AC-4` / `F-044 AC-4` | BR-45 | `F-022 AC-4` / `F-041 AC-3` / `F-049 AC-5` / `F-053 AC-5` / **`F-054 AC-2`（分類 2 / 3 / 4 宛は 0 通）+ `F-007 AC-4`（取引先招待は招待リンク表示）+ `F-054 AC-9`（分類 1 = ホスト所属利用者宛は実送信。A-22）** / **章 7.6 の宛先の分類の判定表 + NFR-ENV-1 ①②③** |
| BR-14 | `F-029 AC-1` / `F-031 AC-1` / 章 8.3 | BR-46 | `F-022 AC-5` / **NFR-ENV-3** / **NFR-ENV-4** |
| BR-15 | `F-020 AC-1`（**5 種。契約書を含む**）/ **`F-014 AC-3`（公開の時点）/ `F-014 AC-6`〜`AC-8`（公開後の再検査と自動解除。A-26）** / `F-038` / `F-047` 処理 ⑥ / `F-048 AC-3` / 章 8.5 | BR-47 | `F-053 AC-1` / **NFR-ENV-8** |
| BR-16 | `F-021 AC-1` | BR-48 | `F-028 AC-1`〜`AC-3` / **NFR-ENV-9** |
| BR-17 | `F-021 AC-2` `AC-3` `AC-5` / `F-035 AC-3` | BR-49 | `F-021 AC-4` / 章 7.12 |
| BR-18 | `F-020 AC-2` / **`F-014 AC-9`**（自動解除を無視して公開を続ける導線が無い）/ 章 8.5 | BR-50 | `F-021 AC-6` / 章 7.12 |
| BR-19 | `F-035 AC-2` / 章 8.4-2 | BR-51 | **決着済み**: `F-049` 外部 API 依存 / `AC-8`（**BYO / DocuSign**）+ `F-001 AC-4` / **NFR-ENV-10**（**独自ドメイン**）/ 章 8.6-6（A-11） |
| BR-20 | `F-035 AC-1` `AC-4` / `F-033 AC-3` / 章 8.4 | BR-52 | `F-008 AC-1` / **`AC-5`**（経験内容・従事期間は収集範囲内）/ 章 8.6-1 |
...
| BR-23 | `F-018 AC-5` / `F-024 AC-2` / `F-051 AC-2` / 章 5.1 の指標対応表 | BR-55 | `F-017 AC-1` / `AC-2` / **`F-008 AC-7`**（経歴を匿名候補に出さない）/ `F-037 AC-3` / 章 6.3 |
| BR-24 | `F-027 AC-1`〜**`AC-7`**（**残量は件数・金額を出さない・ゲートはクォータ外**）/ `F-026 AC-6` / `F-063 AC-5` / `F-061 AC-1` / 章 7.5 | BR-56 | `F-016 AC-5` / `F-017 AC-5` / `F-009 AC-3` |
...
| BR-31 | `F-004 AC-6` / `F-012 AC-3` / **章 4.3** | BR-63 | `F-053 AC-4` / **`F-053 AC-6`** / `F-054 AC-6` / **NFR-ENV-6** |
| BR-32 | 章 7.11 | BR-64 | `F-054 AC-3`〜`AC-5` / **`F-064 AC-1`〜`AC-5` `AC-10`**（期限到来で削除、削除前に返却と予告） / **章 5.4 遷移 1 / 2 / 7（遷移 7 は Phase 1）** / **NFR-ENV-7** |

🔴 **2026-09-01 に `docs/01` へ追加された `BR-65`〜`BR-73` の担保箇所**: **BR-65**（読めるのは当事者分のみ）= `F-065` 処理 ① / `AC-1` / `F-066` 処理 ① / `AC-1`、**BR-66**（開示項目の上限）= `F-065 AC-1` / `F-066 AC-1` `AC-2` / A-23 / 章 6.5、**BR-67**（上流・粗利・内部検討・他社分を出さない）= `F-065 AC-2` `AC-3` / `F-066 AC-3` `AC-4` / `F-045 AC-5` / `F-042` `F-047` の関連ロール、**BR-68**（読み取り専用・意思表示はチャット）= `F-065 AC-4` / `F-066 AC-5` / `F-004 AC-5` / `UC-25` 手順 3、**BR-69**（稼働＝Phase 2 / 契約＝Phase 3）= `F-065 AC-6` / `F-066 AC-6` / 章 1.2 の機能一覧、**BR-70**（電子署名は BYO・未接続でも ⑤ が回る）= `F-049` 外部 API 依存 / 処理 ⑦ / `AC-8` / `AC-9` / `F-047` 処理 ⑧、**BR-71**（取引先へ届く送信は独自ドメイン検証が前提）= `F-001 AC-4` `AC-5` / `F-007 AC-5` / `F-022 AC-7` / `F-041 AC-4` / `F-047 AC-7` / **NFR-ENV-10** / `F-059 AC-5`、**BR-72**（テナント状態ごとの業務可否）= `F-004 AC-7`〜`AC-9` / 章 5.4 の可否表 / `F-062 AC-4` / `F-064 AC-5`、**BR-73**（契約書の片道性・区別・締結後不変・種別共通）= 章 5.5（遷移表・区別表・**種別共通と双方署名の扱い**）/ `F-047 AC-4` `AC-5` / `F-049 AC-2` `AC-3`。
...
1. ~~電子署名サービスの選定と送信元名義~~ — **決着済み（2026-09-01、[Issue #11](https://github.com/Festal-KM/SES-Platform/issues/11)。`docs/03` §3.1.2 / §3.1.2a）。BYO 接続方式・第一コネクタ DocuSign・第二候補クラウドサイン**（A-11 / `F-049`）。**残る論点**: ①**未接続テナントのメール添付経路**（`F-047` 処理 ⑧）の実装コスト ②DocuSign のトークン失効（8 時間）の扱いが `F-049 AC-2`（`SENDING` は片道）と矛盾しないこと ③第二コネクタ追加時に `Contract` の状態機械（章 5.5）が変わらないこと。
2. ~~メール送信基盤の選定~~ — **決着済み（2026-09-01、[Issue #13](https://github.com/Festal-KM/SES-Platform/issues/13)）。Amazon SES + テナント独自ドメインの検証（SPF / DKIM）を、取引先へ届く送信の前提とする**（NFR-ENV-10 / `BR-71`）。**残る論点**: ①**本番アクセス申請のリードタイム**（Phase 1 のクリティカルパス。`CLAUDE.md` §5）②`sandbox` の送信上限（200 通 / 日）が試用の運用に足りるか ③バウンス・苦情の Webhook を `F-059` の監視にどう載せるか ④章 7.7 の上限（1 日 500 通 / 1 分 30 通）との整合。
...
8. **`APP_ENV` による起動時 DI の実現方式**（章 7.6 NFR-ENV-2〜4）— `production` でモックが選ばれたら起動失敗、非本番に本番キーがあれば起動失敗を、環境変数の検証でどう担保するか。あわせて 🔴 **`sandbox` だけは「宛先の分類」（章 7.6 の判定表。ホスト所属利用者と運営者のみ実送信）でメールの実送信 / モックが分かれる**（NFR-ENV-1 / A-22。Issue #9 / Issue #10）。メール送信基盤の選定（`CLAUDE.md` §9-3）で、**送信ドメインの許可リストや宛先制限（allowlist）を基盤側で設定できるか**を裏取りし、アプリ層の分類だけに頼らない二重防御が組めるかを判断すること。
...
8. **上限到達・機能閉鎖・非本番環境の表示**（`F-027` / `F-061` / `F-028`）— いずれも「黙って劣化させない」ことが要件。残量・理由・環境名を常時見える位置に置く。🔴 **`sandbox` のバナーは「①提案先宛は疑似 ②取引先の担当者宛も疑似（招待はリンクを渡す）③自社メンバー宛は実際に届く」の 3 点が読み取れる文言にする**（`F-028 AC-2`）。あわせて **`sandbox` では取引先の招待一覧に招待リンクとコピー導線を出す**（`F-007 AC-4`。`production` では出さない）。
9. 🔴 **代理閲覧中の画面**（`F-060 AC-3`。**決定済み。2026-09-01、[Issue #14](https://github.com/Festal-KM/SES-Platform/issues/14)**）— 章 4.5 の操作は**「押せない状態で残す」のではなく画面から取り除き、取り除いた位置に「代理閲覧中のため操作できない」という理由を常時表示する**。代理閲覧中であること自体も常時分かること。**これを代理閲覧中の画面の共通仕様として扱う**（`docs/01` 申し送り 14）。
...
12. 🔴 **越境経路 5 の「取引先ビュー」を画面として設計する**（`F-065` / `F-066` / `UC-25`）— **稼働の画面（`S-029` 稼働一覧 / `S-030` 稼働詳細 相当。`F-065` = Phase 2）と契約の画面（`S-025` 契約一覧 / `S-026` 契約詳細 相当。`F-066` = Phase 3）に取引先ビューを追加するか、取引先向けの新画面を起こすかを決める**。🔴 **フェーズを取り違えないこと** — 先に開くのは**稼働**（Phase 2）であり、**契約は Phase 3** である（`BR-69` / `F-065 AC-6` / `F-066 AC-6`。**先に開く範囲を広げない**）。**あわせて `docs/04` の既存記述（パートナーのナビゲーションで ⑤ 契約 `S-025` と ⑥ 稼働 `S-029` を非表示としている箇所）を、経路 5 の段階開放に合わせて改めること**（現行の `docs/04` は経路 5 の追加前の前提で書かれている）。いずれの場合も、①**自社が当事者の分だけを `BR-66` の項目だけで**表示し ②**書き込みの導線を持たず**（`BR-68`。延長の意思表示・単価交渉はチャットへ誘導する）③**他社の当事者レコードの存在・件数を示唆する表示（件数バッジ・並び順・「他 N 件」）を作らない**（`BR-07`）④**ホストの販売単価・エンド企業名・粗利・延長確認の論点整理を画面のどこにも置かない**（`BR-67`）。🔴 **取引先のナビゲーションに ⑤ 契約と ⑥ 稼働の導線を追加する**（取引先は 1 日 4〜5 時間滞在する主利用者であり、提案までで終わる画面構成にしない。項目 1 と同じ理由）。**ホスト側には「取引先に何が見えているか」のプレビューを置く**（`F-065` / `F-066` の関連ロール）。
13. 🔴 **送信ドメインの検証と電子署名の接続を、設定画面ではなく「開設フローの工程」として設計する**（`F-001 AC-4` / `F-049 AC-8`。`S-036` 相当を開設フローへ）— **検証が完了するまで取引先へ 1 通も送れない**ため、①テナント開設直後のオンボーディングに DNS 設定の工程を置き ②未完了の状態を「壊れている」ではなく「取引先へ送信できない状態」として理由と手順とともに示し（機能を隠さない）③`F-022` / `F-041` / `F-047` / `F-007` の送信導線に到達したときに、何が足りないかがその場で分かるようにする。**電子署名の接続は任意**であり、未接続でも契約書をメール添付で送れる導線を用意する（`BR-70`）。
14. 🔴 **利用量・残量の表示単位を件数にする**（`F-027 AC-6` / 章 7.5。`S-038` 相当）— **テナント側の画面・通知に金額を出さない**（例外は超過分の請求見込み額のみ）。「スキルシート解析 あと 60 件」のように**業務の言葉で残量を示す**。**運営コンソール（`A-xxx`）には金額と件数の両方 + 消費率 + 基準比の倍率**を出す（`F-063 AC-5`）。**品質ゲートは残量表示の対象に含めない**（`F-027 AC-7`。含めると「クォータ切れでゲートが動かない」と読める）。
15. **`A-005`（運用監視）に監視項目 13「メール送信基盤（環境全体）の送信上限の到達・接近」を置く**（`F-059 AC-7` / 章 7.7）— 保留中の件数・上限値・到達時刻を示し、🔴 **テナント単位の日次上限（`F-027`）とは別項目として並べる**（同じ枠に混ぜない）。**失敗ジョブ・`SUBMIT_FAILED` の枠にも入れない**（保留は障害ではない。項目 6 と同じ理由）。
16. 🔴 **エンジニアの経験内容（`EngineerCareer`）を画面として成立させる**（`F-008 AC-5`〜`AC-8` / A-24。**決定済み。2026-09-10、[Issue #35](https://github.com/Festal-KM/SES-Platform/issues/35) = A**）— ①**`S-007`（エンジニア登録・編集）セクション 3「経験内容と従事期間」を、「後続のリリースで登録できるようになる」から実際に登録できる入力欄へ改める。** 1 行 = 期間（開始年月・終了年月 / 「継続中」）/ 役割 / 業務内容 / 使用技術で、**行の追加・編集・削除**ができること。**0 行での保存を妨げない**（`F-008 AC-5`）。②**`S-006`（エンジニア詳細）に経験内容の表示を置く**（**期間の降順の決定的順序**）。③🔴 **匿名候補の画面（`F-017`）に経験内容を 1 項目も置かない**（`BR-55` / `F-008 AC-7`。項目 3 と同じ理由 — **「詳細な経歴の並び」は個人を特定させる代表例である**）。ホスト側で経歴が見えるのは `Proposal` 作成後の凍結情報に限る。④**提案詳細では凍結された経歴を表示し、台帳の現在値と混ぜない**（`F-019 AC-5`）。⑤**デバイス階層は台帳側（Tier 3）**でよいが、`CLAUDE.md` §13.3 のとおり**モバイルで遮断しない**。⑥Phase 2 で `F-032` の抽出結果を反映する画面は、**同じ入力欄へ追加 / 置換として流し込む**（「AI 抽出結果」専用の別編集画面を作らない。`F-008 AC-8`）。
17. 🔴 **公開後の再検査による「自動解除」を画面として成立させる**（`F-014 AC-6`〜`AC-13` / A-26。**決定済み。2026-09-10、[Issue #42](https://github.com/Festal-KM/SES-Platform/issues/42) = ②**）— ①**ホスト側の案件画面（`S-011` / `S-013` 相当）に「検査で問題が見つかったため公開を解除した」ことと、原因の欄（案件名 / 外部公開用の記載 / 要件の自由記述）を示し、`F-020` の指摘へ導線をつなぐ**（`AC-9`）。**これが②の最大の副作用への手当てである** — 取引先の画面から案件が消えるため、理由が出ないとホストは「なぜ問い合わせが止まったのか」を知る手段が無い。②🔴 **「指摘を無視して公開したままにする」「そのまま再公開する」ボタン・確認ダイアログの類を作らない**（`BR-18` / `F-020 AC-2`。直せるのは元データだけである）。③🔴 **取引先側には解除も理由も出さない** — 既存の「公開されていない案件を開いたとき」の表示に合流させ、はじめから公開されていない場合と区別がつかないようにする（`AC-10`）。④**AI の日次コスト上限で再検査が保留されている間は「公開は維持されているが再検査が未実行」であることをホスト側に示す**（`AC-12`。項目 8 の「黙って劣化させない」と同じ原則。**解除と保留を同じ表示にしない**）。⑤**編集の保存直後に再検査が走る場合があることを、編集画面（`S-012` 相当）で事前に分かるようにする**（3 欄の編集だけが契機であること。`AC-6`）。
...
2. 🔴 **越境 5 経路の実装を、それぞれ 1 箇所に閉じる**（`F-014` / `F-019` / `F-038` / `F-016`〜`F-018` / **`F-065` / `F-066`**）— 越境読み取りが分散すると、6 つ目の経路が事故として生まれる。
3. 🔴 **送信の冪等性**（章 7.8）— `idempotency_key` + `SUBMITTING` への CAS。**自動リトライを設定できない構造**にする。応答不明時の扱い（`SUBMIT_FAILED` に確定 → 人間が再送）を明記する。
4. 🔴 **`gate-inspector` が整合層の合否を左右できない実装**（`F-020 AC-3` / `BR-61`）— 整合層の判定関数に LLM の出力を入力として渡さない。**警告は別のフィールドに載せる。** あわせて 🔴 **「AI の日次コスト上限で実行できない」を「LLM が失敗した」と同じ経路で扱わない**（`F-027 AC-5` / 章 8.5）— 前者はゲートを起動せず対象を `GATE_RUNNING` のまま保持し（`GATE_FAILED` に遷移させず、送信もできない）、停止理由を `F-059 AC-6` の滞留一覧に区別できる形で残す。後者は従来どおり PII 層・商流層を判定不能 = FAIL とする。**この区別のために状態も遷移も追加しない**（章 5.1）。
...
10. **`APP_ENV` による外部連携の差し替えを起動時 1 箇所に集約**（章 7.6 NFR-ENV-2）— リクエストごとの分岐にしない。`production` でのモック選択、非本番での本番キー設定は**起動失敗**にする。🔴 **`sandbox` のメールだけは「宛先の分類」で実送信とモックが分かれる**（章 7.6 の**宛先の分類の判定表** / NFR-ENV-1 / A-22）。これを送信箇所ごとの `if` で実装せず、**メール送信の単一経路に「宛先の分類」（①ホスト所属利用者 / ②パートナー所属利用者 / ③提案先・エンド企業 / ④エンジニア本人 / 分類外＝運営者）を必須の入力として持たせ、分類が未指定の送信をコンパイル時または実行前に成立させない**設計にする（分類を省略できると、新しい通知が既定で実送信側に落ちる）。既定値を置く場合は**モック側**に倒す。🔴 **分類は `Membership` の `partner_company_id` の有無から機械的に導けるようにし、送信側の呼び出しで自己申告させない**（申告制にすると、①と②の取り違えがそのまま実在の取引先への送信になる）。判定順は「`PlatformUser` → パートナー所属 → テナント所属 → それ以外」とし、**パートナー所属の判定をテナント所属の判定より先に置く**。
...
13. 🔴 **越境経路 5 の「当事者」を、行レベルの分離と同じ層で表現する**（`F-065` / `F-066` / A-23）— ①**`Assignment` の当事者は `engineer_id` の所有パートナーから導出し、`Contract` / `Order` は当事者となるパートナー企業を列として持つ**（章 6.5）。②**当事者判定は認証コンテキストのみから行い、リクエスト入力を受け付けない**（`BR-03`）。③🔴 **パートナー所属ユーザーに対する読み取りを、テナント境界・パートナー境界と同じアクセサ（および行レベルセキュリティの述語）で表現する** — 「当事者だから見せる」を業務ロジック側の `if` で書くと、経路 5 が分離の外側に漏れる。④**開示項目の絞り込み（`BR-66`）を、取得後のフィルタではなく取得時の射影として実装する**（取得してから隠す実装は、API 応答・ログ・エクスポートのどこかで漏れる）。⑤**経路 5 に書き込みハンドラを実装しない**（`BR-68`。存在しないことをテストで示す）。
14. 🔴 **取引先へ届く送信の前提条件を、送信の単一経路で判定する**（`F-001 AC-4` / NFR-ENV-10 / `BR-71`）— ①**送信ジョブは、テナントの独自ドメインの検証状態を確認してから外部 API を呼ぶ**。②🔴 **未検証のときに共通ドメインへフォールバックしない**（成功したように見えて `BR-51` に違反する壊れ方になる。NFR-ENV-3 と同型）。③**未検証は `SUBMIT_FAILED` / `SEND_FAILED`（障害）ではなく「設定未了」として区別し**、状態を進めない（`F-022 AC-7` / `F-047 AC-7`）。④**電子署名は `TenantEsignConnection` 相当のテナント単位の接続を前提とし、未接続では `DRAFT` → `SENDING` を起動しない**（`F-049 AC-8`。`Contract` の状態機械は変更しない — 遷移が起きないだけである）。資格情報は暗号化して保存し、ログ・監査ログ・通知・LLM に出さない（`BR-25`）。
15. 🔴 **メール送信基盤（環境全体）の上限到達を「失敗」ではなく「保留」として設計する**（章 7.7「送信基盤（環境全体）の上限到達時の保留と復帰」/ `F-059 AC-7` / `docs/05` `## TBD` TBD-12）— ①**メール配送レコード（`EmailDispatch` 相当）に保留の値（案: `HELD_PROVIDER_QUOTA`）を持たせ**、上限リセット後に自動で送信する（送信を 1 回も試みていないため `BR-22` の自動リトライ禁止に当たらない。この根拠をコード・設計の双方に明記する）。②🔴 **`Proposal` / `Contract` の状態機械（章 5.1 / 章 5.5）に状態を追加せず、`SUBMIT_FAILED` / `SEND_FAILED` にも落とさない**（保留は配送レコードの属性であり、業務状態ではない）。③保留件数・上限到達・接近を `F-059 AC-7` の監視項目として集計できる形にし、**テナント単位の日次上限（`F-027`）のカウンタと同じ指標に合算しない**。④保留の解除が滞留した場合に取りこぼさない起票条件にする（項目 8 と同じ理由。`F-064 AC-10` の削除予告は配送されるまで削除を実行しない）。
16. 🔴 **経験内容（`EngineerCareer`）を子の集合として設計し、`EngineerSnapshot` へ行単位で凍結する**（`F-008 AC-5`〜`AC-8` / `F-019 AC-5` / A-24。**決定済み。2026-09-10、[Issue #35](https://github.com/Festal-KM/SES-Platform/issues/35) = A**）— ①**`Engineer` の 1 属性（自由文・1 塊の構造）として持たない。** 期間 / 役割 / 業務内容 / 使用技術を**個別の項目として**保持する（`F-029` が期間・使用技術を項目として参照し、`F-017` の非開示を項目単位で示す必要があるため）。②🔴 **`EngineerSnapshot` 側も行単位で凍結する** — 台帳の行を参照するのではなく**複製**し、台帳側の編集・削除で既存提案の内容と行数が変わらないこと（`F-019 AC-5`）。**参照で持つと凍結が成立しない。** ③**二重の境界の対象**であり、`Engineer` と同じアクセサ・同じ分離の述語で読み書きする（`F-004`。子表だから素通しになる経路を作らない）。④🔴 **匿名化の出力に経歴が 1 文字も現れないことをテストで固定する**（`F-008 AC-7` / `BR-55`。**「出さない」を実装の注意ではなくテストで押さえる**）。⑤**Phase 2 の `F-032` の反映先を同一構造にし、抽出専用の保存先を作らない**（`F-008 AC-8`。入力元と出所を行に持たせ、手入力と抽出採用を区別できるようにする）。⑥**削除の扱いは A-24 の既定に従う**（`F-046` / `F-064` の削除対象に含める。章 6.8）。⑦**実装順は `T-09-12` を `F-019` の実装より前に置く**（凍結は遡れない）。
17. 🔴 **公開後の再検査と自動解除を、公開の判定と同じ 1 箇所に閉じる**（`F-014 AC-6`〜`AC-13` / A-26。**決定済み。2026-09-10、[Issue #42](https://github.com/Festal-KM/SES-Platform/issues/42) = ②**）— ①🔴 **公開範囲を成立させる経路と落とす経路を 1 箇所のままにする**（項目 2 と同じ理由。**2 箇所目を作ると、どちらか一方だけがゲート結果を見ない経路になる**。`AC-13`）。②**再検査の契機は公開欄 3 欄（案件名 / 外部公開用の記載 / 要件の自由記述）の実際の変更に限る** — **保存のたびに走らせない**（`AC-6`。AI 原価）。🔴 **「検査対象の欄」と「変更検知に用いる値」を取り違えないこと** — 公開先の社名まで含む値で検知すると、**公開先を 1 社増やしただけで本文が同じでも再検査が走る**（`AC-6` に反する。増えた公開先への検査は公開が成立する時点の `AC-3` が行う）。③**AI の日次コスト上限による未実行を「商流層 FAIL」に倒さない** — 保留として保持し、解除後に再検査する（`AC-12` / `F-027 AC-5`。項目 4 と同じ区別。**原価で止まっただけの案件を取引先の画面から消さない**）。④**自動解除の監査ログを、人の操作による公開解除と区別できる形で残す**（実施者 = `system`、変更前後の公開先。`AC-11` / `AC-5`）。⑤**取引先側の応答は「公開されていない」に合流させ、解除の事実・理由を応答・通知・エクスポートのいずれにも載せない**（`AC-10`。取得後に隠すのではなく、境界の述語で成立させる。項目 13 ④ と同じ原則）。
