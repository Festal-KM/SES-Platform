...
**主平面の合計 45 画面。Tier 1 = 13 / Tier 2 = 17 / Tier 3 = 15。**（内訳: T1 = `S-001` `S-002` `S-003` `S-004` `S-017` `S-018` `S-021` `S-024` `S-030` `S-031` `S-032` `S-033` **`S-044`**）
...
**管理平面の合計 14 画面。全画面 Tier 3。**（全体 59 画面 = 主平面 45 + 管理平面 14） ただし `CLAUDE.md` §13.3 のとおり**モバイルで非表示にしない**。運営者の障害対応は移動中にも発生するため、`A-002` / `A-005` の一覧はモバイルで「1 行 = 1 カード状の縦積み（テナント名 / 異常の種別 / 件数 / 経過時間）」に劣化させ、**並び順（異常度の高い順）は保つ**。フィルタと設定操作はデスクトップに寄せる。
