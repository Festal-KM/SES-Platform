...
**主平面の合計 43 画面。Tier 1 = 12 / Tier 2 = 16 / Tier 3 = 15。**（内訳: T1 = `S-001` `S-002` `S-003` `S-004` `S-017` `S-018` `S-021` `S-024` `S-030` `S-031` `S-032` `S-033`）
...
**管理平面の合計 14 画面。全画面 Tier 3。**（全体 57 画面 = 主平面 43 + 管理平面 14） ただし `CLAUDE.md` §13.3 のとおり**モバイルで非表示にしない**。運営者の障害対応は移動中にも発生するため、`A-002` / `A-005` の一覧はモバイルで「1 行 = 1 カード状の縦積み（テナント名 / 異常の種別 / 件数 / 経過時間）」に劣化させ、**並び順（異常度の高い順）は保つ**。フィルタと設定操作はデスクトップに寄せる。
...
- **目的**: 🔴 **取引先へ届くメールが運営者名義で飛ばないようにする**（`BR-51` / `U-04`）。**未検証の状態を「壊れている」ではなく「送信元ドメインが未設定」として示す。**🔴 **未検証のまま送信を実行できる導線を持たない**（**`BR-46`**。「送ったつもりで実際には送られていない」を構造として起こさない。顧客は商機を失ったことにすら気づけないため、送信ボタンを出して失敗させるのではなく、送信の手前で理由と設定導線を出す）。
...
- **目的**: 🔴 **黙って劣化させない**（`BR-24`）。**2 種類の「上限」を区別して表示する**（`docs/03` 申し送り 6 / 12）。🔴 **ここに表示される利用量は、単一経路で記録された AI 利用の全量である**（`BR-09` / `BR-10`）。**記録を経ない AI 呼び出しが存在しないため、この画面に現れない AI 利用も存在しない** — 利用者が残量を信頼できる前提はここにある。
...
- **関連画面**: → `A-003` `A-004` `A-010` `A-011` `A-014`。
...
| **直近の開設** | — | 開設日時 / 企業名 / 環境 / 契約の初期状態 / **招待の状態**（送信済み / 受諾済み / 送信失敗）。**開設して終わりにせず、受諾までを追える**（受諾されないテナントは使われないまま残る） |
...
- **関連画面**: ← `A-002`（テナント 0 件の空状態）/ `A-013`（`SANDBOX` 0 件の空状態）。→ `A-003`（開設したテナントの詳細）/ `A-013`（`SANDBOX` で開設した場合の期限管理）/ `A-010`（契約の操作。Phase 3）/ 主平面の `S-002`（初期 `OWNER` の受諾）。
...
| `UC-02` 初回オンボーディング | **`A-014`**(運営者が開設 + 初期 `OWNER` を招待) → `S-002` → `S-001` → `S-035`(招待) → `S-036`(ドメイン検証) → `S-014`(取引先招待) → `S-007` / `S-012` → `S-013` → （取引先）`S-020` → `S-021` → `S-023` |
...
| 出典 | # | 申し送りの内容 | 反映箇所 |
|---|---|---|---|
| `docs/02` | 1 | 取引先向け画面は自社営業と同じ密度で設計する | `U-09` / §1.1 の Tier 対応表 / `S-004` `S-015` `S-018`（専用画面）/ **共用画面は各画面詳細の「セクション（取引先）」と「権限差分」の 2 項目**（`S-005` `S-006` `S-010` `S-011` `S-016` `S-019` `S-020` `S-023` `S-024` `S-031`。独立した節は設けていない）/ §2.2 の取引先遷移図 / §3.2 / §11-4 |
...
| `docs/02` | 7 | 管理平面は `A-xxx`。9 機能を主平面と同じ密度で設計する | §1.2（**14 画面**）/ §4.9（**`A-001`〜`A-014`** を主平面と同じ 8 項目 + デバイス別で記述）/ §6.7〜6.10 |
...
10. **3 デバイス分（デスクトップ / タブレット / モバイル）を描く対象は Tier 1 の 12 画面**（§1.1 の内訳を参照）。**Tier 2 / Tier 3 は各 1 枚**でよい（劣化のさせ方は各画面の「デバイス別」に文章で定義済み）。管理平面は 14 画面すべて T3 で 1 枚ずつ。**枚数は 1 枚ごとに課金されるため、この内訳を超えて描かない。**
11. **`A-014`（テナントの開設）は運営者が書き込みを行う数少ない画面**である。**フォーム 1 枚**でよいが、🔴 **「開設後に自動で入る既定値の明示」ブロック（自動承認 = 無効 / AI ロール = すべて都度承認 / 公開範囲 = 非公開）を確認ステップの直前に必ず描く**（§4.9 の `A-014`）。
...
11. 🔴 **`A-014`（テナント開設）と `A-010`（契約管理）の API を分ける。** 前者は **Phase 0**（`F-001`。`PLATFORM_OWNER` のみ）、後者は**セクション 4 = Phase 1**（`F-064` の削除完了の確認。閲覧は `PLATFORM_SUPPORT` も可）/ **セクション 1〜3 = Phase 3**（`F-062`）である。**フェーズの異なる 3 つを 1 エンドポイントに束ねない**（Phase 0 / 1 の時点で Phase 3 の認可を実装する必要が生じる）。
12. 🔴 **テナント開設は、テナント作成と初期 `OWNER` の招待を分離して冪等にする。** 招待メールの送信失敗で開設をやり直すと**重複テナント**が生まれる（分離が効いたまま業務が 2 つに割れ、後から気づきにくい）。招待の再送は既存テナントに対する明示操作として扱う。
13. **削除完了の確認（`F-062 AC-7`）を返す API は `A-010` 用の 1 本に限る。** `A-013`（`F-054` / `F-064`）側にも `S-042` 側にも **`A-003`（テナント詳細）側にも**運営者向けの確認エンドポイントを作らない。🔴 **`A-003` の `PURGED` 表示に削除件数を含めない**（返すのはライフサイクル状態のみ。§4.9 の `A-003`）。`A-005` が返すのは**削除ジョブの失敗**であり、完了の事実とは別のフィールドにする（`F-059 AC-2`）。
