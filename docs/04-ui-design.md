...
13. 🔴 **今回の改訂で新規に描く / 描き直す画面**（2026-09-01。それ以外の画面のプロンプトと画像は再生成不要）:
    ...
    - **描き直し（内容が変わった）**: 🔴 **`S-020` / `S-021`**（**AI の日次コスト上限で停止中のゲートを「`検査中` のまま + 停止理由 + 再開条件」で描く。`GATE_FAILED` の橙で描かない。「修正して再実行」のボタンも描かない**）/ **`S-043`**（移行チェックリストに「電子署名アカウントの接続（**任意**）」を加え、**必須と任意をラベルで区別して描く**）/ **`S-038`**（🔴 **金額表記を全廃し、件数 4 種のメーター + メール + ストレージ + 席数 + 請求見込みブロックに差し替える**）/ **`S-037`**（🔴 **資格情報の入力フォームを廃し、「DocuSign に接続する」ボタン → 同意画面 → 戻る の 3 ステップに差し替える。接続済み表示にアカウント名と接続日時**）/ **`S-026`（🔴 品質ゲートの 3 層ブロックを追加し、`先方確認中` に署名者ごとの進捗を並べる。未接続時は「メール添付で送付」）** / **`S-036`**（オンボーディングの工程として描く。影響範囲に `S-014` を含め、**自社メンバー招待は対象外**と明記）/ **`S-004`**（🔴 **ナビに ⑤ ⑥ が増えた**）/ **`A-014`**（送信ドメインの登録欄）/ **`A-005`**（監視項目 11「未検証テナント」と 🔴 **12「`GATE_RUNNING` の滞留」**。**理由の区別（上限 / ジョブ失敗）が絵から読めること**）/ **`A-004`** `A-011`（件数と金額の併記）/ **管理平面の全 14 画面**（🔴 **ナビをサイドバーで描いていたら横並びタブに直す**）。
    - **変更なし**: `S-001`〜`S-003` `S-005`〜`S-019` `S-022`〜`S-024` `S-027`〜`S-035` `S-039`〜`S-042` `A-001`〜`A-003` `A-006`〜`A-010` `A-012` `A-013`（**ナビの形式を除く**）。
