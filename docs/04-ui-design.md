...
- **操作と結果**: 保存 → 案件更新 + 監査ログ。**保存だけでは公開されない**（primary の隣に「保存して公開範囲を設定」の secondary を置く）。
  - ⚠️ **T-06-01 の時点では secondary（`S-013` への導線）を置いていない。** `S-013` は T-06-06 で実装されるため、**存在しない画面へのリンクを作らない**（`S-003` の `S-012` 導線を T-05-01 で保留したのと同じ判断）。代わりに「保存しただけでは、この案件はどの取引先にも公開されません」を画面に常時出し、**`F-014 AC-2` の事実は先に伝える**。T-06-06 で secondary を足す。
