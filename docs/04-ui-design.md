...
| 無効・期限切れのトークンで ③ を開く | 「このリンクは無効か、有効期限が切れています」+ ①（再設定依頼）への導線を表示 | 画面内で①相当の表示へ切替 |
...
| S-046 | 該当なし（一覧を持たない） | 該当なし（検索を持たない） | 該当なし（①③ ともデータ取得を伴わない静的フォーム） | 該当なし | 🔴 **無効・期限切れトークンで ③ を開く → 「このリンクは無効か、有効期限が切れています」+ ①（再設定依頼）への導線 / ネットワークエラー → 「接続できませんでした。時間をおいて再度お試しください」**（本文 §4.1 と同一文言） | 該当なし（未認証画面。全ロール共通） | 該当なし（対象識別子を URL に含む画面だが、無効トークンは 404 ではなく Err 列の専用文言で扱う） | 該当なし（入力項目が少なく離脱確認の対象外） | 🔴 送信中はボタンを送信中表示に置換（二重送信防止。①②③ いずれの送信も同じ規律） | 送信失敗時は同一操作を再試行可能。**②の完了文言はリトライ後も成功・失敗を問わず同一のまま変化しない**（存在有無の漏洩を防ぐため） |
