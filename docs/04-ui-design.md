...
| 4 | **read-only の明示** | 書き込みが許される画面（契約 `A-010` / **開設 `A-014`** / サンドボックスの移行・延長 `A-013` / クォータ `A-004` / 機能フラグ・お知らせ `A-009` / `demo` 限定の合成データ `A-012`。**内訳と根拠は §4.9 の共通前提**）**以外の画面には、画面タイトルの右に `閲覧のみ` を常時表示する**。`A-002` `A-003` `A-005` `A-006` `A-008` `A-011` が該当（`BR-37`） |
...
🔴 **いずれもテナントの業務データ（エンジニア・案件・提案・チャット・契約書）を編集する UI ではない。**🔴 **運営者に開示しないもの**（`BR-40`）: 外部サービスの資格情報の平文、スキルシートの原本と本文、エンジニアの氏名・生年月日・連絡先、チャットの本文。**運営者に必要なのは件数・状態・エラーであって内容ではない。** 全操作（閲覧を含む）を監査ログに記録する。
...
| 🔴 **削除完了の確認** **[Phase 1]** | `F-064`（`CLOSING` → `PURGED`。**Phase 1**）と `F-046`（3 年ルール。**Phase 2 で対象に加わる**）の**完了 / 未完了と件数のみ**。**削除された内容・返却データに到達する導線が存在しない**（`F-062 AC-7` / `BR-40`）。🔴 **これが運営者にとっての唯一の確認経路**であり、`S-042` 側にも `A-013` 側にも運営者向けの確認導線を作らない（`F-062 AC-7` の「`F-064` / `F-046` の側に別途作らない」）。**閲覧は `PLATFORM_SUPPORT` にも許される**（`F-062 AC-7`） |
...
13. **削除完了の確認（`F-062 AC-7`）を返す API は `A-010` 用の 1 本に限る。** `A-013`（`F-054` / `F-064`）側にも `S-042` 側にも運営者向けの確認エンドポイントを作らない。`A-005` が返すのは**削除ジョブの失敗**であり、完了の事実とは別のフィールドにする（`F-059 AC-2`）。
