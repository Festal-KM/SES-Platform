...
| **C8 DIRECTORY** | `<T> = app_tenant_id() AND ( app_is_host() OR <O> IS NULL OR <O> = app_partner_id() )` | `users`(`SELECT`。`owner_partner_company_id`)。🔴 **ホスト所属の行だけが全員に見える**（チャットの送信者名・`ProposalEvent` の実行者名に要る）。**他パートナーの利用者は 1 行も見えない**。パートナー向けシリアライザは `email` を返さない。🔴 **書込（`INSERT` / `UPDATE`）は C3 式**（自分の所属としてしか書けない）。**書き手は §4.4.2 の行由来コンテキスト 3 関数だけ**であり、所属は招待行 / 本人行から取る。🔴 **`ProposalEvent` の実行者名（ホスト担当者の表示名。承認者を含む）が取引先の `S-023` 履歴に出るのは C8 DIRECTORY の意図された開示である**（T-12-14 ④。2026-09-18。**暫定。[Issue #73](https://github.com/Festal-KM/SES-Platform/issues/73) で確認中**。既定 = 現状維持）。`approval`（承認記録の構造）は取引先向けの型に無いまま（#46）。「伏せる」に決まった場合の変更点は **`apps/web/lib/proposals/events.ts` の主体名の解決 1 箇所**（取引先向け view で実行者名を `ホスト担当者` に丸める）であり、**C8 のポリシーは動かさない**（ポリシーを狭めるとチャットの送信者名〔経路 3〕まで消える） |
...
| `app_shared_engineer_ids(tenant_id)` | `engineer_shares` の**共有中エンジニア ID の配列のみ**（共有元は返さない。行は 1 つも返らない） | `SECURITY DEFINER`。所有者 `app_share_probe`。§4.5 の追加ポリシー 2 本と `app_engineer_is_shared()` からのみ使う（T-12-19） |
...
🔴 **T-12-19（migration `20260930000000`）でポリシーの述語は `id IN (SELECT unnest(app_shared_engineer_ids(app_tenant_id())))` の集合評価になり、`app_engineer_is_shared()` は本体を `app_shared_engineer_ids()` へ委譲する 1 実装になった（読める行・読み手・GUC の guard は同一。§17.3「T-12-19 の是正の決着」）。** 上記の SQL 例は改訂前の実装であり、経緯として残す。
...
| 🔴 **以下 7 列は「未判定」として列挙する**（T-12-18 ⑧。2026-09-18。SP-11 T-11-07 申し送り ① + ⑧）。**Phase 1 では REVOKE しない（現状維持 = GRANT が残る）。判定は `A-007`（代理閲覧。SP-16 `F-060`）の設計時に行う** —— 代理閲覧は read-only ロールで主平面の画面をそのまま見る設計（§5.6）であり `app_platform` の GRANT を使わない見込みだが、**Phase 2 の再計画で `A-007` の DB ロールを確定させるまで判断を保留**する（列挙が無いと忘れられるため表に残す。`docs/dev-plan.md` §8 の Phase 2 再計画の入力）。🔴 **判定の基準は上表と同じ**（「件数・状態・エラー」に要らない自由記述・名称は REVOKE する側に倒す）。判定して REVOKE する場合は migration 20260925000000 と同じく REVOKE のみ + `PLATFORM_READ_COLUMN_DENYLIST` への追加 + `tests/static/platform-grants-consistency.test.ts` の更新をセットで行う | |
| `contracts` | `withdraw_reason`（取り下げ理由の自由文。Phase 3。**未判定**） |
| `project_requirements` | `free_text`（要件の自由記述。**未判定**。`A-005` / `A-011` は件数だけを数える） |
| `projects` | `name` / `public_summary`（案件名・外部公開用の要約。**未判定**。§5.7 は `A-002` / `A-003` に案件名を見せないと定めるが、`A-007` が `projects.name` を使うかで判断が変わる） |
| `scheduler_runs` | `detail`（スケジューラ実行の詳細 JSON。**未判定**。`A-005` 項目 6 は件数と時刻だけ） |
| `engineer_skills` | `original_label`（利用者が入力したスキル表記の原文。**未判定**。自由入力であり PII が紛れうる） |
| `skill_aliases` | `alias`（別名の表記そのもの。**未判定**。`#24` の監査が別名の表記を `summary` に載せないのと同じ理由で REVOKE 側に倒す候補） |
...
🔴 **第 2 層（`maskAuditSummary` / `toPlatformAuditLog`）の注記（T-12-18 ⑥ / ⑨。2026-09-18）**:

- **⑥ `auth.login_failed` の `reason`（`PASSWORD_MISMATCH` / `USER_DISABLED` …）は `[masked]` のまま**（`reason` は内容キーの語彙に当たり、値の形を見る前にキーごと落ちる = `A-006` から失敗理由の区別が消える）。**暫定。[Issue #69](https://github.com/Festal-KM/SES-Platform/issues/69) で確認中。既定 = 現状維持**（安全側。区別が要る調査は `A-006` ではなく Sentry / DB の直接照会で行う）。「残す」に決まった場合は **`maskAuditSummary()` で `auth.login_failed` の `reason` に限り列挙値の閉集合（`PASSWORD_MISMATCH` / `USER_DISABLED` / …）だけを通す形状判定**に変える（自由記述は引き続き `[masked]`。閉集合は書き込み側〔`apps/web/lib/auth`〕と `packages/domain` の 1 定数を共有し、`mask-summary.test.ts` に「閉集合の外の `reason` は `[masked]`」の負のケースを足す）。順序は本節 → 実装。
- **⑨ 群 D（生成由来。`targetId` / `modelId` / `promptVersion` / `purpose` = `A-006` 側の列挙値 / ID）は残ってよい。** `BR-40` の非開示は「内容」であり、列挙値と不透明な ID は内容ではない（`maskAuditSummary` の 3 段のとおり）。🔴 **この判断は `S-041` の許可リスト（§6.4「#10 の改訂」）とは独立に行う** —— あちらは「ホスト管理者に見せるキーを選ぶ」（許可したキー以外を落とす）、こちらは「運営者に見せてよい形へ伏せる」（既知の形を前提にしない）であり、**一方の変更を理由に他方を動かさない**（`S-041` に `GATE_RESULT` の verdict や `state.invalid_transition` を載せた〔T-12-18 ⑨〕からといって `A-006` の規則は変えない。逆も同じ）。
...
| 10b | 🔴 `GET /api/audit-logs/export` | `F-005` / `S-041` セクション 4（CSV エクスポート）/ **Phase 1（T-12-18 ⑪。2026-09-18）** | `?from=&to=&action=&actorId=`（#10 と**同じスキーマから `cursor` / `limit` を除いたもの**。🔴 **期間必須・`from <= to` は #10 と同じ判定**（幅の上限は #10 同様に置かない ── `AUDIT_LOG_SEARCH_MAX_PERIOD_DAYS` を使うのは API-A7 だけであり、#10 / #10b には期間幅の上限判定が無い。低-1。2026-09-21）） | `text/csv; charset=utf-8`（UTF-8 BOM 付き RFC 4180。`Content-Disposition: attachment; filename="audit-logs-{from}-{to}.csv"`）。🔴 **ヘッダは 6 列固定**（`createdAt` / `actorKind` / `actorDisplayName` / `action` / `target` / `ipAndDevice`。§6.4「#10 の改訂」の「CSV エクスポート」行）。**`detail` / `summary` の列は無い** | `OWNER` / `ADMIN`（#10 と同じ `requireRole`。読み取り専用 = `requireExecutable` / `requireNotViewer` を掛けない。`CLOSING` でも可）。🔴 **上限 `AUDIT_LOG_EXPORT_MAX_ROWS`（`packages/config`。暫定 10,000）を超える母集団は 400 `AUDIT_LOG_EXPORT_TOO_LARGE`（期間短縮を促す）**。✅ 🔴 **実行のたびに `audit_log.export`（`USER`。1 エクスポート 1 行）を記録する**（2026-09-18。`docs/04` §S-041「エクスポート → CSV（監査ログに記録される）」に合わせて確定。§16.1 / §6.4「CSV エクスポート」行 ⑤。400 で弾いた要求は記録しない。`summary = { rowCount, truncated }` のみ ── 検索条件・CSV の内容は載せない） ✅ **T-12-18 ⑪ で実装（2026-09-21）**: `apps/web/app/api/(main)/audit-logs/export/route.ts`（`collectAuditLogsForExport` が #10 を `cursor` で追う / `auditLogsToCsv` の 6 列 / `recordAuditLogExport`）。結合 `tests/isolation/audit-log-export.test.ts`。 |
...
| CSV エクスポート | `summary` / `detail` の列を足さない（許可リストの 2 実装を避ける。`docs/04` §S-041）。~~⚠️ `S-041` の CSV は現時点で未実装。実装が入るとき、ヘッダ定義を既存 5 列 + `actorDisplayName` に固定するスナップショットを同時に置く~~ → ✅ 🔴 **Phase 1 で実装する（T-12-18 ⑪。2026-09-18。既定 = `pm`。新スコープではない —— `docs/04` §S-041 セクション 4 に定めがあり、`F-005` の監査ログは説明責任の対象〔`CLAUDE.md` §3.5〕で取引先への説明に紙が要る。T-11-09 は「未実装のまま」と申し送っただけで見送りを決めていない）。確定形**: ①API は **#10b `GET /api/audit-logs/export`**（§6.3）。**#10 と同じ `listAuditLogs` を `cursor` で内部的に追って読む**（別の読み出し・別の `where` を書かない = 境界・期間・カテゴリの判定が 1 実装）。読む上限は `AUDIT_LOG_EXPORT_MAX_ROWS`（`packages/config`。暫定 10,000。超えたら 400 `AUDIT_LOG_EXPORT_TOO_LARGE` + `params.maxRows`。**Phase 1 は同期生成 + 上限**であり、`docs/04` §S-041「大量件数のエクスポートはジョブ」は**上限に当たる実例が出てから**ジョブ化する〔早すぎるジョブ化は完了通知・保管・失効の設計を伴い、`DataExportRequest` と第 2 の返却経路を作る〕） ②**ヘッダは 6 列固定 = 画面の既存 5 列 + `actorDisplayName`**（`AUDIT_LOG_CSV_COLUMNS` = `createdAt`〔ISO 8601〕/ `actorKind`〔`USER` / `SYSTEM` / `PLATFORM_USER`〕/ `actorDisplayName`〔#10 の `actorDisplayName` そのまま。`SYSTEM` は空〕/ `action` / `target`〔`targetType:targetId`。画面の「対象」列と同じ合成〕/ `ipAndDevice`〔`ipAddress deviceKind` を空白 1 つで連結。画面の「IP・デバイス種別」列と同じ合成〕）。**`apps/web/lib/audit-logs/csv.ts` の `auditLogsToCsv(items: AuditLogListItem[]): string`（純粋関数）に閉じ、`csv.test.ts` が `toMatchInlineSnapshot` でヘッダ行を固定する**（列の増減は必ず差分になる。`id` / `detail` / `detailSuppressedReason` は載せない） ③🔴 **セルの無害化は `packages/domain/src/export/csv.ts` の `sanitizeCsvCellText`（T-12-17 ⑲ (a)。§9.7 の返却 CSV と同じ関数）を共用**する（返却 CSV と監査 CSV で 2 実装にしない。`action` / `target` / `actorDisplayName` に先頭 `=` / `+` / `-` / `@` / タブ / CR が来ても `'` が前置される） ④🔴 **`detail` / `summary` の識別子が CSV モジュールに現れない**ことを静的 `tests/static/audit-detail-single-path.test.ts` の CSV 検査が固定する（`csv.ts` / #10b の route が `detail` / `summary` / `pickAuditDetail` / `AUDIT_DETAIL_ALLOWLIST` を参照しない = 許可リストの 2 実装が構造的に書けない） ⑤**監査記録**: ~~`audit_log.export` 相当の action は既存に無い。Phase 1 は閲覧（#10）と同じ記録規律に従う = 記録しない~~ → ✅ 🔴 **撤回し、記録するに改める（2026-09-18。`docs/04` §S-041「エクスポート → CSV（監査ログに記録される）」が上流であり、`CLAUDE.md` §8.7 に従い本書を上流に合わせて確定した）**。**`audit_log.export`（`USER`。1 エクスポート 1 行）を §16.1 に追加する**。書き込みは **`#10b` の Route Handler の業務トランザクション内**（`writeAuditLog`。`withApiRoute` の `audit` オプションは使わない — 400 `AUDIT_LOG_EXPORT_TOO_LARGE` で弾いた要求は「エクスポートが行われていない」ため残さない）。`targetType='Tenant'` / `targetId` = エクスポートしたテナント自身（`usage.limit_*` と同じ慣例。§16.1）。`summary` は **`{ rowCount, truncated }` の 2 キーだけ**（`rowCount` = 実際に書き出した行数〔NUMBER〕、`truncated` = `AUDIT_LOG_EXPORT_MAX_ROWS` で打ち切ったか〔BOOLEAN〕）。**検索条件（`from` / `to` / `action` / `actorId`）と CSV の内容・自由記述は載せない**（条件の組み合わせ自体が「誰が誰を検索したか」の手がかりになりうるため。API-A7 の `admin.audit_log.search` とは異なり条件は記録しない）。**閲覧（#10）は引き続き `BR-27` の対象外のまま**であり非対称になるが、理由は「手元にファイルを持って行った」の区別（`skill_sheet.view` / `download` と同じ線引き）——監査ログ全体の一括の外部化は個別閲覧より重い「持ち出し」であり、`F-005` の説明責任の対象である。許可リスト（`AUDIT_DETAIL_ALLOWLIST`）にも `audit_log.export` の 1 行を足す（本節末尾） ⑥画面: `S-041` セクション 4 のボタンは現在の検索条件（期間 / 操作種別 / 主体）をそのまま #10b に渡す（`<a href>` の GET。`OWNER` / `ADMIN` にだけ描く）。**検証**: 結合（`tests/isolation/audit-log-export.test.ts`。①ヘッダ 6 列固定 ②`detail` / `summary` / `note` / `email` の識別子・値が本文に無い〔特権接続で `summary` に禁止名の値を注入した行を作っても CSV に現れない〕③先頭 `=` の `actorDisplayName` が `'=` になる ④取引先 / `SALES` / `VIEWER` は 403、他テナントの行は 0 行 ⑤上限超過は 400 ⑥#10 と同じ期間・カテゴリで行集合が一致）+ 静的 ④ + render（ボタンの有無） ✅ **T-12-18 ⑪ で実装（2026-09-21）**: ①〜⑥のとおり。`AUDIT_LOG_CSV_COLUMNS` は `apps/web/lib/audit-logs/csv.ts`、上限は `AUDIT_LOG_EXPORT_MAX_ROWS`（`packages/config/src/limits.ts`。既定 10,000）、`audit_log.export` の `summary` の組み立ては `packages/db/src/audit-log-export.ts`（CSV 側に `summary` の識別子を置かないため）。 |
 ...
 *  🔴 T-12-18 ⑫（2026-09-18）: 接尾辞族の解決（`resolveAuditDetailKeySpecs`）は **`admin.` で始まる action を除外する**
 *  （`admin.tenant.create` 等が `*.create` の `CRUD_KEYS` で評価される経路を閉じる。運営者の action は `A-006` の側で扱う）。
 *  🔴 T-12-18 ⑩: 内側のキーが `count_*`（末尾 `*`）の spec は**キー接頭辞族** —— `summary` のキーのうち `count_` で始まり、
 *  残りが `^[a-z][a-z0-9_]{0,62}$`（snake_case の表名）のものに `{ kind: 'NUMBER' }` を適用する（`tenant.purge` だけが使う。
 *  値は非負整数でなければキーごと落ちる。entry の `key` は `summary` のキー名そのまま = `count_engineers` …）。 */
export const AUDIT_DETAIL_ALLOWLIST: Readonly<Record<string, Readonly<Record<string, AllowedKeySpec>>>>;
/** 🔴 接尾辞族の評価から除外する action の接頭辞（T-12-18 ⑫）。 */
...
| 🔴 **`state.invalid_transition`**（T-12-18 ⑨。2026-09-18） | `entity`（ENUM。🔴 **閉集合 `STATE_MACHINE_ENTITIES`** = `Proposal` / `ProposalRequest` / `Assignment` / `Contract` / `Tenant`。PascalCase なので大文字スネークの正規表現では通らず、閉集合が必須）/ `from → to`（ENUM。`pair: { id: 'state' }`。状態名は大文字スネーク） | 書き込み側は `apps/web/lib/state/invalid-transition.ts` の 1 実装（§15.3。`summary = { entity, from, to }` だけ）。**不正遷移の監査は状態機械の健全性の読み取り材料**（`A-005` と同じ事実をテナント側でも読める）。主体は発生元に従う（取引先主体でも 4 族ではないので許可リストどおり = 遷移の拒否は状態名しか含まない） ✅ **T-12-18 ⑨ で実装（2026-09-21）**。 |
| 🔴 **`tenant.purge`**（T-12-18 ⑩。2026-09-18。`F-064 AC-8` の画面側） | `cause`（ENUM。閉集合 = `TENANT_PURGED` / `RETENTION`〔`packages/db` の `TENANT_PURGE_CAUSES` と同じ値。🔴 `packages/domain` は `@ses/db` に依存できないため、domain 側に同じ値の定数を置き `schema-value-sets.ts` との一致を `pick-detail.test.ts` で固定する = `PROPOSAL_REQUEST_OPERATIONS` と同じ作法〕）/ `tables`（NUMBER。対象種別の数）/ 🔴 **`count_*`（キー接頭辞族。NUMBER）** = `count_engineers` / `count_skill_sheets` / … | 書き込み側は `packages/db/src/tenant-purge.ts`（`SYSTEM` 主体。`{ cause, runId, tables, count_{table}: number }` —— **`counts` の入れ子ではなく平坦な `count_{table}`** で書かれている〔T-10-09〕。既定の「`counts` のキー = `PURGE_SPEC.delete` の表名と数値のみ」はこの実キー形で満たす: 接頭辞の後ろは snake_case の表名だけ、値は非負整数だけ。**表名の集合が `PURGE_SPEC.delete` と一致すること**は書き込み側の `tenant-purge.test.ts` が固定し〔`packages/domain` は `@ses/config` に依存できないので許可リスト側は形で検査する〕、画面は表名を `S-042` セクション 2 と同じ種別ラベル〔`packages/i18n`〕で描く）。**`runId` は落ちる**（不透明 ID の表示種類を作らない。`対象` 列の `targetId` = テナント ID）。🔴 **削除された内容・返却データの内容には到達しない**（件数と種別だけ） ✅ **T-12-18 ⑩ で実装（2026-09-21）**: `count_*` はキー接頭辞族として実装（接頭辞の後ろが snake_case の表名・値が非負整数のキーだけを、キー名の昇順で出す）。 |
| 🔴 **`data_export.download`**（T-12-18 ⑩。2026-09-18。`F-064 AC-8` の画面側） | `kind`（ENUM。閉集合 = `CLOSING_RETURN`〔Phase 1〕/ `OPERATIONAL`〔`F-052`。Phase 3〕。`packages/db` の `DATA_EXPORT_KINDS` と同じ値を domain 側の定数で持ち、一致をユニットテストで固定する〔`tenant.purge` の `cause` と同じ作法〕） | 書き込み側は `apps/web/lib/data-exports/service.ts` → `dataExportDownloadSummary`（`packages/db/src/data-export.ts`。`{ kind, exportRequestId }`）。既定の `format` = 実キー **`kind`**。既定の `requestId` = 実キー `exportRequestId` は **`detail` に載せない** —— 同じ行の `targetType = 'DataExportRequest'` / `targetId = exportRequestId` として **`対象` 列に既に出ている**（`detail` に重ねると「④ それ以外の ID の不透明表示」の種類を型に作ることになり、本表の規律〔該当キーは 1 つも無い〕を破る）。⚠️ `docs/04` §S-041 の表が `requestId` を挙げる場合は `対象` 列への読み替えとして `ui-design` へ申し送る。`data_export.create`（`*.create` 族。`{ kind, scope }`）は `CRUD_KEYS` に無いので 0 キーのまま ✅ **T-12-18 ⑩ で実装（2026-09-21）**。 |
| 🔴 **`audit_log.export`**（2026-09-18。`docs/04` §S-041 セクション 4「CSV エクスポート（監査ログに記録される）」に追随して確定） | `rowCount`（NUMBER）/ `truncated`（BOOLEAN） | ⚠️ **書き込み側は本改訂の対象外**（`programmer` が `#10b` の実装時に `pick-detail.ts` の `AUDIT_DETAIL_ALLOWLIST` へこの 1 行を足す）。値は `#10b` が実際に書き出した行数と、`AUDIT_LOG_EXPORT_MAX_ROWS` に達して打ち切ったかの真偽（§6.3 #10b / §6.4「CSV エクスポート」行 ⑤）。検索条件（`from` / `to` / `action` / `actorId`）は `summary` に元から無い（載せない設計） ✅ **T-12-18 ⑪ で実装（2026-09-21）**: 書き込み側は `packages/db/src/audit-log-export.ts` の `recordAuditLogExport`。 |
...
| 上記以外のすべて（`esign.*` / `sending_domain.state_change` / `assignment.view` / `contract.view` / `contract_document.download` / `retention.delete` / `admin.*` / `tenant.update` …。~~`state.invalid_transition` / `tenant.purge`~~ は T-12-18 ⑨ / ⑩ で上に行を得た） | （なし） | `entries: []`。画面は「詳細はありません」。~~⚠️ **接尾辞の族に当たる action は例外**（T-11-09 レビューで訂正。2026-09-17）: `admin.tenant.create` は接尾辞 `.create` で `*.create` の `CRUD_KEYS` が評価される（現行の `before` / `after` / `platformRole` / `reason` / `domain` は `CRUD_KEYS` に無く 0 キー）。将来 `admin.*.update` が `status` 等を書けば出るため、**`resolveAuditDetailKeySpecs` で `admin.` を接尾辞族から除外するか、記述をこのままにするかを SP-12 で決める**~~ → ✅ 🔴 **決定（T-12-18 ⑫。2026-09-18）: `admin.` は接尾辞族（`*.create` / `*.update` / `*.delete` の `CRUD_KEYS`）から除外する。** `resolveAuditDetailKeySpecs` は exact 一致の後、`AUDIT_DETAIL_SUFFIX_FAMILY_EXCLUDED_PREFIXES`（`['admin.']`）のいずれかで始まる action には接尾辞族を当てず `null`（= 0 キー）を返す。理由: 運営者の action（`admin.tenant.create` / `admin.quota.change` …）は **`S-041` の閲覧者（テナントの `OWNER` / `ADMIN`）に見せる対象ではなく `A-006` の側で扱う**ものであり、将来 `admin.*.update` が `status` / `operation` 等を書いた瞬間に CRUD の許可キーが**誤って**適用される経路を閉じる。`admin.*` の行そのものは従来どおり `S-041` に出る（`impersonation.*` と同じく主体は `運営者` + 不透明 ID。`BR-41`）が、展開は常に「詳細はありません」。**ユニット**（`pick-detail.test.ts`）: `admin.tenant.create` / `admin.tenant.update` / `admin.quota.change` に `CRUD_KEYS` の全キーを持つ `summary` を渡しても `entries: []`（対照: `partner_company.update` は出る）+ `AUDIT_DETAIL_SUFFIX_FAMILY_EXCLUDED_PREFIXES` をスナップショットに含める。順序 §6.4（本行）→ `pick-detail.ts` |
...
- 🔴 **T-12-18 ⑨ / ⑩ / ⑫ の追加分（2026-09-18）**: ユニット ①のスナップショット差分（`proposal.update` +6 キー / `state.invalid_transition` / `tenant.purge` / `data_export.download` / `AUDIT_DETAIL_SUFFIX_FAMILY_EXCLUDED_PREFIXES`）+ ②の禁止名の検査は**新しい行にも自動で及ぶ**（`counts` / `count_*` / `kind` / `entity` は語彙に無い）。結合 `tests/isolation/audit-log-detail.test.ts` に **正のケース**（実 `gate.run` の `GATE_RESULT` 行で 4 つの verdict が `PASS` / `FAIL` の ENUM として出る / #48 の不正遷移で `entity` = `Proposal`・`from → to` が対で出る / 実 `tenant.purge` の行で `count_{table}` が `PURGE_SPEC.delete` の表名ぶん NUMBER で出て `runId` が出ない / #78 の行で `kind` = `CLOSING_RETURN` が出て `exportRequestId` が出ない）と **負のケース**（`DRAFT_UPDATE` の `fields` / `skill_sheet.update` の `wasLatest` / `GATE_RESULT` の `aiFailed` / `count_` の後ろが表名の形でない `count_Engineers!` / 負数の `count_engineers` が落ちる）を足す。(f) の走査は許可リストのキーを読むので追加行も自動で対象になる。**`A-006`（`maskAuditSummary` / `admin-audit-logs.test.ts`）は本改訂でも変更なし**（§5.5 ⑨「独立に判断する」） ✅ **2026-09-21 に実装**（結合は `tests/isolation/audit-log-detail.test.ts` の (i)。ユニットのスナップショットは `proposal.update` +6 キー / `state.invalid_transition` / `tenant.purge`〔`count_*`〕/ `data_export.download` / `audit_log.export` の差分）。
...
| 40b | 🔴 `GET /api/proposals/{id}/gate-results` | `F-020 AC-7`（再実行で上書きせず履歴として残す）/ `S-023` セクション 4「実行ごとの履歴」/ **SP-12（T-12-14 ②。2026-09-18）**。✅ **T-12-14 で実装**（`listReviewGateResults` / `readProposalGateResults` / `heldViewFor`。結合 `proposal-gate-api.test.ts` の検証 ①〜⑤） | — | `{ items: GateResultHistoryItem[] }`（`review_gates` を `(tenant_id, target_type, target_id)` で**複数行**、実行日時の降順。各行 = `GateResultView` と同じ射影〔`contentHash` / 層別 PASS・FAIL / 指摘 / `aiWarnings` / `aiFailed` / `held`〕+ `reviewGateId` / `executedAt` / `heldSince` / `matchesCurrentContent`。下記「#40b と `S-023` セクション 4 の設計」） | 🔴 **境界は #40 と同一**（取引先は自社提案分のみ。他社・他テナント・不存在は同じ 404〔本文まで同一〕）。**認可・射影は #40 と同じ関数を共有する**（`loadTarget` / `toGateResultView` / `held` の組み立て。別実装にしない）。ガードも #40 の `GET` と同じ **`requireRole([...PROPOSAL_GATE_REQUEST_ROLES, 'VIEWER'])`**（読み取り = `requireExecutable` / `requireNotViewer` を掛けない）。監査は書かない（#40 / #46 と同じ線引き） |
...
🔴 **#40b と `S-023` セクション 4 の設計（T-12-14 ②。2026-09-18。SP-09 T-09-09 ③-③ の申し送りの回収。`F-020 AC-7` / `docs/02` `ui-design` 申し送り 5 / `docs/04` §S-023 セクション 4〔`ui-design` が「現在の結果」→「実行ごとの履歴」に並行改訂中。同じ既定〕）**:

- **何が足りなかったか**: `review_gates` は再実行で上書きせず行を残す（`F-020 AC-7`。§3.6 の部分 UNIQUE は HELD 行だけを 1 行に絞り、`DONE` 行は内容ごとに積まれる）が、**読む API が #40（最新の 1 件）しか無く、`S-023` セクション 4 は「現在の結果」だけを描いていた**。足りないのは読む API と描く場所であり、**書き込み側・状態機械・`jobId`・CAS は 1 つも変えない**。
- **#40b の応答型**（`apps/web/lib/proposals/gate.ts` に `readProposalGateResults` を足し、型は `packages/domain/src/gate/view.ts` の `GateResultView` を**拡張**する）:

```ts
export type GateResultHistoryItem = {
  readonly reviewGateId: string;                       // 🔴 review_gates.id。S-023 の「検査 #<id>」（承認の履歴行の reviewGateId）と突合するため
  readonly execution: 'DONE' | 'HELD_AI_COST_LIMIT';   // 🔴 履歴に 'RUNNING' は無い（RUNNING = 「確定した行がまだ無い」。§11.10 ⑦）
  readonly executedAt: string | null;                  // DONE のとき ISO 8601、HELD は null
  readonly heldSince: string | null;                   // HELD のとき ISO 8601、DONE は null
  readonly matchesCurrentContent: boolean;             // 🔴 row.contentHash === 現在の内容のハッシュ（computeProposalContentHash）。画面が「現在の内容に対する結果」を印で示す
  readonly layers: GateResultView['layers'];           // 🔴 #40 と同じ射影（toGateResultView の出力をそのまま写す）
  readonly aiWarnings: GateResultView['aiWarnings'];
  readonly aiFailed: boolean;
  readonly contentHash: string;
  readonly held?: GateResultView['held'];              // execution='HELD_AI_COST_LIMIT' のときだけ（#40 と同じ GateHeldView）
};
export type GateResultHistoryView = { readonly items: readonly GateResultHistoryItem[] };   // 降順。0 件 = まだ一度も依頼していない
```

- **手順**（`readProposalGateResults(ctx, proposalId, meta)`）: ①**#40 と同じ `loadTarget(ctx, proposalId)`**（C5。見えなければ `requireFound` が 404。本文まで #40 と同一）②`listReviewGateResults(ctx, { targetType: 'PROPOSAL', targetId })`（`packages/db/src/review-gate.ts` に**新設**。`readReviewGateResult` と同じ `select`〔+ `executedAt`〕・同じ `where`・同じ `AuthenticatedTenantCtx`〔§11.10 ⑧ = 分離キーを ctx からそのまま取る〕で**全行**を返す。🔴 **`readReviewGateResult` はこの関数の結果から「保留を優先して 1 行選ぶ」だけに書き換え、母集団のクエリを 1 実装にする**）③並びは **`COALESCE(executed_at, held_since) DESC, id DESC`**（HELD 行は部分 UNIQUE により高々 1 行で、常に最新側に来る）④各行を **#40 と同じ `toGateResultView(row, held?)`** に通し、`held` は #40 の HELD 分岐（`gateHoldTimestamps` + `limitRaise: 'PLATFORM_OPERATOR'` + `rerun`）を**関数に切り出して共有**する（`heldViewFor(row, now)`。#40 / #40b の 2 呼び出し元。書き写さない）⑤`matchesCurrentContent` は `target.contentHash`（#40 が `RUNNING` のときに返す「現在の内容のハッシュ」と同じ値）との等値。
- 🔴 **境界の担保は #40 と同じ 2 枚**: RLS（`review_gates` は C5 PARTY = 取引先は自社作成の提案の行だけ）+ `loadTarget` の 404。**取引先が他社提案の履歴に到達しない**ことは `tests/isolation/proposal-gate-api.test.ts` の #40 の境界ケース（他社・他テナント・不存在が同じ 404）に **#40b の同じケースを足して固定する**（§11.10 ⑨ / §17.3 #2）。応答に `owner` / 主体 / 提案先は無く、`GateResultView` と同じ材料だけ（`GateInput` を返す API は引き続き存在しない。§11.14）。
- 🔴 **`S-023` セクション 4 = 「実行ごとの履歴」**（サーバコンポーネント内で `readProposalGateResults` を直接呼ぶ。Route Handler を経由せず、#46b / `S-006` と同じ作法）: 1 実行 = 1 ブロック（見出し = 実行日時〔HELD は「上限到達で未実行（保留開始: …）」〕+ `matchesCurrentContent` の印「現在の内容に対する結果」/ 古い内容には「以前の内容に対する結果」）。**各ブロックの層別結果は `S-021` のゲート結果と同じ view model 関数 `approvalGateRows(gate: GateResultView)`（`apps/web/lib/proposals/approval-rows.ts`）と同じ描画部品**で描く（`GateResultHistoryItem` は `GateResultView` の部分構造をそのまま持つので、そのまま渡せる。履歴用の別実装を書かない）。🔴 **整合層の機械照合の合否（`layers.consistency`。PASS / FAIL + `findings`）と AI の警告（`aiWarnings`）は履歴でも別の見出しで描く** —— 不合格と警告を混ぜない（`CLAUDE.md` §3.3 第 3 層 / `docs/02` `ui-design` 申し送り 5。`aiFailed = true` の行は「検査を完了できなかった」の文言で PII / 商流の FAIL と区別する = `S-021` と同じ）。`aiWarnings` の見出しは「AI の警告（合否に影響しません）」。**折りたたみは `docs/04` §10.3 の共通規約（直近 10 件 + 「すべて表示」）に合流し、`S-023` 固有の例外を作らない**（T-12-14 ③ と同じ判断。1 提案の実行回数は通常 1〜数回）。モバイルは既存のアコーディオン（`docs/04` §S-023「デバイス別」）のまま。取引先にも同じセクションが出る（自社提案の履歴 = #40 と同じ開示範囲。`held.resetAt` を含む —— §5.8.1 ⑦ の ⚠️〔`F-027 AC-1` との字義の食い違い〕は #40b にもそのまま当てはまり、判断は #40 と一括）。
- **#46 の `gate`（現在の結果）は変えない**。`S-023` の固定ヘッダ・承認の履歴行（「検査 #<id>」）は従来どおり #46 から描き、セクション 4 だけが #40b を読む（同じ `reviewGateId` で突合できる）。
- **検証**: 結合 `tests/isolation/proposal-gate-api.test.ts` に ①同じ提案で内容を変えて #39 → `gate.run` を 2 回 → #40b が **2 行を降順**で返し、新しい行だけ `matchesCurrentContent = true`、古い行の `contentHash` が残る（上書きされない = `F-020 AC-7`）②HELD 行がある提案は先頭が `HELD_AI_COST_LIMIT` + `held`、`DONE` 行の `held` は `undefined` ③🔴 **取引先 A1 が他社（A2）の提案 / テナント B の提案 / 不存在の ID で #40b を叩くと同じ 404（本文まで #40 と同一）、自社提案では 200** ④一度も依頼していない `DRAFT` は `items: []`（404 にしない）⑤`?force=true` 等の未知クエリが 1 バイトも結果を変えない。render `proposal-detail-screen.render.test.tsx` に「履歴が実行ごとに分かれ、警告が不合格と別の見出しで描かれ、`aiFailed` の行が『検査を完了できなかった』になる」を足す。**境界領域として `code-reviewer` 1 回**（`CLAUDE.md` §8.3）。
...
- 🔴 **2 本のクエリ**: ①ホスト文脈なら `withSharedCandidateScope(ctx, projectId, …)` で共有候補の**全件**を読み、**同じトランザクションで `replaceAnonymousCandidates` により `MatchCandidate`（`isAnonymous = true`）を案件全体で置き換える**（Phase 1 に `match.build` は無いため、**候補一覧の読み取りが `MatchCandidate` の唯一の生成経路**である。`POST /api/proposal-requests`（#31。T-08-06）はこの行から `candidateRef` を逆引きする）。T-12-19 で生 SQL（`$executeRaw`。`packages/db/src/shared-candidate.ts`）になったが、書く列・行数・`tenant_id` の出所は不変。生成を `match.build` に移す (d) は SP-13 `T-13-07` の着手条件。②`withTenant` で案件の共通部分・自社候補（`engineerSearchPlan(criteria).buckets` ごとに `id` / `updatedAt` の全件）・ページ分の自社詳細を読み、監査を書く。**パートナー文脈は①を実行しない**（`F-017 AC-5`。匿名候補は応答に 0 件であり、`withSharedCandidateScope` 自体も `requireHost` で拒む二重）。
...
| API-A2 | `GET /api/admin/tenants` | `F-056` / `A-002` | 0→1 | `PO`/`PP`（閲覧）。🔴 **T-11-01（Phase 1）: 既定の並びは異常度の高い順**（`?sort=health`〔既定〕/ `name` / `createdAt`。`F-056 AC-2`）。各行に `health: { score, signals }` と `activeMemberCount` を載せる。**T-11-01 の実装の決着は本表の直下**。✅ 🔴 **T-12-18 ③（2026-09-18。Phase 1）: 応答に `summary: Record<TenantHealthSignal, number>`（種別ごとのテナント数。`A-002` セクション 1「異常の要約」）を追加し、`?signal=<TenantHealthSignal>` の絞り込みを受ける**（要約のクリック = 絞り込み。決着表の「応答の形」行の追記） ✅ **T-12-18 ③ で実装（2026-09-21）**。 |
...
| 応答の形 | `{ items: PlatformTenantListItemView[], nextCursor, observedAt }`。行 = SP-03 の 11 キー + `activeMemberCount` + `health: { score: number; signals: TenantHealthSignal[] }`。`sort` の値集合は `TENANT_LIST_SORT_KEYS`（`@ses/domain`）を境界検証（`apps/web/lib/admin-tenants/schemas.ts`。未知の値は 400）とクエリが共有する。✅ 🔴 **T-12-18 ③（2026-09-18。SP-11 T-11-01 申し送り ④。既定 = Phase 1 で実装）: `summary: Record<TenantHealthSignal, number>` を応答の最上位に足す**（`{ items, nextCursor, observedAt, summary }`）。**5 キーを必ず全部持つ**（0 件も `0`。型が `Record` なので欠けられない）。値 = **その種別のシグナルを持つテナントの数**（1 テナントが 2 種別を持てば両方に数える。`docs/04` §A-002「閾値を割った件数」= 種別ごとの件数）。🔴 **母集団は絞り込み・カーソルを外した同じ算出結果**（並びと同じくメモリで確定した全行から数える。`CLOSING` / `PURGED` は `signals: []` なのでどこにも数えない）—— #45 の `byState` と同じ考え方で、絞り込み中も他の種別の件数が見えて切り替えられる。**`?signal=<TenantHealthSignal>`**（任意。値集合は `TENANT_HEALTH_SIGNALS`〔`@ses/domain`〕を境界検証とクエリが共有。未知の値は 400）を付けると `items` は **`health.signals` にその種別を含む行だけ**（並び・カーソルの意味は不変。絞り込みの切替でカーソルを捨てる = `sort` と同じ）。🔴 **件数だけであり、テナントの内容には立ち入らない（`BR-40`）** —— `summary` のキーは列挙値、値は整数。**E2E #15 の禁止キー走査は `summary` をキーとしては禁じていない**（`tests/support/admin-forbidden-keys.ts` 群 A〜D に `summary` は無い。禁じているのは `A-006` の `summary` の**生値**であり、API-A2 の `summary` の下に現れるキー集合は `TENANT_HEALTH_SIGNALS` の列挙値だけ = 走査を通る。E2E #15 (a) に「API-A2 の `summary` のキー集合が `TENANT_HEALTH_SIGNALS` と一致し、値がすべて整数」の正のケースを 1 つ足す）。画面 `A-002` セクション 1 = 要約チップ（種別ラベル + 件数。クリックで `?signal=`。選択中は解除の導線）を一覧の最上部（集計日時の上）に置く。**検証**: 結合 `tests/isolation/admin-tenant-health.test.ts` に「`summary` の各値が `items`（全ページ）の `signals` の集計と一致 / `?signal=X` の `items` に X を持たない行が 0 件かつ `summary` は絞り込み前と同じ / 未知の `signal` は 400」、render `admin-tenants-list.render.test.tsx` に「要約が最上部・0 件の種別も描く」、`schemas.test.ts` に `signal` の境界 ✅ **T-12-18 ③ で実装（2026-09-21）**: 要約は `countTenantHealthSignals`（`packages/domain/src/health/tenant-health.ts`。純粋関数）、絞り込みは `listPlatformTenants` の `query.signal`（母集団はメモリで確定した全行のまま）、境界検証は `parseAdminTenantListQuery` / 画面の門番は `parseTenantListSignal`。 |
...
| 結合（実 DB + 実 Redis） | `DRAFT → GATE_RUNNING` の CAS・`contentHash`・`ProposalEvent`・監査・enqueue / 🔴 **§9.10 の 5 手順**（failed の削除 → `waiting` への復帰 → 状態不変）/ `waiting` を消さないこと / `DONE` 行で 422 かつ積まないこと / HELD の手動再開（同じ `jobId`・`GATE_FAILED` にしない）/ #40 の 3 値と `held` / 他社・他テナント・不存在が同じ 404（本文まで同一）/ `?force=true` が 1 バイトも結果を変えないこと。🔴 **履歴 API（#40b `GET /api/proposals/{id}/gate-results`。T-12-14 ②。2026-09-18）も同じ結合で固定する** —— 同じ spec に「他社・他テナント・不存在が #40 と同じ 404（本文まで同一）/ 自社提案は 200 / 内容を変えて 2 回実行すると 2 行が降順」を足す（§6.5「#40b と `S-023` セクション 4 の設計」の検証 ①〜⑤） |
...
| 8 | ✅ **T-12-17 ⑲ (b)（2026-09-18。[Issue #68](https://github.com/Festal-KM/SES-Platform/issues/68)）**: `TENANT_PURGE_GRACE_DAYS` は **`production` / `sandbox` / `staging` で 30 に固定**（`CLAUDE.md` §4.2 `Tenant` の `CLOSING` → 30 日 → `PURGED`。30 以外なら `EnvValidationError`。メッセージは変数名だけで値を出さない = 規則 5）。`development` / `demo` は可変のまま（結合 / E2E #24 が短い値を使う）。規則 7 と同じ「顧客データのある環境では固定」の形（`crossFieldChecks`） |
...
packages/db/seed/
  ...
  presets/{demo,isolation,perf}.ts  # demo: 複数の取引先・数十人の台帳・進行中の提案・満了が近い稼働・ゲートで止まる資料
                                    # isolation: 2 テナント × 2 パートナー（CLAUDE.md §5 Phase 0）/ perf: 1 万 / 1 万 / 匿名共有 2,000（docs/03 §3.7.2。✅ T-12-01。CLI 専用）
...
| 🔴 **複数テナント** | `demo` は 2 テナント（片方は取引先 5 社・もう片方は 1 社）。`isolation` は 2 テナント × 2 パートナー（**各パートナーが当事者の `Assignment` / `Contract` / `Order` を 1 件ずつ含め、同一案件に両社の稼働を置く** — §4.7 #8〜#10 / §17.3 #21 の母集団）。`perf` は 30 テナント（最大テナントにエンジニア 3,000 / 案件 3,000 / パートナー 15 社。`docs/03` §3.7.2。✅ T-12-01: 配分表は下表「T-12-01 の実装の決着」） |
...
**T-12-01 の実装の決着（`seed:perf`。2026-09-18。`docs/03` §3.7.2「検証方法」/ `docs/02` 章 7.1 / `CLAUDE.md` §7 / `BR-47`）**

| 論点 | 決着 |
|---|---|
| 🔴 **配分表（均等割にしない）** | `packages/db/seed/presets/perf.ts` の `PERF_SEED_PROFILES`（30 行の定数。`seed.test.ts` が合計を固定する）。**1 位 = エンジニア 3,000 / 案件 3,000 / 取引先 15 社**、2 位 1,500 / 1,500 / 5 社、3 位 1,000 / 1,000 / 5 社、4 位 700 / 5 社、5 位 500 / 4 社、6 位 400 / 4 社、7 位 300 / 4 社、8 位 250 / 3 社、9〜10 位 200 / 3 社、11〜20 位 130 / 3 社、21〜30 位 65 / 2 社（案件数はエンジニア数と同じ）。合計はちょうど **10,000 / 10,000**、残り 29 社の取引先は 2〜5 社（`docs/03` §3.7.2）。各テナントの取引先所属は表の `partnerEngineers`（1 位 2,250 = 15 社 × 150 名。以下おおむね 4 割）で、残りがホスト所属 |
| 🔴 **匿名共有 2,000 件の偏り** | **全件を最大テナントの取引先 15 社に置く**（`perfPartnerShareCount`: 133 × 15 + 端数 5 = 先頭 5 社が 134。他の 29 テナントは 0 件）。§3.7.1「自社スコープ + 共有スコープの 2 本のクエリ + アプリ層マージ」の最悪ケース = 最大テナントのホスト `SALES` が検索したとき、自社 750 名 + 共有 2,000 名が同じ条件で当たる形。開示は `EngineerShare(revoked_at IS NULL)` の opt-in だけ（経路 4）で、共有元 = `engineers.owner_partner_company_id` を結合テストが全行で突き合わせる |
| **検索が当たる分布** | スキルは辞書（`global-skills`）から**重み付き**（カテゴリ重み + `Java` / `AWS` / `SQL` / `TypeScript` / `基本設計` 等のブースト）で 3〜8 個、経験年数 0.5〜15 年（0.5 刻み）、単価 45〜95 万 + 5〜20 万、稼働状況 WORKING 55 / STANDBY_SCHEDULED 30 / STANDBY 15（%）と稼働可能時期は状況に応じた相対日、都道府県は全 47 を 1 ずつ + 東京 40 / 大阪 10 / 神奈川 8 / 埼玉・千葉・愛知 6 … の重み、リモート区分 30 / 50 / 20、フリーワードは `preference_note` の定型句（12 種から 1〜2）。案件は `name` / `public_summary` が業種 × 案件種別の組、状態 OPEN 70 / FILLED 25 / SUCCESSOR_WANTED 5、`start_date` は `T-60`〜`T+120`、要件 MUST 1〜3 + NICE 0〜2（辞書 ID 付き）。**半数を公開**（経路 1。`review_gates(PROJECT_PUBLISH, 全層 PASS)` + `project_visibilities` を 1〜3 社に。ハッシュは `gateContentHash` に実物と同じ材料）。経歴は 1 人 2〜4 行（実測 30,155 行）。**最大テナントで `Java ≥ 3 年` / 東京 / フルリモート / `STANDBY` / 単価 70 万 / 「フルリモート」の各条件が数百件以上当たり、47 都道府県すべてが現れる**ことをユニット（計画）と結合（実 DB）の両方で固定 |
| 🔴 **`createMany` と `transition()` の線引き** | 1 万件の母集団（エンジニア / スキル / 経歴 / 案件 / 要件 / 公開ゲート / 公開範囲 / 共有 / 監査）は **`createMany`（バッチ 1,000）**で入れる。**`advanceState`（`transition()` + CAS）で進めるのは各テナント 4 件の提案（`DRAFT` / `APPROVAL_PENDING` / `SUBMITTED`〔`T-5`〕/ `WON`〔`T-40`〕）と `WON` からの `Assignment(ACTIVE, 満了 T+55)` だけ**（30 テナント × 4 = 120 件。1 件ずつ CAS で進める設計を 1 万件に適用しない）。提案の承認・送信の三つ巴（`content_hash`）と `SendAttempt` / `ProposalEvent` / `AuditLog` は `demo` と同じ形 |
| 🔴 **冪等な再生成（`F-053 AC-2`）** | 計画（`buildPerfTenantPlans`）は**純粋関数**（固定シード `ses-perf-v1`、`rng` の消費順が配分表の順に固定）で、ID は `seedUuid`（プリセットコード `fe4f`。`demo` `de00` / `isolation` `150a` / 辞書 `5c11` と分ける）。ユニットは 2 回生成の SHA-256 一致を、結合は `reset` → 再投入で**全表の行数が不変・最大テナントのエンジニア 3,000 行の ID / 氏名 / 値と提案 120 件の状態・`content_hash` が一致**し、**同居する `isolation` の 2 テナントの行数が 1 行も変わらない**ことを固定。`readSeedPresence` / `ALREADY_SEEDED` / `SeedIncompleteError` の契約は `demo` と同じ（`provisioning_request_id = seed-perf-provisioning-{n}`） |
| 🔴 **規模の検証（黙って少なく作らない）** | `seed()` の最後に `readPerfSeedScale`（テナント ID で絞った実測: 30 / 10,000 / 10,000 / 共有 2,000 / `skill_sheets` 0 / 最大テナント 3,000 / 3,000 / 15 社 / 共有 2,000）を `PERF_SEED_TOTALS` と突き合わせ、1 つでもずれれば `PerfSeedScaleError` で投入を失敗させる |
| 🔴 **合成データのみ（`BR-47`）** | 氏名は `DEMO_SEED_NAME_RULES` の姓 × 名（96 通りを循環）、商号は同じ接頭辞に「性能」+ 連番（`株式会社サンプル性能01`〜`30` / `株式会社ダミー性能0101`〜）、エンド企業は `架空〜株式会社`、メール・送信ドメインは `perf-{nn}.example`（送信ドメインは Issue #57 の暫定対応どおり `VERIFIED` 行を直接作る）。電話・生年月日は置かない。`skill_sheets` は 0 行。静的テスト `tests/static/demo-seed-no-real-names.test.ts` の走査対象に `perf.ts` を追加（同じ禁止リスト。辞書名 `Oracle Database` 等をソースに書かないため、スキルの重みは**カテゴリ + 一部の名前**で持つ） |
| 🔴 **`deleteTenantData` のトランザクション上限** | `packages/db/src/seed-sql.ts` の `deleteTenantData`（`reset()` の実体。1 トランザクションで `tenant_id` を持つ全表を消す）は Prisma の既定 5 秒のままだと `perf` の約 13 万行の削除が負荷次第で切れる（結合テストで 9.4 秒。CLI 単体では 5 秒未満で通ることもある = 不安定）。**上限を 10 分（`DELETE_TENANT_DATA_TIMEOUT_MS`）に置いた**。切れた場合はロールバックされ半端には残らないが、`--reset` が成立しないため。`isolation` / `demo` の挙動は変わらない |
| 🔴 **CLI 専用** | `pnpm seed --preset=perf --reset`（`APP_ENV ∈ {demo, development}` のみ。`assertSeedableAppEnv` は `runSeed` の先頭）。**API-A16（`A-012`）からは投入できない**（`apps/web/app/api/admin/demo/_lib/service.ts` は `preset: 'demo'` 固定のまま）。1 万件の投入は Vercel の関数タイムアウトに掛かる規模であり、管理平面の同期実行（上表「同期実行」）に載せない。`SeedPresetNotImplementedError` は 3 プリセットが揃ったため撤去 |
| **所要時間の実測（2026-09-18。Docker Desktop for Windows 上の Testcontainers PostgreSQL 17。CLI）** | **1 回目（空 → 投入）158 秒、2 回目（`--reset`: 削除 → 投入）111 秒、`--reset` 無し（`ALREADY_SEEDED`）2.4 秒**。投入行数: engineers 10,000 / projects 10,000 / engineer_shares 2,000 / engineer_careers 30,155 / engineer_skills 54,952 / project_requirements 29,889 / project_visibilities 9,765 / review_gates 5,094 / audit_logs 7,334 / proposals 120 / assignments 30。内訳（1 回 122 秒の計測）: `engineer_skills` 31 秒 / `engineer_careers` 24 秒 / 案件 + 要件 21 秒 / 提案 + 稼働 19 秒 / 公開 10 秒 / エンジニア 8 秒 —— **`createMany` 1,000 行あたり 0.5〜0.8 秒（Prisma クライアント側の入力検証が支配的で、行数に比例）**。目安の「2 分以内」は 2 回目で満たし、1 回目はコンテナのコールドキャッシュ分だけ超える。短縮するなら `packages/db/src/seed-sql.ts` に `COPY` / `unnest` の一括投入（生 SQL の置き場所は §2.2 どおりそこだけ）を足すのが次の一手であり、本タスクでは触らない |
| ⚠️ **T-12-02 への申し送り（負荷測定の母集団の使い方）** | ① `APP_ENV=development SEED_DATABASE_URL=… pnpm seed --preset=perf --reset` で投入（`isolation` / `demo` と同居できる）。② **計測は最大テナント（`perfSeedIds(1)`。`株式会社サンプル性能01`）のホスト `SALES`**（`perfSeedIds(1).hostSalesUserIds[0]` / `perfSeedEmails(1).hostSales[0]` = `sales-1@perf-01.example`、合成パスワード `PERF_SEED_PASSWORD`）で行う。`GET /api/engineers?skills=<Java の辞書 ID>&yearsMin=3&prefecture=13&remote=FULL_REMOTE&q=フルリモート…`（`F-009`。共有スコープ 2,000 件を含む混在）と `GET /api/projects?status=OPEN&startFrom=…&prefecture=13&q=受発注`（`F-015`）の p50 / p95 / p99 を、条件の組（絞り込みなし / 1 条件 / 全条件 / フリーワードのみ）ごとに N=50〜100 回で採る。③ 置き場所の提案: **`tests/perf/**`（`vitest.perf.config.ts` を新設。`tests/isolation` と同じ Testcontainers ハーネスで `seed:perf` を `beforeAll` に置き、Route Handler を `seed-demo.test.ts` と同じ `requireTenantCtx` のモックで直接呼ぶ）**。CI の毎回実行には載せず（1 回 3〜5 分）、`docs/dev-plan.md` §8 に結果を記録する手動 / 定期ジョブとする。④ 匿名候補の評価は丸め後の区分に対して行う（T-08-05）ため、`yearsMin` / 単価 / 稼働時期の条件で共有スコープの当たり方が自社側と違うのは仕様であり、比較は「両スコープの合計件数と応答時間」で行う |
...
| 🔴 **運営者の全操作（閲覧を含む）** | `withPlatformRead` / `withPlatformWrite`（§5.3。`fn` の前に書く）。🔴 **`admin.*` の `summary` の形（T-12-18 ⑦。2026-09-18）**: `withPlatformWrite` の `before` / `after` は **`JSON.stringify` した文字列のまま**（64 文字を超えると `A-006` の `maskAuditSummary` の形状判定〔トークン形状 = 空白なし 1〜64 文字〕で `[masked]` に畳まれ、`admin.tenant.create` の内容が `A-006` から読めない）。**暫定。[Issue #69](https://github.com/Festal-KM/SES-Platform/issues/69) で確認中。既定 = 現状維持**（安全側。T-11-07 では扱われなかった）。「読めるようにする」に決まった場合は **記録側**を直す —— `before` / `after` を文字列化せず **キーごとの構造**（`§6.4` の許可リスト方式と同型 = `{ name, environment, lifecycleState, planId }` のように列挙値・ID・件数だけを平坦なキーで置く）で書く。🔴 **画面（`A-006`）で文字列を復元・パースしない**（自由文を正規表現で読む経路になる。`CLAUDE.md` §3.2 と同じ理由）。順序は本節 → 実装（`packages/db/src/platform.ts` の `withPlatformWrite` の呼び出し元ごと） | `PLATFORM_USER` |
...
| 🔴 **`audit_log.export`**（`S-041` セクション 4。2026-09-18。`docs/04` §S-041「CSV エクスポート（監査ログに記録される）」に追随して確定） | `#10b`（§6.3）の業務トランザクション内（`writeAuditLog`。`withApiRoute` の `audit` オプションは使わない — 400 `AUDIT_LOG_EXPORT_TOO_LARGE` で弾いた要求は残さない）。**閲覧（#10）は `BR-27` の対象外のまま**で非対称になるが、監査ログ全体の一括の外部化は個別閲覧より重い「持ち出し」であり `F-005` の説明責任の対象（§6.4「CSV エクスポート」行 ⑤）。`summary = { rowCount, truncated }` のみ（検索条件・CSV の内容は載せない）。`targetType='Tenant'` / `targetId` = エクスポートしたテナント自身 | `USER` |
...
| 項目 | クエリ | Phase |
|---|---|---|
| 失敗ジョブ数 | BullMQ の failed（キュー別）。🔴 `gate.run` の failed は §9.10 の再依頼（#39）で `Job.remove()` されるため、再依頼が行われると減る（意図どおり。未対応分だけが残る）。✅ **T-12-17 ⑥（2026-09-18）**: 読み取り口 `createBullMqFailedJobsReader` は `gate.run` 以外のキューで **`Job` ハッシュを取得しない**（件数 = `ZCARD` / 最終失敗時刻 = failed ZSET の先頭スコア〔BullMQ は `ZADD failed <finishedOn> <jobId>` で積む〕）。`account.mail` の failed ジョブの `data`（平文トークン）がプロセスのメモリに乗る経路を `FailedJobsSource` の型で塞いだ（読むだけの口という性質は不変。`admin-no-gate-retry.test.ts` ①） | 1 |
...
| 🔴 削除ジョブの失敗 | `TenantPurgeRun.status='FAILED'`（`kind='PURGE_JOB_FAILED'`）。**完了の事実は返さない**（`docs/04` 申し送り 15）。✅ **T-12-17 ⑱（2026-09-18）**: 同じ項目の**別区分** `runningOverdue`（`kind='RUNNING_OVERDUE'`）に `status='RUNNING' AND started_at <= now − PURGE_RUN_STALL_ALERT_MINUTES`（`packages/config`。既定 30。`SUBMITTING_STALL_ALERT_MINUTES` と同型）を `GROUP BY tenant_id, cause` で出す（SP-10 T-10-09 の「④成功 → ⑤失敗」= 削除は済んだが完了の書き込みに失敗した疑い）。🔴 `FAILED` の `total` に加算しない / 項目 15 と混ぜない / `counts` / `failure_reason` / `completed_at` は読まない（完了の事実は引き続き API-A12 のみ）。重さは `hold`（`FAILED` があれば `failure` が勝つ） | 1 |
...
| 2 | パートナーの全画面・API・集計・通知・エクスポートに他社由来の値が 0 件。**件数バッジ・並び順の変化・示唆も 0 件**（🔴 **ゲート結果の履歴 API #40b〔T-12-14 ②。2026-09-18〕は #40 の境界の結合テスト `tests/isolation/proposal-gate-api.test.ts` に同じケース〔他社・他テナント・不存在が同じ 404〕を足して固定する。E2E は本行の既存の走査〔取引先セッションで `S-023` を開く〕がセクション 4 の DOM を含むので新しい spec を足さない**） | `F-004 AC-3` / `AC-4` |
...
| 20 | エンジニア 1 万件 / 案件 1 万件 / 匿名共有 2,000 件で `F-009` の p95 が 1 秒以内（✅ **T-12-02: 計測ハーネスを実装し実測した（2026-09-18）。証跡 = `tests/perf/phase1-load.test.ts` + `vitest.perf.config.ts`（`pnpm test:perf`。CI には載せず手動 / リリース前）と `docs/dev-plan.md` §8 の 2026-09-18 の行。** `F-009` の複合検索（#15 自社スコープ。7 組）と `F-015`（#25。6 組）は p95 ≤ 190 ms で green、`gate.run` / `send.proposal` は起動〜確定が p95 0.4 秒で green。🔴 **匿名候補が混在する候補一覧（#15 `projectId` あり = #30。2 組）は p95 2.3 秒 / 3.9 秒で未達**であり、テストは赤のまま置く（予算を上げない）。原因と是正候補は下記「T-12-02 の実測の決着」。人間判断の材料は `docs/dev-plan.md` §8） | 🔴 `docs/03` §3.7.2 の分布で計測。Phase 1 の完了条件 → **T-12-19 で是正済み（2026-09-18。下記「T-12-19 の是正の決着」）** |
...
**T-12-02 の実測の決着（#20。2026-09-18。`CLAUDE.md` §7 / `docs/02` 章 7.1 / `docs/03` §3.7.2「検証方法」/ `F-009 AC-4` / `F-015 AC-2`）**

| 論点 | 決着 |
|---|---|
| **ハーネス** | `tests/perf/phase1-load.test.ts`（`vitest.perf.config.ts` / `pnpm test:perf`）。`tests/isolation` と同じ Testcontainers（PostgreSQL 17 + Redis）に `beforeAll` で `seed:perf`（`reset: false`。フレッシュなコンテナでは `SEEDED`、投入済み DB では `ALREADY_SEEDED`）を入れ、**Route Handler を `requireTenantCtx` のモックで直接呼ぶ**（`seed-demo.test.ts` と同じ作法。DB・RLS・検索・共有スコープ・キュー・Worker は実物）。最大テナント（`perfSeedIds(1)`）のホスト `SALES` で `GET /api/engineers`（9 組）/ `GET /api/projects`（6 組）を N = 60（ウォームアップ 5 を除く）直列に叩き、最近接順位法の p50 / p95 / p99 を採る。`gate.run` / `send.proposal` は実 BullMQ Worker（`createBullMqWorker` = 本番と同じ配線）+ モック AI / モックメールで、30 テナントの提案 1 件ずつ（N = 30）の #39 → `APPROVAL_PENDING` / #43 → `SUBMITTED` を API のポーリング（25 ms）で観測する。予算は `tests/perf/budget.ts` の定数（`packages/config` に置かない = 環境ごとに緩められない）。🔴 **CI の毎回実行には載せない**（投入 1〜3 分 + 計測 5 分）。`tests/perf/**` は ESLint の CATCH_ALL ゾーンであり `@ses/db/testing` と生 SQL を使わない（母集団の確認は seed の戻り値と API の `total` で行う） |
| **母集団の確認** | `GET /api/engineers` の `total` = 750（ホスト所属）、`GET /api/engineers?projectId=` の `total` = 2,750。**差 2,000 = 共有スコープの行数 = `engineer_shares` の全件**（経路 4 の母集団が入っている）。1 ページ目（50 件）に匿名候補 33 件が混在 |
| ✅ **`F-009 AC-4`（#15 自社スコープ）** | 7 組すべて green。p95 は 81〜188 ms（絞り込みなし 102 / スキル 1 つ 129 / スキル 2 つ OR 167 / スキル + 経験年数 + 単価 97 / 都道府県 + リモート 81 / フリーワード 188 / 全部盛り 136）。**フリーワード（`ILIKE`。索引条件に降りない〔`docs/03` §3.7.2 懸念 4〕）でも 3,000 件の走査が 1 テナント分に閉じるため p95 188 ms** |
| ✅ **`F-015 AC-2`（#25）** | 6 組すべて green。p95 30〜55 ms |
| ✅ **`gate.run` / `send.proposal`** | 起動〜確定の p95 = 387 ms / 332 ms（ハンドラ本体 138 / 151 ms）。**実 Anthropic / SES のレイテンシを含まない**（非本番は全モック。`CLAUDE.md` §11.1）。実 API を含む値は本番の `A-005` の滞留監視で見る |
| 🔴 **未達: 匿名候補が混在する候補一覧（#15 `projectId` あり = #30 / `S-016`）** | 絞り込みなし p50 1,835 / **p95 2,313** / p99 2,886 ms、全部盛り p50 1,776 / **p95 3,932** / p99 4,412 ms。**索引では直らない**（下記の実行計画は既に索引走査）。内訳（`withSharedCandidateScope` の関数単位で実測。4 回の中央値）: ①`listSharedEngineers` **0.73〜0.96 秒** = DB 0.37 秒（`engineers` 126 ms + `engineer_skills` 243 ms。いずれも Filter の **`app_engineer_is_shared()` を 1 行ごとに呼ぶ**コストで、`engineer_skills` は 10,928 行 × SECURITY DEFINER 関数〔`Buffers: shared hit=33113`〕）+ Prisma クライアント側 ≈ 0.5 秒（`select: { skill: {…} }` の入れ子解決 10,928 行 + `Decimal`）②`replaceAnonymousCandidates` **0.33〜0.45 秒**（DB の DELETE は 4 ms。**`createMany` 2,000 行の Prisma クライアント側の入力検証が支配的** = T-12-01 の「1,000 行 0.5〜0.8 秒」と同じ性質）③`buildAnonymousCandidateViews` 0.06〜0.13 秒 ④自社スコープ + マージ + 監査 ≈ 0.2〜0.3 秒 |
| **実行計画の要点（`app_tenant` / 共有スコープ `on` / `EXPLAIN (ANALYZE, BUFFERS)`）** | `engineers`（`owner_partner_company_id IS NOT NULL`）: `Bitmap Index Scan on engineers_tenant_id_owner_partner_company_id_availability_a_idx`（rows=2250, 0.5 ms）→ `Bitmap Heap Scan` の **`Filter: (app_engineer_is_shared(id, tenant_id) OR …)`**（rows=2000, Removed 250, **Buffers: shared hit=6709**, actual 122 ms）→ Sort（Execution 126 ms）。`engineer_skills`（`engineer_id = ANY(2,000 件)`）: `Bitmap Index Scan on engineer_skills_tenant_id_engineer_id_skill_id_key`（rows=10928, 3 ms）→ `Bitmap Heap Scan` の **`Filter: (app_engineer_is_shared(engineer_id, tenant_id) OR …)`**（**Buffers: shared hit=33113**, actual 232〜523 ms）→ Sort（Execution 243〜572 ms）。`match_candidates` の DELETE: `Index Scan using match_candidates_tenant_id_project_id_score_idx`（rows=2000, Execution 4 ms）。🔴 走査は索引で 1 テナント分に閉じており、時間は **1 行ごとの関数呼び出し**（インライン展開されない SECURITY DEFINER）に消えている |
| **OpenSearch の要否** | 🔴 **不要（無関係）。** フリーワード無しの組でも 1.8 秒であり、遅い部分は転置索引で解ける形ではない（RLS の関数呼び出しと Prisma クライアントの処理）。`docs/03` §3.7.3 の段階 4 に進む根拠にならない |
| **是正候補（使い捨ての実験で効果を実測。いずれも未実装 = 人間判断）** | **(a) 共有スコープの RLS ポリシーを集合化する**（migration。境界）: `app_shared_engineer_ids(t uuid) RETURNS uuid[]`（SECURITY DEFINER。所有者 `app_share_probe`。返すのは ID の配列だけで共有元は返さない）を 1 文 1 回だけ評価し、ポリシーを `id IN (SELECT unnest(app_shared_engineer_ids(app_tenant_id())))` にする → 実行計画が `hashed SubPlan` になり **`engineers` 126 → 6 ms / `engineer_skills` 243 → 18 ms**（`Buffers` 33,113 → 358）。**(b) `replaceAnonymousCandidates` を `packages/db/src/shared-candidate.ts` 内の生 SQL（`DELETE` + `INSERT … SELECT unnest($1::uuid[]) ON CONFLICT DO NOTHING`）にする** → **0.33〜0.45 → 0.03〜0.17 秒**（生 SQL の置き場所は §2.2 どおり `packages/db/src/**`）。**(c) `listSharedEngineers` の `engineer_skills` を平坦な `select`（`skillId` のみ）にし、辞書 `skills`（39 行）を別クエリで引いて JS で結ぶ** → Prisma の入れ子解決 ≈ 0.5 秒を削る（実験では (a) だけを入れても `listSharedEngineers` が 0.58〜0.68 秒残り、この 0.5 秒が Prisma 側であることを示す）。**(a) だけでは route 1.09〜1.67 秒で未達のまま。(a)+(b)+(c) で絞り込みなしの推定 ≈ 0.5〜0.7 秒。** より根本的には **(d) `MatchCandidate` の置き換えを読み取りから外す**（§4.6「この読み取りが `MatchCandidate` の唯一の生成経路」= Phase 1 の暫定。Phase 2 の `match.build` で生成側へ移す） |
| 🔴 **予算の読み方（人間判断）** | `CLAUDE.md` §7 / `docs/02` 章 7.1 は「複合検索（`F-009` / `F-015`）p95 1 秒」と「**マッチング候補の初回提示（`F-029`）p95 3 秒**」を分けている。混在の候補一覧（`S-016`）を後者で読めば絞り込みなし（2.3 秒）は満たすが、全部盛り（3.9 秒）は満たさない。**本タスクは T-12-02 の表どおり 1 秒で判定し、赤のまま置く**（判定を緩めない）。どちらで読むか・是正 (a)〜(d) をいつ入れるかは `docs/dev-plan.md` §8 の 2026-09-18 の行（Issue はオーケストレーターが起票） |
| **環境差** | 計測はローカル（AMD Ryzen 5 7530U / 13.8 GB / Docker Desktop for Windows 上の Testcontainers / node 24）。p99 が p95 の 1.2〜1.7 倍に散るのはこの環境の分散であり、RDS / ECS の値は本番の `A-005` で見る。**CI マシンの値を正としない** |

**T-12-19 の是正の決着（#20。2026-09-18。[Issue #71](https://github.com/Festal-KM/SES-Platform/issues/71) 既定 A）**

| 論点 | 決着 |
|---|---|
| **(a) 集合化** | migration `20260930000000_shared_scope_set_policy`。`app_shared_engineer_ids(t uuid) RETURNS uuid[]`（SQL / STABLE / SECURITY DEFINER / `SET search_path = public, pg_temp` / 所有者 `app_share_probe` / `REVOKE ALL FROM PUBLIC` + `GRANT EXECUTE TO app_tenant`）。本体は `CASE WHEN GUC 'on' AND app_is_host() THEN ARRAY(SELECT engineer_id FROM engineer_shares WHERE tenant_id = t AND revoked_at IS NULL) ELSE '{}'::uuid[] END`（NULL を返さない）。ポリシー 2 本は `tenant_id = app_tenant_id() AND COALESCE(current_setting('app.shared_scope', true), 'off') = 'on' AND id IN (SELECT unnest(app_shared_engineer_ids(app_tenant_id())))`（`engineer_skills` は `engineer_id IN`）。GUC の guard をポリシー本文にも明示した理由 = ①§4.7 #15 の走査（式に `app_engineer_is_shared` か `shared_scope` を含むポリシー = 2 本）を満たす ②`'off'` の通常経路で関数呼び出しを短絡。`unnest(...)` の引数を行に相関させない（`app_tenant_id()` を渡す）。`app_share_probe` の列 GRANT / 索引 / 射程は不変 |
| **`app_engineer_is_shared()` の扱い** | DROP せず `CREATE OR REPLACE` で本体を `COALESCE(eng = ANY(app_shared_engineer_ids(t)), false)` に寄せて 1 実装（所有者・EXECUTE・`search_path=public, pg_temp` を再指定して不変）。結合 §4.7 #15 / #17 と `shared-candidate-scope` が関数を直接呼ぶため残す |
| **`EXPLAIN (ANALYZE, BUFFERS)`** | `engineers` / `engineer_skills` の `Filter` が `ANY (id = (hashed SubPlan 1).col1)` に。Execution 126 → 9.7 ms / 243〜572 → 26 ms、`Buffers: shared hit` 6,709 → 132 / 33,113 → 358。SubPlan 本体（`unnest`）は 1 文 1 回 1.5 ms |
| **(b) `replaceAnonymousCandidates` の生 SQL 化**（`packages/db/src/shared-candidate.ts`。`$executeRaw` + `Prisma.sql`） | `DELETE FROM match_candidates WHERE tenant_id = $1 AND project_id = $2 AND is_anonymous = true` → `INSERT (id, tenant_id, project_id, engineer_id, is_anonymous, computed_at) SELECT … FROM unnest($ids::uuid[], $engineerIds::uuid[], $computedAts::timestamptz[]) ON CONFLICT (tenant_id, project_id, engineer_id) DO NOTHING`。`id` は `uuidV7(computedAt)`。戻り値は実 INSERT 行数。根拠 = Prisma の `createMany` 入力検証 2,000 行 ≈ 0.35 秒。`tests/static/db-raw-access.test.ts` ④ の既存許可（`packages/db/src/**`）の中で許可リストは広げない。GUC に触れない |
| **(c) `listSharedEngineers` の平坦化** | `engineer_skills` の select を `{ engineerId, skillId, yearsOfExperience }` の 3 列に、辞書は `tx.skill.findMany({ select: { id, name, sortKey } })` 1 回、JS の `Map` で結合。辞書に無い `skillId` は例外。`engineers` の 7 列・`orderBy`・返す形は不変 |
| **残る内訳（(b)(c) 後）** | `listSharedEngineers` 0.46〜0.54 秒（Prisma の 1 万行の行変換が主）/ `replaceAnonymousCandidates` 0.19〜0.40 秒（INSERT の FK 検査 4 本 × 2,000 行）。🔴 **§4.6 の暫定「読み取りが唯一の生成経路」を (d) `match.build`（Phase 2。SP-13 `T-13-07` の着手条件）に移せばこの 0.2〜0.4 秒と DELETE が消える** |
| **再測定**（`pnpm test:perf`。2026-09-18。seed perf v1 / N = 60 / ウォームアップ 5） | 混在 2 組 p95 **1,021 / 1,038 ms**（是正前 2,313 / 3,932。予算 `MATCH_P95_BUDGET_MS = 3,000` = `CLAUDE.md` §7「マッチング候補の初回提示 3 秒」/ `F-029 AC-5` / Issue #71 既定 A）、自社スコープ 7 組 p95 44〜86 ms（是正前 81〜188。ポリシー本文の GUC guard の短絡で改善）、案件 6 組 30〜83 ms、`gate.run` 351 / `send.proposal` 415 ms（不変）。表は `docs/dev-plan.md` §8「T-12-19 の再測定」。🔴 **Issue #71 が B（1 秒のまま）なら 1 秒を僅かに超えるため (d) の前倒しが要る** |
| **開示範囲** | 1 つも変わっていない（匿名 5 項目 / `revoked_at IS NULL` の行だけ / 読み手はホスト / 案件をまたいで突合できない参照子）。結合 `rls-enforced` 60 / `double-defense-matrix` 59 / `engineer-shares` 30 / `anonymous-candidate-view` 10 / `shared-candidate-scope` 29 / `engineer-careers` 28 / `search-indexes` 13 / `project-candidates` 22、静的 1,016 件が**無改変で** green |
...
| F-005 | §3.8(`AuditLog`) / **§16.1** / §6.3(#10, 🔴 #10b CSV) / §6.4(#10 の改訂 = 許可リスト表) | F-027 | **§5.8** / §7.6 / §8.7 / §6.7(#69,#70) | F-049 | **§8.4** / §10.2 / §12.3 |
...
| F-020 | **§11 全体** / §9.3(`gate.run`) / §6.5(#39,#40, 🔴 #40b = AC-7 の履歴) | F-042 | §3.7(`Assignment`) / §6.6(#53-#56) | F-064 | §9.7(`tenant.closing-notify` / `tenant.purge-scan` の配送確認 = AC-10 / `tenant.purge`) / §3.9(`TenantPurgeRun`) / §6.9(API-A12) / §6.4(#10 の改訂: `tenant.purge` / `data_export.download` の許可リスト = AC-8 の画面側) / §17.3(#17,#24) |
