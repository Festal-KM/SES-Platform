...
- ⚠️ **暫定の遷移先**: 保存後・キャンセル・**404 境界（`[id]/edit/not-found.tsx`。`docs/04` §10.1 `S-012`「対象が削除済み → 一覧へ」）**はいずれも `PROJECT_FORM_CANCEL_HREF = '/'`（`_form/form-props.ts`。`S-010` が入る **T-06-03** で `/projects` に差し替えれば 3 経路が同時に動く）、登録直後は `PROJECT_CREATED_HREF_PATTERN`（**`apps/web/lib/projects/created-href.ts`**）。🔴 **後者は関数ではなく文字列**である —— サーバコンポーネントからクライアントコンポーネントへ渡す props は直列化できる必要があり、関数は渡せない（§6.1「Server Actions を使わない」）。
