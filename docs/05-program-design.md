...
| 7 | 送信を 2 回起動しても外部呼び出しが 1 回（同一 `idempotency_key`）（✅ T-09-06: 結合 `tests/isolation/send-proposal.test.ts` ① で実 Redis + 実 Worker により証明。ブラウザ経路は `home.mobile.spec.ts` が #43 の 2 回目が同じ `jobId` で 202 になるところまで。worker を立てる T-09-11 で送信の確定まで通す） | `F-022 AC-1` |
| 8 | 🔴 **応答不明 → `SUBMIT_FAILED` → 自動再送されない → 人手再送で 1 回だけ送信**（✅ T-09-07: 結合 `tests/isolation/send-proposal-unknown.test.ts` ①②④ で実 Redis + 実 Worker + モックの台本 `unknown` により証明〔外部 1 → 自動再送 0 → #44 で 1 = 合計 2〕。ブラウザ経路は `tests/e2e/send-unknown.spec.ts` が前提をシームで作り契約を確かめる。worker を立てる T-09-11 で通しに置き換える） | `F-022 AC-3` / `F-023` / `UC-20` |
| 9 | 🔴 **保留（ドメイン未検証）が `SUBMIT_FAILED` にならず、検証後に自動復帰して送信される**（✅ T-09-06: 結合 `send-proposal.test.ts` ④。ブラウザ経路は T-09-11） | §10.4 |
