...
| 10b | 🔴 `GET /api/audit-logs/export` | `F-005` / `S-041` セクション 4（CSV エクスポート）/ **Phase 1（T-12-18 ⑪。2026-09-18）** | `?from=&to=&action=&actorId=`（#10 と**同じスキーマから `cursor` / `limit` を除いたもの**。期間必須・上限 `AUDIT_LOG_SEARCH_MAX_PERIOD_DAYS` は #10 と同じ判定） | `text/csv; charset=utf-8`（UTF-8 BOM 付き RFC 4180。`Content-Disposition: attachment; filename="audit-logs-{from}-{to}.csv"`）。🔴 **ヘッダは 6 列固定**（`createdAt` / `actorKind` / `actorDisplayName` / `action` / `target` / `ipAndDevice`。§6.4「#10 の改訂」の「CSV エクスポート」行）。**`detail` / `summary` の列は無い** | `OWNER` / `ADMIN`（#10 と同じ `requireRole`。読み取り専用 = `requireExecutable` / `requireNotViewer` を掛けない。`CLOSING` でも可）。🔴 **上限 `AUDIT_LOG_EXPORT_MAX_ROWS`（`packages/config`。暫定 10,000）を超える母集団は 400 `AUDIT_LOG_EXPORT_TOO_LARGE`（期間短縮を促す）**。✅ 🔴 **実行のたびに `audit_log.export`（`USER`。1 エクスポート 1 行）を記録する**（2026-09-18。`docs/04` §S-041「エクスポート → CSV（監査ログに記録される）」に合わせて確定。§16.1 / §6.4「CSV エクスポート」行 ⑤。400 で弾いた要求は記録しない。`summary = { rowCount, truncated }` のみ ── 検索条件・CSV の内容は載せない） |
...
| 🔴 **`audit_log.export`**（2026-09-18。`docs/04` §S-041 セクション 4「CSV エクスポート（監査ログに記録される）」に追随して確定） | `rowCount`（NUMBER）/ `truncated`（BOOLEAN） | ⚠️ **書き込み側は本改訂の対象外**（`programmer` が `#10b` の実装時に `pick-detail.ts` の `AUDIT_DETAIL_ALLOWLIST` へこの 1 行を足す）。値は `#10b` が実際に書き出した行数と、`AUDIT_LOG_EXPORT_MAX_ROWS` に達して打ち切ったかの真偽（§6.3 #10b / §6.4「CSV エクスポート」行 ⑤）。検索条件（`from` / `to` / `action` / `actorId`）は `summary` に元から無い（載せない設計） |
...
| 40b | 🔴 `GET /api/proposals/{id}/gate-results` | `F-020 AC-7`（再実行で上書きせず履歴として残す）/ `S-023` セクション 4「実行ごとの履歴」/ **SP-12（T-12-14 ②。2026-09-18）** | — | `{ items: GateResultHistoryItem[] }`（`review_gates` を `(tenant_id, target_type, target_id)` で**複数行**、実行日時の降順。各行 = `GateResultView` と同じ射影〔`contentHash` / 層別 PASS・FAIL / 指摘 / `aiWarnings` / `aiFailed` / `held`〕+ `reviewGateId` / `executedAt` / `heldSince` / `matchesCurrentContent`。下記「#40b と `S-023` セクション 4 の設計」） | 🔴 **境界は #40 と同一**（取引先は自社提案分のみ。他社・他テナント・不存在は同じ 404〔本文まで同一〕）。**認可・射影は #40 と同じ関数を共有する**（`loadTarget` / `toGateResultView` / `held` の組み立て。別実装にしない）。ガードも #40 の `GET` と同じ **`requireRole([...PROPOSAL_GATE_REQUEST_ROLES, 'VIEWER'])`**（読み取り = `requireExecutable` / `requireNotViewer` を掛けない）。監査は書かない（#40 / #46 と同じ線引き） |
