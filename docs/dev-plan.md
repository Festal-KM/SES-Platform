...
| `U-7` / TBD-7 | GuardDuty のスキャンが 2 分以内か | `SCAN_STALL_ALERT_MINUTES`（既定 10）で滞留を検知する設計にし、目標値に依存させない（**T-05-05 で実装済み**） | T-05-05 | ~~SP-05 内で実測~~ → **AWS 環境の構築時に実測（E-13。2026-09-06 に変更。[Issue #37](https://github.com/Festal-KM/SES-Platform/issues/37) で確認中）** |
...
| 6 | ~~Phase 1 の `F-011` 実装時に~~ **AWS 環境の構築時に GuardDuty のスキャン所要時間を実測**（2026-09-06 変更）。2 分を満たせないなら目標値の見直しを人間に提起 | **§5 E-13** / §9（`U-7` / TBD-7 の行） | **AWS 環境構築タスク**（設計側の対応は T-05-05 で完了。時期の変更は [Issue #37](https://github.com/Festal-KM/SES-Platform/issues/37) で確認中） |
