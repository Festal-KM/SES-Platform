...
> **前提条件（本スプリントには画面が無いためブロッカーではなかった。SP-03 以降のブロッカーは解消済み）**: **ワイヤーフレーム画像は全 88 枚が生成済みである**（2026-09-03。[Issue #17](https://github.com/Festal-KM/SES-Platform/issues/17)（`S-032` の停止通知の種別が `docs/04` に無い）が **= A** で決着したため残り 82 枚を生成し、`docs/04` 改訂 5 の `S-046` 分 3 枚を追加した。**85 → 88 枚**）（`docs/dev-plan.md` §5 E-15 / §6.4 R-11）。**ワイヤーフレームは `programmer` の入力である**ため、以後も画面が新設・改訂されるたびに `--screen` で 1 枚ずつ生成する。
