...
- ✅ **追補（2026-09-09、コミット `8efd493`）** — 🔴 **`K-3` の証明テスト `tests/isolation/gate-injection.test.ts`（26 件 / 4 describe）を載せた。** 上の本体コミット `be524fe` は**囲いと denylist**（構文レベル）までを固定していたが、🔴 **「囲いが破れない」ことと「判定が変わらない」ことは別の主張**であり、後者は**プロンプトの組み立てから合否の保存まで**を通さないと実証できない（`docs/05` §11.13 ①）。**完了確認が NG としたのは「テストが無い」ことではなく「コミットにも CI にも載っていない」ことである。** 実 DB + RLS で `gate.run` の本番と同じ 1 本の経路を通し、`MockAnthropicClient` に「釣られた応答」を与えて 4 つを固定する（詳細は §5 の E2E 欄 / `docs/dev-plan.md` §6.1 K-3）。**同コミットには `tests/static/gate-hold-release-enqueue.test.ts` への `removeFailedJob` 追加、`packages/ai/src/untrusted.test.ts` のコメント実体化、`packages/ai/src/client.ts` の例外メッセージ是正（挙動不変）、`docs/05` §7.8 / §7.9 ⑥ / §11.13 の更新、および計画側（本ファイル / SP-08 / SP-09 / SP-12 / `docs/dev-plan.md`）の更新が同梱されている。** **CI green: run [`34338285489`](https://github.com/Festal-KM/SES-Platform/actions/runs/34338285489)**（E2E 35 passed）。
...
- 🔴 **持ち越し（2026-09-09 に確定。`docs/dev-plan.md` §3.2 / §4.1 / §8 に記載済み）**: **本タスクは SP-07 の期間内に着手しない。実施は SP-08 と並走、遅くとも SP-09 の先頭タスクとする。** 🔴 **SP-09 より後ろに置いてはならない** —— **`gate.run` の Worker が無主のままでは、ブラウザ経路の E2E #3（1 サイクル完遂）/ #4（ゲート FAIL が送信できない）/ #23（AI 上限と HELD）が成立せず、`CLAUDE.md` §5 の Phase 1 成功条件 1・2 に到達できない**（`docs/sprints/SP-09-proposal-flow.md` T-09-11）。**タスク ID は `T-07-11` のまま変えない**（`CLAUDE.md` §8.8。ID を振り直すと SP-07 の完了記録・`docs/05` §11.12 ⑧・Issue #44 からの参照が切れる）。**着手条件は下記 `Q-07-1` / `Q-07-2` の 2 件であり、いずれも既定値が置いてあるため回答を待って止めない。**<br>🔴 **更新（2026-09-10）: `Q-07-2`（[Issue #44](https://github.com/Festal-KM/SES-Platform/issues/44)）は回答 ① で決着した** —— **`demo` は常時 PASS / `development` は未設定のまま**。🔴 **実装の前に `docs/05` §13.2 へ追記する**（`CLAUDE.md` §8.7。`program-design` の仕事）。**残る着手条件は `Q-07-1` のみ。** 🔴 **`docs/dev-plan.md` に `SP-21`（UI 基盤）が SP-07 の直後に差し込まれたが、本タスクの配置制約は変わらない**（SP-08 と並走 / 遅くとも SP-09 の先頭。**SP-21 と並走してよい** —— 触る範囲が重ならない）。
- ✅ **完了（2026-09-10、コミット `89545bb`。CI run `34438822934` green。⚠️ 本記録は 2026-09-15 の SP-08 完了確認で補填した** —— SP-07 の完了確認〔2026-09-09〕より後にコミットされたため完了記録が無く、**SP-08 の確認で本タスクが「未着手」と誤読された**。記録の漏れは誤判定を生む） — `apps/worker/src/{bootstrap,runtime,scheduler}.ts` を新設し `main.ts` に配線。**`bootstrapWorker()` を `main.ts` から切り出した**（`main.ts` は import しただけでワーカーが常駐するエントリになったため。起動経路テストの harness が Redis へ繋がずに初期化を検証できるように。呼び出し連鎖 `main.ts` → `bootstrap.ts` → `initializeRuntimeConfig` は `tests/static/startup-di-callers.test.ts` が 2 段とも固定。`docs/05` §13.1）。**スケジュール基盤**: `runScheduled`（`scheduler.ts`。1 slot 1 回。`SchedulerRun.runKey` の `UNIQUE` により二重起動してもハンドラは 1 回〔受け入れ基準③〕）+ **テナントファンアウト**（専用ロール **`app_scheduler_probe`** + `SECURITY DEFINER` の **`app_list_scheduler_tenants()`** の migration。ジョブ実行文脈がテナント一覧を得るための最小権限であり、業務テーブルは読めない。**ロール走査の対象に含め、除外リストに足していない**）。宣言済みスケジュール 5 本（`gate.hold-release` 10 分ごと等）が実際に走る（受け入れ基準①②）。起動経路テスト `tests/startup/**` green（④）。`QUEUE_CONSTRUCTION_ALLOWLIST` は 1 件のまま（⑤）。**`docs/05` §9.1 の設計どおりであり、§11.12 ⑧ の「見送り」は解消した。** 着手条件の消化: `Q-07-2` は [Issue #44](https://github.com/Festal-KM/SES-Platform/issues/44) の回答 ①（`demo` = 常時 PASS / `development` = 未設定）で決着、`Q-07-1` は既定値（`development` の `mock` だけで受け入れ基準を満たし、実接続は E-3 の後）で進行。**後続への効果**: SP-08 の `T-08-07` は `proposal-request.expire` を本タスクの `runtime.ts` の既存経路に 6 本目として乗せた（`runtime.test.ts:135`）。⚠️ 🔴 **ブラウザ経路の E2E #3 / #4 / #23（`T-09-11`）に残る制約は本タスクとは別である** —— **E2E ハーネス（`tests/e2e/harness`）に Redis とワーカーが無い**（PostgreSQL と MinIO だけ。`docs/05` §11.12 ⑦）。ワーカーは配線済みだが、E2E から `gate.run` を走らせるには**ハーネス側に Redis + ワーカーを足す**必要があり、それは `T-09-11` の仕事である。
...
コミットは **11 本**である。T-07-01 `024b67c` / T-07-02 `be524fe` / T-07-03 `d915465` / T-07-04 `ba30053` / T-07-05 `4314694` / **T-07-07 `7f0d7af`** / **T-07-06 `19d6b47`**（以上 2026-09-08）/ T-07-08 `9a23602` / T-07-09 `2ba9be7` / T-07-10 `c58623f` / 🔴 **T-07-02 の追補 `8efd493`**（以上 2026-09-09）。

- 🔴 **T-07-07 を T-07-06 より先に実装した** —— パイプライン（T-07-06）が整合層（T-07-07）を**呼ぶ側**であり、逆順に作ると「整合層が無い間だけ PASS を返す」暫定分岐が生まれるためである。**計画の番号順との差は意図的であり、依存の向き（`docs/dev-plan.md` §4.3 の「DB → AI 層 → API → UI → E2E」）に従った結果である。**
- 🔴 **11 本目 `8efd493` は「T-07-02 の追補」である**（新しいタスクではない）—— **`K-3` の証明テストがコミットにも CI にも載っていなかった**ことが完了確認で判明したため、`tests/isolation/gate-injection.test.ts`（26 件 / 4 describe）を新規に載せた。**内容**: 同テストの新規追加 / `tests/static/gate-hold-release-enqueue.test.ts` の `FORBIDDEN_CALLEES` に **`removeFailedJob`** を追加（**失敗ジョブの削除を自動復帰の側から呼べないことまで固定する** —— 削除は人間の明示操作である `#39` の入口だけが行う。`docs/05` §9.10 / [Issue #16](https://github.com/Festal-KM/SES-Platform/issues/16)）/ `packages/ai/src/untrusted.test.ts` のコメント実体化 / `packages/ai/src/client.ts` の例外メッセージ是正（**挙動は変えていない**）/ `docs/05` §7.8 のポインタ・**§7.9 ⑥ の是正**（「SDK 依存はまだ入っていない」→ **`@anthropic-ai/sdk` は T-07-01 で追加済み**）・**§11.13 の新設**（K-3 の読み替えの一次資料）。**本ファイル・SP-08 / SP-09 / SP-12 / `docs/dev-plan.md` の計画側の更新も同梱されている。**
...
🔴 **テスト green の証跡（2026-09-09。SP-07 の最終状態）**: **unit 193 files / 3557**、**isolation 51 files / 1345**、**E2E 35**（**desktop 23 + mobile 12**。🔴 **SP-06 から増えていない** —— 本スプリントの検証はすべて結合層と静的層に置いた。理由は §5 の E2E 欄の読み替えのとおりで、**提案の画面・承認 API・送信 API が無く、`gate.run` の Worker も未配線（T-07-11）であるため、ブラウザ経路から本スプリントの機能に到達できない**）、**startup 14**。

- 🔴 **isolation が 50 files / 1320 → 51 files / 1345 に増えたのは `8efd493` である** —— 増分の 1 ファイル / 25 件が **`tests/isolation/gate-injection.test.ts`**（26 件のうち 25 件がここで新規計上。`K-3` の証明。unit の +1 は `packages/ai/src/untrusted.test.ts` の追補）。🔴 **完了確認が NG としたのは「テストが無い」ことではなく「コミットにも CI にも載っていない」ことであり、この 1 行がその解消の証跡である。**
- **CI green は 11 コミットすべて**（単一ジョブ直列）: **T-07-10 の `c58623f` は run [`34332902675`](https://github.com/Festal-KM/SES-Platform/actions/runs/34332902675)**（E2E 35 passed）/ 🔴 **スプリント最終の `8efd493` は run [`34338285489`](https://github.com/Festal-KM/SES-Platform/actions/runs/34338285489)**（E2E 35 passed。2026-09-09）。**この 2 つを両方残す** —— 前者は T-07-10 の完了時点の、後者は **`K-3` の証明テストを載せた最終状態**の証跡であり、片方だけでは「いつ何が緑だったか」が復元できない。
- [Issue #25](https://github.com/Festal-KM/SES-Platform/issues/25) の**運用 C の確認記録**として残す —— 機械的強制は未達であり「CI があるから守られている」と読み替えない（`docs/dev-plan.md` §6.4 R-05）。⚠️ **あわせて `docs/05` §11.13 ④**: 🔴 **`tests/isolation/**` は `dist` 越しに走るため、`pnpm -r build` を挟まないと防御を壊しても緑のままになる。** 上の緑はこの手順を踏んだ実行である。

**全タスクで `code-reviewer` APPROVED。**
...
| ~~[#44](https://github.com/Festal-KM/SES-Platform/issues/44)~~ | **`development` / `demo` でモックのゲートが返す既定応答**（= `Q-07-2`） | ✅ **決着（2026-09-10、人間の回答 ①）: `demo` は常時 PASS / `development` は未設定のまま**（= 既定として置いていた内容と同じ。⚠️ **Issue 側の選択肢番号は ①、本表の旧記載は ③。内容が同一であることを確認済み** —— 番号だけで照合しない）。🔴 **実装の前に `docs/05` §13.2 へ追記する**（`CLAUDE.md` §8.7。`program-design` の仕事） | **T-07-11**（着手条件が 1 件外れた。残るのは `Q-07-1` のみ）/ `docs/05` §13.2 の追記 | ✅ **決着済み**（`docs/05` §13.2 への反映は T-07-11 の着手前） |
