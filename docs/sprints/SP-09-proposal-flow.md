...
- ✅ **決着（2026-09-16。T-09-07）**: `docs/05` §10.6「T-09-07 の実装の決着」のとおり。応答不明の確定は T-09-06 のまま。足したのは ①モックの `EmailSender` の台本（`MockEmailStep`: `unknown` = 記録してから `UNKNOWN`〔`callCount` 加算 = 届いた可能性〕/ `unreachable` = 記録せず `TRANSIENT`〔`callCount` 不変〕/ `reject`。注入口は `ConnectorRuntimeOptions.mockEmail` と `startWorkerRuntime(config, { mockEmailScript })`。`real` に渡すと起動時に `MockEmailScriptNotApplicableError`）②`send.settle-unknown`（毎 10 分。`SUBMITTING` × `updated_at <= now − SUBMITTING_STALL_ALERT_MINUTES`〔`A-005` 項目 2 と同じ列・閾値〕を `RESERVED → UNKNOWN(UNKNOWN:SETTLE_TIMEOUT)` + `SUBMIT_FAILED` に**確定させるだけ**。予約が無い滞留は `SETTLE_TIMEOUT:UNSENT`〔外部を呼んでいない = 届いた可能性を出さない〕。外部 0・`APPROVED` に戻さない・試行を作らない = 自動リトライではない。**§10.6 の「自動で `SUBMIT_FAILED` に落とさない」は暫定で改訂**〔`F-022 AC-2` を満たす機構が他に無く、`S-022` は `SUBMITTING` を出さず運営者は read-only〕）。結合 `tests/isolation/send-proposal-unknown.test.ts`（8 件。①応答不明 = 外部 1 / 対照のネットワーク断 = 外部 0 ②再実行・`hold-release`・実 Worker の再配送・`settle-unknown`・1 日後で外部 1 のまま ③⑤ 後の注入で `SUBMITTING` + `RESERVED` 滞留 → `readSubmittingStalls` に 1 件 → `settle-unknown` が確定 → 0 件・`S-022` に届いた可能性 / 予約なしは `UNSENT` ④実 #44 → seq 2 → 外部合計 2・`SendAttempt` 2 行 ⑤`byState` の 4 区分）+ `worker-runtime`（キュー 14 本・台本 × `real` は起動失敗）+ ユニット 4 本 + 静的 2 本 + E2E `tests/e2e/send-unknown.spec.ts`（契約の確認。worker 不在）。
- 🔴 **T-09-07 からの申し送り（2026-09-16）**:
  1. **T-09-11 へ（E2E #8 の通し）**: worker をハーネスに立てるとき、モックの台本を **`startWorkerRuntime(config, { mockEmailScript: [{ kind: 'unknown' }, { kind: 'deliver' }] })`** で注入する（`@ses/connectors` の `MockEmailStep`。環境変数から渡すなら `parseMockEmailScript(JSON.parse(...))` で検証してから）。`tests/e2e/send-unknown.spec.ts` の `settleProposalSendAsFailedForE2e` を外し、#43 → worker の ⑤⑥（`UNKNOWN` + `SUBMIT_FAILED`）→ `S-022` → #44 → worker が seq 2 を送って `SUBMITTED`（`sendAttempts` = `[UNKNOWN(1), SUCCEEDED(2)]`）に置き換える。「外部 1 回」の掴み手は `EmailSender.callCount()`（worker プロセスの外からは読めないので、worker をハーネスのプロセス内で `startWorkerRuntime` するか、`S-023` の試行の記録〔`SUCCEEDED` の件数〕で代える）。🔴 `real` の環境に台本を渡す配線を作らない（起動が落ちる）。
  2. **T-09-11 へ（`send.settle-unknown` の E2E）**: ⑤ の後のプロセス消失をブラウザ経路で再現する手段は無い（結合テストは `settleProposalSubmission` の throw 注入）。E2E では対象外とし、結合テスト ③ を根拠にする。
  3. **SP-11 T-11-04 へ**: `A-005` 項目 2（`SUBMITTING` 滞留）は `SUBMITTING_STALL_ALERT_MINUTES` + 10 分以内に `send.settle-unknown` が確定させる。**それを超えて残る滞留は「ジョブが動いていない」の兆候**であり、`SCHEDULER_HEARTBEAT` / 項目 3 と並べて読む旨を `A-005` の説明文に足すことを検討（本タスクでは `docs/05` §10.6 の運用手順に書いた）。
  4. **Issue 起票（オーケストレーター）**: `docs/05` §10.6 の「`SUBMITTING` のままプロセスが消えた → 自動で `SUBMIT_FAILED` に落とさない」を暫定で改訂した（人間が確定する経路が無く `F-022 AC-2` と両立しないため）。人間判断の確認を提案。
...
- ✅ **決着（2026-09-17。T-09-11）**: `docs/05` §17.5「T-09-11 の実装の決着」/ §17.6 ⑦ / §17.3 #3 #4 #7 #8 #9 #10 #13 / §13.2 / §6.5「`S-020` の『修正する（下書きに戻す）』と E2E ハーネスのシームの撤去」のとおり。[Issue #47](https://github.com/Festal-KM/SES-Platform/issues/47) は未回答のため**既定値（選択肢 1）で実装した**。作ったもの: ①**E2E ハーネスの worker**（`tests/e2e/harness/worker.ts`。globalSetup ⑦ で Playwright のメインプロセス内に `startWorkerRuntime` を本番と同じ配線で起動。起動前に web と同じ外向き遮断フックをこのプロセスにも入れて自己診断。env は web と同じ 1 組 = `harness/app-env.ts`。`send.hold-release` の周期を 1 分に上書き〔E2E #9 の自動復帰を待てる長さ。ジョブ本体・fan-out は本番と同じ〕）②**台本の注入口** `startWorkerRuntime(config, { mockAnthropicScript, mockEmailScriptByRecipientDomain })`（`real` に渡すと起動時 throw。`@ses/ai` に `MockAnthropicScriptNotApplicableError` を追加し `createAiClient('real', { mock })` も落とす。`development` の既定応答は置かず、ハーネスが `DEMO_MOCK_ANTHROPIC_SCRIPT` を明示的に渡す = #44 維持）③**`MockEmailSender.scriptByRecipientDomain`**（宛先ドメイン別・`token.attemptSeq` で引く無状態の台本。1 プロセスの worker を全 spec が共有するため呼び出し順の台本では #8 の位置が実行順に依存する。`unknown-once.e2e.example.test` への試行 1 = 応答不明 / 試行 2 = 届く）④**`S-020` の「修正する（下書きに戻す）」**（`GATE_FAILED → DRAFT` = #48 `MANUAL`。**シナリオ 2 が検出した UI の欠落** —— API にはあったが呼ぶ導線が無く、不合格の提案をブラウザで直せなかった。`proposal-editor-reopen-draft` + 理由文。render テスト 2 件）⑤**`tests/e2e/proposal-cycle.spec.ts`**（7 test = シナリオ 1 / 2 / 3〔#7・#8・#9 の 3 test〕/ 4 / 5〔iPhone 15〕。ゲート FAIL は AI の台本ではなく機械的検出〔実名を本文に〕で起こす。「外部 1 回」は `send_attempts` 1 行で表明〔予約を経ない送信は無い〕）⑥**`db-admin.ts` のシーム整理**（`settleProposalGateAsPassedForE2e` / `settleProposalSendAsFailedForE2e` / `settleProposalSendAsSucceededForE2e` を削除。残るのは「API を通らない経路の模擬」= `registerVerifiedSendingDomainForE2e`〔#9。`domain.verify` は SES の identity API を要求し development に無い。Issue #57 未回答〕/ `shiftProposalContentHashForE2e`〔#10 送信側〕と後始末）⑦`home.mobile.spec.ts` を実経路に置き換え（判断材料の判定は `support/proposal-flow.ts` の 1 実装をシナリオ 5 と共有）、`send-unknown.spec.ts` は削除して #8 に統合。実測: Playwright **51 test 全 green**（desktop 38 + mobile 13。5 シナリオ = 7 test green）。
- 🔴 **T-09-11 からの申し送り（2026-09-17）**:
  1. **SP-12（Phase 1 hardening）へ**: ① #44（再送）の 202 後に遷移する `S-021` は `APPROVED` として描かれ、送信中のポーリング（#46）は「送信する」を押した直後（`SUBMIT_REQUESTED`）と `SUBMITTING` の間しか走らない。**再送の確定（`SUBMITTED`）を画面が拾わず、読み直すまで「送信する」が押せる状態に見える**（押しても同じ `attemptSeq` で 1 本に畳まれるので二重送信にはならないが、利用者には分からない）。`S-022` から遷移した直後もポーリングを走らせるか、遷移先を `S-023` にするかを決める（E2E は `reload()` で回避している。`proposal-cycle.spec.ts` シナリオ 3 #8 の注記）。② `S-021` の `data-can-approve` は立場の表明で状態を含まない（`GATE_FAILED` でも `true`）。属性名から状態の可否と誤読しうるので、`data-can-approve-now` を足すか render テストに注記する。③ `S-023` の履歴は worker の実経路だとゲート・送信の `TRANSITION` 4 本が増える（商談 4 本と合わせて 8 本）。履歴が長くなったときの折りたたみ方針は `docs/04` §S-023 に無い。
  2. **SP-10 / SP-11 へ**: §17.3 #23 前半（AI 上限と HELD → `S-038` の残量表示 → 承認・送信の 409 / 422）はブラウザ経路では未実装のまま結合層にある。ハーネスの env は 1 組で他の全シナリオがゲート PASS を前提にするため、E2E で上限到達を再現するには **spec 単位で `AI_DAILY_COST_LIMIT_USD_DEFAULT` を変えた worker を起動する**か、運営者の `A-004` で上限を下げる経路（Phase 3）が要る。
  3. **Issue #47 へ（オーケストレーター）**: 既定値（選択肢 1）で実装済み。決着は本節 ①〜③。**worker を別プロセスにする案は採らなかった**（台本を値で渡す・`callCount()` は spec から読めないので `send_attempts` を掴み手にした）。
  4. **Issue #57 へ（オーケストレーター）**: E2E は `tenant_sending_domains` を直接 `VERIFIED` にしている（`registerVerifiedSendingDomainForE2e`）。非本番で検証を成立させる設計が決まったら、このシームをその経路に置き換える。
  5. **レビュー申し送り（任意）**: ハーネスの外向き遮断フックは Playwright のメインプロセスに入るため、Docker デーモンへの接続が TCP の非ループバック（`DOCKER_HOST=tcp://<remote>`）な環境では teardown の `container.stop()` が遮断される（Ryuk が回収する）。CI（ubuntu-latest）とローカル（Docker Desktop の名前付きパイプ）では起きない。
...
## 8. 完了記録（§7 は欠番。完了判定は §6）

- **2026-09-17**（`MODE: REVIEW` / `TARGET: SP-09`）: **T-09-01〜T-09-13 の 13 タスクすべてに実装・テスト・`docs/05` §6.5 の決着節が存在する。** コミット（すべて main。CI は E2E を含む）: T-09-12 `e36c06c` / T-09-13 `0ec4e26` / T-09-01 `7c6f2f3` / T-09-02 `027ebcb` / T-09-03 `1171dc8` / T-09-04 `450f506` + 修正 `4f332cb` / T-09-05 `a1bd28a` / T-09-06 `bb09e22` / T-09-08 `182c4e4` / T-09-09 `fb710d1` / T-09-10 `80aef76` / T-09-07 `ac811a3`（以上 CI green）/ 🔴 **T-09-11 `39ab36e`（CI 実行中。ローカル Playwright 51 passed〔`test-results/.last-run.json` = passed〕、code-reviewer APPROVED。CI green の確認をもって完了とする —— SP-08 と同じ扱い）**。Phase 1 成功条件 1・2 の証跡は `tests/e2e/proposal-cycle.spec.ts` シナリオ 1 / 2。未回答 Issue は既定値で進行し、ブロッカーに数えない: [#47](https://github.com/Festal-KM/SES-Platform/issues/47)（E2E ハーネスの worker。既定 = 選択肢 1 で実装済み）/ [#54](https://github.com/Festal-KM/SES-Platform/issues/54)（`APPROVED → DRAFT`。既定 = 追加しない）/ [#57](https://github.com/Festal-KM/SES-Platform/issues/57)（非本番の送信ドメイン。既定 = 現状維持）/ [#58](https://github.com/Festal-KM/SES-Platform/issues/58)（添付の実体。既定 = 送らない）/ [#59](https://github.com/Festal-KM/SES-Platform/issues/59)（`S-023` の閲覧記録。既定 = 記録しない）/ [#61](https://github.com/Festal-KM/SES-Platform/issues/61)（`send.settle-unknown`。既定 = 実装どおり）。⚠️ **完了確認で見つかった転記漏れ（コードではなく計画側。`pm` 計画モードで解消する）**: ①T-09-09 ③ / T-09-10 ③ / T-09-11 ① の「SP-12 へ」の申し送りが `docs/sprints/SP-12-phase1-hardening.md` に無い ②T-09-09 ③-④ の `S-003` 要対応キュー（承認待ち / 送信失敗 / ゲート差し戻し / 提案依頼）は「SP-10 の範囲」とされたが SP-10 に無く、`apps/web/lib/home/types.ts` の `HomeBlock` は `SCAN_QUARANTINE` のみ ③`docs/05` §6.5 #46b（`GET /api/proposals/{id}/snapshot-diff`。**SP-09** と記載）/ `docs/04` §S-006 セクション 4・5 / `docs/05` §17.3 #25 の差分ビューは `apps/web/app/(main)/engineers/[id]/page.tsx:284` のコメントで SP-09 に委ねられたまま SP-09 に持ち主が無い ④`docs/dev-plan.md` §9 に #57 / #58 / #59 / #61 の行が無い。
- 再判定 2026-09-17: PHASE_COMPLETE（CI `35179774366` success、申し送りは T-12-13〜T-12-16）
