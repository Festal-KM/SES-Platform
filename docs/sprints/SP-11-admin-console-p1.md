...
- ✅ **決着（2026-09-16 実装）**: スコアは `@ses/domain` の `scoreTenantHealth`（`packages/domain/src/health/tenant-health.ts`。純粋関数。`now` は引数）。シグナル 5 値 = `TRIAL_EXPIRED`（40）/ `INACTIVE`（20）/ `SEATS_UNUSED`（10）/ `NO_PARTNERS`（6）/ `TRIAL_EXPIRING`（3）で、**重みは辞書式**（どの 1 つも下位の合計より大きい。運用の並び順であり Issue 不要）。閾値は `packages/config` の `TENANT_HEALTH_*` 4 キー（14 日 / 7 日 / 30% / 7 日）を `tenantHealthRuntime()` から渡す（`packages/db` は既定値へフォールバックしない）。API-A2 は `?sort=health`（既定）/ `name` / `createdAt`、各行に `health: { score, signals }` + `activeMemberCount`、応答に `observedAt`。同点は `createdAt` 昇順 → `id`。`CLOSING` は最下位の層、`PURGED` はさらに下（対象外）。最終アクティビティ = `users.last_login_at` の最大値（`audit_logs` の MAX はパーティション全走査 + 運営者行の除外が要るため採らず）。使われている席 = 有効な所属のうち停滞閾値内にログインした利用者。材料は 1 回の `withPlatformRead` で小さい 4 表（`tenants` / `memberships` / `users` / `partner_companies`）だけを全件集計し、エンジニア数・案件数はページ分だけ。`A-002` は「異常の種別」列（`Badge`）+ 席「利用中 / 有効」+ 並び替えリンク + 集計日時と閾値 + 異常 0 件の帯。詳細は `docs/05` §6.9「API-A2 の実装の決着」。テスト: ユニット 23 / 結合 `tests/isolation/admin-tenant-health.test.ts` 24 / render 9。
- **T-11-01 の申し送り（2026-09-16）**: ①🔴 **T-11-07（E2E #15）**: API-A2 の応答に `health.signals`（列挙値）と `activeMemberCount` が加わった。禁止値の走査対象に `users.display_name` / `email`（席の母集団として `memberships × users` を `groupBy` するが select しない）と `partner_companies.name` を含め、`GET /api/admin/tenants?sort=name` / `?sort=createdAt` も同じ禁止値で走査すること（並びを変えても行の形は同じ）。`tests/isolation/admin-tenant-health.test.ts` の `FORBIDDEN_STRINGS` / `ITEM_KEYS` をそのまま流用できる ②`docs/04` §A-002 の「並び替え → 異常度 / 最終アクティビティ / 席利用率 / エンジニア数」に対し実装は `health` / `name` / `createdAt`（`docs/05` の決着表に注記）。`ui-design` の改訂で確定させる（T-11-03 の A-006 フィルタ差分と同じ扱い）③`docs/04` §A-002 の異常の種別にある「未対応の送信失敗あり」はスコアに入れていない（`A-005` 項目 1 と二重計上になる。足すなら `docs/04` / `docs/05` §5.7 を先に改訂）④`docs/04` §A-002 セクション 1「異常の要約（種別ごとの件数。クリックで絞り込み）」と「閾値を割った件数を最上部の要約に出す」は未実装（本タスクは列の追加と並び替えのみ）。全件の材料はクエリ内に揃っているので、応答に `summary: Record<TenantHealthSignal, number>` を足せば実装できる。要否は `ui-design` / `pm` で ⑤契約席数（`Subscription.seat_count`）は Phase 3（`A-010`）で GRANT が入る。その時点で `seatCount` の上限側を差し替える（現状は有効な所属数を代理に使う）。
...
- ✅ **決着（2026-09-16 実装）**: API-A6 = `GET /api/admin/usage`（`readPlatformUsage`。**1 回の `withPlatformRead`（`admin.usage.view`）で件数 4 単位 + 当日 AI コスト（USD）+ 当月 AI 原価（USD。ロール別）+ 消費率 + 2 つの倍率〔`baselineRatio` = ÷ $12.82 / **`unitCostRatio`** = 実原価 ÷ Σ(件数 × docs/03 §7.6.1 の標準原価)〕**、環境全体の当月 AI 支出は `summarizeProviderSpend` で同じ集計行から**別の行**）/ `PUT /api/admin/tenants/{id}/quota`（`setTenantQuotaOverride`。`PLATFORM_OWNER` のみ = `requirePlatformOwnerCtx`。**新表 `tenant_quota_overrides` に INSERT だけ**〔`withPlatformWrite(domain='QUOTA')`。migration 20260924000000。C2 / `app_tenant` SELECT / `app_platform` 列 GRANT / `app_platform_write` INSERT のみ + WITH CHECK〔操作者 = 自分・対象 = `app.target_tenant_id`・適用日 ≥ 今日・**引き下げは明日以降**〕〕）。🔴 **引き下げは `effectiveFrom` 翌日以降 + `notifyTenantAdmins: true` が必須**（`@ses/domain` の `decideQuotaChange`。違反は 400 `QUOTA_CHANGE_REJECTED`）。**通知（`QUOTA_LOWERED`）はワーカーの `usage.limit-check` 手順 ⑥ が `email.dispatch` 単一経路で積む**（管理平面は `EmailDispatch` を書かない = `app_platform_write` を `email_dispatches` に広げない。`dedupeKey` = 上書き行 ID）。判定側（`usage.limit-check`）と表示側（`GET /api/usage`）は `resolveTenantQuotas`（既定値 + 上書き。`@ses/domain` の `resolveQuotaLimit` 1 実装）で同じ上限値を受ける。抽出は `classifyConsumptionBand`（`QUOTA_LOW_CONSUMPTION_PERCENT` 20 / 警告閾値 80）。`packages/config` に `AI_MONTHLY_COST_CAP_USD_DEFAULT`（40 = Standard）を新設（当月の金額上限の分母。運営者の内部指標）。画面 `A-004`（`/admin/usage`。フォームと行の操作導線は OWNER にだけ描く）、`adminTenantQuotaHref` を `/admin/usage?targetTenantId=…` に差し替え、管理ホームと `A-003` セクション 4 に導線。監査 `admin.quota.change`（`summary` = metric / from / to / effectiveFrom / kind / reasonLength。理由の本文は載せない）。書き込み API の固定一覧 6 本 → 7 本（`tests/static/admin-no-gate-retry.test.ts` ⑤ / `admin-tenants-read-only` / `execute-guard`。**意図した摩擦**）。詳細は `docs/05` §3.2 / §3.10 / §5.2 / §5.8.1 ⑧ / §6.9「API-A6 と `A-004` の実装の決着」。テスト: 結合 `tests/isolation/admin-usage-quota.test.ts`（18 件）/ ユニット 57 件〔domain 24 / db 9 / web schemas 10 / worker 14（うち新規 4）〕/ render 13 件。
- ⚠️ **T-11-02 の申し送り**: ①`Subscription.unitQuotaOverride` / `quotaOverrideEffectiveFrom` の 2 列は**使わない**（SP-20 で削除を検討。金額上限の上書き `quotaOverrideUsd` は SP-20 のまま）②「消化率が**常に**低い」は Phase 1 では当月だけで判定する（月次の履歴 `tenant_monthly_costs.quota_consumption_rate` が契約無しで `null`。SP-20 で履歴に置き換える）③2 人の `PLATFORM_OWNER` が同じ計測を同時に変えたときの `from` は先に読んだ値（後の行が効く。両方の監査行が残る）④`AI_MONTHLY_COST_CAP_USD_DEFAULT` の値（$40）は `Q-T-3`②と同じ論点（プラン別の値が入るまでの既定値。実装は値に依存しない）⑤`docs/04` §A-004 の「クォータ設定は件数と金額上限を別の入力として持つ」のうち**金額上限の入力は SP-20**（本タスクは **AI の月次件数 4 単位のみ**。`EMAIL_COUNT` / `STORAGE_BYTES` は上書きの対象に含めない。NG-1 で 6 計測から修正。理由と後続タスクは⑥）—— `ui-design` で `docs/04` に注記を。⑥ 🔴 **`EMAIL_COUNT` / `STORAGE_BYTES` は上書きの対象外**（code-reviewer NG-1。2026-09-16）: 執行点（`apps/worker/src/jobs/email-send.ts` の `decideEmailRate` / `reserveEmailDailyQuota`、`send-hold-release.ts`、`apps/web/lib/skill-sheets/service.ts` の `decideStorageUpload`）が既定値しか読まず、「上限を上げても執行が変わらない / 停止と表示されているのに送信される」事故になるため。`resolveTenantQuotas` の `emailDailyLimit` / `storageLimitBytes` は常に既定値・`sources` は常に `'DEFAULT'`。**執行点の配線と、パートナー文脈での読み取り**〔`HostTenantCtx` 要求の `resolveTenantQuotas` では `PARTNER_*` のアップロードで型が通らない。`usage_counters_storage_select` と同じ判断 + `reason` / `set_by_platform_user_id` の列 GRANT 制限が要る〕**を含む後続タスクは SP-12 に `T-12-xx` として起票する**。
- 🔴 **T-11-07 への申し送り（E2E #15 の禁止値。API-A6 / `A-004` の応答と HTML を JSON 化して 0 件であること）**: ①利用者の氏名・メールアドレス（`users.display_name` / `email`。宛先の引き当ては worker 側で行い、応答には席数だけ）②`ai_usage` の対象（`target_type` / `target_id`）・モデル・プロンプト版・`purpose`（ロール別の**金額**だけを載せる）③エンジニア・案件・提案・チャットの内容④`tenant_quota_overrides.reason` は **`A-004` の一覧 DTO には載せていない**（監査ログにも長さだけ）。**禁止キー**: `displayName` / `recipientEmail` / `targetId` / `modelId` / `promptVersion` / `purpose` / `reason`（`A-006` の `summary` は別）。🔴 **金額（`costUsd` / `capUsd` / `limitUsd` / `spentUsd` / `standardCostUsd`）が現れてよいのは管理平面（API-A6 / `/admin/usage`）だけ**である —— E2E #15 は逆に**主平面（`GET /api/usage` / `/settings/usage`）の応答と HTML に `Usd` / `$` が 1 つも無い**ことを同じ seed で確かめる（`apps/web/lib/usage/view.types.test.ts` が型の側から固定済み）。抽出の禁止値は `tests/isolation/admin-usage-quota.test.ts` ⑧ の `MODEL_ID` / `PROMPT_VERSION` / seed のメールと同じ値を使えばよい。⑤`PLATFORM_SUPPORT` でサインインした `/admin/usage` の HTML に `admin-usage-quota-form` / `admin-usage-quota-open-` が**存在しない**こと（グレーアウトではなく不在。`F-057 AC-2`）、`PUT /api/admin/tenants/{id}/quota` が 403 であることを E2E でも押さえる。⑥モバイルビューポートでも `A-004` の表が `hidden` にならず横スクロールで読めること（T3。`CLAUDE.md` §13.3）。
...
- ✅ **決着（2026-09-16 実装）**: API-A8 `GET /api/admin/monitoring`（`apps/web/app/api/admin/monitoring/route.ts`。`GET` のみ）は **15 項目**（1 / 2 / 3 / 4 / 5 / 6 / 7 / 11 / 12 / 13 / 14 / 15 / 16 / 17 / スケジューラ）を `docs/04` §A-005 の順で**常に全部**返し、各項目は `{ ok: true, … } | { ok: false, errorKind }`（`apps/web/lib/admin-monitoring/view.ts`）。🔴 **項目ごとに `withPlatformRead` を 1 回**（Postgres は 1 クエリの失敗でトランザクション全体を中断するため、独立性は 1 項目 1 トランザクションでしか成立しない = 1 回の GET で `admin.monitoring.view` が 14 本。`summary.item` で区別）。新設の材料は `packages/db/src/platform/queries/monitoring.ts`（内容を持つ表には `groupBy` / `count` / `aggregate` だけ）、既存の 4 材料（6 / 11 / 12 / 17）は再実装せずに写す。failed セットは `@ses/connectors/bullmq` の**読むだけ**の口 `createBullMqFailedJobsReader`（`gate.run` 以外の payload は読まない = `account.mail` の平文トークンに触れない）で 1 回読み、項目 3 / 12 で共有。Redis を読めなければ項目 3 は `ok: false`、項目 12 は `failedJobsAvailable: false` で保留だけを残し閾値超過を応答不明に畳まない。項目 13 は `providerQuotaUsage` / `isProviderQuotaWarning`（判定と同じ 1 実装）で消費率と接近を出し、`getQuota()` の失敗は `available: false`（0 で埋めない）。**`nearingSince` の目印（`mail:provider:nearingSince`）は API-A8 の読み取り時に `SET NX` / `DEL` で維持する表示専用の揮発値**（テナントの業務データではない。`send.hold-release` に同じ `observe()` を足すのは T-09-08 後の申し送り）。閾値は `SUBMITTING_STALL_ALERT_MINUTES`（新設。30）/ `MAIL_DISPATCH_STUCK_ALERT_MINUTES`（新設。15）+ 既存キー、方針値は `limits.ts`（`SCHEDULER_HEARTBEAT_STALE_HOURS` 24 / `GATE_FAIL_RATE_WINDOW_HOURS` 24 / `GATE_FAIL_RATE_BASELINE_DAYS` 7）で、ルートは `bootstrap.ts` の `monitoringRuntime()`（`gateStallRuntime()` / `providerSpendRuntime()` を含む）からだけ受ける。画面 `A-005`（`/admin/monitoring`。管理ホームに導線）は保留 = `warning` / 障害 = `danger`（`severityOf`）、空の表現 3 種（13 = 消費率 / 14「保留中の送信はありません」/ 15「削除待ちのテナントはありません」）、`RATE_LIMIT` / `AI_COST_LIMIT_HELD` → `adminTenantQuotaHref`（`A-004` が入るまで `A-003` に倒す 1 行）、`targetId` はリンクにしない、操作導線なし。`docs/04` 改訂 11 で項目 16 / 17 と項目 12 の `応答不明` を先に追記した。テスト: 結合 `tests/isolation/admin-monitoring.test.ts`（23 件。実 DB + 実 Redis）/ render 19 件 / ユニット（db 9 + web 17 + connectors 8）。詳細は `docs/05` §6.9「API-A8 と `A-005` の実装の決着」。
- 🔴 **T-11-07 への申し送り（E2E #15 の禁止値。API-A8 / `A-005` の応答を JSON 化して 0 件であること）**: ①提案の件名（`subject`）・本文（`body` / `draft_body`）・提案先（`recipient_company_name` / `recipient_email`）・単価（`offered_unit_price`）②エンジニアの氏名・生年月日・連絡先③`review_gates.findings` / `ai_warnings` の抜粋④**DKIM トークン（`dkim_tokens`）/ MAIL FROM ドメイン（`mail_from_domain`）/ `ses_identity_arn` / `last_failure_reason`**（項目 11）⑤`email_dispatches.recipient_email`（項目 13 / 15 / 16）⑥`tenant_purge_runs.failure_reason` / `counts`（項目 7。失敗理由の自由文と完了件数は返さない）⑦スキルシートの `object_key` / `note`（項目 4）⑧ジョブの payload（`account.mail` の平文トークン。項目 3 は件数と時刻だけ）⑨`ai_usage` の `target_*` / モデル / プロンプト版（項目 17）⑩提案依頼の `message` / `decline_reason`。**禁止キー**: `subject` / `body` / `recipientEmail` / `recipientCompanyName` / `displayName` / `dkimTokens` / `mailFromDomain` / `lastFailureReason` / `failureReason` / `objectKey` / `note` / `findings` / `payload` / `token` / `byRole`。`apps/web/lib/admin-monitoring/view.types.test.ts` が型の側から、結合 ⑪ が JSON の側から固定済み。E2E は `tests/isolation/admin-monitoring.test.ts` の `FORBIDDEN` と同じ値を seed に仕込んで `GET /api/admin/monitoring` と `/admin/monitoring` の HTML を走査すればよい。
- 🔴 **T-11-02 への申し送り（`A-004` の URL）**: `apps/web/lib/admin-monitoring/hrefs.ts` の `adminTenantQuotaHref(tenantId)` が `A-005` 項目 12（`AI_COST_LIMIT_HELD`）/ 14（`RATE_LIMIT`）からクォータへ繋ぐ唯一の出所で、**現在は `A-003` に倒している**（404 にしない）。`A-004` を置いたらこの 1 行を `/admin/usage?targetTenantId=…` に差し替える（呼び出し側は変わらない。render テストは「クォータ」の文言で導線の存在を見ており URL は固定していない）。
- ⚠️ **T-09-08 完了後の申し送り（`apps/worker`）**: `send.hold-release` が既に計算している `warning`（`isProviderQuotaWarning`）の結果を `ProviderQuotaNearingMarker.observe()`（`@ses/connectors`）に渡すと、誰も `A-005` を開かなくても `nearingSince` の目印が 10 分ごとに維持される。実体は `createRedisProviderQuotaNearingMarker`（`@ses/connectors/bullmq`）。

- **T-11-04 のレビュー申し送り（2026-09-16。code-reviewer。非ブロッキング）**: ①`apps/worker/src/jobs/send-hold-release.ts` に `ProviderQuotaNearingMarker.observe()` を足す（現状は誰も `A-005` を開かないと接近時刻の目印が維持されず TTL 24h で消える。T-09-08 後） ②`packages/connectors/src/bullmq.ts` の `queue.getFailed(0, 0)` は全キューで `Job` ハッシュ全体（`account.mail` の `data.token` を含む）を Redis から取得してから時刻だけを写している。応答・ログには出ないが、failed ZSET のスコアだけを読む形に寄せられる ③`readers.ts` の `readMailProviderHeld`（DB）失敗が `PROVIDER_READ_FAILED`（「Redis を読めませんでした」）になり原因を誤案内する → `DB_READ_FAILED` に分ける ④`bootstrap.quotaEmailSender()` が `quotaCache` を渡していないためプロセス内 60 秒キャッシュ（設計は Redis の 60 秒キャッシュ。worker も同じで T-11-04 の退行ではない）⑤結合テストは `bootstrap` をモックしているため `monitoringRuntime()` の実配線は typecheck のみ。`development` で `GET /api/admin/monitoring` を 1 回叩く smoke を T-11-07 で。
