...
| T-13-07 | `MatchCandidate` の生成ジョブと性能 | M | マッチング候補の初回提示が **p95 3 秒以内**（`CLAUDE.md` §7）。🔴 **着手条件（2026-09-18。SP-12 `T-12-19` からの転記 / [Issue #71](https://github.com/Festal-KM/SES-Platform/issues/71)）**: `T-12-02` の実測で `S-016`（#30。匿名候補 2,000 件混在）が p95 2.3 / 3.9 秒だった原因のうち、**(d)「`MatchCandidate` の置き換えを読み取りから外す」**（`docs/05` §4.6「この読み取りが `MatchCandidate` の唯一の生成経路」= Phase 1 の暫定）を本タスクの `match.build` で生成側へ移す。**(a) RLS 集合化 / (b) 生 SQL 化 / (c) 平坦化 は `T-12-19` で済み**（再測定の値は `docs/dev-plan.md` §8「T-12-19 の再測定」の行）。**#71 の回答が B（1 秒のまま）で、`T-12-19` の再測定が 1 秒超なら (d) は SP-12 に前倒し済み**であり、本条件は「済み」に読み替える。🔴 **生成側へ移しても、開示範囲（匿名 5 項目 / 共有可の行だけ / 読み手はホストだけ）と参照子の非突合性は不変**（`CLAUDE.md` §3.1 経路 4） |
