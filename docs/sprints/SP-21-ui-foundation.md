...
✅ **追記（2026-09-15。`T-08-11` で常設化が完了した。コミット `0ebc742` + `1a1e7f8`）** —— ⚠️ **訂正**: 初回計測（T-21-06）の時点で偽陽性 4 件の原因を「和文・全角括弧・英数の混在によるフォント切替」と見ていたが、**素朴な版と本実装を同じ DOM に当てて名指しで確認した実測では、原因は次の 2 種であり、フォント切替ではない**: ①② `S-005` / `S-010` の `<legend class="sr-only">検索条件</legend>`（1×1px + `overflow: hidden` で**隠すことが目的**の要素。幾何条件 `isScreenReaderOnly` で除く。🔴 クラス名では見ない）③④ `S-012` / `S-007` の `<legend>単価レンジ（…）（円）</legend>`（**JSX の補間が隣接する 4 つのテキストノードとして描画され、`Range.getClientRects()` が 1 行でも矩形を 4 つ返す**。矩形を縦方向の重なりで束ねて行数を数える `countLines` で除く）。フォント切替による `top` の 1〜2px のずれは同じ規則で吸収されるが、**初回計測の偽陽性の原因ではなかった**。🔴 **閾値は 1 つも動かしておらず、要素を名指しで除外するセレクタも 0 件である。** 一次記録は `tests/e2e/support/assertions.ts:133`〜（「初回計測で出た偽陽性 4 件と、その解消」）。**導入初日に CI で実バグ（`S-010` 案件名列が Linux フォントでだけ 62px に潰れて 3 行に折れる）を捕捉した** —— §8.3-2 と同じ「壊れているのに緑」を、今度は機械が止めた。
