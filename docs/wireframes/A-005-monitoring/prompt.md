...
- A second, thinner row under the active tab: `運用監視` (current, underlined).
...
### Section 1: `閾値超過` — three bordered warning strips at the very top, each containing four facts
...
### Section 2: `監視項目` — one dense table, 10 body rows, columns are NEVER hidden
...
### Section 3: `本日の照合結果` — a compact table of 10 rows echoing each item with its count, including the zeros
Columns `監視項目` / `本日の照合`. Example rows: `満了 60 日前の未起票 / 1 件`, `削除ジョブ失敗 / 0 件`, `計測欠測 / 1 件`, `ウイルススキャン失敗 / 2 件`, and six more, several of which read `0 件`.
...
- 異常が上位に来ていることが一目で分かる構成にする（申し送り 6 / §6.9）。最上部を閾値超過の 3 ストリップにし、各ストリップに「何が・どのテナントで・いつから・閾値はいくつか」の 4 点を必ず含める（§5-9）。
