...
// 🔴 T-10-02: 計測の連続性・検算・月次集計（docs/05 §9.8 / §5.9 / `F-026 AC-3`〜`AC-5`）。
//    判定と算出は純粋関数としてここに置き、材料を揃えるのは `packages/db`、起動は `apps/worker`。
export {
  compareDayKeys,
  dayRangeOfMonth,
  daysInMonth,
...
// 🔴 T-07-01: AI ロールの値集合（docs/05 §7.1 / §3.8 / CLAUDE.md §12.2）。
