...
function buildEnv(database: E2eDatabase, guardMarker: string): EnvRecord {
 ...
 *    テストが走り始める窓ができる。
 */
export async function startWebServer(database: E2eDatabase): Promise<WebServer> {
  buildWebApp();
  ...
  const child = spawn(
    ...
    { cwd: WEB_APP_DIR, env: buildEnv(database, guardMarker) },
  );
