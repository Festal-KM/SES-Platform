...
/**
 * 🔴 Issue #33（docs/05 §3.3.1）で `invitations.partner_company_id` は複合 FK になったが、
 *    **このブロックは 1 文字も変えない。** アプリ層照合の役割が「防御の本体」から
 *    「一次防御（正しい応答を返す責務）」へ降格しただけであり、期待値は不変である:
 *      ① FK 違反（`23503`）が利用者応答に到達すると 500 になり、**500 と 404 の差で
 *        他テナントに実在する ID かどうかを探れる**（docs/05 §4.8 が崩れる）。
 *      ② FK はテナント境界しか見ない。同一テナント内の第二境界（下の PARTNER_1_2 の 403）は
 *        FK では判定できない。
 *    したがって **ここが緑のままであること自体が、複合 FK 化の「アプリ経路は不変」の対照**である。
 */
