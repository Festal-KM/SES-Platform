...
//    `globals.css` の `@media (max-width: 640px)`（`S-041` の列の間引き）は SP-21 の
//    T-21-04 / T-21-07 で Tailwind 側へ移る。**移るまでの間、値が既定から外れないことを守る。**
//
...
describe('🔴 ブレークポイントは Tailwind の既定に従う（CLAUDE.md §13.3 / SP-21 T-21-01 ①）', () => {
  ...
  it('手書き `@media` の幅が Tailwind の既定値だけを使っている', () => {
    // 🔴 `globals.css` の `@media (max-width: 640px)`（`S-041` の列の間引き）は T-21-04 /
    //    T-21-07 で Tailwind 側へ移る。**移るまでの間、値が既定から外れないこと**を守る。
